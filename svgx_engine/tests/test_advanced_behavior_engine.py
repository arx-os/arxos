...
class TestAdvancedBehaviorEngine:
    ...
    def sample_rule(self):
        ...
        return BehaviorRule(
            ...
            actions=[
                {"type": "update", "target_property": "alert_level", "value": "high"},
                {
                    "type": "log",
    ...
    def sample_state_machine(self):
        ...
        states = [
            ...
            BehaviorState(
                ...
                exit_actions=[
                    {"type": "log", "message": "Equipment stopping", "level": "info"}
                ],
            ),
            BehaviorState(
                state_id="error",
    ...
    def sample_time_trigger(self):
        ...
        return TimeTrigger(
            ...
            schedule_data={"interval": 3600, "start_time": datetime.now()},  # 1 hour
            actions=[
                {
                    "type": "update",
    ...
    def test_register_state_machine(self, behavior_engine, sample_state_machine):
        ...
        behavior_engine.register_state_machine(
            element_id, sample_state_machine, initial_state
        )
    ...
    @pytest.mark.asyncio
    async def test_evaluate_rules_simple(
        self, behavior_engine, sample_rule, sample_context
    ):
        ...
        applicable_rules = await behavior_engine.evaluate_rules(
            "test_element", sample_context
        )
    ...
    async def test_evaluate_rules_priority(self, behavior_engine, sample_context):
        ...
        rule1 = BehaviorRule(
            ...
            actions=[
                {"type": "log", "message": "Low priority action", "level": "info"}
            ],
            priority=1,
        )
        ...
        rule2 = BehaviorRule(
            ...
            actions=[
                {"type": "log", "message": "High priority action", "level": "warning"}
            ],
            priority=3,
        )
        ...
        assert (
            applicable_rules[0]["rule_id"] == "high_priority"
    ...
    async def test_state_transition_invalid(
        self, behavior_engine, sample_state_machine
    ):
    ...
    @pytest.mark.asyncio
    async def test_state_transition_with_actions(
        self, behavior_engine, sample_state_machine
    ):
    ...
    def test_threshold_condition_evaluation(self, behavior_engine, sample_context):
        ...
        sample_context["temperature"] = 35.0
        assert (
            behavior_engine._evaluate_single_condition(condition, sample_context)
            is True
        )
    ...
    def test_cad_parity_action_execution(self, behavior_engine):
        ...
        with pytest.MonkeyPatch().context() as m:
            ...
            # Execute action
            asyncio.run(
                behavior_engine._execute_cad_parity_action(action, "test_element", {})
            )
    ...
    def test_infrastructure_action_execution(self, behavior_engine):
        ...
        with pytest.MonkeyPatch().context() as m:
            ...
            # Execute action
            asyncio.run(
                behavior_engine._execute_infrastructure_action(
                    action, "test_element", {}
                )
            )
    ...
    async def test_event_handling(self, behavior_engine):
        ...
        rule = BehaviorRule(
            ...
            actions=[
                {"type": "log", "message": "User interaction detected", "level": "info"}
            ],
        )
        ...
        await behavior_engine._handle_event(
            "test_element", "user_interaction", event_data
        )

        # Verify rule was evaluated (would need to mock action execution to verify)

    def test_condition_registration_and_retrieval(
        self, behavior_engine, sample_condition
    ):
    ...
    def test_rule_metadata_handling(self, behavior_engine):
        ...
        assert (
            behavior_engine._rule_applies_to_element(rule, "element1", context) is True
        )
        assert (
            behavior_engine._rule_applies_to_element(rule, "element3", context) is False
        )
    ...
    def test_comprehensive_behavior_scenario(self, behavior_engine):
        ...
        safety_rule = BehaviorRule(
            ...
            actions=[
                {"type": "update", "target_property": "alert", "value": "critical"}
            ],
            priority=3,
        )
        ...
        operational_rule = BehaviorRule(
            ...
            actions=[
                {"type": "log", "message": "Equipment operational", "level": "info"}
            ],
            priority=1,
        )
        ...
        context = {"element_id": element_id, "temperature": 85, "status": "active"}

        # Evaluate rules
        applicable_rules = asyncio.run(
            behavior_engine.evaluate_rules(element_id, context)
        )
