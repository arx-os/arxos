...
class KnotVector:
    ...
    def __post_init__(self):
        ...
        if self.is_clamped:
            ...
            if self.knots[0] != self.knots[multiplicity-1] or self.knots[-1] != self.knots[-multiplicity]:
                raise ValueError(f"Knot vector not properly clamped for degree {self.degree}")


class BezierCurve:
    """Bezier curve implementation with precision support."""
    
    def __init__(self, control_points: List[ControlPoint], precision_config: Optional[PrecisionConfig] = None):
    ...
    def _calculate_tangent(self, t: decimal.Decimal) -> PrecisionCoordinate:
        ...
        for i in range(self.degree):
            dx = self.control_points[i+1].position.x - self.control_points[i].position.x
            dy = self.control_points[i+1].position.y - self.control_points[i].position.y
            dz = self.control_points[i+1].position.z - self.control_points[i].position.z
    ...
    def _calculate_curvature(self, t: decimal.Decimal) -> float:
        ...
        for i in range(self.degree - 1):
            dx = self.control_points[i+2].position.x - 2*self.control_points[i+1].position.x + self.control_points[i].position.x
            dy = self.control_points[i+2].position.y - 2*self.control_points[i+1].position.y + self.control_points[i].position.y
            dz = self.control_points[i+2].position.z - 2*self.control_points[i+1].position.z + self.control_points[i].position.z
    ...
    def subdivide(self, t: Union[float, decimal.Decimal]) -> Tuple['BezierCurve', 'BezierCurve']:
...
class BSplineCurve:
    ...
    def __init__(self, control_points: List[ControlPoint], knot_vector: KnotVector,
        ...
        # Validate inputs
        if len(self.control_points) != len(self.knot_vector.knots) - self.knot_vector.degree - 1:
    ...
    def _calculate_basis_functions(self, span: int, u: decimal.Decimal) -> List[decimal.Decimal]:
        ...
        for j in range(1, p + 1):
            for i in range(p - j + 1):
                ...
                basis_functions[i] = left_term * temp + right_term * basis_functions[i + 1]
        
        return basis_functions
    
    def _calculate_position(self, span: int, basis_functions: List[decimal.Decimal]) -> PrecisionCoordinate:
...
class CurveFitter:
    ...
    def fit_bezier_to_points(self, points: List[PrecisionCoordinate], 
                            degree: int = 3) -> BezierCurve:
        """Fit Bezier curve to points using least squares."""
        if len(points) < degree + 1:
            raise ValueError(f"Need at least {degree + 1} points for degree {degree} curve")
    ...
    def _solve_least_squares(self, matrix: np.ndarray, 
                            points: List[PrecisionCoordinate]) -> List[ControlPoint]:
...
class CurveConstraint:
    ...
    def __init__(self, curve1: Any, curve2: Any, constraint_type: str,
    ...
    def _validate_tangent(self) -> bool:
        ...
        if point1.tangent and point2.tangent:
            angle = self.precision_math.angle_between_vectors(point1.tangent, point2.tangent)
...
class CurveSystem:
    ...
    def create_bspline_curve(self, control_points: List[ControlPoint], 
                           degree: int = 3, clamped: bool = True) -> BSplineCurve:
    ...
    def fit_curve_to_points(self, points: List[PrecisionCoordinate], 
    ...
    def add_curve_constraint(self, curve1: Any, curve2: Any, 
                           constraint_type: str) -> CurveConstraint:
    ...
    def get_curve_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'bezier_curves': len([c for c in self.curves if isinstance(c, BezierCurve)]),
            'bspline_curves': len([c for c in self.curves if isinstance(c, BSplineCurve)]),
            'total_constraints': len(self.constraints),
            'valid_constraints': sum(1 for c in self.constraints if c.validate())
        }


def create_curve_system(precision_config: Optional[PrecisionConfig] = None) -> CurveSystem:
    """Create curve system with optional precision configuration."""
    return CurveSystem(precision_config)


def create_bezier_curve(control_points: List[ControlPoint], 
                       precision_config: Optional[PrecisionConfig] = None) -> BezierCurve:
    """Create Bezier curve from control points."""
    return BezierCurve(control_points, precision_config)


def create_bspline_curve(control_points: List[ControlPoint], degree: int = 3,
                        precision_config: Optional[PrecisionConfig] = None) -> BSplineCurve:
    """Create B-spline curve from control points."""
    n_control = len(control_points)
    n_knots = n_control + degree + 1
