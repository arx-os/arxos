...
class DocumentationFixer:
    ...
    def fix_documentation(self):
        ...
        # First, restore original files from backups
    ...
    def _restore_from_backups(self):
        ...
        for repo in self.repositories:
            ...
            for backup_file in backup_files:
                ...
                # If original doesn't exist, restore from backup
    ...
    def _organize_repository_docs(self, repo_path: Path):
        ...
        for md_file in repo_path.rglob("*.md"):
            if "docs/" not in str(
                md_file.relative_to(repo_path)
            ) and not md_file.name.endswith(".backup"):
    ...
    def _organize_md_file(self, md_file: Path, docs_dir: Path):
        ...
        if md_file != target_path and not target_path.exists():
            print(
                f"  📄 Moving {md_file.name} → docs/{target_subdir}/{target_filename}"
            )
    ...
    def _determine_target_subdir(self, md_file: Path) -> str:
        ...
        if any(
            keyword in filename
            for keyword in ["architecture", "design", "system", "component", "platform"]
        ):
            return "architecture"
        ...
        if any(
            keyword in filename
            for keyword in ["api", "endpoint", "rest", "graphql", "integration"]
        ):
            return "api"
        ...
        if any(
            keyword in filename
            for keyword in ["user", "guide", "tutorial", "howto", "usage", "training"]
        ):
            return "user-guides"
        ...
        if any(
            keyword in filename
            for keyword in ["trouble", "faq", "support", "debug", "error"]
        ):
        ...
        if any(
            keyword in content
            for keyword in ["api", "endpoint", "request", "response", "integration"]
        ):
            return "api"
        elif any(
            keyword in content
            for keyword in ["deploy", "infrastructure", "kubernetes", "monitoring"]
        ):
            return "deployment"
        elif any(
            keyword in content
            for keyword in ["setup", "install", "development", "build", "developer"]
        ):
            return "development"
        elif any(
            keyword in content
            for keyword in ["user", "guide", "tutorial", "usage", "training"]
        ):
            return "user-guides"
        elif any(
            keyword in content for keyword in ["trouble", "error", "debug", "fix"]
        ):
            return "troubleshooting"
        else:
