...
class MemoryManager:
    ...
    def _perform_cleanup(self):
        ...
        if current_memory.used_memory_mb > self.memory_threshold_mb:
            self.logger.info(f"Memory threshold exceeded ({current_memory.used_memory_mb:.1f}MB), "
            ...
            self.logger.info(f"Cleanup completed, memory usage: {self._get_memory_usage().used_memory_mb:.1f}MB")
    ...
    def get_memory_trends(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'average_usage_mb': sum(s.used_memory_mb for s in recent_stats) / len(recent_stats),
    ...
    def optimize_memory_settings(self) -> Dict[str, Any]:
        ...
        if current_stats.memory_percentage > 80:
            recommendations.append("High memory pressure - consider reducing cache sizes")
            self.max_cache_size = max(1000, self.max_cache_size // 2)
        
        # Check object count
        if current_stats.object_count > 100000:
            recommendations.append("High object count - consider more aggressive cleanup")
            self.cleanup_interval = max(60, self.cleanup_interval // 2)
        
        # Check cache efficiency
        pool_stats = self.object_pool.get_stats()
        hit_rate = pool_stats['stats']['hits'] / max(1, pool_stats['stats']['hits'] + pool_stats['stats']['misses'])
