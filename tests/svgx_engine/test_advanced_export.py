...
class TestSVGXAdvancedExportService:
    ...
    def export_service(self, temp_db_path):
        ...
        return SVGXAdvancedExportService(
            db_path=temp_db_path, max_workers=2, cache_size=100
        )
    ...
    def test_create_export_batch(self, export_service, sample_building_data):
        ...
        jobs = [
            (
                sample_building_data["building_id"],
                AdvancedExportFormat.IFC_LITE,
                ExportQuality.STANDARD,
                {},
            ),
            (
                sample_building_data["building_id"],
                AdvancedExportFormat.GLTF,
                ExportQuality.HIGH,
                {},
            ),
            (
                sample_building_data["building_id"],
                AdvancedExportFormat.SVGX,
    ...
    def test_list_export_jobs(self, export_service, sample_building_data):
        ...
        building_jobs = export_service.list_advanced_export_jobs(
        ...
        pending_jobs = export_service.list_advanced_export_jobs(
    ...
    def test_performance_monitoring(self, export_service, sample_building_data):
        ...
        with patch.object(
            export_service.performance_monitor, "monitor"
        ) as mock_monitor:
            job_id = export_service.create_advanced_export_job(
                building_id=sample_building_data["building_id"],
                format=AdvancedExportFormat.IFC_LITE,
    ...
    def test_format_support(self, export_service, sample_building_data):
        ...
        formats = [
            ...
            AdvancedExportFormat.GLTF,
            AdvancedExportFormat.SVGX,
            AdvancedExportFormat.EXCEL,
            AdvancedExportFormat.PARQUET,
            AdvancedExportFormat.GEOJSON,
        ]
    ...
    def test_service_factory(self, temp_db_path):
        ...
        service = create_svgx_advanced_export_service(
            db_path=temp_db_path, max_workers=4, cache_size=500
        )
    ...
    def test_concurrent_job_processing(self, export_service, sample_building_data):
        ...
        completed_jobs = export_service.list_advanced_export_jobs(
            status=AdvancedExportStatus.COMPLETED
        )
        processing_jobs = export_service.list_advanced_export_jobs(
            status=AdvancedExportStatus.PROCESSING
        )
    ...
    def test_database_operations(self, export_service, sample_building_data):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM advanced_export_jobs WHERE job_id = ?", (job_id,)
        )
