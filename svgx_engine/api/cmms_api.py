...
async def sync_work_orders(request: SyncRequest):
    ...
    try:
        result = await data_sync_service.sync_work_orders(
            cmms_connection_id=request.cmms_connection_id, force_sync=request.force_sync
        )
        return {
            "success": True,
...
async def get_work_orders(
    ...
    try:
        work_orders = await work_order_service.get_work_orders(
            status=status, asset_id=asset_id, assigned_to=assigned_to
        )
...
async def create_maintenance_task(request: MaintenanceTaskRequest):
    ...
    try:
        ...
        if not task:
            raise HTTPException(
                status_code=404, detail="Maintenance schedule not found"
            )
...
async def get_assets(
    ...
    try:
        assets = await asset_tracking_service.get_all_assets(
            asset_type=asset_type, status=status, department=department
        )
...
async def acknowledge_asset_alert(asset_id: str, alert_id: UUID, acknowledged_by: str):
    ...
    try:
        alert = await asset_tracking_service.acknowledge_alert(
...
async def resolve_asset_alert(
    ...
    try:
        alert = await asset_tracking_service.resolve_alert(
