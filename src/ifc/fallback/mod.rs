...
impl FallbackIFCParser {
    ...
    pub fn parse_ifc_file(&self, file_path: &str) -> Result<(Building, Vec<SpatialEntity>), Box<dyn std::error::Error>> {
    ...
    pub fn parse_ifc_file_parallel(&self, file_path: &str) -> Result<(Building, Vec<SpatialEntity>), Box<dyn std::error::Error>> {
    ...
    pub fn parse_ifc_file_with_progress(&self, file_path: &str, progress: ProgressContext) -> Result<(Building, Vec<SpatialEntity>), Box<dyn std::error::Error>> {
    ...
    fn parse_step_content(&self, content: &str) -> Result<(Building, Vec<SpatialEntity>), Box<dyn std::error::Error>> {
    ...
    fn parse_step_content_parallel(&self, content: &str) -> Result<(Building, Vec<SpatialEntity>), Box<dyn std::error::Error>> {
        let entities = self.entity_extractor.extract_entities(content)?;
    ...
    fn parse_step_content_with_progress(&self, content: &str, progress: ProgressContext) -> Result<(Building, Vec<SpatialEntity>), Box<dyn std::error::Error>> {
        ...
        for (i, line) in lines.iter().enumerate() {
            if line.starts_with("#") && line.contains("=") {
                match self.entity_extractor.parse_entity_line(line) {
                    ...
                    None => {
                        // Try error recovery
                        if let Some(recovered) = self.error_recovery.recover_entity(line, "Parse error") {
        ...
        let spatial_entities: Vec<SpatialEntity> = entities
            ...
            .filter(|e| self.spatial_extractor.is_spatial_entity(&e.entity_type))
            .filter_map(|e| {
                self.spatial_extractor.extract_spatial_data(e)
