...
class DrawingView:
    ...
    def is_point_in_view(self, point: PrecisionPoint) -> bool:
        ...
        return (
            0 <= transformed_point.x <= self.width
            and 0 <= transformed_point.y <= self.height
        )
...
class ViewGenerator:
    ...
    def generate_front_view(
        self, assembly_data: Dict[str, Any], config: ViewConfig
    ) -> DrawingView:
        """Generate a front view of an assembly."""
        view = DrawingView(view_id="front_view", name="Front View", config=config)
    ...
    def generate_top_view(
        ...
        view = DrawingView(view_id="top_view", name="Top View", config=config)
    ...
    def generate_side_view(
        ...
        view = DrawingView(view_id="side_view", name="Side View", config=config)
    ...
    def generate_isometric_view(
        ...
        view = DrawingView(
            view_id="isometric_view", name="Isometric View", config=config
        )
    ...
    def generate_section_view(
        ...
        view = DrawingView(view_id="section_view", name="Section View", config=config)
    ...
    def _get_section_projection_matrix(
        self, section_plane: Dict[str, Any]
    ) -> List[List[Decimal]]:
...
class DrawingViewManager:
    ...
    def create_view(
        self, name: str, view_type: ViewType, config: Optional[ViewConfig] = None
    ) -> str:
        ...
        view = DrawingView(view_id=view_id, name=name, config=config)
    ...
    def generate_standard_views(self, assembly_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        front_view = self.view_generator.generate_front_view(
            assembly_data, front_config
        )
    ...
    def generate_isometric_view(self, assembly_data: Dict[str, Any]) -> str:
        ...
        isometric_view = self.view_generator.generate_isometric_view(
            assembly_data, isometric_config
        )
    ...
    def generate_section_view(
        self, assembly_data: Dict[str, Any], section_plane: Dict[str, Any]
    ) -> str:
        """Generate a section view for an assembly."""
        section_config = ViewConfig(view_type=ViewType.SECTION)
        section_view = self.view_generator.generate_section_view(
            assembly_data, section_plane, section_config
        )
    ...
    def get_views_by_type(self, view_type: ViewType) -> List[DrawingView]:
        ...
        return [
            view for view in self.views.values() if view.config.view_type == view_type
        ]
    ...
    def export_data(self) -> Dict[str, Any]:
        ...
        return {
            "views": {view_id: view.to_dict() for view_id, view in self.views.items()},
            "projection_type": self.projection_type.value,
...
def create_drawing_view(
    view_id: str, name: str, view_type: ViewType, config: Optional[ViewConfig] = None
) -> DrawingView:
    """Create a drawing view."""
    if config is None:
        config = ViewConfig(view_type=view_type)

    return DrawingView(view_id=view_id, name=name, config=config)
