...
class SQLAlchemyUserRepository(BaseRepository[User, UserModel], UserRepository):
    ...
    def get_by_id(self, user_id: UserId) -> Optional[User]:
        ...
        try:
            model = (
                ...
                .filter(
                    and_(UserModel.id == user_id.value, UserModel.deleted_at.is_(None))
                )
                .first()
            )
    ...
    def get_by_email(self, email: str) -> Optional[User]:
        ...
        try:
            model = (
                self.session.query(UserModel)
                .filter(and_(UserModel.email == email, UserModel.deleted_at.is_(None)))
                .first()
            )
    ...
    def get_by_role(self, role: UserRole) -> List[User]:
        ...
        try:
            models = (
                self.session.query(UserModel)
                .filter(and_(UserModel.role == role, UserModel.deleted_at.is_(None)))
                .order_by(UserModel.username)
                .all()
            )
    ...
    def get_active_users(self) -> List[User]:
        ...
        try:
            models = (
                self.session.query(UserModel)
                .filter(
                    and_(UserModel.is_active == True, UserModel.deleted_at.is_(None))
                )
                .order_by(UserModel.username)
                .all()
            )
