...
class BuildingAggregate:
    ...
    def update_address(self, new_address: Address, updated_by: str) -> None:
        ...
        event = BuildingLocationChangedEvent(
            building_id=str(self.id),
            old_coordinates=(
                old_address.coordinates
                if old_address.coordinates
                else Coordinates(0, 0)
            ),
            new_coordinates=(
                new_address.coordinates
                if new_address.coordinates
                else Coordinates(0, 0)
            ),
    ...
    def update_status(
        self, new_status: Status, updated_by: str, reason: Optional[str] = None
    ) -> None:
        """
    ...
    def update_cost(
        self, new_cost: Money, updated_by: str, reason: Optional[str] = None
    ) -> None:
        """
    ...
    def delete(self, deleted_by: str, reason: Optional[str] = None) -> None:
        ...
        event = BuildingDeletedEvent(
            building_id=str(self.id), deleted_by=deleted_by, deletion_reason=reason
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cost": {"amount": self.cost.amount, "currency": self.cost.currency},
    ...
    def create(
        ...
        building = Building.create(
            name=name, address=address, dimensions=dimensions, metadata=metadata or {}
        )
