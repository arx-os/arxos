...
"#;
    ...
    fn test_arxos_load_ar_model_invalid_format() {
        unsafe {
            ...
            assert!(parsed.get("error").is_some() || !parsed.get("success").unwrap_or(&serde_json::Value::Bool(false)).as_bool().unwrap_or(false));
    ...
    fn test_arxos_load_ar_model_with_temp_file() {
        ...
        unsafe {
            ...
            if parsed.get("success").and_then(|s| s.as_bool()).unwrap_or(false) {
    ...
    fn test_arxos_load_ar_model_with_custom_path() {
        ...
        unsafe {
            ...
            if parsed.get("success").and_then(|s| s.as_bool()).unwrap_or(false) {
                assert_eq!(parsed["file_path"].as_str().unwrap(), output_path.to_str().unwrap());
    ...
    fn test_arxos_list_pending_equipment_empty() {
        ...
        unsafe {
            ...
            if parsed.get("success").and_then(|s| s.as_bool()).unwrap_or(false) {
                ...
                let items = parsed.get("items").and_then(|i| i.as_array()).unwrap_or(&empty_vec);
        ...
        unsafe {
            ...
            if save_parsed.get("success").and_then(|s| s.as_bool()).unwrap_or(false) {
                ...
                if !pending_ids.is_empty() {
                    ...
                    if confirm_parsed.get("success").and_then(|s| s.as_bool()).unwrap_or(false) {
        ...
        unsafe {
            ...
            if save_parsed.get("success").and_then(|s| s.as_bool()).unwrap_or(false) {
                ...
                if !pending_ids.is_empty() {
                    ...
                    let reject_parsed: serde_json::Value = serde_json::from_str(reject_json_str).unwrap();
                    
                    // Should succeed if pending ID exists
                    if reject_parsed.get("success").and_then(|s| s.as_bool()).unwrap_or(false) {
