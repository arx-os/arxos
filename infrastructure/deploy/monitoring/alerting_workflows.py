...
class MonitoringAlertingWorkflows:
    ...
    async def _create_alert(
        self, metric: MetricData, threshold: float, severity: AlertSeverity
    ):
    ...
    async def resolve_alert(
        self, alert_id: str, resolved_by: str, resolution_notes: str = None
    ) -> bool:
        """
        Resolve an alert.
    ...
    async def _update_metrics_tracking(self):
        ...
        while True:
            try:
                ...
                if total_alerts > 0:
                    self.metrics_tracking["alert_resolution_rate"] = (
                ...
                if response_times:
                    self.metrics_tracking["average_response_time"] = sum(
                        response_times
                    ) / len(response_times)
    ...
    def get_metrics(
        ...
        for metric in self.metrics.values():
            if (name is None or metric.name == name) and (
                source is None or metric.source == source
            ):
