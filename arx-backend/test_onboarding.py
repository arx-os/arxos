#!/usr/bin/env python3
"""
Arx Backend Onboarding Test Script

This script verifies that the development environment is correctly set up
and operational for the Arx Backend service.
"""
...
def check_go_version():
    ...
    if version:
        ...
        if "go1.23" in version or "go1.24" in version:
            return True
        else:
            print("   ⚠️  Warning: Go 1.23+ is recommended")
            return False
    return False
...
def check_build():
    ...
    return run_command("go build -o arx-backend cmd/server/main.go", "Building application") is not None


def main():
    """Run all onboarding checks."""
    print("🚀 Arx Backend Onboarding Test")
    print("=" * 50)
    ...
    if passed == total:
        ...
        print("2. Visit http://localhost:8080/health to verify the server")
        print("3. Check the API documentation at http://localhost:8080/docs")
        ...
        print("4. Check the ONBOARDING.md file for detailed instructions")
