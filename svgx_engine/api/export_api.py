    ...
    try:
        ...
        if request.config is None:
            request.config = create_export_config(
                format=request.format, quality=ExportQuality.MEDIUM
            )
...
async def get_export_history(limit: int = 50):
    ...
    try:
        history = (
            export_system.export_history[-limit:]
            if export_system.export_history
            else []
        )
...
async def get_export_statistics():
    ...
    try:
        ...
        if history:
            average_export_time = sum(result.export_time for result in history) / len(
                history
            )
        else:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get export statistics: {str(e)}"
        )
...
async def validate_export_data(request: ValidateExportRequest):
    ...
    try:
        # Basic validation
        if not request.data:
            return ValidateExportResponse(valid=False, message="No data provided")
        ...
        if request.format == ExportFormat.IFC:
            if "elements" not in request.data:
                return ValidateExportResponse(
                    valid=False, message="IFC export requires 'elements' field"
                )
        elif request.format == ExportFormat.GLTF:
            if "elements" not in request.data:
                return ValidateExportResponse(
                    valid=False, message="GLTF export requires 'elements' field"
                )
        elif request.format == ExportFormat.DXF:
            if "elements" not in request.data:
                return ValidateExportResponse(
                    valid=False, message="DXF export requires 'elements' field"
                )
        elif request.format == ExportFormat.STEP:
            if "elements" not in request.data:
                return ValidateExportResponse(
                    valid=False, message="STEP export requires 'elements' field"
                )
        elif request.format == ExportFormat.IGES:
            if "elements" not in request.data:
                return ValidateExportResponse(
                    valid=False, message="IGES export requires 'elements' field"
                )

        return ValidateExportResponse(valid=True, message="Data validation passed")

    except Exception as e:
        logger.error(f"Validation failed: {e}")
        return ValidateExportResponse(
            valid=False, message=f"Validation failed: {str(e)}"
        )
            ...
            try:
                ...
                if export_request.config is None:
                    export_request.config = create_export_config(
                        format=export_request.format, quality=ExportQuality.MEDIUM
                    )
...
async def start_export_job(
    request: ExportDataRequest, background_tasks: BackgroundTasks
):
...
async def get_export_progress(job_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get export progress: {str(e)}"
        )
...
async def cancel_export_job(job_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to cancel export job: {str(e)}"
        )
...
async def get_export_system_info():
    ...
    try:
        return {
            ...
            "active_jobs": len(
                [job for job in export_jobs.values() if job["status"] == "running"]
            ),
...
if __name__ == "__main__":
    ...
    # Run the application
    uvicorn.run(
        "export_api:app", host="0.0.0.0", port=8001, reload=True, log_level="info"
    )
