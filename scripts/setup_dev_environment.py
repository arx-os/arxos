...
class DevelopmentEnvironmentSetup:
    ...
    def load_configuration(self) -> None:
        ...
        try:
            ...
            raise ConfigurationError("config_loading", f"Failed to load configuration: {e}")
    ...
    def validate_application_services(self) -> None:
        ...
        try:
            from application.factory import (
                get_building_service, get_device_service, get_room_service,
                get_user_service, get_project_service
            )
            ...
            for service_name, service_factory in services:
                try:
                    ...
                    self.logger.info(f"Application service {service_name} created successfully")
                except Exception as e:
                    self.logger.error(f"Application service {service_name} creation failed: {e}")
                    raise
        ...
        try:
            ...
            for var_name, default_value in env_vars.items():
                if not os.getenv(var_name):
                    ...
                    self.logger.info(f"Set environment variable: {var_name}={default_value}")
