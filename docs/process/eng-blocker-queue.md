...
**Pin baseline:** `v2.0.0-pilot.5` @ `ad5213dca08cef52cc90d9b80037f0dbaaa14a8d`  
**Rule:** Field evidence before features · CLI honesty · no spine rewrite · LOC minimal.  
**Note:** E1–E3 remain optional polish; day-1 field does not depend on them.
...
Resource refuse already names `ARX_MAX_*` and `docs/reference/resource-limits.md`.  
...
+            println!(
+                "  Tip: copy warning [codes] into docs/pilot/field-truth-log.md §A2 (LossReport evidence)."
+            );
+        }
...
+            "{kind} file too large for pilot defaults: {} ({} bytes) exceeds limit {} bytes. \
+             Re-export a lighter model, use a stronger capture node, or raise {} (see docs/reference/resource-limits.md).",
             path.display(),
             len,
-            max_bytes
+            max_bytes,
...
+mapped into Arx rooms/equipment). Copy codes into docs/pilot/field-truth-log.md §A2.
+
+Size: pilot default max IFC size is ARX_MAX_IFC_BYTES (see docs/reference/resource-limits.md).
+
