...
class AIAgent:
    ...
    async def process_query(
        ...
        try:
            self.logger.info(
                f"Processing AI query for user {user_id}", query=query, model=model
            )
    ...
    async def validate_geometry(
        ...
        try:
            self.logger.info(
                f"Validating geometry for user {user_id}",
    ...
    async def process_voice_input(
        self, audio_data: str, user_id: str, language: str = "en"
    ) -> AIResponse:
        """Process voice input and convert to text"""
        try:
            self.logger.info(
                f"Processing voice input for user {user_id}", language=language
            )

            # Convert base64 audio to text
            text_result = await self.voice_processor.process_audio(audio_data, language)

            # Process the transcribed text
            nlp_result = await self.nlp_engine.process_query(text_result["text"], {})

            return AIResponse(
                success=True,
                message="Voice input processed successfully",
                data={"transcription": text_result, "nlp_analysis": nlp_result},
                timestamp=datetime.utcnow(),
            )
    ...
    async def execute_task(
        ...
        try:
            self.logger.info(
                f"Executing agent task for user {user_id}",
                task=task,
