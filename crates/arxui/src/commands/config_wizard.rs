...
impl ConfigWizardState {
    ...
    fn validate_field(&mut self, field_idx: usize) {
        ...
        match section {
            ConfigSection::User => match field.label.as_str() {
                "Email" => {
            ...
            ConfigSection::Performance => match field.label.as_str() {
    ...
    fn apply_field_to_config(&mut self, field_idx: usize) {
        ...
        match section {
            ...
            ConfigSection::Path => match field.label.as_str() {
                "Default Import Path" => {
            ...
            ConfigSection::Building => match field.label.as_str() {
...
fn render_header<'a>(state: &'a ConfigWizardState, theme: &'a Theme) -> Paragraph<'a> {
...
/// Render fields list
fn render_fields<'a>(state: &'a ConfigWizardState, _area: Rect, theme: &'a Theme) -> List<'a> {
    let items: Vec<ListItem> = state
        ...
        .map(|(idx, field)| {
            ...
            let status_icon = if field.is_valid { "✓" } else { "✗" };
...
fn render_preview<'a>(
    ...
    if state.config.building.default_coordinate_system
        != state.original_config.building.default_coordinate_system
    {
        lines.push(Line::from(vec![
            Span::styled("Coordinate System: ", Style::default().fg(theme.muted)),
            Span::styled(
                state.config.building.default_coordinate_system.clone(),
                Style::default().fg(theme.text),
            ),
        ]));
    }

    if state.config.performance.max_parallel_threads
        != state.original_config.performance.max_parallel_threads
    {
        lines.push(Line::from(vec![
            Span::styled("Max Threads: ", Style::default().fg(theme.muted)),
            Span::styled(
                state.config.performance.max_parallel_threads.to_string(),
                Style::default().fg(theme.text),
            ),
        ]));
    }
...
/// Render footer
fn render_footer<'a>(theme: &'a Theme, has_changes: bool, editing: bool) -> Paragraph<'a> {
        ...
        terminal.terminal().draw(|frame| {
            ...
            if editing_mode {
                let edit_text = if edit_buffer.is_empty() {
                    ...
                    format!(
                        "Editing {}: {}_",
                        state.fields[state.selected_field].label, edit_buffer
                    )
                };
