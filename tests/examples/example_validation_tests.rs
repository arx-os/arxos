...
fn test_example_buildings_are_valid() {
    ...
    for file_name in example_files {
        ...
        // Parse YAML
        let building_data: BuildingData = serde_yaml::from_str(&content).unwrap_or_else(|e| {
            panic!(
        ...
        // Validate structure
        assert!(
            !building_data.building.name.is_empty(),
            "Building name should not be empty in {}",
            file_name
        );
        assert!(
            !building_data.building.id.is_empty(),
            "Building ID should not be empty in {}",
            file_name
        );

        // Validate metadata
        assert_eq!(
            building_data.metadata.units, "meters",
            "Units should be meters in {}",
            file_name
        );
        assert!(
            !building_data.metadata.coordinate_system.is_empty(),
            "Coordinate system should not be empty in {}",
            file_name
        );

        // Validate coordinate systems
        assert!(
            !building_data.coordinate_systems.is_empty(),
            "Should have at least one coordinate system in {}",
            file_name
        );
        ...
        for floor in &building_data.floors {
            assert!(
                !floor.id.is_empty(),
                "Floor ID should not be empty in {}",
                file_name
            );
            assert!(
                !floor.name.is_empty(),
                "Floor name should not be empty in {}",
                file_name
            );
            ...
            for wing in &floor.wings {
                for room in &wing.rooms {
                    assert!(
                        !room.id.is_empty(),
                        "Room ID should not be empty in {}",
                        file_name
                    );
                    assert!(
                        !room.name.is_empty(),
                        "Room name should not be empty in {}",
                        file_name
                    );
                }
            }
            ...
            for equipment in &floor.equipment {
                assert!(
                    ...
                    "Equipment ID should not be empty in {}",
                    file_name
                );
                assert!(
                    !equipment.name.is_empty(),
                    "Equipment name should not be empty in {}",
                    file_name
                );
                // equipment_type is an enum, check if it's Other("")
                assert!(
                    !equipment.path.is_empty(),
                    "Equipment path should not be empty in {}",
                    file_name
                );
            }
        }
...
fn test_example_metadata_consistency() {
    ...
    for file_name in example_files {
        ...
        let total_rooms: usize = building_data
            .floors
            .iter()
            .flat_map(|f| f.wings.iter())
            .map(|w| w.rooms.len())
            .sum();
        let total_equipment: usize = building_data.floors.iter().map(|f| f.equipment.len()).sum();
...
fn test_complete_building_has_equipment() {
    ...
    let building_data: BuildingData =
        serde_yaml::from_str(&content).expect("Failed to parse building.yaml");
