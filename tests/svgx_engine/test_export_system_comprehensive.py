...
class TestComprehensiveExportSystem(unittest.TestCase):
    ...
    def test_all_export_formats(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for format in formats:
                ...
                self.assertTrue(result.success, f"Export to {format.value} failed: {result.error_message}")
    ...
    def test_export_configurations(self):
        ...
        configs = [
            create_export_config(ExportFormat.IFC, ExportQuality.STANDARD, True, True, True, True),
            create_export_config(ExportFormat.IFC, ExportQuality.STANDARD, False, True, True, True),
            create_export_config(ExportFormat.IFC, ExportQuality.STANDARD, True, False, True, True),
            create_export_config(ExportFormat.IFC, ExportQuality.STANDARD, True, True, False, True),
            create_export_config(ExportFormat.IFC, ExportQuality.STANDARD, True, True, True, False),
        ]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for i, config in enumerate(configs):
                ...
                self.assertTrue(result.success)
                self.assertEqual(result.metadata["export_config"]["include_metadata"], config.include_metadata)
                self.assertEqual(result.metadata["export_config"]["include_properties"], config.include_properties)
                self.assertEqual(result.metadata["export_config"]["include_materials"], config.include_materials)
                self.assertEqual(result.metadata["export_config"]["include_dimensions"], config.include_dimensions)
    ...
    def test_export_statistics(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            formats = [ExportFormat.IFC, ExportFormat.GLTF, ExportFormat.DXF, ExportFormat.STEP, ExportFormat.IGES]
    ...
    def test_export_history_tracking(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i in range(3):
                ...
                self.export_system.export_data(self.test_data, output_path, ExportFormat.IFC)

        self.assertEqual(len(self.export_system.export_history), initial_history_length + 3)
    ...
    def test_export_with_empty_data(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for format in [ExportFormat.IFC, ExportFormat.GLTF, ExportFormat.DXF, ExportFormat.STEP, ExportFormat.IGES]:
    ...
    def test_export_file_formats(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = self.export_system.export_data(self.test_data, output_path, ExportFormat.IFC)
            self.assertTrue(result.success)
            ...
            result = self.export_system.export_data(self.test_data, output_path, ExportFormat.GLTF)
            self.assertTrue(result.success)
            ...
            result = self.export_system.export_data(self.test_data, output_path, ExportFormat.DXF)
            self.assertTrue(result.success)
    ...
    def test_export_performance(self):
        ...
        for i in range(100):
            element = {
                "type": "wall" if i % 4 == 0 else "window" if i % 4 == 1 else "door" if i % 4 == 2 else "column",
                "name": f"Element_{i}",
                "x": float(i),
                "y": float(i * 2),
                "z": 0.0,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for format in [ExportFormat.IFC, ExportFormat.GLTF, ExportFormat.DXF]:
                ...
                self.assertTrue(result.success)
                self.assertGreater(result.file_size, 0)
                self.assertLess(result.export_time, 10.0)  # Should complete within 10 seconds
