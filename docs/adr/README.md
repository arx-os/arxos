...
Accepted architecture and product-surface decisions. On conflict with other docs, **ADRs win for their stated scope**. See also [`../INDEX.md`](../INDEX.md).

## Index (current)

| ID | Doc | Summary |
| :--- | :--- | :--- |
| **0001 / D13** | [0001-identity-and-addressing.md](./0001-identity-and-addressing.md) | Hierarchical `address` primary ops; GlobalId provenance; postal roots; IFC round-trip |
| **D9** | [web-demotion.md](./web-demotion.md) | Interactive WASM/PWA abandoned; web = static landing only |
| **D10** | [capture-model.md](./capture-model.md) | Proposed-first capture; geometry as input |
| **D11** | [native-capture-interface.md](./native-capture-interface.md) | Native iOS ↔ agent thin hand-off (v1 = file LiDAR) |
| **D12** | [repo-structure.md](./repo-structure.md) | Core (`arxos`) vs `ios` separate repos |

**Design detail (not binding alone):** [`../reference/identity-and-addressing.md`](../reference/identity-and-addressing.md) · code map: [`../reference/identity.md`](../reference/identity.md)

**Historical note:** Decisions 5–7 below assumed a Leptos PWA field/owner UI. Their **implementation history** remains; their **product role as the field device path** is superseded by Decision 9.
...
- **Decision:** See full record — [`0001-identity-and-addressing.md`](./0001-identity-and-addressing.md). Summary: `arx_address` is primary operational identity; `ifc_global_id` is provenance only; UUID is internal; dots legal in segments; building roots are fully qualified `bldg.<country>.<region>.<city>.…`; IFC export must update-or-create with honesty.
...
- **Full record:** [`adr-web-demotion.md`](web-demotion.md)
...
- **Full record:** [`adr-capture-model.md`](capture-model.md)
...
- **Full record:** [`adr-native-capture-interface.md`](native-capture-interface.md)
...
- **Full record:** [`adr-repo-structure.md`](repo-structure.md)
