...
class AdvancedRuleEngine:
    ...
    async def evaluate_rules(
        self, context: Dict[str, Any], rule_types: Optional[List[RuleType]] = None
    ) -> List[RuleResult]:
        """
        ...
        try:
            with self._lock:
                ...
                for priority in priority_order:
                    priority_rules = [
                        r for r in rules_to_evaluate if r.priority == priority
                    ]
    ...
    def _get_rules_to_evaluate(
        self, rule_types: Optional[List[RuleType]] = None
    ) -> List[Rule]:
    ...
    def _evaluate_conditions(
        self, conditions: List[RuleCondition], condition_results: List[bool]
    ) -> bool:
        ...
        for i in range(1, len(conditions)):
            ...
            if condition.logical_operator == "AND":
                ...
                logger.warning(
                    f"Unknown logical operator: {condition.logical_operator}"
                )
                result = result and condition_result
    ...
    async def _execute_actions(
        self, actions: List[RuleAction], context: Dict[str, Any]
    ) -> List[str]:
    ...
    def _update_performance_stats(
        self, results: List[RuleResult], start_time: datetime
    ):
        """Update performance statistics."""
        ...
        if total_evaluations > 0:
            ...
            self.performance_stats["average_evaluation_time"] = (
                current_avg * (total_evals - total_evaluations) + total_time
            ) / total_evals
