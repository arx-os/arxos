...
class ThermalAnalysisService:
    ...
    def analyze_thermal_behavior(
        ...
        try:
            ...
            if self.has_advanced_features and self._should_use_advanced_features(
                request
            ):
            ...
            result.max_temperature = (
                max(result.temperature_field) if result.temperature_field else 0.0
            )
            result.min_temperature = (
                min(result.temperature_field) if result.temperature_field else 0.0
            )
    ...
    def _conduction_analysis(
        ...
        thermal_gradients = self._calculate_thermal_gradients(
            temperature_field, request.mesh
        )
    ...
    def _convection_analysis(
        ...
        # Calculate heat flux field
        heat_flux_field = self._calculate_convection_heat_flux(
            request, temperature_field
        )
    ...
    def _radiation_analysis(
        ...
        # Calculate heat flux field
        heat_flux_field = self._calculate_radiation_heat_flux(
            request, temperature_field
        )
    ...
    def _solve_heat_conduction(self, request: ThermalAnalysisRequest) -> List[float]:
        ...
        for iteration in range(100):
            temperature_correction = self._temperature_correction_conduction(
                temperature_field, request
            )
    ...
    def _solve_convection_heat_transfer(
        self, request: ThermalAnalysisRequest
    ) -> List[float]:
        """Solve convection heat transfer equation."""
        n_nodes = len(request.mesh)
        ...
        for iteration in range(150):
            temperature_correction = self._temperature_correction_convection(
                temperature_field, request
            )
            temperature_field += temperature_correction
    ...
    def _solve_radiation_heat_transfer(
        self, request: ThermalAnalysisRequest
    ) -> List[float]:
        """Solve radiation heat transfer equation."""
        n_nodes = len(request.mesh)
        ...
        for iteration in range(200):
            temperature_correction = self._temperature_correction_radiation(
                temperature_field, request
            )
            temperature_field += temperature_correction
    ...
    def _solve_combined_heat_transfer(
        self, request: ThermalAnalysisRequest
    ) -> List[float]:
        """Solve combined heat transfer equation."""
    ...
    def _calculate_heat_flux(
        self, request: ThermalAnalysisRequest, temperature_field: List[float]
    ...
    def _calculate_convection_heat_flux(
        self, request: ThermalAnalysisRequest, temperature_field: List[float]
    ) -> List[Tuple[float, float, float]]:
    ...
    def _calculate_radiation_heat_flux(
        self, request: ThermalAnalysisRequest, temperature_field: List[float]
    ) -> List[Tuple[float, float, float]]:
    ...
    def _calculate_combined_heat_flux(
        ...
        for i in range(len(temperature_field)):
            combined_x = (
                conduction_flux[i][0] + convection_flux[i][0] + radiation_flux[i][0]
            ) / 3
            combined_y = (
                conduction_flux[i][1] + convection_flux[i][1] + radiation_flux[i][1]
            ) / 3
            combined_z = (
                conduction_flux[i][2] + convection_flux[i][2] + radiation_flux[i][2]
            ) / 3
            combined_flux.append((combined_x, combined_y, combined_z))

        return combined_flux

    def _calculate_thermal_gradients(
        self, temperature_field: List[float], mesh: List[Dict[str, Any]]
    ) -> List[Tuple[float, float, float]]:
        """Calculate thermal gradients."""
        gradients = []
    ...
    def _calculate_thermal_stress(
        self, request: ThermalAnalysisRequest, result: ThermalAnalysisResult
        ...
        for i, temp in enumerate(result.temperature_field):
            ...
            thermal_stress.append(
                (sigma_xx, sigma_yy, sigma_zz, sigma_xy, sigma_xz, sigma_yz)
            )

        return thermal_stress

    def _temperature_correction_conduction(
        self, temperature_field: np.ndarray, request: ThermalAnalysisRequest
    ) -> np.ndarray:
        """Calculate temperature correction for conduction."""
        # Simplified temperature correction
        return np.zeros_like(temperature_field)

    def _temperature_correction_convection(
        self, temperature_field: np.ndarray, request: ThermalAnalysisRequest
    ) -> np.ndarray:
    ...
    def _temperature_correction_radiation(
        self, temperature_field: np.ndarray, request: ThermalAnalysisRequest
    ) -> np.ndarray:
    ...
    def _calculate_heat_transfer_rate(self, result: ThermalAnalysisResult) -> float:
        ...
        return (
            total_flux / len(result.heat_flux_field) if result.heat_flux_field else 0.0
        )

    def _find_nodes_in_boundary(
        self,
        boundary_location: List[Tuple[float, float, float]],
        mesh: List[Dict[str, Any]],
    ) -> List[int]:
    ...
    def _should_use_advanced_features(self, request: ThermalAnalysisRequest) -> bool:
        ...
        if hasattr(request, "materials") and request.materials:
            for material in request.materials.values():
                if (
                    hasattr(material, "temperature_dependent")
                    and material.temperature_dependent
                ):
                    return True
        ...
        if hasattr(request, "boundary_conditions") and request.boundary_conditions:
            for bc in request.boundary_conditions:
                if hasattr(bc, "type") and bc.type in [
                    "time_varying",
                    "non_linear",
                    "phase_change",
                ]:
    ...
    def _perform_advanced_analysis(
        self, request: ThermalAnalysisRequest, start_time: float
    ) -> ThermalAnalysisResult:
        ...
        try:
            ...
            result = ThermalAnalysisResult(
                ...
                thermal_gradients=[],
                heat_transfer_rate=self._calculate_heat_transfer_rate_from_advanced(
                    advanced_results
                ),
                max_temperature=(
                    max(advanced_results["final_temperature"])
                    if advanced_results["final_temperature"]
                    else 0.0
                ),
                min_temperature=(
                    min(advanced_results["final_temperature"])
                    if advanced_results["final_temperature"]
                    else 0.0
                ),
                thermal_stress=[],
    ...
    def _convert_to_advanced_request(
        ...
        if hasattr(request, "boundary_conditions") and request.boundary_conditions:
            for bc in request.boundary_conditions:
                bc_type = (
                    BoundaryConditionType(bc.type.value)
                    if hasattr(bc, "type")
                    else BoundaryConditionType.TEMPERATURE
                )
    ...
    def _calculate_heat_transfer_rate_from_advanced(
        self, advanced_results: Dict[str, Any]
    ) -> float:
