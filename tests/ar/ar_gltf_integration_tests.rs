...
use arxos::ar_integration::processing::{validate_ar_scan_data, ARScanData, DetectedEquipmentData};
...
fn test_batch_operations() {
    ...
    let id1 = manager
        .add_pending_equipment(&detected1, "scan_1", 1, None, 0.8, None)
        .unwrap()
        .unwrap();
    let id2 = manager
        .add_pending_equipment(&detected2, "scan_1", 1, None, 0.8, None)
        .unwrap()
        .unwrap();
...
fn test_ar_scan_validation_valid() {
    let valid_scan = ARScanData {
        detected_equipment: vec![DetectedEquipmentData {
        ...
        floor_level: Some(1),
        room_name: Some("Validation Room".to_string()),
    };
...
fn test_ar_scan_validation_invalid() {
    ...
    let invalid_position = ARScanData {
        ...
        floor_level: Some(2),
        room_name: Some("Invalid Position".to_string()),
    };
    ...
    let invalid_confidence = ARScanData {
        ...
        floor_level: Some(2),
        room_name: Some("Invalid Confidence".to_string()),
    };
...
fn create_realistic_test_building() -> BuildingData {
    fn equipment_position(point: &Point3D) -> Position {
        Position {
            x: point.x,
            y: point.y,
            z: point.z,
    ...
    fn make_equipment(
        ...
        Equipment {
            ...
            address: None,
            equipment_type,
            position: equipment_position(&position),
            properties: HashMap::new(),
    ...
    ground_floor.equipment.push(make_equipment(
        "vav-001",
        "VAV-001",
        EquipmentType::HVAC,
        Point3D {
        ...
        "Building::Integration::Floor::1::Equipment::VAV-001",
    ));
    ...
    ground_floor.equipment.push(make_equipment(
        ...
        "Building::Integration::Floor::1::Equipment::Panel-A",
    ));
    ...
    second_floor.equipment.push(make_equipment(
        "switch-01",
        "Network Switch 01",
        ...
        "Building::Integration::Floor::2::Equipment::Switch-01",
    ));
    ...
    second_floor.equipment.push(make_equipment(
        ...
        "Building::Integration::Floor::2::Equipment::Water-Heater",
    ));
    ...
    BuildingData {
        ...
        floors: vec![ground_floor, second_floor],
...
fn test_gltf_export_to_file() {
    ...
    let file_contents = fs::read_to_string(&output_path).expect("Should read glTF file");

    // Parse as JSON to verify structure
    let json_value: serde_json::Value =
        serde_json::from_str(&file_contents).expect("glTF file should be valid JSON");

    // Verify required glTF fields exist
    assert!(
        json_value.get("asset").is_some(),
        "glTF should have 'asset' field"
    );
    assert_eq!(
        json_value["asset"]["version"], "2.0",
        "glTF version should be 2.0"
    );
    assert!(
        json_value.get("scenes").is_some(),
        "glTF should have 'scenes' field"
    );
    assert!(
        json_value.get("nodes").is_some(),
        "glTF should have 'nodes' field"
    );
}
...
fn test_gltf_export_material_properties() {
    ...
    for material in materials {
        ...
        assert!(
            pbr.get("baseColorFactor").is_some(),
        ...
        assert_eq!(
            color_array.len(),
            4,
            "baseColorFactor should be RGBA (4 values)"
        );
    }
}
...
fn test_gltf_export_multiple_equipment_same_type() {
    ...
    building.floors[0].equipment.push(Equipment {
        ...
        path: "Building::Integration::Floor::1::Equipment::VAV-002".to_string(),
        address: None,
    ...
    let unique_types: std::collections::HashSet<String> = building
        ...
        .flat_map(|f| {
            f.equipment
                .iter()
                .map(|e| format!("{:?}", e.equipment_type).to_lowercase())
        })
        .collect();
