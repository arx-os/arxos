...
use crate::identity::{get_key_fingerprint_for_email, is_gpg_available};
use crate::identity::{PendingUserRegistry, User, UserRegistry};
    ...
    for user in users {
        ...
        if user.verified {
            ...
            if let Some(ref verified_at) = user.verified_at {
                println!(
                    "   Verified at: {}",
                    verified_at.format("%Y-%m-%d %H:%M:%S UTC")
                );
            }
        }
        ...
        if let Some(ref last_active) = user.last_active {
            println!(
                "   Last active: {}",
                last_active.format("%Y-%m-%d %H:%M:%S UTC")
            );
        }
        println!();
    }
