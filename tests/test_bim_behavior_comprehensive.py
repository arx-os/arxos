...
class TestBIMBehaviorComprehensive:
    ...
    async def test_advanced_state_machine(self):
        ...
        print(
            f"✅ Executed {stats['processing_stats']['total_transitions']} transitions"
        )
    ...
    async def test_conditional_logic_engine(self):
        ...
        threshold_result = await self.logic_engine.evaluate_condition(
            "temperature_threshold", context={"temperature": 35.0}
        )
        ...
        complex_result = await self.logic_engine.evaluate_condition(
            "complex_condition",
            context={"temperature": 30.0, "humidity": 65.0, "power_level": 85.0},
        )
        ...
        print(
            f"✅ Evaluated {stats['processing_stats']['total_evaluations']} conditions"
        )
    ...
    async def test_physics_engine_integration(self):
        ...
        electrical_result = self.physics_engine.electrical.calculate_power_flow(
            electrical_data
        )
        ...
        structural_result = self.physics_engine.structural.calculate_beam_analysis(
            structural_data
        )
    ...
    async def test_comprehensive_bim_behavior_scenario(self):
        ...
        temp_result = await self.logic_engine.evaluate_condition(
            "temperature_threshold", context={"temperature": 28.0}
        )
        ...
        if temp_result.result:
            ...
            success = await self.state_machine.execute_transition(
                hvac_element_id,
                "equipment_on",  # Already on, but update properties
            ...
            cooling_result = self.physics_engine.fluid_dynamics.calculate_air_flow(
                cooling_data
            )
        ...
        print(
            f"✅ State transitions: {state_stats['processing_stats']['total_transitions']}"
        )
        print(
            f"✅ Condition evaluations: {logic_stats['processing_stats']['total_evaluations']}"
        )
    ...
    async def test_performance_optimization(self):
        ...
        print(
            f"✅ Average event processing time: {stats['processing_stats']['average_processing_time']*1000:.2f}ms"
        )
        ...
        for i in range(50):
            await self.logic_engine.evaluate_condition(
                "temperature_threshold", context={"temperature": 25.0 + i}
            )
        ...
        logic_stats = self.logic_engine.get_processing_stats()
        assert (
            logic_stats["processing_stats"]["average_evaluation_time"] < 0.005
        )  # <5ms
        print(
            f"✅ Average condition evaluation time: {logic_stats['processing_stats']['average_evaluation_time']*1000:.2f}ms"
        )
        ...
        for i in range(20):
            ...
            await self.state_machine.execute_transition(
                element_id, "equipment_on", context={"test": True}
            )
        ...
        state_stats = self.state_machine.get_processing_stats()
        assert (
            state_stats["processing_stats"]["average_transition_time"] < 0.01
        )  # <10ms
        print(
            f"✅ Average state transition time: {state_stats['processing_stats']['average_transition_time']*1000:.2f}ms"
        )
    ...
    async def test_error_handling_and_recovery(self):
        ...
        try:
            await self.logic_engine.evaluate_condition(
                "nonexistent_condition", context={}
            )
            assert False, "Should have failed for nonexistent condition"
        except Exception as e:
            print(
                f"✅ Invalid condition evaluation properly handled: {type(e).__name__}"
            )
    ...
    async def test_real_world_bim_scenario(self):
        ...
        temperature_result = await self.logic_engine.evaluate_condition(
            "temperature_threshold", context={"temperature": 32.0}
        )
        ...
        if temperature_result.result:
            ...
            hvac_result = self.physics_engine.fluid_dynamics.calculate_air_flow(
                hvac_data
            )
            assert hvac_result.success
        ...
        fire_condition = await self.logic_engine.evaluate_condition(
            "complex_condition",
            context={"smoke_detected": True, "temperature": 45.0, "occupancy": 10},
        )
        ...
        print(
            f"✅ Total events processed: {event_stats['processing_stats']['total_events']}"
        )
        print(
            f"✅ Total state transitions: {state_stats['processing_stats']['total_transitions']}"
        )
        print(
            f"✅ Total condition evaluations: {logic_stats['processing_stats']['total_evaluations']}"
        )
