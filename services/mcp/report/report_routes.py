...
class ReportGenerationRequest(BaseModel):
    building_id: str
    report_type: str = "comprehensive"  # comprehensive, violation_summary, executive_summary
...
async def generate_report(
    ...
    try:
        ...
        if not validation_data:
            raise HTTPException(
                status_code=404,
                detail="Validation data not found for building"
            )
...
async def delete_report(
    ...
    try:
        ...
        if not result['success']:
            raise HTTPException(
                status_code=404,
                detail=result.get('error', 'Failed to delete report')
            )
