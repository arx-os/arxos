...
impl PendingUserRegistry {
    ...
    pub fn load(repo_path: &Path) -> Result<Self, PendingRegistryError> {
        ...
        info!(
            "Loaded {} pending requests from registry",
            data.requests.len()
        );
    ...
    pub fn find_pending_by_email(&self, email: &str) -> Option<&PendingUserRequest> {
        ...
        self.requests.iter().find(|r| {
            r.email.to_lowercase() == email_lower && r.status == PendingRequestStatus::Pending
        })
    }
    ...
    pub fn approve_request(
        ...
        email: &str,
        approved_by: &str,
    ) -> Result<PendingUserRequest, PendingRegistryError> {
        let request = self
            .requests
            .iter_mut()
        ...
        if request.status != PendingRequestStatus::Pending {
            return Err(PendingRegistryError::RequestAlreadyReviewed(
                request.id.clone(),
            ));
        }
...
mod tests {
    ...
    fn test_pending_registry_find_pending_by_email() {
        ...
        // Approve the request
        registry
            .approve_request("test@example.com", "admin@example.com")
            .unwrap();
    ...
    fn test_pending_registry_approve_already_approved() {
        ...
        // Approve once
        registry
            .approve_request("test@example.com", "admin1@example.com")
            .unwrap();
    ...
    fn test_pending_registry_deny_request() {
        ...
        registry
            .deny_request("test@example.com", "admin@example.com", reason.clone())
            .unwrap();
    ...
    fn test_pending_registry_deny_without_reason() {
        ...
        registry
            .deny_request("test@example.com", "admin@example.com", None)
            .unwrap();
    ...
    fn test_pending_registry_deny_already_denied() {
        ...
        // Deny once
        registry
            .deny_request("test@example.com", "admin1@example.com", None)
            .unwrap();
    ...
    fn test_pending_registry_pending_requests() {
        ...
        // Approve one
        registry
            .approve_request("user1@example.com", "admin@example.com")
            .unwrap();
        ...
        assert!(pending_after
            .iter()
            .all(|r| r.status == PendingRequestStatus::Pending));
    }
    ...
    fn test_pending_registry_save_and_load_with_status() {
        ...
        assert_eq!(
            loaded.find_by_email("user1@example.com").unwrap().status,
            PendingRequestStatus::Approved
        );
        assert_eq!(
            loaded.find_by_email("user2@example.com").unwrap().status,
            PendingRequestStatus::Denied
        );
        assert_eq!(
            loaded.find_by_email("user3@example.com").unwrap().status,
            PendingRequestStatus::Pending
        );
    }
