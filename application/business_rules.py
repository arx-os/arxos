...
from application.exceptions import BusinessRuleError, ValidationError, ApplicationError
        ...
        for rule_name in rules_to_check:
            ...
            try:
                if not rule.validator(data):
                    ...
                    self.logger.debug(
                        f"Business rule '{rule.name}' failed: {rule.error_message}"
                    )
    ...
    def _register_default_rules(self) -> None:
        ...
        self.register_rule(
            BusinessRule(
                ...
                validator=lambda data: data.get("device_type")
                in ["sensor", "controller", "monitor", "actuator"],
        ...
        self.register_rule(
            BusinessRule(
                ...
                validator=lambda data: re.match(
                    r"^[A-Z]?\d+[A-Z]?$", data.get("room_number", "")
                ),
        ...
        self.register_rule(
            BusinessRule(
                ...
                validator=lambda data: self._validate_password_strength(
                    data.get("password", "")
                ),
...
class ValidationEngine:
    ...
    def validate_data(self, data: Dict[str, Any], validators: List[str] = None) -> None:
        ...
        try:
            ...
            raise ValidationError(
                message="Data validation failed", field="general", value=str(e)
            )
...
class BusinessRuleService:
    ...
    def validate_entity_creation(self, entity_type: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Entity creation validation failed for {entity_type}: {e}"
            )
            raise
    ...
    def validate_entity_update(
        ...
        try:
            ...
            self.logger.info(
                f"Entity update validation passed for {entity_type} {entity_id}"
            )

        except Exception as e:
            self.logger.error(
                f"Entity update validation failed for {entity_type} {entity_id}: {e}"
            )
            raise
    ...
    def validate_business_operation(self, operation: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Business operation validation failed for {operation}: {e}"
            )
            raise
    ...
    def _get_operation_rules(self, operation: str) -> List[str]:
        ...
        rule_mapping = {
            ...
            "building_construction": [
                "building_address_valid",
                "building_coordinates_valid",
            ],
        }
