...
class SVGXParser:
    ...
    def __init__(self):
        self.supported_elements = {
            ...
            'arx:object', 'arx:system', 'arx:behavior'
        }
    ...
    def _parse_arx_object(self, element: ET.Element) -> ArxObject:
        ...
        for key, value in element.attrib.items():
            if key.startswith('arx:') and not key in ['arx:object-id', 'arx:object-type', 'arx:system']:
        ...
        if physics_elem is not None:
            ...
            if forces_elem is not None:
                for force_elem in forces_elem.findall('force'):
                    ...
                    value = float(force_elem.get('value', 0)) if force_elem.get('value') else None
