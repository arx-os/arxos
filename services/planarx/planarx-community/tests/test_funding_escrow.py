...
class TestEscrowEngine:
    ...
    def test_deposit_funds(self):
        ...
        success = escrow_engine.deposit_funds(
            escrow_id=escrow_account.id, amount=Decimal("5000"), user_id="backer-1"
        )
    ...
    def test_deposit_funds_activate_escrow(self):
        ...
        escrow_engine.deposit_funds(
            escrow_id=escrow_account.id, amount=Decimal("10000"), user_id="backer-1"
        )
    ...
    def test_submit_milestone(self):
        ...
        assert (
            submit_transaction.transaction_type == TransactionType.MILESTONE_SUBMITTED
        )
    ...
    def test_reject_milestone(self):
        ...
        assert (
            escrow_account.milestones[0].rejection_reason
    ...
    def test_invalid_escrow_id(self):
        ...
        with pytest.raises(ValueError, match="Escrow account invalid-id not found"):
            escrow_engine.deposit_funds(
                escrow_id="invalid-id", amount=Decimal("1000"), user_id="user-1"
            )
...
class TestEscrowAPI:
    ...
    def test_deposit_funds_api(self, client):
        ...
        # Deposit funds
        deposit_data = {"amount": 5000, "user_id": "backer-1"}

        response = client.post(
            f"/api/funding/escrow/{escrow_id}/deposit", json=deposit_data
        )
    ...
    def test_submit_milestone_api(self, client):
        ...
        # Activate escrow
        client.post(
            f"/api/funding/escrow/{escrow_id}/deposit",
            json={"amount": 10000, "user_id": "backer-1"},
        )

        # Get milestone ID from summary
        ...
        response = client.post(
            f"/api/funding/escrow/{escrow_id}/milestones/{milestone_id}/submit",
            json=submit_data,
        )
    ...
    def test_approve_milestone_api(self, client):
        ...
        # Approve milestone
        approve_data = {"approver_id": "board-1", "is_override": False}

        response = client.post(
            f"/api/funding/escrow/{escrow_id}/milestones/{milestone_id}/approve",
            json=approve_data,
        )
