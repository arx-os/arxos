...
class GUSCLI:
    ...
    async def query(self, query: str, user_id: str = "cli_user") -> Dict[str, Any]:
        ...
        try:
            response = await self.client.post(
                ...
                json={"query": query, "user_id": user_id, "context": {"source": "cli"}},
            )
    ...
    async def get_knowledge(
        ...
        try:
            response = await self.client.post(
                ...
                json={"topic": topic, "context": context or {}},
            )
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
    ...
    async def execute_task(
        self, task: str, parameters: Dict[str, Any], user_id: str = "cli_user"
    ) -> Dict[str, Any]:
        """Execute a specific task"""
        try:
            response = await self.client.post(
                f"{self.gus_api_url}/api/v1/task",
                json={"task": task, "parameters": parameters, "user_id": user_id},
            )
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
...
@click.pass_obj
def task(
    gus_cli: GUSCLI, task: str, parameters: tuple, user_id: str, output_format: str
):
    """Execute a specific task"""
