...
class PredictionRequest(BaseModel):
    ...
    model_type: str = Field(
        default="linear_regression", description="Type of prediction model"
    )
    prediction_horizon: int = Field(
        default=30, description="Prediction horizon in days"
    )
...
class InsightRequest(BaseModel):
    ...
    insight_types: List[str] = Field(
        default_factory=list, description="Types of insights to generate"
    )
...
async def get_user_recommendations(
    user_id: str, recommendation_type: Optional[str] = None, active_only: bool = True
):
...
async def process_htmx_request(request: HTMXRequestModel):
    ...
    try:
        # Convert location to tuple if provided
        location_tuple = (
            tuple(request.context.get("location", []))
...
async def get_ai_components(
    component_type: Optional[str] = None, active_only: bool = True
):
    """Get AI components"""
    try:
        component_type_enum = (
            AIComponentType(component_type) if component_type else None
        )

        components = await ai_frontend_service.get_ai_components(
            component_type=component_type_enum, active_only=active_only
        )
...
async def analyze_trends(dataset_id: UUID, variable_name: str):
    ...
    try:
        trend = await ai_analytics_service.analyze_trends(
            dataset_id=dataset_id, variable_name=variable_name
        )
...
async def analyze_correlations(dataset_id: UUID, variable1: str, variable2: str):
    """Analyze correlations between variables"""
    try:
        correlation = await ai_analytics_service.analyze_correlations(
            dataset_id=dataset_id, variable1=variable1, variable2=variable2
        )
...
async def detect_anomalies(
    dataset_id: UUID, variable_name: str, threshold: float = 2.0
):
    """Detect anomalies in dataset"""
    try:
        anomalies = await ai_analytics_service.detect_anomalies(
            dataset_id=dataset_id, variable_name=variable_name, threshold=threshold
        )
...
async def generate_insights(request: InsightRequest):
    ...
    try:
        insight_types = (
            [InsightType(it) for it in request.insight_types]
            if request.insight_types
            else None
        )

        insights = await ai_analytics_service.generate_insights(
            user_id=request.user_id, insight_types=insight_types
        )
