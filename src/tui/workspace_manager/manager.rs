...
use super::{discovery::discover_workspaces, types::Workspace};
...
impl WorkspaceManager {
    ...
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let active_workspace = std::env::current_dir().ok().and_then(|dir| {
...
mod tests {
    ...
    fn test_workspace_manager_active_detection() {
        ...
        let is_active1 = active_workspace
            .as_ref()
            .map(|active| active == &workspace1.path)
            .unwrap_or(false);
        let is_active2 = active_workspace
            .as_ref()
