        ...
        try:
            ...
            for gateway_id in self.gateway_nodes:
                try:
                    ...
                    logger.error(
                        f"Failed to calculate routes from gateway {gateway_id}: {e}"
                    )
...
class ArxLinkSyncProtocol:
            ...
            if response and response.type == MessageType.SYNC_RESPONSE:
                if response.payload.get("status") == "accepted":
                    ...
                    self.topology.update_node(
                        self.node_id, {"hop_count": self.node_info.hop_count}
                    )
    ...
    async def _send_message(self, message: SyncMessage) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Sent message {message.id} to {message.dest_id or 'broadcast'}"
            )
            return True
    ...
    async def _send_message_with_response(
        self, message: SyncMessage, timeout: float = 5.0
        ...
        try:
            ...
            response = await asyncio.wait_for(
                self.pending_responses[message.id], timeout=timeout
            )
        ...
        try:
            ...
            if action == "connect":
                ...
                if can_accept:
                    ...
                    self.topology.update_node(
                        self.node_id, {"children": self.node_info.children}
                    )
    ...
    async def _handle_error(self, message: SyncMessage):
        ...
        try:
            ...
            logger.error(
                f"Received error from {message.source_id}: {error_code} - {error_message}"
            )
    ...
    async def _cleanup_stale_nodes(self):
        ...
        try:
            ...
            for node_id, node in self.topology.nodes.items():
                ...
                if (
                    node.last_seen
                    and (current_time - node.last_seen).total_seconds() > stale_timeout
                ):
    ...
    def _sign_message(self, message: SyncMessage) -> str:
        ...
        signature_data = (
            f"{message.id}:{message.type.value}:{message.source_id}:{message.sequence}"
        )

        # Create HMAC signature
        signature = hmac.new(
            str(self.session_key).encode(), signature_data.encode(), hashlib.sha256
        ).hexdigest()
