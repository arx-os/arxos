...
class ArxPipelineCLI:
    ...
    def list_systems(self):
        ...
        for system in self.supported_systems:
            status = "✅ Available" if self._check_system_availability(system) else "⚠️  Not configured"
    ...
    def validate_system(self, system: str, verbose: bool = False):
        ...
        if verbose:
            print(f"Schema Validation: {'✅ PASS' if schema_result.get('success') else '❌ FAIL'}")

        # Validate symbols
        symbols_result = self.service.handle_operation("validate-symbols", {"system": system})
        if verbose:
            print(f"Symbol Validation: {'✅ PASS' if symbols_result.get('success') else '❌ FAIL'}")

        # Validate behaviors
        behaviors_result = self.service.handle_operation("validate-behavior", {"system": system})
        if verbose:
            print(f"Behavior Validation: {'✅ PASS' if behaviors_result.get('success') else '❌ FAIL'}")

        # Overall result
        all_passed = (
            schema_result.get('success', False) and
            symbols_result.get('success', False) and
            behaviors_result.get('success', False)
        )
    ...
    def execute_pipeline(self, system: str, object_type: Optional[str] = None, dry_run: bool = False):
        ...
        if result.get('success'):
            print(f"✅ Pipeline execution completed successfully in {execution_time:.2f}s")
            if dry_run:
        ...
        if result.get('success'):
            ...
            if executions:
                for execution in executions[:limit]:
                    status_icon = "✅" if execution.get('status') == 'completed' else "🔄" if execution.get('status') == 'running' else "❌"
                    print(f"  {status_icon} {execution.get('id', 'Unknown')} - {execution.get('status', 'Unknown')} - {execution.get('created_at', 'Unknown')}")
            else:
                print("  No executions found")
        else:
            print(f"❌ Failed to list executions: {result.get('error', 'Unknown error')}")
...
def main():
    ...
    parser.add_argument("--list-systems", action="store_true", help="List all supported systems")
    parser.add_argument("--validate", action="store_true", help="Validate a system")
    parser.add_argument("--execute", action="store_true", help="Execute pipeline for a system")
