...
def test_enhanced_physics_engine():
    ...
    try:
        ...
        fluid_result = physics_engine.calculate_physics(
            PhysicsType.FLUID_DYNAMICS, duct_data
        )
        print(f"✅ Fluid dynamics calculation completed: {fluid_result.state.value}")
        print(
            f"   - Pressure drop: {fluid_result.metrics.get('total_pressure_drop', 0):.2f} Pa"
        )
        ...
        electrical_result = physics_engine.calculate_physics(
            PhysicsType.ELECTRICAL, circuit_data
        )
        ...
        print(
            f"   - Power factor: {electrical_result.metrics.get('power_factor', 0):.2f}"
        )
        ...
        structural_result = physics_engine.calculate_physics(
            PhysicsType.STRUCTURAL, beam_data
        )
        print(f"✅ Structural analysis completed: {structural_result.state.value}")
        print(
            f"   - Bending stress: {structural_result.metrics.get('bending_stress', 0):.2f} Pa"
        )
        print(
            f"   - Safety factor: {structural_result.metrics.get('bending_safety_factor', 0):.2f}"
        )
        ...
        thermal_result = physics_engine.calculate_physics(
            PhysicsType.THERMAL, thermal_data
        )
        print(f"✅ Thermal analysis completed: {thermal_result.state.value}")
        print(
            f"   - Heat transfer: {thermal_result.metrics.get('total_heat_transfer', 0):.2f} W"
        )
        ...
        acoustic_result = physics_engine.calculate_physics(
            PhysicsType.ACOUSTIC, acoustic_data
        )
        print(f"✅ Acoustic analysis completed: {acoustic_result.state.value}")
        print(
            f"   - SPL at receiver: {acoustic_result.metrics.get('spl_receiver', 0):.2f} dB"
        )
        print(
            f"   - Reverberation time: {acoustic_result.metrics.get('reverberation_time', 0):.2f} s"
        )

        # Get physics summary
        summary = physics_engine.get_physics_summary()
        print(
            f"✅ Physics summary generated: {summary['total_calculations']} total calculations"
        )
...
def test_physics_bim_integration():
    ...
    try:
        ...
        print(
            f"✅ Integration summary generated: {summary['total_sessions']} active sessions"
        )
...
def main():
    ...
    if physics_success and integration_success:
        print(
            "✅ All tests passed! Enhanced Physics Engine Integration is working correctly."
        )
