...
class Parameter:
    ...
    def _validate_parameter(self) -> None:
        ...
        if self.parameter_type == ParameterType.STRING and not isinstance(self.value, str):
...
class ExpressionEvaluator:
    ...
    def evaluate_expression(self, expression: str, parameters: Dict[str, Parameter]) -> Optional[Decimal]:
    ...
    def _substitute_parameters(self, expression: str, parameters: Dict[str, Parameter]) -> str:
        ...
        for param_name, parameter in parameters.items():
            if parameter.status == ParameterStatus.VALID:
                ...
                if numeric_value is not None:
                    ...
                    processed_expression = re.sub(pattern, str(numeric_value), processed_expression)
    ...
    def _evaluate_math_expression(self, expression: str) -> Decimal:
        ...
        if '+' in expression:
            ...
            return self._evaluate_math_expression(parts[0]) + self._evaluate_math_expression(parts[1])
        elif '-' in expression:
            parts = expression.split('-', 1)
            return self._evaluate_math_expression(parts[0]) - self._evaluate_math_expression(parts[1])
        elif '*' in expression:
...
class ParameterRelationship:
    ...
    def __init__(self, name: str, expression: str, target_parameter: str,
...
class ParametricModel:
    ...
    def remove_parameter(self, parameter_name: str) -> bool:
        ...
        if parameter_name in self.parameters:
            ...
            logger.debug(f"Removed parameter '{parameter_name}' from model '{self.name}'")
            return True
        return False
    ...
    def set_parameter_value(self, parameter_name: str, value: Union[Decimal, str, bool]) -> bool:
    ...
    def remove_relationship(self, relationship_name: str) -> bool:
        ...
        if relationship_name in self.relationships:
            ...
            logger.debug(f"Removed relationship '{relationship_name}' from model '{self.name}'")
            return True
        return False
            ...
            if changed_parameter in relationship.source_parameters:
                ...
                if result is not None:
                    ...
                    if target_param and target_param.parameter_type in [
                        ParameterType.LENGTH, ParameterType.ANGLE, ParameterType.RADIUS,
                        ParameterType.DIAMETER, ParameterType.AREA, ParameterType.VOLUME,
                        ParameterType.COUNT
                    ]:
                        target_param.set_value(result)
                        logger.debug(f"Updated dependent parameter '{target_param.name}' to {result}")
    ...
    def get_model_statistics(self) -> Dict[str, Any]:
        ...
        valid_parameters = sum(1 for p in self.parameters.values() if p.status == ParameterStatus.VALID)
        valid_relationships = sum(1 for r in self.relationships.values() if r.status == ParameterStatus.VALID)
...
class ParametricModelingSystem:
    ...
    def add_relationship_to_model(self, model_name: str, relationship: ParameterRelationship) -> bool:
    ...
    def get_system_statistics(self) -> Dict[str, Any]:
        ...
        total_relationships = sum(len(model.relationships) for model in self.models.values())
...
def create_parameter(name: str, value: Union[float, str, bool],
                    parameter_type: ParameterType, unit: str = "",
...
def create_parameter_relationship(name: str, expression: str, target_parameter: str,
                                source_parameters: List[str]) -> ParameterRelationship:
