...
impl RemoteCommand {
    pub fn execute(self) -> Result<()> {
        match self.action {
            ...
            RemoteAction::Connect { building } => Self::run_client(building, None),
        }
    }
    ...
    fn list_buildings() -> Result<()> {
        ...
        for (name, host) in config.buildings {
            println!(
                "{:<20} {:<30} {:<10} {:<15}",
                name, host.host, host.port, host.user
            );
        }
        Ok(())
    }
    ...
    fn run_client(building_name: String, command: Option<String>) -> Result<()> {
        ...
        println!(
            "Connecting to {} ({}) as {}...",
            building_name, host_addr, user
        );

        // Run async client in blocking runtime
        let runtime = tokio::runtime::Runtime::new()?;
        runtime.block_on(async { connect_and_run(host_addr, user, key_path, command).await })
    }
}
...
async fn connect_and_run(
    ...
    session
        .disconnect(russh::Disconnect::ByApplication, "Bye", "English")
        .await?;
    Ok(())
}
