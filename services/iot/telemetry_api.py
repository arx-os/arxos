...
class TelemetryProcessor:
    ...
    def transform_point(
        self, point: TelemetryPoint, rules: Dict[str, Any]
    ) -> TelemetryPoint:
        """Transform a telemetry point according to rules."""
        try:
            for transformer_name, transformer_config in rules.get(
                "transformers", {}
            ).items():
    ...
    def aggregate_points(
        self, points: List[TelemetryPoint], aggregation_type: str, interval: str
    ) -> List[TelemetryPoint]:
    ...
    def _validate_rate_of_change(
        self, point: TelemetryPoint, config: Dict[str, Any]
    ) -> bool:
    ...
    def _group_by_interval(
        self, points: List[TelemetryPoint], interval: str
    ) -> Dict[datetime, List[TelemetryPoint]]:
        """Group points by time interval."""
...
class TelemetryAPI:
        ...
        try:
            ...
            if rules:
                if not self.processor.validate_point(point, rules):
                    return web.json_response({"error": "Validation failed"}, status=400)
            ...
            if success:
                ...
                return web.json_response({"error": "Failed to store point"}, status=500)
    ...
    async def store_batch(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if success:
                ...
                return web.json_response({"error": "Failed to store batch"}, status=500)
    ...
    async def query_points(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            device_ids = (
                request.query.get("device_ids", "").split(",")
                if request.query.get("device_ids")
                else None
            )
            capability_names = (
                request.query.get("capability_names", "").split(",")
                if request.query.get("capability_names")
                else None
            )
            start_time = (
                datetime.fromisoformat(request.query["start_time"])
                if request.query.get("start_time")
                else None
            )
            end_time = (
                datetime.fromisoformat(request.query["end_time"])
                if request.query.get("end_time")
                else None
            )
            limit = int(request.query["limit"]) if request.query.get("limit") else 1000
            quality_threshold = (
                float(request.query["quality_threshold"])
                if request.query.get("quality_threshold")
                else None
            )
    ...
    async def get_aggregated(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            # Aggregate points
            aggregated_points = self.processor.aggregate_points(
                points, aggregation, interval
            )
    ...
    async def get_stats(self, request: web.Request) -> web.Response:
        ...
        try:
            with sqlite3.connect(self.storage.db_path) as conn:
                ...
                cursor = conn.execute(
                    "SELECT COUNT(DISTINCT capability_name) FROM telemetry"
                )
