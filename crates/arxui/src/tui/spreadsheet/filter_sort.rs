...
use crate::tui::spreadsheet::types::{
    CellValue, ColumnFilter, ColumnSort, FilterCondition, Grid, SortOrder,
};
    ...
    for row_idx in 0..grid.original_row_count {
        ...
        for filter in &grid.filters {
            if let Some(cell) = grid
                .rows
                .get(row_idx)
                .and_then(|r| r.get(filter.column_idx))
            {
...
fn matches_filter(value: &CellValue, condition: &FilterCondition) -> bool {
    match condition {
        ...
        FilterCondition::InRange(min, max) => match value {
        ...
        FilterCondition::IsIn(values) => {
            ...
            values
                .iter()
                .any(|v| v.to_lowercase() == value_str.to_lowercase())
        }
...
mod tests {
    use super::*;
    use crate::tui::spreadsheet::types::{CellType, CellValue, ColumnDefinition};
    ...
    fn test_filter_starts_with() {
        ...
        apply_filter(
            &mut grid,
            0,
            FilterCondition::StartsWith("test".to_string()),
        )
        .unwrap();
    ...
    fn test_filter_not_equals() {
        ...
        apply_filter(
            &mut grid,
            0,
            FilterCondition::NotEquals("test0".to_string()),
        )
        .unwrap();
    ...
    fn test_filter_is_in() {
        ...
        apply_filter(
            &mut grid,
            0,
            FilterCondition::IsIn(vec!["test0".to_string(), "test1".to_string()]),
        )
        .unwrap();
    ...
    fn test_glob_filter() {
        ...
        // Set address values
        grid.get_cell_mut(0, 0).unwrap().value =
            CellValue::Text("/usa/ny/brooklyn/boiler-01".to_string());
        grid.get_cell_mut(1, 0).unwrap().value =
            CellValue::Text("/usa/ny/manhattan/boiler-02".to_string());
        grid.get_cell_mut(2, 0).unwrap().value = CellValue::Text("/usa/ca/boiler-03".to_string());
        grid.get_cell_mut(3, 0).unwrap().value =
            CellValue::Text("/usa/ny/brooklyn/valve-01".to_string());
