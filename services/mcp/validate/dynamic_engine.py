...
class DynamicRuleEngine:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # Dynamic settings
        self.enable_runtime_modification = self.config.get(
    ...
    def create_dynamic_rule(self, rule: MCPRule, description: str = "") -> str:
        ...
        with self.rule_lock:
            dynamic_rule = DynamicRule(
                rule_id=rule.rule_id, base_rule=rule, current_version="v1.0"
            )
    ...
    def modify_rule(
        ...
        with self.rule_lock:
            ...
            # Apply modifications
            try:
                modified_rule = self._apply_modifications(
                    dynamic_rule.base_rule, modifications
                )
                ...
                if len(dynamic_rule.versions) > self.max_rule_versions:
                    dynamic_rule.versions = dynamic_rule.versions[
                        -self.max_rule_versions :
                    ]
    ...
    def _apply_modifications(
        self, rule: MCPRule, modifications: Dict[str, Any]
    ) -> MCPRule:
        ...
        for key, value in modifications.items():
            if key == "name":
                ...
                modified_rule.conditions = self._modify_conditions(
    ...
    def _modify_conditions(
        self, conditions: List[RuleCondition], modifications: List[Dict[str, Any]]
    ) -> List[RuleCondition]:
    ...
    def _modify_actions(
        self, actions: List[RuleAction], modifications: List[Dict[str, Any]]
    ) -> List[RuleAction]:
        """Modify rule actions"""
        modified_actions = []
    ...
    def evaluate_adaptive_conditions(
    ...
    def _evaluate_time_based_condition(
        self, condition: Dict[str, Any], context: Dict[str, Any]
    ) -> bool:
    ...
    def validate_building_model_dynamic(
        self,
            ...
            for mcp_file in loaded_mcp_files:
                for rule in mcp_file.rules:
                    ...
                    if rule.rule_id in self.dynamic_rules:
                        ...
                        adaptive_satisfied = self.evaluate_adaptive_conditions(
                            rule.rule_id, context
                        )

                        if adaptive_satisfied:
                            # Execute the current version of the rule
                            result = self.base_engine._execute_rule(
                                dynamic_rule.base_rule, building_model
                            )
        ...
        return {
            ...
            "dynamic_rules": len(
                [r for r in dynamic_results if r["version"] != "static"]
            ),
            "adaptive_rules_executed": len(
                [r for r in dynamic_results if r["adaptive_satisfied"]]
            ),
        }
    ...
    def _notify_modification_callbacks(
        self, rule_id: str, modification: Dict[str, Any]
    ):
    ...
    def _condition_to_dict(self, condition: RuleCondition) -> Dict[str, Any]:
        ...
        return {
            "type": (
                ...
                if hasattr(condition.type, "value")
                else condition.type
            ),
    ...
    def _action_to_dict(self, action: RuleAction) -> Dict[str, Any]:
        ...
        return {
            ...
            "severity": (
                ...
                if action.severity and hasattr(action.severity, "value")
                else action.severity
            ),
    ...
    def _dict_to_rule(self, rule_data: Dict[str, Any]) -> MCPRule:
        ...
        for cond_data in rule_data.get("conditions", []):
            condition = RuleCondition(
                type=(
                    ConditionType(cond_data["type"])
                    if isinstance(cond_data["type"], str)
                    else cond_data["type"]
                ),
        ...
        for act_data in rule_data.get("actions", []):
            action = RuleAction(
                type=(
                    ActionType(act_data["type"])
                    if isinstance(act_data["type"], str)
                    else act_data["type"]
                ),
                message=act_data.get("message"),
                severity=(
                    RuleSeverity(act_data["severity"])
                    if act_data.get("severity")
                    and isinstance(act_data["severity"], str)
                    else act_data.get("severity")
                ),
        ...
        return MCPRule(
            ...
            category=(
                RuleCategory(rule_data["category"])
                if isinstance(rule_data["category"], str)
                else rule_data["category"]
            ),
