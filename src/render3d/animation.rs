...
impl AnimationSystem {
    ...
    fn update_animation(&mut self, animation: &mut Animation, _delta_time: f64) {
        ...
        let eased_progress = animation::apply_easing(animation.progress, &animation.easing);
    ...
    pub fn animate_linear(
        ...
        let animation = animation::create_linear_animation(id, start_value, end_value, duration);
    ...
    pub fn animate_camera_move(
        ...
        let animation = animation::create_camera_move_animation(id, start_pos, end_pos, duration);
        self.start_animation(animation)
    }
    ...
    pub fn animate_status_transition(
        ...
        let animation =
            animation::create_status_transition_animation(id, from_status, to_status, duration);
        self.start_animation(animation)
    }
...
mod tests {
    ...
    fn test_easing_functions() {
        // Test linear easing
        assert_eq!(
            animation::apply_easing(0.5, &EasingFunction::Linear),
            0.5
        );

        // Test ease-in
        let ease_in = animation::apply_easing(0.5, &EasingFunction::EaseIn);
        assert!(ease_in < 0.5);

        // Test ease-out
        let ease_out = animation::apply_easing(0.5, &EasingFunction::EaseOut);
        assert!(ease_out > 0.5);
    }
