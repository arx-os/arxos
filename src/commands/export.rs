...
use crate::export::{ARExporter, ARFormat, IFCExporter};
use crate::git;
...
fn init_git_repository(path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match git2::Repository::init(repo_path) {
        ...
        Err(e) => Err(format!("Failed to initialize Git repository: {}", e).into()),
    }
}
...
pub fn handle_export_with_format(
    ...
    match format.to_lowercase().as_str() {
        ...
        _ => Err(format!(
            "Unsupported export format: {}. Supported: git, ifc, gltf, usdz",
            format
        )
        .into()),
    }
...
pub fn handle_export(repo: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let yaml_file = yaml_files.first().ok_or("No YAML files found to export")?;
    ...
    let yaml_content = PathSafety::read_file_safely(std::path::Path::new(yaml_file), &base_dir)
    ...
    let mut git_manager =
        match git::manager::BuildingGitManager::new(&repo, &building_data.building.name, config) {
            Ok(manager) => manager,
            Err(e) => {
...
fn handle_export_ifc(output_path: PathBuf, delta: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ Successfully exported IFC file: {}",
        output_path.display()
    );
    Ok(())
}
