    ...
    def execute(self, request: UpdateBuildingRequest) -> BuildingResponse:
        ...
        try:
            ...
            building_aggregate = self.building_service.update_building(
                building_id=request.building_id, updates=updates
            )
...
class ListBuildingsUseCase:
    ...
    def execute(
        self, request: Optional[BuildingSearchRequest] = None
    ) -> BuildingListResponse:
        """
        ...
        try:
            ...
            logger.info(
                f"Listed {len(building_responses)} buildings out of {total_count}"
            )
            return result
    ...
    def _apply_filters(
        self, buildings: List[BuildingAggregate], request: BuildingSearchRequest
        ...
        if request.min_area or request.max_area:
            filtered_buildings = [
                b
                for b in filtered_buildings
                if self._area_in_range(
                    b.building.dimensions.area, request.min_area, request.max_area
                )
            ]
        ...
        if request.min_cost or request.max_cost:
            filtered_buildings = [
                b
                for b in filtered_buildings
                if self._cost_in_range(
                    b.cost.amount, request.min_cost, request.max_cost
                )
            ]
    ...
    def _area_in_range(
        self, area: float, min_area: Optional[float], max_area: Optional[float]
    ) -> bool:
    ...
    def _cost_in_range(
        self, cost: float, min_cost: Optional[float], max_cost: Optional[float]
    ) -> bool:
