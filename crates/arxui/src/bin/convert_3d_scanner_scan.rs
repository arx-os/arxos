...
//!
//! ```bash
//! cargo run -p arxui --bin convert_3d_scanner_scan -- ~/Downloads/scan_directory [output.yaml]
//! ```
//!
//! # Example
//!
//! ```bash
//! cargo run -p arxui --bin convert_3d_scanner_scan -- ~/Downloads/2025_11_01_08_21_41 scan_output.yaml
//! ```
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let conversion_context = ScanConversionContext {
        name: &title,
        device: &device,
        bounding_box,
        ...
        floor_polygon: floor_polygon.as_deref(),
        detection_counts: DetectionCounts {
            walls: walls_count,
            doors: doors_count,
            windows: windows_count,
        },
        walls_data: walls_data.as_deref(),
    };

    let building_data = generate_building_data(conversion_context)?;
...
struct ScanConversionContext<'a> {
    name: &'a str,
    device: &'a str,
    ...
    frame_count: usize,
    scan_dir: &'a Path,
    floor_polygon: Option<&'a [Point3D]>,
    detection_counts: DetectionCounts,
    walls_data: Option<&'a str>,
}

fn generate_building_data(
    context: ScanConversionContext<'_>,
) -> Result<BuildingData, Box<dyn std::error::Error>> {
    let ScanConversionContext {
        name,
        device,
        bounding_box,
        geometry,
    ...
    room_properties.insert("width_meters".to_string(), format!("{:.2}", geometry.width));
    room_properties.insert("depth_meters".to_string(), format!("{:.2}", geometry.depth));
    room_properties.insert(
        "height_meters".to_string(),
        format!("{:.2}", geometry.height),
    );
    room_properties.insert(
        "walls_detected".to_string(),
        detection_counts.walls.to_string(),
    );
    room_properties.insert(
        "doors_detected".to_string(),
        detection_counts.doors.to_string(),
    );
    room_properties.insert(
        "windows_detected".to_string(),
        detection_counts.windows.to_string(),
    );
    room_properties.insert(
