...
class TestBezierCurve(unittest.TestCase):
    ...
    def test_bezier_curve_subdivision(self):
        ...
        self.assertAlmostEqual(
            float(left_start.position.x), float(original_start.position.x), places=6
        )
        self.assertAlmostEqual(
            float(left_start.position.y), float(original_start.position.y), places=6
        )
        ...
        self.assertAlmostEqual(
            float(right_end.position.x), float(original_end.position.x), places=6
        )
        self.assertAlmostEqual(
            float(right_end.position.y), float(original_end.position.y), places=6
        )
...
class TestBSplineCurve(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.bspline_curve = BSplineCurve(
            self.control_points, self.knot_vector, self.config
        )
    ...
    def test_bspline_basis_functions(self):
        ...
        span = self.bspline_curve._find_knot_span(Decimal("0.5"))
        basis_functions = self.bspline_curve._calculate_basis_functions(
            span, Decimal("0.5")
        )

        self.assertIsInstance(basis_functions, list)
        self.assertEqual(
            len(basis_functions), self.bspline_curve.knot_vector.degree + 1
        )
...
class TestCurveSystem(unittest.TestCase):
    ...
    def test_curve_fitting(self):
        ...
        curve = self.curve_system.fit_curve_to_points(
            points, CurveType.BEZIER, degree=3
        )
...
class TestCurveSystemIntegration(unittest.TestCase):
    ...
    def test_curve_constraint_integration(self):
        ...
        tangent_constraint = self.constraint_system.add_curve_tangent_constraint(
            curve1, curve2
        )
        curvature_constraint = (
            self.constraint_system.add_curvature_continuous_constraint(curve1, curve2)
        )
    ...
    def test_curve_fitting_with_constraints(self):
        ...
        position_constraint = self.constraint_system.add_curve_position_constraint(
            curve, target_position
        )

        self.assertIsInstance(position_constraint, CurvePositionConstraint)

        # Add length constraint
        length_constraint = self.constraint_system.add_curve_length_constraint(
            curve, 3.0
        )
