...
use crate::tui::layouts::dashboard_layout;
use crate::tui::{StatusColor, TerminalManager, Theme};
use crate::utils::loading;
use crate::yaml::BuildingData;
use arx::identity::UserRegistry;
...
impl WatchDashboardState {
    fn new(building_name: Option<String>) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let user_registry = find_git_repository()
            .ok()
            .flatten()
            .and_then(|repo_path| UserRegistry::load(std::path::Path::new(&repo_path)).ok());
    ...
    fn load_sensor_data(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for floor in &self.building_data.floors {
            for equipment in &floor.equipment {
                ...
                let (value, unit, sensor_type) =
                    if let Some(health_status) = &equipment.health_status {
                        match health_status {
                            EquipmentHealthStatus::Healthy => {
                                (22.5, "°C".to_string(), "Temperature".to_string())
                            }
                            EquipmentHealthStatus::Warning => {
                                (25.5, "°C".to_string(), "Temperature".to_string())
                            }
                            EquipmentHealthStatus::Critical => {
                                (30.0, "°C".to_string(), "Temperature".to_string())
                            }
                            EquipmentHealthStatus::Unknown => {
                                (20.0, "°C".to_string(), "Temperature".to_string())
                            }
                        }
                    } else {
    ...
    fn load_alerts(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for floor in &self.building_data.floors {
            for equipment in &floor.equipment {
                ...
                let should_alert = if let Some(health_status) = &equipment.health_status {
                    matches!(
                        health_status,
                        EquipmentHealthStatus::Warning | EquipmentHealthStatus::Critical
                    )
                } else {
                    matches!(
                        equipment.status,
                        EquipmentStatus::Maintenance | EquipmentStatus::OutOfOrder
                    )
                };
                    ...
                    let (severity, message) = if let Some(health_status) = &equipment.health_status
                    {
                        match health_status {
                            ...
                            EquipmentHealthStatus::Critical => (
                                ...
                                format!(
                                    "{}: Critical condition requires attention",
                                    equipment.name
                                ),
                            ),
                            _ => continue,
        ...
        alerts.sort_by(|a, b| match (a.severity, b.severity) {
            (StatusColor::Critical, StatusColor::Critical) => {
            ...
            .filter(|reading| {
                if let Some(ref filter_room) = self.filter_room {
                    reading
                        .room
                        .as_ref()
                        .is_some_and(|r| r.contains(filter_room))
                } else {
                    true
                }
            })
                                ...
                                for room in &wing.rooms {
                                    if room.equipment.iter().any(|e| e.id == *equipment_id) {
                                        ...
                                        return room
                                            .name
                                            .to_lowercase()
                                            .contains(&filter_room.to_lowercase());
                                    }
                                }
                            }
                        }
    ...
    fn load_recent_changes(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for commit in commits {
            ...
            let email =
                extract_email_from_author(&commit.author).unwrap_or_else(|| commit.author.clone());
...
fn render_header<'a>(state: &'a WatchDashboardState, theme: &'a Theme) -> Paragraph<'a> {
    let tabs_text: String = state
        .tabs
        .iter()
        .enumerate()
        .map(|(idx, tab)| {
...
fn render_recent_changes<'a>(state: &'a WatchDashboardState, theme: &'a Theme) -> Paragraph<'a> {
...
fn render_overview_alerts<'a>(state: &'a WatchDashboardState, theme: &'a Theme) -> Paragraph<'a> {
    let recent_alerts: Vec<Line> = state
        .filtered_alerts()
        .iter()
        .take(5)
        .map(|alert| {
...
/// Render sensors tab
fn render_sensors<'a>(state: &'a WatchDashboardState, _area: Rect, theme: &'a Theme) -> Table<'a> {
...
/// Render alerts tab
fn render_alerts<'a>(state: &'a WatchDashboardState, _area: Rect, theme: &'a Theme) -> List<'a> {
...
fn render_equipment<'a>(
    ...
    let equipment_list: Vec<Line> = state
        .building_data
        .floors
        .iter()
        .flat_map(|floor| {
