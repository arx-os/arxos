...
def demonstrate_geometric_validation():
    ...
    distance_tests = [
        ...
        {"name": "3D Distance", "points": (0, 0, 0, 1, 1, 1), "expected": 1.732},
        {
            ...
            "points": (1.000001, 2.000001, 3.000001, 4.000001),
            "expected": 3.606,
        },
    ]
    ...
    for test in distance_tests:
        ...
        results = validator.validate_geometric_operation(
            test["name"].lower().replace(" ", "_"), distance
        )
...
def demonstrate_constraint_validation():
    ...
    for constraint in valid_constraints:
        ...
        results = validator.validate_constraint(
            constraint["name"].lower().replace(" ", "_"), constraint["data"]
        )
...
def demonstrate_custom_validation_rules():
    ...
    def validate_precision_accuracy(coordinate, precision):
        ...
        x_valid = abs(
            coordinate.x - round(coordinate.x / float(precision)) * float(precision)
        ) <= float(precision)
        y_valid = abs(
            coordinate.y - round(coordinate.y / float(precision)) * float(precision)
        ) <= float(precision)
        z_valid = abs(
            coordinate.z - round(coordinate.z / float(precision)) * float(precision)
        ) <= float(precision)
    ...
    for i, coord in enumerate(test_coordinates, 1):
        ...
        custom_results = [
            r for r in results if any(rule.name in r.message for rule in custom_rules)
        ]
    ...
    constraints = [
        {
            ...
            "data": {"type": "distance", "values": [5.000]},
        },
...
def demonstrate_error_handling():
    ...
    for validation_type, stats in summary["by_type"].items():
        print(
            f"     {validation_type}: {stats['passed']} passed, {stats['failed']} failed"
        )
    ...
    for validation_level, stats in summary["by_level"].items():
        print(
            f"     {validation_level}: {stats['passed']} passed, {stats['failed']} failed"
        )
