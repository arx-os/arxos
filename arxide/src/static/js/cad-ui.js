...
class CadApplication {
    ...
    initializePrecisionControls() {
        ...
        const precisionLevelSelect = document.getElementById('precision-level');
        if (precisionLevelSelect) {
            precisionLevelSelect.addEventListener('change', (e) => {
        ...
        if (gridSizeSelect) {
            gridSizeSelect.addEventListener('change', (e) => {
                ...
                this.cadEngine.gridSize = gridSize;
        ...
        if (gridSnapCheckbox) {
            gridSnapCheckbox.addEventListener('change', (e) => {
                this.cadEngine.gridSnappingEnabled = e.target.checked;
                this.updateGridSnapUI(e.target.checked);
            });
        }
    }
     ...
     * Initialize constraint tools
     */
    initializeConstraintTools() {
        // Distance constraint tool
        const distanceConstraintBtn = document.getElementById('distance-constraint');
        if (distanceConstraintBtn) {
            distanceConstraintBtn.addEventListener('click', () => {
        ...
        if (parallelConstraintBtn) {
            parallelConstraintBtn.addEventListener('click', () => {
        ...
        if (perpendicularConstraintBtn) {
            perpendicularConstraintBtn.addEventListener('click', () => {
        ...
        if (clearConstraintsBtn) {
            clearConstraintsBtn.addEventListener('click', () => {
    ...
    /**
     * Update precision UI
     * @param {string} level - Precision level
     */
    updatePrecisionUI(level) {
        ...
        if (drawingInfo) {
            const currentText = drawingInfo.textContent;
    ...
    /**
     * Update grid snap UI
     * @param {boolean} enabled - Grid snap enabled
     */
    updateGridSnapUI(enabled) {
        const gridSnapCheckbox = document.getElementById('grid-snap');
        if (gridSnapCheckbox) {
    ...
    /**
     * Set current tool with enhanced constraint support
     * @param {string} tool - Tool to set
     */
    setCurrentTool(tool) {
        this.currentTool = tool;
    ...
    /**
     * Handle constraint tool selection
     * @param {string} tool - Constraint tool
     */
    handleConstraintTool(tool) {
        switch (tool) {
            case 'distance-constraint':
                this.enableDistanceConstraintMode();
                break;
            case 'parallel-constraint':
                this.enableParallelConstraintMode();
                break;
            case 'perpendicular-constraint':
                this.enablePerpendicularConstraintMode();
                break;
            default:
    ...
    /**
     * Enable distance constraint mode
     */
    ...
    /**
     * Enable parallel constraint mode
     */
    ...
    /**
     * Enable perpendicular constraint mode
     */
    ...
    /**
     * Disable constraint mode
     */
    disableConstraintMode() {
        this.constraintMode = null;
        this.constraintSelection = [];
        this.removeConstraintSelectionListeners();
    }

    /**
     * Add constraint selection listeners
     */
    addConstraintSelectionListeners() {
        // Add click listener to canvas for object selection
        const canvas = document.getElementById('cad-canvas');
        if (canvas) {
            this.constraintClickListener = (event) => {
                this.handleConstraintObjectSelection(event);
            };
            canvas.addEventListener('click', this.constraintClickListener);
        }
    }

    /**
     * Remove constraint selection listeners
     */
    removeConstraintSelectionListeners() {
        const canvas = document.getElementById('cad-canvas');
        if (canvas && this.constraintClickListener) {
            canvas.removeEventListener('click', this.constraintClickListener);
            this.constraintClickListener = null;
        }
    }

    /**
     * Handle constraint object selection
    ...
    handleConstraintObjectSelection(event) {
        ...
        if (selectedObject) {
            ...
            if (this.constraintSelection.length === 2) {
                ...
                this.showNotification(`Selected object for ${this.constraintMode} constraint. Select another object.`, 'info');
            }
        }
    }
        ...
        try {
            ...
            switch (this.constraintMode) {
                case 'distance':
                    ...
                    constraintId = this.cadEngine.constraintSolver.addConstraint('distance', {
                        object1Id: obj1.id,
                        object2Id: obj2.id,
                        distance: distance
                    });
     ...
     */
    clearAllConstraints() {
        if (this.cadEngine && this.cadEngine.constraintSolver) {
            this.cadEngine.constraintSolver.clearConstraints();
            this.updateConstraintsList();
            this.showNotification('All constraints cleared', 'info');
        }
    }

    /**
     * Update constraints list in UI
     */
    ...
    removeConstraint(constraintId) {
        if (this.cadEngine && this.cadEngine.constraintSolver) {
            this.cadEngine.constraintSolver.removeConstraint(constraintId);
            this.updateConstraintsList();
            this.showNotification('Constraint removed', 'info');
        }
    }

    /**
     * Update performance info with constraint count
     */
