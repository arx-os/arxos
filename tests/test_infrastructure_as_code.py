...
class TestPrecisionManagement:
    ...
    def test_format_for_display(self):
        ...
        # Test decimal format
        decimal_str = precision_manager.format_for_display(
            value, PrecisionDisplayMode.DECIMAL
        )
        assert "100.001 mm" in decimal_str

        # Test scientific format
        scientific_str = precision_manager.format_for_display(
            value, PrecisionDisplayMode.SCIENTIFIC
        )
        assert "100001 μm" in scientific_str

        # Test micron format
        micron_str = precision_manager.format_for_display(
            value, PrecisionDisplayMode.MICRON
        )
    ...
    def test_precision_validation(self):
        ...
        assert not precision_manager.validate_precision(
            invalid_value, PrecisionLevel.MICRON
        )
...
class TestDigitalElement:
    ...
    def test_element_volume_calculation(self):
        ...
        element = DigitalElement(
            width=Decimal("100.000"), height=Decimal("200.000"), depth=Decimal("50.000")
        )
    ...
    def test_element_constraints(self):
        ...
        element.add_constraint(
            "parallel", target_id="element_1", tolerance=Decimal("0.001")
        )
        element.add_constraint(
            "perpendicular", target_id="element_2", tolerance=Decimal("0.001")
        )
...
class TestInfrastructureAsCodeService:
    ...
    def test_get_element(self):
        ...
        element = infrastructure_service.create_element(
            element_type=ElementType.WALL, name="Test Wall"
        )
    ...
    def test_update_element(self):
        ...
        element = infrastructure_service.create_element(
            element_type=ElementType.WALL, name="Original Name"
        )

        # Update element
        success = infrastructure_service.update_element(
            element.id, name="Updated Name", position_x=Decimal("150.000")
        )
    ...
    def test_gus_instructions_generation(self):
        ...
        # Create test elements
        infrastructure_service.create_element(
            element_type=ElementType.WALL, name="Wall 1"
        )

        infrastructure_service.create_element(
            element_type=ElementType.DOOR, name="Door 1"
        )
...
class TestPrecisionIntegration:
    ...
    def test_micron_precision_handling(self):
        ...
        # Verify precision
        assert precision_manager.validate_precision(
            element.position_x, PrecisionLevel.MICRON
        )
        assert precision_manager.validate_precision(
            element.position_y, PrecisionLevel.MICRON
        )
        assert precision_manager.validate_precision(
            element.width, PrecisionLevel.MICRON
        )

        # Test display formatting
        display_x = precision_manager.format_for_display(
            element.position_x, PrecisionDisplayMode.MICRON
        )
    ...
    def test_engineering_compliance(self):
        ...
        assert PrecisionValidator.validate_engineering_compliance(
            valid_inch_value, "ASME"
        )
