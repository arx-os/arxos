...
class SignalPropagationService:
    ...
    def analyze_signal_propagation(
        ...
        try:
            logger.info(
                f"Starting signal propagation analysis for request {request.id}"
            )

            # Calculate distance between source and receiver
            distance = self._calculate_distance(
                request.source.position, request.receiver_position
            )
            ...
            signal_strength = self._calculate_signal_strength(
                received_power, request.environment
            )
    ...
    def _calculate_path_loss(
        self, request: SignalAnalysisRequest, distance: float
    ) -> float:
        """Calculate path loss based on propagation model."""
    ...
    def _calculate_received_power(
        self, source: SignalSource, path_loss: float
    ) -> float:
    ...
    def _calculate_signal_strength(
        self, received_power: float, environment: PropagationEnvironment
    ) -> float:
    ...
    def _calculate_environmental_attenuation(
    ...
    def _calculate_snr(
        self, signal_strength: float, environment: PropagationEnvironment
    ) -> float:
        ...
        if request.environment.ground_reflectivity > 0:
            ground_component = {
                ...
                "path_loss": self._calculate_path_loss(
                    request, distance + 2 * request.source.height
                ),
            }
    ...
    def _calculate_coverage_area(
        ...
        if request.propagation_model == PropagationModel.FREE_SPACE:
            coverage_radius = self._estimate_coverage_radius_free_space(
                request, min_signal_strength
            )
        else:
            coverage_radius = self._estimate_coverage_radius_empirical(
                request, min_signal_strength
            )
    ...
    def _estimate_coverage_radius_free_space(
        self, request: SignalAnalysisRequest, min_signal_strength: float
    ) -> float:
        ...
        distance = (
            c
            / (4 * math.pi * request.source.frequency)
            * 10 ** (required_path_loss / 20)
        )

        return distance

    def _estimate_coverage_radius_empirical(
        self, request: SignalAnalysisRequest, min_signal_strength: float
    ) -> float:
    ...
    def _calculate_interference(
        self, request: SignalAnalysisRequest, signal_strength: float
    ) -> float:
        """Calculate interference level."""
    ...
    def get_environment_model_info(
