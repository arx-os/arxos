...
class RoomModel(BaseModel):
    ...
    # Status
    status = Column(
        Enum(RoomStatus), nullable=False, default=RoomStatus.PLANNED, index=True
    )
    ...
    devices = relationship(
        "DeviceModel", back_populates="room", cascade="all, delete-orphan"
    )
    ...
    def update_status(
        self, new_status: RoomStatus, updated_by: Optional[str] = None
    ) -> None:
        """Update room status."""
