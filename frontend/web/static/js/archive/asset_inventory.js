...
function loadAssets() {
  ...
  let url = `/api/buildings/${building}/assets?`;
  ...
  if (window.arxLogger) {
    window.arxLogger.info('Loading assets', {
      ...
      filters: { system, assetType, room, status },
      url: url
    });
  }
  ...
  fetch(url, {
    ...
    .catch(error => {
      const duration = performance.now() - startTime;
      if (window.arxLogger) {
        window.arxLogger.apiError('GET', url, error, 0);
        window.arxLogger.error('Failed to load assets', {
          building_id: building,
          filters: { system, assetType, room, status },
          error: error.message
        });
...
function renderAssetTable() {
  ...
  const pageAssets = sorted.slice(start, end);
...
function sortAssets(field) {
  if (window.arxLogger) {
    window.arxLogger.info('Sorting assets', {
      field: field,
...
function prevPage() {
  if (currentPage > 1) {
    if (window.arxLogger) {
      window.arxLogger.info('Navigating to previous page', {
...
function nextPage() {
  if (currentPage < Math.ceil(assets.length / pageSize)) {
    if (window.arxLogger) {
      window.arxLogger.info('Navigating to next page', {
        from_page: currentPage,
        to_page: currentPage + 1
      });
    }
...
function toggleSelectAllAssets() {
  ...
  if (window.arxLogger) {
    window.arxLogger.info('Toggled select all assets', {
      selected: checked,
      total_assets: checkboxes.length,
      selected_count: checkedCount
    });
  }
}
...
function exportAssetsAdvanced() {
  const building = document.getElementById('filter-building').value;
  const format = document.getElementById('export-format').value;
  const system = document.getElementById('export-system').value;
  ...
  if (window.arxLogger) {
    window.arxLogger.info('Starting advanced asset export', {
      format: format,
      building_id: building,
      filters: { system, status, dateFrom, dateTo },
      options: { includeHistory, includeMaintenance, includeValuations },
      url: url
    });
  }
  ...
  fetch(url, {
    ...
    .then(blob => {
      ...
      a.download = `asset_inventory_export.${format}`;
    ...
    .catch((error) => {
      ...
      if (window.arxLogger) {
        window.arxLogger.apiError('GET', url, error, 0);
        window.arxLogger.error('Asset export failed', {
          format: format,
          building_id: building,
          error: error.message
        });
...
function deleteSelectedAssets() {
  ...
  if (selected.length === 0) {
    if (window.arxLogger) {
      window.arxLogger.warn('No assets selected for deletion');
    }
  ...
  Promise.all(selected.map(id => fetch(`/api/assets/${id}`, {
    ...
    const duration = performance.now() - startTime;
    if (window.arxLogger) {
      window.arxLogger.error('Failed to delete assets', {
        selected_count: selected.length,
        error: error.message,
        duration_ms: duration
      });
  ...
  if (assetId) {
    ...
    fetch(`/api/assets/${assetId}`, {
      ...
      .then(res => {
        const duration = performance.now() - startTime;
        if (window.arxLogger) {
          window.arxLogger.apiRequest('GET', `/api/assets/${assetId}`, res.status, duration);
        }
        return res.json();
      })
      .then(asset => {
      ...
      .catch(error => {
        const duration = performance.now() - startTime;
        if (window.arxLogger) {
          window.arxLogger.apiError('GET', `/api/assets/${assetId}`, error, 0);
          window.arxLogger.error('Failed to load asset details', {
            asset_id: assetId,
            error: error.message
          });
        }
...
function renderAssetForm(asset, isEdit) {
  if (window.arxLogger) {
    window.arxLogger.info('Rendering asset form', {
      is_edit: isEdit,
  ...
  fetch('/api/buildings', {
    ...
    .then(res => {
      const duration = performance.now() - startTime;
      if (window.arxLogger) {
        window.arxLogger.apiRequest('GET', '/api/buildings', res.status, duration);
      }
      return res.json();
    })
    .then(buildings => {
      let buildingOptions = '<option value="">-- Select --</option>';
      for (const b of buildings) {
        buildingOptions += `<option value="${b.id}"${asset.building_id == b.id ? ' selected' : ''}>${b.name}</option>`;
      }
...
function loadFloorsForAsset(selectedFloorId) {
  ...
  if (buildingId) {
    ...
    fetch(`/api/buildings/${buildingId}/floors`, {
      ...
      .then(res => {
        const duration = performance.now() - startTime;
        if (window.arxLogger) {
          window.arxLogger.apiRequest('GET', `/api/buildings/${buildingId}/floors`, res.status, duration);
        }
        return res.json();
      })
      .then(floors => {
...
function submitAssetForm(assetId) {
  if (window.arxLogger) {
    window.arxLogger.info('Submitting asset form', {
      is_edit: !!assetId,
  ...
  const roomId = document.getElementById('asset-room').value;
  const assetType = document.getElementById('asset-type').value;
  const system = document.getElementById('asset-system').value;
  const x = parseFloat(document.getElementById('asset-x').value);
  const y = parseFloat(document.getElementById('asset-y').value);
  const specsText = document.getElementById('asset-specs').value;
  const value = parseFloat(document.getElementById('asset-value').value);
  const replacement = parseFloat(document.getElementById('asset-replacement').value);
  const fundingSource = document.getElementById('asset-funding-source').value;
  const status = document.getElementById('asset-status').value;
  let specifications;
  try {
    specifications = specsText ? JSON.parse(specsText) : {};
    document.getElementById('specs-error').textContent = '';
  } catch (e) {
    if (window.arxLogger) {
      window.arxLogger.error('Invalid JSON in asset specifications', {
        error: e.message,
        specs_text: specsText
      });
    }
    document.getElementById('specs-error').textContent = 'Invalid JSON';
    showToast('Specifications must be valid JSON.', 'error');
    return;
  }
  if (!buildingId || !assetType || !system) {
    if (window.arxLogger) {
      window.arxLogger.warn('Asset form validation failed', {
        building_id: buildingId,
        asset_type: assetType,
        system: system
      });
    }
    alert('Building, Asset Type, and System are required.');
    return;
  }
  const assetData = {
    building_id: buildingId,
    ...
    replacement_cost: isNaN(replacement) ? null : replacement,
  ...
  fetch(url, {
    ...
    headers: {
      ...
      ...(localStorage.getItem('arx_jwt') ? { 'Authorization': 'Bearer ' + localStorage.getItem('arx_jwt') } : {})
    },
    ...
    .then(res => {
      ...
      if (res.ok) {
        ...
        res.text().then(t => {
          if (window.arxLogger) {
            window.arxLogger.error('Asset submission failed', {
              method: method,
              url: url,
              status: res.status,
              error: t
    ...
    .catch(error => {
      const duration = performance.now() - startTime;
      if (window.arxLogger) {
        window.arxLogger.apiError(method, url, error, 0);
        window.arxLogger.error('Asset submission failed', {
          method: method,
          url: url,
          error: error.message,
...
function loadAssetDetails(asset) {
  if (window.arxLogger) {
    window.arxLogger.info('Loading asset details', {
      asset_id: asset.id,
      asset_type: asset.asset_type
    });
  }
  ...
  fetch(`/api/assets/${asset.id}/history`, { headers: localStorage.getItem('arx_jwt') ? { 'Authorization': 'Bearer ' + localStorage.getItem('arx_jwt') } : {} })
    .then(res => {
      const duration = performance.now() - startTime;
      if (window.arxLogger) {
        window.arxLogger.apiRequest('GET', `/api/assets/${asset.id}/history`, res.status, duration);
      }
      return res.json();
    })
    .then(history => {
      document.getElementById('asset-history-timeline').innerHTML = history.map(h => `<div>${h.event_type}: ${h.description} (${new Date(h.event_date).toLocaleDateString()})</div>`).join('') || '<div class="text-gray-400">No history records.</div>';
    }).catch(() => {
      document.getElementById('asset-history-timeline').innerHTML = '<div class="text-red-500">Failed to load history.</div>';
    });
  // Fetch and render maintenance
  fetch(`/api/assets/${asset.id}/maintenance`, { headers: localStorage.getItem('arx_jwt') ? { 'Authorization': 'Bearer ' + localStorage.getItem('arx_jwt') } : {} })
    .then(res => res.json())
    .then(maint => {
      document.getElementById('asset-maintenance-records').innerHTML = maint.map(m => `<div>${m.maintenance_type}: ${m.description} (${new Date(m.scheduled_date).toLocaleDateString()})</div>`).join('') || '<div class="text-gray-400">No maintenance records.</div>';
    }).catch(() => {
      document.getElementById('asset-maintenance-records').innerHTML = '<div class="text-red-500">Failed to load maintenance.</div>';
    });
  // Fetch and render valuation
  fetch(`/api/assets/${asset.id}/valuations`, { headers: localStorage.getItem('arx_jwt') ? { 'Authorization': 'Bearer ' + localStorage.getItem('arx_jwt') } : {} })
    .then(res => res.json())
    .then(vals => {
      document.getElementById('asset-valuation-history').innerHTML = vals.map(v => `<div>${v.valuation_type}: $${v.value} (${new Date(v.valuation_date).toLocaleDateString()})</div>`).join('') || '<div class="text-gray-400">No valuation records.</div>';
    }).catch(() => {
      document.getElementById('asset-valuation-history').innerHTML = '<div class="text-red-500">Failed to load valuations.</div>';
    });
  // Documents placeholder
  document.getElementById('asset-documents').innerHTML = '<div class="text-gray-400">No documents linked.</div>';
}

function clearAssetDetails() {
  document.getElementById('asset-history-timeline').textContent = '';
  document.getElementById('asset-maintenance-records').textContent = '';
  document.getElementById('asset-valuation-history').textContent = '';
  document.getElementById('asset-documents').textContent = '';
}
...
function updateSummaryCards() {
  ...
  const avgAge = assets.length > 0 ? assets.reduce((sum, asset) => sum + (asset.age || 0), 0) / assets.length : 0;
  const activeAssets = assets.filter(asset => asset.status === 'active').length;
  const activePercentage = totalAssets > 0 ? Math.round((activeAssets / totalAssets) * 100) : 0;
  ...
  document.getElementById('total-assets-building').textContent = buildingName;
...
function renderAgeChart() {
  ...
  const ageRanges = { '0-5': 0, '6-10': 0, '11-15': 0, '16-20': 0, '20+': 0 };
  assets.forEach(asset => {
...
function renderEfficiencyChart() {
  ...
  const efficiencyRanges = { '0-50%': 0, '51-75%': 0, '76-90%': 0, '91-100%': 0 };
  assets.forEach(asset => {
    const efficiency = parseFloat(asset.efficiency_rating?.replace('%', '') || '0');
    if (efficiency <= 50) efficiencyRanges['0-50%']++;
    else if (efficiency <= 75) efficiencyRanges['51-75%']++;
    else if (efficiency <= 90) efficiencyRanges['76-90%']++;
    else efficiencyRanges['91-100%']++;
  });
...
function renderMaintenanceChart() {
  ...
  // Placeholder data - in a real implementation, you'd fetch maintenance cost data
  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
  const costs = [1200, 1900, 3000, 5000, 2000, 3000];
...
function highlightAssetOnSVG(assetId) {
  // Open SVG viewer with asset highlighted
  window.open(`/svg_view.html?asset=${assetId}`, '_blank');
}

function showAssetTab(tab) {
  document.querySelectorAll('.asset-tab-content').forEach(el => el.classList.add('hidden'));
  document.getElementById(`asset-tab-${tab}`).classList.remove('hidden');
}

function showToast(message, type = 'info') {
  if (window.arxLogger) {
    window.arxLogger.info('Showing toast message', {
      message: message,
      type: type
    });
  }
...
function addAssetFromSymbol() {
  if (window.arxLogger) {
    window.arxLogger.info('Adding asset from symbol', {
...
document.addEventListener('DOMContentLoaded', () => {
  if (window.arxLogger) {
    window.arxLogger.info('Asset inventory page loaded', {
