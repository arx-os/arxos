...
impl BuildingRenderer {
            ...
            if let Some(polygon_str) = room.properties.get("floor_polygon") {
                if !polygon_str.is_empty() {
                    ...
                    for point_str in polygon_str.split(';') {
                        ...
                        if parts.len() >= 2 {
                            if let (Ok(x_val), Ok(y_val)) = (parts[0].parse::<f64>(), parts[1].parse::<f64>()) {
                                // Transform based on coordinate space
                                let x = if is_world_space { x_val } else { x_val + room_x };
                                let y = if is_world_space { y_val } else { y_val + room_y };
                                min_x = min_x.min(x);
                                min_y = min_y.min(y);
                                max_x = max_x.max(x);
                                max_y = max_y.max(y);
    ...
    fn draw_room(&self, grid: &mut Vec<Vec<char>>, room: &RoomData, bounds: (f64, f64, f64, f64), _scale: f64) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(polygon_str) = room.properties.get("floor_polygon") {
            if !polygon_str.is_empty() {
                ...
                if !polygon_points.is_empty() {
                    ...
                    self.draw_polygon_outline(grid, &polygon_points, bounds, grid_width, grid_height)?;
                    
                    // Draw room name in center
                    let (center_x, center_y) = self.calculate_polygon_center(&polygon_points, bounds, grid_width, grid_height);
                    self.draw_room_name(grid, &room.name, center_x, center_y, grid_width, grid_height);
        ...
        let start_y = (((room_min_y - min_y) / (max_y - min_y) * grid_height as f64) as usize).min(grid_height.saturating_sub(1));
        let end_y = (((room_max_y - min_y) / (max_y - min_y) * grid_height as f64) as usize).min(grid_height.saturating_sub(1));
    ...
    fn draw_polygon_outline(
        ...
        let grid_points: Vec<(usize, usize)> = points
            .iter()
            .map(|(x, y)| {
                let gx = (((x - min_x) / (max_x - min_x) * grid_width as f64) as usize).min(grid_width.saturating_sub(1));
                // Flip Y: larger world Y should be at top (smaller screen Y)
                let gy = grid_height.saturating_sub(1) - (((*y - min_y) / (max_y - min_y) * grid_height as f64) as usize).min(grid_height.saturating_sub(1));
                (gx, gy)
            })
            .collect();
        ...
        for y in 0..grid_height {
            ...
            for i in 0..grid_points.len() {
                ...
                if y >= y1 && y < y2 && y1 != y2 {
                    let x = (p1.0 as f64 + (y as f64 - p1.1 as f64) * (p2.0 as f64 - p1.0 as f64) / (p2.1 as f64 - p1.1 as f64)) as usize;
            ...
            for pair in intersections.chunks(2) {
                if pair.len() == 2 {
                    ...
                    for x in start..=end.min(grid_width.saturating_sub(1)) {
                        if y < grid_height && grid[y][x] == ' ' {
                            grid[y][x] = '·';
                        }
                    }
                }
    ...
    fn draw_line(&self, grid: &mut Vec<Vec<char>>, p1: (usize, usize), p2: (usize, usize), grid_width: usize, grid_height: usize) {
        ...
        loop {
            if x >= 0 && x < grid_width as i32 && y >= 0 && y < grid_height as i32 {
                ...
                let char = if (x == p1.0 as i32 && y == p1.1 as i32) || (x == p2.0 as i32 && y == p2.1 as i32) {
                ...
                } else if (current_char == '─' || current_char == '│' || current_char == '+') && (char == '─' || char == '│') {
                    // Merge wall segments
    ...
    fn calculate_polygon_center(&self, points: &[(f64, f64)], bounds: (f64, f64, f64, f64), grid_width: usize, grid_height: usize) -> (usize, usize) {
        let (min_x, min_y, max_x, max_y) = bounds;
        let (avg_x, avg_y) = points.iter().fold((0.0, 0.0), |acc, p| (acc.0 + p.0, acc.1 + p.1));
        let n = points.len() as f64;
        let gx = (((avg_x / n - min_x) / (max_x - min_x) * grid_width as f64) as usize).min(grid_width.saturating_sub(1));
        // Flip Y-axis to match polygon rendering
        let gy = grid_height.saturating_sub(1) - (((avg_y / n - min_y) / (max_y - min_y) * grid_height as f64) as usize).min(grid_height.saturating_sub(1));
        (gx, gy)
    }
        ...
        for wall_str in walls_str.split('|') {
            ...
            if parts.len() >= 4 {
                if let (Ok(x1), Ok(y1), Ok(x2), Ok(y2)) = (
                    ...
                    let in_bounds = x1 >= (min_x - margin) && x1 <= (max_x + margin) &&
                                    x2 >= (min_x - margin) && x2 <= (max_x + margin) &&
                                    y1 >= (min_y - margin) && y1 <= (max_y + margin) &&
                                    y2 >= (min_y - margin) && y2 <= (max_y + margin);
                    ...
                    if in_bounds {
                        ...
                        // Convert to grid
                        let gx1 = (((cx1 - min_x) / bounds_width * grid_width as f64) as usize).min(grid_width.saturating_sub(1));
                        // Flip Y-axis to match polygon rendering
                        let gy1 = grid_height.saturating_sub(1) - (((cy1 - min_y) / bounds_height * grid_height as f64) as usize).min(grid_height.saturating_sub(1));
                        let gx2 = (((cx2 - min_x) / bounds_width * grid_width as f64) as usize).min(grid_width.saturating_sub(1));
                        let gy2 = grid_height.saturating_sub(1) - (((cy2 - min_y) / bounds_height * grid_height as f64) as usize).min(grid_height.saturating_sub(1));
    ...
    fn draw_equipment(&self, grid: &mut Vec<Vec<char>>, equipment: &EquipmentData, bounds: (f64, f64, f64, f64), _scale: f64) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let eq_x = (((equipment.position.x - min_x) / (max_x - min_x) * width as f64) as usize).min(width.saturating_sub(1));
        // Flip Y-axis to match polygon rendering (larger world Y = smaller screen Y)
        let eq_y = height.saturating_sub(1) - (((equipment.position.y - min_y) / (max_y - min_y) * height as f64) as usize).min(height.saturating_sub(1));
