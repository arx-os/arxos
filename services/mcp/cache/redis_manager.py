...
class RedisManager:
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379", 
                 default_ttl: int = 3600, max_connections: int = 10):
        """
    ...
    async def cache_validation(self, building_id: str, validation_data: dict, 
        ...
        try:
            ...
            logger.info(f"Cached validation results for building {building_id} (TTL: {ttl}s)")
            return True
    ...
    async def cache_building_model(self, building_id: str, building_model: dict, 
                                 ttl: Optional[int] = None) -> bool:
    ...
    async def cache_mcp_file(self, mcp_id: str, mcp_data: dict, 
                            ttl: Optional[int] = None) -> bool:
        """Cache MCP file data"""
        try:
    ...
    async def cache_jurisdiction_match(self, building_id: str, jurisdiction_data: dict, 
                                     ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            entry = CacheEntry(
                ...
                metadata={
                    "building_id": building_id,
                    "applicable_codes": len(jurisdiction_data.get("applicable_codes", [])),
                    "jurisdiction_level": jurisdiction_data.get("jurisdiction_level", "unknown")
                }
            )
    ...
    async def get_cached_jurisdiction_match(self, building_id: str) -> Optional[dict]:
        ...
        try:
            ...
            logger.error(f"Error getting cached jurisdiction match for {building_id}: {e}")
            return None
    
    async def invalidate_cache(self, identifier: str, cache_type: str = "validation") -> bool:
    ...
    async def invalidate_building_cache(self, building_id: str) -> bool:
        ...
        try:
            ...
            if keys_to_delete:
                ...
                logger.info(f"Invalidated {result} cache entries for building {building_id}")
                return True
                ...
                try:
                    ...
                    if cached_data:
                        ...
                        if isinstance(entry, CacheEntry):
                            ...
                            if not stats["oldest_entry"] or entry.created_at < stats["oldest_entry"]:
                                stats["oldest_entry"] = entry.created_at
                            if not stats["newest_entry"] or entry.created_at > stats["newest_entry"]:
