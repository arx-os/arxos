...
class StructuralAnalysisService:
    ...
    def _deflection_analysis(self, element: StructuralElement) -> AnalysisResult:
        ...
        deflection_ratio = (
            max_deflection / deflection_limit if deflection_limit > 0 else 0.0
        )
        ...
        return AnalysisResult(
            ...
            forces=[],
            safety_factor=(
                1.0 / deflection_ratio if deflection_ratio > 0 else float("inf")
            ),
    ...
    def _calculate_stiffness_matrix(self, element: StructuralElement) -> np.ndarray:
        ...
        for i in range(n_nodes):
            for j in range(n_nodes):
                if i == j:
                    K[i * 3 : (i + 1) * 3, j * 3 : (j + 1) * 3] = (
                        np.eye(3) * element.material.elastic_modulus
                    )
                else:
                    K[i * 3 : (i + 1) * 3, j * 3 : (j + 1) * 3] = (
                        np.eye(3) * element.material.elastic_modulus * 0.1
                    )
    ...
    def _calculate_mass_matrix(self, element: StructuralElement) -> np.ndarray:
        ...
        # Lumped mass matrix
        for i in range(n_nodes):
            M[i * 3 : (i + 1) * 3, i * 3 : (i + 1) * 3] = (
                np.eye(3) * element.material.density
            )

        return M

    def _calculate_damping_matrix(
        self, element: StructuralElement, M: np.ndarray, K: np.ndarray
    ) -> np.ndarray:
    ...
    def _calculate_load_vector(self, element: StructuralElement) -> np.ndarray:
        ...
        for load in element.loads:
            ...
            for i, node in enumerate(element.nodes):
                F[i * 3 : (i + 1) * 3] += (
                    np.array(load.direction) * load.magnitude / len(element.nodes)
                )

        return F

    def _calculate_dynamic_load_vector(
        self, element: StructuralElement, time: float
    ) -> np.ndarray:
        ...
        for load in element.loads:
            ...
            for i, node in enumerate(element.nodes):
                F[i * 3 : (i + 1) * 3] += (
                    np.array(load.direction) * load_magnitude / len(element.nodes)
                )

        return F

    def _calculate_stresses(
        self, element: StructuralElement, u: np.ndarray
    ) -> np.ndarray:
    ...
    def _calculate_safety_factor(
        self, element: StructuralElement, stresses: np.ndarray
    ) -> float:
    ...
    def _calculate_geometric_stiffness_matrix(
    ...
    def _calculate_buckling_mode(
        self, element: StructuralElement, buckling_load: float
    ) -> np.ndarray:
        """Calculate buckling mode shape."""
        n_nodes = len(element.nodes)
    ...
    def _calculate_fatigue_life(
        self, element: StructuralElement, stress_range: np.ndarray
    ) -> float:
        """Calculate fatigue life using S-N curve."""
        ...
        cycles = (
            (fatigue_strength / max_stress_range) ** b
            if max_stress_range > 0
            else float("inf")
        )
    ...
    def _calculate_deflection(
        self, element: StructuralElement, load: Load
    ) -> np.ndarray:
        ...
        for i in range(n_nodes):
            deflection[i * 3 : (i + 1) * 3] = (
                np.array(load.direction)
                * load.magnitude
                / element.material.elastic_modulus
            )
    ...
    def _calculate_max_dynamic_stress(
        self, element: StructuralElement, u: np.ndarray
    ) -> float:
    ...
    def _calculate_max_dynamic_strain(
        self, element: StructuralElement, u: np.ndarray
    ) -> float:
