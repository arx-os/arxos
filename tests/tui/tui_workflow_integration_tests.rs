...
fn test_command_palette_workflow() {
    ...
    assert!(
        filtered_count <= palette.commands().len(),
    ...
    assert!(
        _after_next.is_some(),
        "Should still have selection after navigation"
    );
    ...
    if let Ok(mut manager) = WorkspaceManager::new() {
        ...
        if !manager.is_empty() {
            ...
            if filtered_count > 1 {
                ...
                assert_ne!(
                    initial_workspace, after_next,
                    "Selection should change with multiple workspaces"
                );
            } else {
                // With single filtered workspace, navigation wraps around to same
                assert_eq!(
                    initial_workspace, after_next,
                    "Single workspace should wrap around"
                );
            }
...
fn test_theme_switching_workflow() {
    ...
    assert_eq!(
        theme2.primary, default_primary,
...
fn test_mouse_interaction_workflow() {
    ...
    assert!(
        disabled_action.is_none(),
        "Should ignore mouse when disabled"
    );
}
...
fn test_command_palette_search_workflow() {
    ...
    if let Some(cmd) = selected {
        assert!(
            cmd.name.contains("equipment") || cmd.description.contains("equipment"),
...
fn test_help_system_integration_workflow() {
    ...
    palette.help_system_mut().toggle_overlay();
    assert!(
        palette.help_system().show_overlay,
...
fn test_error_recovery_workflow() {
    ...
    if let Some(details_idx) = error_modal
        .actions
        .iter()
        .position(|a| matches!(a, ErrorAction::ViewDetails))
    {
