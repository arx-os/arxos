            ...
            if self.config.enable_coordinate_validation:
                ...
                if not validation_result.is_valid:
                    if self.config.should_fail_on_violation():
                        raise ValueError(
                            f"Invalid coordinate: {validation_result.errors}"
                        )
...
class PrecisionGeometryFactory:
    ...
    def create_circle_2d(
        ...
        area = self.precision_math.multiply(
            self.precision_math.multiply(radius, radius), self.precision_math.pi()
        )
        perimeter = self.precision_math.multiply(
            self.precision_math.multiply(radius, 2), self.precision_math.pi()
        )
    ...
    def create_rectangle_2d(
        ...
        perimeter = self.precision_math.multiply(
            self.precision_math.add(width, height), 2
        )
    ...
    def _calculate_polygon_area(self, coordinates: List[PrecisionCoordinate]) -> float:
        ...
        for i in range(len(coordinates)):
            ...
            area = self.precision_math.add(
                area, self.precision_math.multiply(coordinates[i].x, coordinates[j].y)
            )
            area = self.precision_math.subtract(
                area, self.precision_math.multiply(coordinates[j].x, coordinates[i].y)
            )

        return abs(area) / 2.0

    def _calculate_polygon_perimeter(
        self, coordinates: List[PrecisionCoordinate]
    ) -> float:
        ...
        if len(coordinates) > 2:
            perimeter = self.precision_math.add(
                perimeter, self.precision_math.distance(coordinates[-1], coordinates[0])
            )
...
class PrecisionGeometryTransformer:
    ...
    def translate(
        self, geometry: Dict[str, Any], dx: float, dy: float, dz: float = 0.0
    ) -> Dict[str, Any]:
        ...
        if "precision_coordinates" in geometry:
            ...
            geometry["coordinates"] = self.adapter.from_precision_coordinates(
                transformed_coords
            )

        elif "precision_coordinate" in geometry:
            # Transform single coordinate
            transformed_coord = geometry["precision_coordinate"].transform(
                translation_coord
            )
            geometry["precision_coordinate"] = transformed_coord
            geometry["coordinates"] = self.adapter.from_precision_coordinate(
                transformed_coord
            )
    ...
    def rotate(
        ...
        center_coord = self.adapter.to_precision_coordinate(center_x, center_y, 0.0)
        angle_radians = self.precision_math.multiply(
            angle_degrees, self.precision_math.pi() / 180.0
        )
        ...
        if "precision_coordinates" in geometry:
            ...
            rotated_coords = []
            for coord in geometry["precision_coordinates"]:
                rotated_coord = self._rotate_coordinate(
                    coord, center_coord, angle_radians
                )
            ...
            rotated_coord = self._rotate_coordinate(
                geometry["precision_coordinate"], center_coord, angle_radians
            )
    ...
    def _rotate_coordinate(
        self, coord: PrecisionCoordinate, center: PrecisionCoordinate, angle: float
    ) -> PrecisionCoordinate:
    ...
    def validate_geometry(self, geometry: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "precision_coordinates" in geometry:
                ...
                coord_validation = self.coordinate_validator.validate_coordinate(
                    geometry["precision_coordinate"]
                )
                if not coord_validation.is_valid:
                    validation_result["is_valid"] = False
                    validation_result["errors"].append(
                        f"Coordinate: {coord_validation.errors}"
                    )
    ...
    def _check_precision_violations(self, geometry: Dict[str, Any]) -> List[str]:
        ...
        if "precision_coordinates" in geometry:
            for i, coord in enumerate(geometry["precision_coordinates"]):
                ...
                if (
                    abs(coord.x - round(coord.x / precision_value) * precision_value)
                    > 1e-10
                ):
                    violations.append(f"Coordinate {i} x-value not at precision level")
                if (
                    abs(coord.y - round(coord.y / precision_value) * precision_value)
                    > 1e-10
                ):
...
class PrecisionIntegrationManager:
    ...
    def create_geometry(self, geometry_type: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            if geometry_type == "point_2d":
                ...
                return self.factory.create_point_3d(
                    kwargs["x"], kwargs["y"], kwargs.get("z", 0.0)
                )
    ...
    def transform_geometry(
        self, geometry: Dict[str, Any], transform_type: str, **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            if transform_type == "translate":
                ...
                return self.transformer.rotate(
                    geometry,
                    kwargs["angle"],
                    kwargs.get("center_x", 0.0),
                    kwargs.get("center_y", 0.0),
                )
            else:
    ...
    def convert_legacy_geometry(self, geometry: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if geometry_type in ["point_2d", "point_3d"]:
                ...
                return self.create_geometry(
                    geometry_type, center_x=center[0], center_y=center[1], radius=radius
                )
                ...
                return self.create_geometry(
                    geometry_type, x=x, y=y, width=width, height=height
                )
