...
fn create_test_building(temp_dir: &TempDir, building_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    use std::fs;
...
"#, building_name, building_name);
    ...
    repo.commit(Some("HEAD"), &signature, &signature, "Initial building file", &tree, &[])?;
...
fn test_user_attribution_e2e_ar_scan() -> Result<(), Box<dyn std::error::Error>> {
    ...
    unsafe {
        ...
        // Should succeed
        assert!(parsed.get("success").and_then(|s| s.as_bool()).unwrap_or(false),
    ...
    let git_manager = BuildingGitManager::new(
        &repo_path_str,
        building_name,
        config
    )?;
    ...
    let commit_with_attribution = commits.iter()
    ...
    assert!(commit_with_attribution.message.contains("ArxOS-User-ID"),
    ...
    let user_from_id = registry_after.find_by_id(&extracted_user_id)
    ...
    let user_commits: Vec<_> = all_commits.iter()
        .filter(|c| {
...
fn test_user_attribution_e2e_confirm_equipment() -> Result<(), Box<dyn std::error::Error>> {
    ...
    unsafe {
        ...
        let save_result = ffi::arxos_save_ar_scan(
            json_ptr,
            building_ptr,
            user_email_ptr,
            0.7
        ...
        let pending_ids = parsed.get("pending_ids")
    ...
    let confirm_commit = commits.iter()
