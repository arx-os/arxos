...
def example_complex_use_cases():
    ...
    try:
        with uow:
            ...
            create_response = create_with_floors_uc.execute(
                building_request, floors_data
            )
            ...
            if create_response.success:
                ...
                if hierarchy_response.success:
                    ...
                    for floor in building["floors"]:
                        print(
                            f"    Floor {floor['floor_number']}: {floor['name']} ({floor['room_count']} rooms)"
                        )

                        for room in floor["rooms"]:
                            print(
                                f"      Room {room['room_number']}: {room['name']} ({room['device_count']} devices)"
                            )

                            for device in room["devices"]:
                                print(
                                    f"        Device: {device['name']} ({device['device_type']})"
                                )
                else:
                    print(
                        f"❌ Failed to get hierarchy: {hierarchy_response.error_message}"
                    )
            else:
                print(
                    f"❌ Failed to create building with floors: {create_response.error_message}"
                )
    ...
    try:
        with uow:
            ...
            if create_response.success:
                ...
                add_room_response = add_room_uc.execute(
                    floor_id, room_data, devices_data
                )
                ...
                if stats_response["success"]:
                    ...
                    print(
                        f"❌ Failed to get statistics: {stats_response['error_message']}"
                    )
            else:
                print(
                    f"❌ Failed to create smart building: {create_response.error_message}"
                )
