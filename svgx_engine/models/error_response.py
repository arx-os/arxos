...
class ErrorResponse(BaseModel):
    ...
    category: ErrorCategory = Field(
        ErrorCategory.INTERNAL, description="Error category"
    )
...
class NotFoundErrorResponse(ErrorResponse):
    ...
    search_criteria: Optional[Dict[str, Any]] = Field(
        None, description="Search criteria used"
    )
...
class ConflictErrorResponse(ErrorResponse):
    ...
    conflicting_resource: Optional[str] = Field(
        None, description="Conflicting resource"
    )
...
def error_response_to_json_response(
    error_response: ErrorResponse, status_code: int = 400
) -> Dict[str, Any]:
