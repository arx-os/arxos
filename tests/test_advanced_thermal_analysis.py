...
class TestAdvancedThermalAnalysis(unittest.TestCase):
    ...
    def test_temperature_dependent_properties(self):
        ...
        thermal_conductivity_solid = water_material.get_property(
            "thermal_conductivity", temp_solid
        )
        ...
        thermal_conductivity_liquid = water_material.get_property(
            "thermal_conductivity", temp_liquid
        )
        ...
        thermal_conductivity_gas = water_material.get_property(
            "thermal_conductivity", temp_gas
        )
    ...
    def test_phase_change_materials(self):
        ...
        self.assertEqual(
            aluminum_material.get_phase(solid_al_temp), MaterialPhase.SOLID
        )
        self.assertEqual(
            aluminum_material.get_phase(liquid_al_temp), MaterialPhase.LIQUID
        )
        ...
        self.assertAlmostEqual(
            water_material.latent_heat_vaporization, 2260000, delta=10000
        )
    ...
    def test_advanced_boundary_conditions(self):
        ...
        self.assertNotEqual(
            value_t5, value_t0
        )  # Should be different due to oscillation
        self.assertAlmostEqual(
            value_t10, value_t0, delta=0.1
        )  # Should be same after period
    ...
    def test_non_linear_solver(self):
        ...
        for solver_type in solver_types:
            ...
            results = self.service.solve_advanced_thermal_analysis(
                self.test_mesh, self.test_materials, self.test_boundary_conditions
            )

            # Check convergence
            self.assertTrue(results["convergence_info"]["converged"])
            self.assertLess(
                results["convergence_info"]["residual"],
    ...
    def test_error_handling(self):
        ...
        with self.assertRaises(KeyError):
            self.service.solve_advanced_thermal_analysis(
                self.test_mesh, invalid_materials, self.test_boundary_conditions
            )

        # Test with empty mesh
        with self.assertRaises(Exception):
            self.service.solve_advanced_thermal_analysis(
                [], self.test_materials, self.test_boundary_conditions
            )
        ...
        results = self.service.solve_advanced_thermal_analysis(
            self.test_mesh, self.test_materials, [invalid_bc]
        )
    ...
    def test_performance_benchmark(self):
        ...
        for i in range(50):  # 50 nodes
            large_mesh.append(
                {"id": str(i), "position": [i * 0.01, 0.0, 0.0], "size": 0.01}
            )
        ...
        results = self.service.solve_advanced_thermal_analysis(
            large_mesh, large_materials, self.test_boundary_conditions
        )
    ...
    def test_visualization_capabilities(self):
        ...
        material_phases = {
            ...
            "1": MaterialPhase.SOLID,
            "2": MaterialPhase.LIQUID,
            "3": MaterialPhase.LIQUID,
        ...
        try:
            self.service.visualize_thermal_results(
                self.test_mesh, temperature_field, heat_flux, material_phases
            )
...
def run_performance_tests():
    ...
    for size in mesh_sizes:
        mesh = []
        for i in range(size):
            mesh.append({"id": str(i), "position": [i * 0.01, 0.0, 0.0], "size": 0.01})
