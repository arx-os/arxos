// Audit Logs Management JavaScript
let currentPage = 1;
let currentFilters = {};
...
function getObjectTypeColor(objectType) {
    switch (objectType) {
...
function updatePagination(data) {
    ...
    const endRecord = Math.min(data.page * data.page_size, data.total);
...
function updateLogCount(total) {
    document.getElementById('logCount').textContent = `Total: ${total} audit log entries`;
}
...
function exportLogs() {
    ...
    link.href = `/api/audit-logs?${queryParams}`;
...
function showLogDetails(logId) {
    fetch(`/api/audit-logs?object_id=${logId}`)
        .then(response => response.json())
        .then(data => {
                            ...
                            ${Object.entries(fieldChanges).map(([field, change]) => `
                                <div class="bg-gray-50 p-3 rounded">
                                    ...
                                    <div class="text-sm text-gray-600">
                                        <div>Before: <span class="font-mono">${change.before || 'null'}</span></div>
                                        <div>After: <span class="font-mono">${change.after || 'null'}</span></div>
                                    </div>
                                </div>
                            `).join('')}
                        </div>
