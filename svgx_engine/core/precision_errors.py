    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "corrected_coordinates": [
                str(coord) for coord in self.corrected_coordinates
            ],
        }
...
class PrecisionErrorHandler:
    ...
    def handle_error(
        ...
        if self.current_report:
            if (
                severity == PrecisionErrorSeverity.CRITICAL
                or severity == PrecisionErrorSeverity.ERROR
            ):
        ...
        if attempt_recovery and error_type in self.recovery_strategies:
            try:
                ...
                self.logger.error(
                    f"Recovery strategy failed for error {error.error_id}: {recovery_error}"
                )
    ...
    def register_recovery_strategy(
        self, error_type: PrecisionErrorType, strategy: callable
    ) -> None:
    ...
    def _register_default_recovery_strategies(self) -> None:
        ...
        self.register_recovery_strategy(
            PrecisionErrorType.TRANSFORMATION_ERROR, self._recover_transformation_error
        )

        # Constraint violation recovery
        self.register_recovery_strategy(
            PrecisionErrorType.CONSTRAINT_VIOLATION, self._recover_constraint_violation
        )
    ...
    def _recover_coordinate_range_violation(self, error: PrecisionError) -> bool:
        ...
        try:
            ...
            for coord in error.coordinates:
                ...
                corrected_coord = PrecisionCoordinate(
