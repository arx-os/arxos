...
def test_svgx_simulated_telemetry():
    ...
    try:
        ...
        types = [
            SVGXTelemetryType.SVGX_PARSER,
            SVGXTelemetryType.SVGX_RUNTIME,
            SVGXTelemetryType.SVGX_COMPILER,
            SVGXTelemetryType.SVGX_CACHE,
        ]

        simulated_data = generate_svgx_simulated_telemetry(
            sources=sources, types=types, count=50, namespace="simulation_namespace"
        )
