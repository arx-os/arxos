...
type PullRequestUseCase struct {
	...
	assignmentRepo versioncontrol.PRAssignmentRuleRepository
...
func (uc *PullRequestUseCase) ApprovePullRequest(
	...
	if pr.ApprovedCount >= pr.RequiredReviewers {
		pr.Status = versioncontrol.PRStatusApproved
		uc.logger.Info("PR approved and ready to merge", "pr", prID)
	} else {
		pr.Status = versioncontrol.PRStatusInReview
	}
...
func (uc *PullRequestUseCase) MergePullRequest(
	...
	// Validate PR status
	if pr.Status != versioncontrol.PRStatusApproved && pr.RequiresReview {
...
func (uc *PullRequestUseCase) inferPRType(sourceBranch *versioncontrol.Branch) versioncontrol.PRType {
	switch sourceBranch.BranchType {
	case versioncontrol.BranchTypeContractor:
		return versioncontrol.PRTypeContractorWork
	case versioncontrol.BranchTypeVendor:
		return versioncontrol.PRTypeVendorService
	case versioncontrol.BranchTypeIssue:
		return versioncontrol.PRTypeIssueFix
	case versioncontrol.BranchTypeScan:
		return versioncontrol.PRTypeScanUpload
	default:
		return versioncontrol.PRTypeOther
	}
}
...
func (uc *PullRequestUseCase) autoAssignPR(
	ctx context.Context,
	repositoryID types.ID,
	prType versioncontrol.PRType,
	priority versioncontrol.PRPriority,
) (*types.ID, bool) {
...
func (uc *PullRequestUseCase) GetPRStats(
	...
	openStatus := versioncontrol.PRStatusOpen
	approvedStatus := versioncontrol.PRStatusApproved
	inReviewStatus := versioncontrol.PRStatusInReview

	openCount, _ := uc.prRepo.Count(versioncontrol.PRFilter{
		RepositoryID: &repositoryID,
		Status:       &openStatus,
	})

	approvedCount, _ := uc.prRepo.Count(versioncontrol.PRFilter{
		RepositoryID: &repositoryID,
		Status:       &approvedStatus,
	})

	inReviewCount, _ := uc.prRepo.Count(versioncontrol.PRFilter{
		RepositoryID: &repositoryID,
...
func (uc *PullRequestUseCase) must(prs []*versioncontrol.PullRequest, err error) []*versioncontrol.PullRequest {
	if err != nil {
		return []*versioncontrol.PullRequest{}
	}
	return prs
}
