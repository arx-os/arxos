...
fn test_complete_ar_workflow_with_git() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let initial_result =
        git_manager.export_building(&initial_building, Some("Initial building setup"))?;
    ...
    let pending_id = pending_manager
    ...
    let commit_result =
        git_manager.export_building(&building_data, Some("Added VAV-301-New from AR scan"))?;
    ...
    assert!(
        building_yaml_path.exists(),
        "building.yml should exist after export"
    );
    let content = fs::read_to_string(&building_yaml_path)?;
    assert!(
        content.contains("VAV-301-New"),
        "YAML should contain new equipment"
    );
    ...
    assert!(
        floor_content.contains("VAV-301-New"),
        "Floor YAML should contain new equipment"
    );
...
fn test_ar_workflow_reject_pending() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pending_id = pending_manager
        .add_pending_equipment(&detected, "scan_1", 3, None, 0.7, None)?
    ...
    assert!(
        ...
        "Should have no pending items after rejection"
    );
    ...
    assert_eq!(
        ...
        0,
        "Default building should have no floors"
    );
