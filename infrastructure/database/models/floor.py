...
class FloorModel(BaseModel):
    ...
    # Status
    status = Column(
        Enum(FloorStatus), nullable=False, default=FloorStatus.PLANNED, index=True
    )

    # Relationships
    building = relationship("BuildingModel", back_populates="floors")
    rooms = relationship(
        "RoomModel", back_populates="floor", cascade="all, delete-orphan"
    )
    ...
    def update_status(
        self, new_status: FloorStatus, updated_by: Optional[str] = None
    ) -> None:
        """Update floor status."""
    ...
    def update_floor_number(
        self, new_floor_number: int, updated_by: Optional[str] = None
    ) -> None:
        """Update floor number."""
