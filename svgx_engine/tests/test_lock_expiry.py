...
class TestLockExpiryAndTimeouts:
    ...
    def test_lock_acquisition_with_timestamp(self):
        ...
        result = self.engine.lock_object(self.canvas_id, self.object_id, self.session_id, self.user_id)
    ...
    def test_lock_expiry_detection(self):
        ...
        # Create a lock
        self.engine.lock_object(self.canvas_id, self.object_id, self.session_id, self.user_id)

        # Manually expire the lock by modifying timestamp
        lock_info = self.engine.locks[self.canvas_id][self.object_id]
        expired_timestamp = (datetime.utcnow() - timedelta(seconds=400)).isoformat()  # 400 seconds ago
    ...
    def test_lock_conflict_with_expiry(self):
        ...
        result = self.engine.lock_object(self.canvas_id, self.object_id, new_session_id, self.user_id)
        assert result["status"] == "lock_acquired"
    ...
    def test_cleanup_expired_locks(self):
        ...
        self.engine.locks[self.canvas_id]["obj1"]["timestamp"] = (datetime.utcnow() - timedelta(seconds=400)).isoformat()
        self.engine.locks[self.canvas_id]["obj2"]["timestamp"] = (datetime.utcnow() - timedelta(seconds=400)).isoformat()
    ...
    async def test_api_set_lock_timeout(self):
        ...
        response = self.client.post("/runtime/lock-timeout/", json={"timeout_seconds": 600})
    ...
    async def test_api_set_lock_timeout_invalid(self):
        ...
        response = self.client.post("/runtime/lock-timeout/", json={"timeout_seconds": -1})
        assert response.status_code == 400

        # Test zero timeout
        response = self.client.post("/runtime/lock-timeout/", json={"timeout_seconds": 0})
        assert response.status_code == 400
    ...
    async def test_api_release_session_locks(self):
        ...
        response = self.client.post("/runtime/release-session-locks/", json={"session_id": self.session_id})
    ...
    def test_lock_expiry_integration(self):
        ...
        result = self.engine.lock_object(self.canvas_id, self.object_id, new_session, self.user_id)
        assert result["status"] == "lock_acquired"
    ...
    def test_lock_expiry_edge_cases(self):
        ...
        result = self.engine.lock_object(self.canvas_id, self.object_id, "new-session", self.user_id)
        assert result["status"] == "lock_acquired"
    ...
    def test_multiple_canvas_locks(self):
        ...
        # Expire one lock
        self.engine.locks["canvas1"]["obj1"]["timestamp"] = (datetime.utcnow() - timedelta(seconds=400)).isoformat()
