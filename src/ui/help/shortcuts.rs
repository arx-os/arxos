...
mod tests {
    ...
    fn test_shortcuts_by_category() {
        let shortcuts = get_all_shortcuts();
        let navigation: Vec<_> = shortcuts
            .iter()
    ...
    fn test_shortcut_categories_coverage() {
        ...
        let categories: std::collections::HashSet<_> =
            shortcuts.iter().map(|s| s.category).collect();
    ...
    fn test_shortcut_no_empty_fields() {
        ...
        for shortcut in &shortcuts {
            assert!(
                !shortcut.key.trim().is_empty(),
                "Shortcut key should not be empty or whitespace"
            );
            assert!(
                !shortcut.description.trim().is_empty(),
                "Shortcut description should not be empty or whitespace"
            );
        }
    }
}
