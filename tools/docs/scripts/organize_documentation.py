...
class DocumentationOrganizer:
    ...
    def _determine_target_subdir(self, md_file: Path) -> str:
        ...
        # API documentation
        if any(
            keyword in filename for keyword in ["api", "endpoint", "rest", "graphql"]
        ):
            return "api"
        ...
        if any(
            keyword in filename
            for keyword in ["deploy", "infra", "kubernetes", "docker", "aws"]
        ):
            return "deployment"
        ...
        if any(
            keyword in filename
            for keyword in ["dev", "setup", "install", "build", "test"]
        ):
            return "development"
        ...
        if any(
            keyword in filename
            for keyword in ["user", "guide", "tutorial", "howto", "usage"]
        ):
            return "user-guides"
        ...
        if any(
            keyword in content for keyword in ["api", "endpoint", "request", "response"]
        ):
            return "api"
        elif any(
            keyword in content for keyword in ["deploy", "infrastructure", "kubernetes"]
        ):
            return "deployment"
        elif any(
            keyword in content
            for keyword in ["setup", "install", "development", "build"]
        ):
            return "development"
        elif any(
            keyword in content for keyword in ["user", "guide", "tutorial", "usage"]
        ):
            return "user-guides"
        elif any(
