...
use super::types::{FilterConfig, SearchConfig, SearchResult};
...
impl SearchEngine {
    ...
    pub fn new(building_data: &BuildingData) -> Self {
        ...
        let building = Building {
            ...
            coordinate_systems: building_data
                .coordinate_systems
                .iter()
                .map(|cs| crate::core::CoordinateSystemInfo {
                    name: cs.name.clone(),
                    origin: cs.origin,
                    x_axis: cs.x_axis,
                    y_axis: cs.y_axis,
                    ...
                    results.push(SearchResult {
                        ...
                        status: Some(format!(
                            ...
                            equipment
                                .health_status
                                .as_ref()
                                .unwrap_or(&crate::core::EquipmentHealthStatus::Unknown)
                        )),
    ...
    fn matches_filter(&self, equipment: &Equipment, config: &FilterConfig) -> bool {
        ...
        if let Some(ref building) = config.building {
            if !self.buildings[0]
                .name
                .to_lowercase()
                .contains(&building.to_lowercase())
            {
                return false;
            }
        }
