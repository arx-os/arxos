...
class MCPIntegrationTest:
    ...
    async def test_authentication(self):
        ...
        try:
            ...
            has_permission = auth_manager.has_permission(
                user, Permission.READ_VALIDATION
            )
    ...
    async def test_rule_engine_integration(self):
        ...
        try:
            ...
            assert (
                jurisdiction_info["location_found"] == True
    ...
    async def test_cache_integration(self):
        ...
        try:
            ...
            success = await redis_manager.cache_building_model(
                building_model.building_id, building_model.dict()
            )
            assert success, "Failed to cache building model"

            # Test retrieving cached building model
            cached_model = await redis_manager.get_cached_building_model(
                building_model.building_id
            )
            assert cached_model is not None, "Failed to retrieve cached building model"
            assert (
                cached_model["building_id"] == building_model.building_id
            ), "Cached model mismatch"

            # Test cache invalidation
            success = await redis_manager.invalidate_cache(
                building_model.building_id, "building_model"
            )
