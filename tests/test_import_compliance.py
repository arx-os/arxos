...
class ImportComplianceChecker:
    ...
    def _check_file(self, file_path: str) -> None:
        ...
        try:
            ...
            if relative_imports:
                ...
                logger.warning(
                    f"Found {len(relative_imports)} relative imports in {file_path}"
                )
    ...
    def _suggest_import_fix(self, import_line: str, file_path: str) -> str:
        ...
        if levels_up == 0:
            ...
            if current_dir == ".":
                ...
                absolute_path = (
                    f"svgx_engine.{current_dir.replace(os.sep, '.')}.{module_path}"
                )
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"  Files with relative imports: {self.stats['files_with_relative_imports']}"
        )
        report.append(
            f"  Total relative imports found: {self.stats['relative_imports_found']}"
        )
        ...
        if self.issues:
            ...
            for issue in self.issues:
                ...
                for rel_import in issue["relative_imports"]:
                    report.append(
                        f"  Line {rel_import['line_number']}: {rel_import['line_content']}"
                    )
        ...
        report.append(
            "3. Import specific classes/functions rather than using 'import *'"
        )
...
def test_import_compliance():
    ...
    assert results[
        "compliant"
    ], f"Found {len(results['issues'])} files with relative imports"
