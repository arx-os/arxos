...
class SDKTester:
    ...
    def test_all_sdks(
        self, language: str = None, service: str = None, test_type: str = None
    ):
    ...
    def _run_typescript_unit_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            subprocess.run(
                ["npm", "install"], cwd=sdk_dir, check=True, capture_output=True
            )
        ...
        try:
            result = subprocess.run(
                ["npm", "test"], cwd=sdk_dir, capture_output=True, text=True
            )
            ...
            return TestResult(
                name="TypeScript Unit Tests", status="FAILED", duration=0, error=str(e)
            )
    ...
    def _run_python_unit_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                name="Python Unit Tests", status="FAILED", duration=0, error=str(e)
            )
    ...
    def _run_go_unit_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            result = subprocess.run(
                ["go", "test", "./..."], cwd=sdk_dir, capture_output=True, text=True
            )
            ...
            return TestResult(
                name="Go Unit Tests", status="FAILED", duration=0, error=str(e)
            )
    ...
    def _run_java_unit_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            result = subprocess.run(
                ["mvn", "test"], cwd=sdk_dir, capture_output=True, text=True
            )
            ...
            return TestResult(
                name="Java Unit Tests", status="FAILED", duration=0, error=str(e)
            )
    ...
    def _run_csharp_unit_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            result = subprocess.run(
                ["dotnet", "test"], cwd=sdk_dir, capture_output=True, text=True
            )
            ...
            return TestResult(
                name="C# Unit Tests", status="FAILED", duration=0, error=str(e)
            )
    ...
    def _run_php_unit_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            subprocess.run(
                ["composer", "install"], cwd=sdk_dir, check=True, capture_output=True
            )
        except subprocess.CalledProcessError as e:
            return TestResult(
                name="PHP Unit Tests",
        ...
        try:
            result = subprocess.run(
                ["vendor/bin/phpunit"], cwd=sdk_dir, capture_output=True, text=True
            )
            ...
            return TestResult(
                name="PHP Unit Tests", status="FAILED", duration=0, error=str(e)
            )
    ...
    def _run_typescript_build_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            result = subprocess.run(
                ["npm", "run", "build"], cwd=sdk_dir, capture_output=True, text=True
            )
            ...
            return TestResult(
                name="TypeScript Build Tests", status="FAILED", duration=0, error=str(e)
            )
    ...
    def _run_python_build_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                name="Python Build Tests", status="FAILED", duration=0, error=str(e)
            )
    ...
    def _run_go_build_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            result = subprocess.run(
                ["go", "build", "./..."], cwd=sdk_dir, capture_output=True, text=True
            )
            ...
            return TestResult(
                name="Go Build Tests", status="FAILED", duration=0, error=str(e)
            )
    ...
    def _run_java_build_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            result = subprocess.run(
                ["mvn", "compile"], cwd=sdk_dir, capture_output=True, text=True
            )
            ...
            return TestResult(
                name="Java Build Tests", status="FAILED", duration=0, error=str(e)
            )
    ...
    def _run_csharp_build_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            result = subprocess.run(
                ["dotnet", "build"], cwd=sdk_dir, capture_output=True, text=True
            )
            ...
            return TestResult(
                name="C# Build Tests", status="FAILED", duration=0, error=str(e)
            )
    ...
    def _run_php_build_tests(self, sdk_dir: Path) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                name="PHP Build Tests", status="FAILED", duration=0, error=str(e)
            )
...
def main():
    ...
    parser.add_argument(
        "--sdk-path", help="Path to generated SDKs", default="generated"
    )
