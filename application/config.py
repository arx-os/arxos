    ...
    def load_from_environment(self) -> ApplicationConfig:
        ...
        try:
            ...
            raise ConfigurationError(
                "env_loading", f"Failed to load environment configuration: {e}"
            )
    ...
    def load_from_file(self, config_file: str) -> ApplicationConfig:
        ...
        try:
            ...
            if not config_path.exists():
                raise ConfigurationError(
                    "file_not_found", f"Configuration file not found: {config_file}"
                )
            ...
            self.logger.error(
                f"Failed to load configuration from file {config_file}: {e}"
            )
            raise ConfigurationError(
                "file_loading", f"Failed to load file configuration: {e}"
            )
    ...
    def validate_config(self, config: ApplicationConfig) -> None:
        ...
        try:
            ...
            if config.database.port <= 0 or config.database.port > 65535:
                raise ConfigurationError(
                    "db_port", "Database port must be between 1 and 65535"
                )
            ...
            if config.cache.port <= 0 or config.cache.port > 65535:
                raise ConfigurationError(
                    "redis_port", "Redis port must be between 1 and 65535"
                )
            ...
            if config.message_queue.port <= 0 or config.message_queue.port > 65535:
                raise ConfigurationError(
                    "mq_port", "Message queue port must be between 1 and 65535"
                )
            ...
            raise ConfigurationError(
                "validation", f"Configuration validation failed: {e}"
            )
...
# Compatibility function for new configuration structure
def get_settings() -> ApplicationConfig:
    """Get application settings (compatibility with new config structure)."""
    return get_config()


# Alias for new configuration structure
