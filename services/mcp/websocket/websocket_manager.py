...
class WebSocketManager:
    ...
    async def connect(
        self, websocket: WebSocket, building_id: str, session_id: Optional[str] = None
    ):
        ...
        try:
            ...
            self.logger.info(
                f"Client connected to building {building_id} (total connections: {self.connection_count})"
            )
            ...
            if building_id in self.active_connections:
                if websocket in self.active_connections[building_id]:
                    ...
                    if building_id in self.building_sessions:
                        ...
                        if (
                            self.building_sessions[building_id]["connected_clients"]
                            <= 0
                        ):
                    ...
                    self.logger.info(
                        f"Client disconnected from building {building_id} (remaining connections: {self.connection_count})"
                    )
    ...
    async def broadcast_validation(
        self, building_id: str, validation_data: dict, session_id: Optional[str] = None
    ):
        ...
        self.logger.info(
            f"Broadcasted validation update to {len(self.active_connections[building_id])} clients for building {building_id}"
        )

    async def broadcast_violation_highlight(
        self, building_id: str, violation_data: dict, session_id: Optional[str] = None
    ):
    ...
    async def broadcast_compliance_score(
        self, building_id: str, compliance_data: dict, session_id: Optional[str] = None
    ):
    ...
    async def broadcast_rule_check(
        self, building_id: str, rule_data: dict, session_id: Optional[str] = None
    ):
    ...
    async def broadcast_error(
        self, building_id: str, error_data: dict, session_id: Optional[str] = None
    ):
