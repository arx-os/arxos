...
class ProjectApplicationService:
            ...
            if result.success:
                ...
                if self.metrics:
                    self.metrics.record_timing(
                        "project.create", time.time() - start_time
                    )
    ...
    def update_project(
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    "Updating project", project_id=project_id, updated_by=updated_by
                )
            ...
            if result.success:
                ...
                if self.event_store:
                    project_updated_event = ProjectUpdated(
                        project_id=project_id,
                        updated_fields=[
                            field for field in [name, status] if field is not None
                        ],
                        updated_by=updated_by,
                    )
                ...
                if self.metrics:
                    self.metrics.record_timing(
                        "project.update", time.time() - start_time
                    )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to update project", project_id=project_id, error=str(e)
                )
        ...
        try:
            ...
            if self.cache_service:
                ...
                if cached_project:
                    ...
                    return GetProjectResponse(success=True, project=cached_project)
            ...
            if result.success:
                ...
                if self.cache_service:
                    self.cache_service.set(
                        f"project:{project_id}", result.project, ttl=300
                    )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to get project", project_id=project_id, error=str(e)
                )
    ...
    def delete_project(self, project_id: str) -> DeleteProjectResponse:
        ...
        try:
            # Log the operation
            if self.logger:
                self.logger.info("Deleting project", project_id=project_id)
            ...
            if result.success:
                ...
                if self.metrics:
                    self.metrics.record_timing(
                        "project.delete", time.time() - start_time
                    )
    ...
    def get_project_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.metrics:
                self.metrics.record_timing(
                    "project.statistics", time.time() - start_time
                )
            ...
            # Log error
            if self.logger:
                self.logger.error("Failed to get project statistics", error=str(e))

            # Record error metrics
            if self.metrics:
                self.metrics.increment_counter("project.statistics.error")

            return {"error": f"Failed to get project statistics: {str(e)}"}
