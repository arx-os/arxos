...
def test_iot_integration():
    ...
    try:
        ...
        print(
            f"✅ Device registration: Sensor={sensor_success}, Actuator={actuator_success}"
        )
        ...
        print(
            f"✅ IoT system statistics: {stats['iot_stats']['total_devices']} devices"
        )
...
def test_advanced_visualization():
    ...
    try:
        ...
        print(
            f"✅ Visualization statistics: {stats['visualization_stats']['total_dashboards']} dashboards"
        )
...
def test_enterprise_integration():
    ...
    try:
        ...
        erp_success = enterprise_system.connect_erp_system(
            ERPSystem.SAP, "https://test-erp.example.com", "test_user", "test_password"
        )
        ...
        report_id = enterprise_system.create_compliance_report(
            ComplianceType.SAFETY, "Q1 2024", compliance_data
        )
        ...
        print(
            f"✅ Enterprise statistics: {stats['enterprise_stats']['total_erp_syncs']} syncs"
        )
...
async def test_phase3_integration():
    ...
    try:
        ...
        iot_config = IoTConfig(
            mqtt_broker="localhost", mqtt_port=1883, data_buffer_size=1000
        )
        iot_system = IoTIntegrationSystem(iot_config)

        viz_config = VisualizationConfig(
            render_engine="plotly", update_frequency=1.0, max_data_points=1000
        )
        ...
        enterprise_system.sync_to_erp(
            ERPSystem.SAP, "sensor_data", {"sensor_id": "test_001", "value": 24.5}
        )
