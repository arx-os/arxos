...
class TestSVGXSerializer:
    ...
    def test_to_json_pretty_print(self):
        ...
        element = SVGXElement(
            id="test-element", tag="circle", properties={"radius": 25}
        )

        document = SVGXDocument(id="test-document", elements=[element])
    ...
    def test_from_dict_reconstruction(self):
        ...
        assert (
            reconstructed.metadata["project"] == original_document.metadata["project"]
        )
        assert len(reconstructed.elements) == 1
        assert reconstructed.elements[0].id == original_element.id
        assert (
            reconstructed.elements[0].properties["width"]
            == original_element.properties["width"]
        )
...
class TestPersistenceExportService:
    ...
    def test_create_export_job(self):
        ...
        try:
            ...
            job_id = self.service.create_export_job(
                self.test_document, file_path, options
            )
    ...
    def test_database_operations(self):
        ...
        assert (
            loaded_document.metadata["project"]
            == self.test_document.metadata["project"]
        )
            ...
            element = SVGXElement(
                ...
                metadata={
                    ...
                    "system": (
                        "electrical"
                        if i % 3 == 0
                        else "hvac" if i % 3 == 1 else "plumbing"
                    ),
                },
            )
    ...
    def test_full_export_workflow(self):
        ...
        try:
            ...
            for format_type in formats:
                ...
                # Export document
                self.service.export_svgx_document(
                    self.test_document, file_path, options
                )
        ...
        try:
            ...
            for i in range(3):
                with tempfile.NamedTemporaryFile(
                    mode="w", suffix=f".json", delete=False
                ) as f:
                    file_path = f.name
