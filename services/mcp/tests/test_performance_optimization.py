...
class TestPerformanceOptimization:
    ...
    def setup_method(self):
        ...
        self.cached_engine = CachedRuleEngine(
            self.base_engine, self.cache_manager
        )
    ...
    def create_large_building_model(self, num_objects: int) -> BuildingModel:
        ...
        for i in range(num_objects):
            ...
            obj = BuildingObject(
                ...
                location=location,
                connections=[f"object_{(i+1) % num_objects:06d}"] if i < num_objects - 1 else []
            )
            objects.append(obj)
    ...
    def test_parallel_processing_performance(self):
        ...
        base_result = self.base_engine.validate_building_model(building_model, mcp_files)
        ...
        if len(building_model.objects) > 2000 and base_execution_time > 0.01:
            assert parallel_execution_time < base_execution_time * 2.0, \
        ...
        assert base_result.overall_compliance_score == parallel_result.overall_compliance_score
        
        print(f"Parallel processing: base {base_execution_time:.3f}s, parallel {parallel_execution_time:.3f}s")
    ...
    def test_memory_optimization_performance(self):
        ...
        initial_memory = self.process.memory_info().rss / 1024 / 1024
        optimized_result = self.memory_optimized_engine.validate_building_model(building_model, mcp_files)
        optimized_memory = self.process.memory_info().rss / 1024 / 1024
        optimized_memory_used = optimized_memory - initial_memory
        
        # Results should be equivalent
        assert base_result.overall_compliance_score == optimized_result.overall_compliance_score
        
        # Memory usage should be reasonable
        assert optimized_memory_used < 1000, f"Memory usage {optimized_memory_used:.1f}MB exceeds 1GB limit"
        
        print(f"Memory optimization: base {base_memory_used:.1f}MB, optimized {optimized_memory_used:.1f}MB")
    ...
    def test_caching_performance(self):
        ...
        print(f"Caching performance: miss {first_execution_time:.3f}s, hit {second_execution_time:.3f}s")
    ...
    def test_parallel_engine_scalability(self):
        ...
        for size in model_sizes:
            ...
            start_time = time.time()
            result = self.parallel_engine.validate_building_model_parallel(building_model, mcp_files)
            execution_time = time.time() - start_time
            
            results[size] = {
                'execution_time': execution_time,
                'objects_per_second': size / execution_time if execution_time > 0 else 0
            }
            
            print(f"Size {size}: {execution_time:.3f}s, {results[size]['objects_per_second']:.0f} obj/s")
    ...
    def test_memory_manager_optimization(self):
        ...
        for i in range(100):
            obj = self.memory_manager.get_object(f"type_{i}", lambda: {"data": f"value_{i}"})
    ...
    def test_combined_optimization_performance(self):
        ...
        assert execution_time < 30.0, f"Execution time {execution_time:.3f}s exceeds 30.0s limit"
    ...
    def test_parallel_engine_configuration(self):
        ...
        for config in configs:
            ...
            assert result is not None
            assert execution_time < 60.0, f"Configuration {config} took too long: {execution_time:.3f}s"
    ...
    def test_memory_manager_cleanup(self):
        ...
        for i in range(1000):
            obj = self.memory_manager.get_object(f"type_{i}", lambda: {"data": "x" * 1000})
            self.memory_manager.release_object(f"type_{i}", obj)
