...
class ElectricalAnalysisService:
    ...
    def analyze_circuit(
        self, request: ElectricalAnalysisRequest
    ...
    def _dc_analysis(
        ...
        power_dissipation = self._calculate_power_dissipation(
            node_voltages, branch_currents, request
        )
    ...
    def _ac_analysis(
        ...
        n_nodes = len(request.nodes)
        frequency = (
            request.voltage_sources[0].frequency if request.voltage_sources else 60.0
        )
        ...
        # Calculate branch currents
        branch_currents = self._calculate_ac_branch_currents(
            node_voltages, request, frequency
        )

        # Calculate power dissipation
        power_dissipation = self._calculate_ac_power_dissipation(
            node_voltages, branch_currents, request
        )

        # Calculate electromagnetic fields
        voltage_field = self._calculate_ac_voltage_field(
            node_voltages, request, frequency
        )
        current_field = self._calculate_ac_current_field(
            branch_currents, request, frequency
        )
        electric_field = self._calculate_ac_electric_field(
            voltage_field, request, frequency
        )
        magnetic_field = self._calculate_ac_magnetic_field(
            current_field, request, frequency
        )
    ...
    def _frequency_analysis(
        ...
        frequencies = np.logspace(
            np.log10(start_freq), np.log10(end_freq), n_frequencies
        )
        ...
        for freq in frequencies:
            ...
            # Solve nodal equations
            try:
                node_voltages_complex = np.linalg.solve(
                    admittance_matrix, current_vector
                )
        ...
        branch_currents = self._calculate_ac_branch_currents(
            node_voltages, request, frequencies[mid_idx]
        )
        power_dissipation = self._calculate_ac_power_dissipation(
            node_voltages, branch_currents, request
        )
        ...
        current_field = self._calculate_ac_current_field(
            branch_currents, request, frequencies[mid_idx]
        )
        electric_field = self._calculate_ac_electric_field(
            voltage_field, request, frequencies[mid_idx]
        )
        magnetic_field = self._calculate_ac_magnetic_field(
            current_field, request, frequencies[mid_idx]
        )
    ...
    def _build_conductance_matrix(
        ...
        # Add conductances from components
    ...
    def _calculate_branch_currents(
        self, node_voltages: np.ndarray, request: ElectricalAnalysisRequest
    ) -> np.ndarray:
        """Calculate branch currents."""
    ...
    def _calculate_ac_branch_currents(
        self,
        node_voltages: np.ndarray,
        request: ElectricalAnalysisRequest,
        frequency: float,
    ) -> np.ndarray:
    ...
    def _calculate_ac_power_dissipation(
        self,
        node_voltages: np.ndarray,
        branch_currents: np.ndarray,
        request: ElectricalAnalysisRequest,
        ...
        return self._calculate_power_dissipation(
            node_voltages, branch_currents, request
        )

    def _calculate_voltage_field(
        self, node_voltages: np.ndarray, request: ElectricalAnalysisRequest
    ) -> np.ndarray:
    ...
    def _calculate_current_field(
        self, branch_currents: np.ndarray, request: ElectricalAnalysisRequest
    ) -> np.ndarray:
    ...
    def _calculate_electric_field(
        self, voltage_field: np.ndarray, request: ElectricalAnalysisRequest
    ) -> np.ndarray:
    ...
    def _calculate_magnetic_field(
        self, current_field: np.ndarray, request: ElectricalAnalysisRequest
    ) -> np.ndarray:
    ...
    def _calculate_ac_voltage_field(
        self,
        node_voltages: np.ndarray,
        request: ElectricalAnalysisRequest,
        frequency: float,
    ...
    def _calculate_ac_current_field(
        self,
        branch_currents: np.ndarray,
        request: ElectricalAnalysisRequest,
        frequency: float,
    ...
    def _calculate_ac_electric_field(
        self,
    ...
    def _calculate_ac_magnetic_field(
        self,
    ...
    def _update_sources_for_time(
        self, request: ElectricalAnalysisRequest, time: float
    ) -> None:
        """Update source values for current time."""
        for source in request.voltage_sources:
            if source.type == "sine":
                source.magnitude = source.magnitude * math.sin(
                    2 * math.pi * source.frequency * time + source.phase
                )
    ...
    def _calculate_efficiency(
        self, request: ElectricalAnalysisRequest, result: ElectricalAnalysisResult
    ) -> float:
