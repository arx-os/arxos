...
class SlotFiller:
    ...
    def _load_patterns(self):
        ...
        self.slot_patterns = [
            ...
            SlotPattern(
                r"in\s+(\w+)", SlotType.LOCATION, "room_name", confidence_boost=0.05
            ),
            SlotPattern(
                r"at\s+(\w+)", SlotType.LOCATION, "position", confidence_boost=0.05
            ),
            SlotPattern(
                r"to\s+(\w+)", SlotType.LOCATION, "destination", confidence_boost=0.05
            ),
            # Property patterns
            SlotPattern(
                r"color\s+(\w+)", SlotType.PROPERTY, "color", confidence_boost=0.1
            ),
            SlotPattern(
                r"size\s+(\w+)", SlotType.PROPERTY, "size", confidence_boost=0.05
            ),
            SlotPattern(
                r"type\s+(\w+)", SlotType.PROPERTY, "type", confidence_boost=0.05
            ),
            SlotPattern(
                r"width\s+(\d+)", SlotType.PROPERTY, "width", confidence_boost=0.05
            ),
            SlotPattern(
                r"height\s+(\d+)", SlotType.PROPERTY, "height", confidence_boost=0.05
            ),
            SlotPattern(
                r"length\s+(\d+)", SlotType.PROPERTY, "length", confidence_boost=0.05
            ),
            # Value patterns
            SlotPattern(
                r"(\d+)\s*(?:x|by)\s*(\d+)",
            ...
            # Format patterns
            SlotPattern(
                r"format\s+(\w+)", SlotType.FORMAT, "format", confidence_boost=0.05
            ),
            SlotPattern(
                r"as\s+(\w+)", SlotType.FORMAT, "format", confidence_boost=0.05
            ),
            SlotPattern(
                r"in\s+(\w+)\s+format", SlotType.FORMAT, "format", confidence_boost=0.05
            ),
            # Target patterns
            SlotPattern(
                r"to\s+(\w+)", SlotType.TARGET, "target", confidence_boost=0.05
            ),
            SlotPattern(
                r"target\s+(\w+)", SlotType.TARGET, "target", confidence_boost=0.05
            ),
            SlotPattern(
                r"destination\s+(\w+)", SlotType.TARGET, "target", confidence_boost=0.05
            ),
            # Source patterns
            SlotPattern(
                r"from\s+(\w+)", SlotType.SOURCE, "source", confidence_boost=0.05
            ),
            SlotPattern(
                r"source\s+(\w+)", SlotType.SOURCE, "source", confidence_boost=0.05
            ),
            SlotPattern(
                r"origin\s+(\w+)", SlotType.SOURCE, "source", confidence_boost=0.05
            ),
            # Condition patterns
            SlotPattern(
                r"if\s+(\w+)", SlotType.CONDITION, "condition", confidence_boost=0.05
            ),
            SlotPattern(
                r"when\s+(\w+)", SlotType.CONDITION, "condition", confidence_boost=0.05
            ),
            SlotPattern(
                r"where\s+(\w+)", SlotType.CONDITION, "condition", confidence_boost=0.05
            ),
            SlotPattern(
                r"(\w+)\s+only", SlotType.CONDITION, "condition", confidence_boost=0.05
            ),
            # Unit patterns
            SlotPattern(
                r"(\d+)\s*(feet|ft|inches|in|meters|m)",
                SlotType.UNIT,
                "unit",
                confidence_boost=0.05,
            ),
            ...
            # Object ID patterns
            SlotPattern(
                r"id\s+(\w+)", SlotType.OBJECT_ID, "object_id", confidence_boost=0.1
            ),
            SlotPattern(
                r"object\s+(\w+)",
                SlotType.OBJECT_ID,
    ...
    def extract_slots(self, text: str, intent_type: IntentType) -> SlotResult:
            ...
            for match in matches:
                ...
                if self._validate_slot_value(slot_value, pattern):
                    slot = Slot(
                        ...
                        confidence=0.8
                        + pattern.confidence_boost,  # Base confidence for pattern matches
        ...
        return SlotResult(
            slots=final_slots, confidence=min(1.0, len(final_slots) * 0.1 + 0.5)
        )
    ...
    def _extract_intent_specific_slots(
        ...
        if intent_type == IntentType.CREATE:
            ...
            room_match = re.search(
                r"(bedroom|bathroom|kitchen|living|office|dining|study)", text
            )
            if room_match:
                slots.append(
                    Slot(
    ...
    def _merge_overlapping_slots(self, slots: List[Slot]) -> List[Slot]:
        ...
        for slot in sorted_slots:
            ...
            for existing_slot in merged_slots:
                if (
                    slot.start_pos < existing_slot.end_pos
                    and slot.end_pos > existing_slot.start_pos
                ):
    ...
    def get_slot_help(self, slot_type: SlotType) -> str:
        ...
        return help_texts.get(
            slot_type, f"No help available for slot type: {slot_type}"
        )


# Convenience function for quick slot extraction
def extract_slots(
    text: str, intent_type: IntentType, config: Optional[Dict[str, Any]] = None
) -> SlotResult:
    """
    Convenience function for quick slot extraction
