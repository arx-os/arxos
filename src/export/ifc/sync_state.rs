...
impl IFCSyncState {
    ...
    pub fn load(path: &Path) -> Option<Self> {
        ...
        match fs::read_to_string(path) {
            Ok(content) => {
                match serde_json::from_str::<IFCSyncState>(&content) {
                    Ok(state) => {
    ...
    pub fn update_after_export(&mut self, equipment_paths: HashSet<String>, rooms_paths: HashSet<String>) {
        self.last_export_timestamp = Utc::now();
        self.exported_equipment_paths = equipment_paths;
        self.exported_rooms_paths = rooms_paths;
        info!("Updated sync state: {} equipment, {} rooms", 
...
mod tests {
    ...
    fn test_sync_state_save_and_load() {
        ...
        state.exported_equipment_paths.insert("building/floor-1/equipment-1".to_string());
        state.exported_rooms_paths.insert("building/floor-1/room-101".to_string());
        ...
        assert!(loaded.exported_equipment_paths.contains("building/floor-1/equipment-1"));
    }
    ...
    fn test_sync_state_update_after_export() {
        ...
        let rooms_paths: HashSet<String> = [
