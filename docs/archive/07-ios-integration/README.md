...
The Arxos iOS app is intentionally minimal: just a terminal emulator with camera access. All intelligence stays on the mesh nodes where it belongs.

## 🎯 Design Philosophy

### What the App Is
- **Terminal Emulator**: Connect to mesh nodes via Bluetooth/Serial
...
struct ArxosApp {
    // Two views, no more:
    let terminal: TerminalEmulatorViewController
...
$ arxos connect bluetooth://building-node
# or via serial: arxos connect serial:///dev/tty.usbserial

# Connected - full terminal access to building:
$ arxos status
...
6. Data sent to mesh node via Bluetooth/Serial
...
struct TerminalView: View {
    ...
    var body: some View {
        VStack {
            ...
            // Terminal display
            TerminalViewRepresentable(connection: connection)
                .onReceive(connection?.dataPublisher) { data in
            ...
            HStack {
                Button("Scan") { 
                    connection?.send("arxos scan --room=current\n")
                }
                Button("Markup") {
                    connection?.send("arxos markup --ar-guided\n")
                }
                Button("Status") {
