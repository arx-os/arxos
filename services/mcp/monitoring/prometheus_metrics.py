...
class MetricsCollector:
    """Comprehensive metrics collector for MCP service"""
    ...
    def _initialize_metrics(self):
        ...
        self.system_uptime = Gauge(
            "mcp_system_uptime_seconds", "System uptime in seconds"
        )

        self.system_memory_usage = Gauge(
            "mcp_system_memory_bytes", "System memory usage in bytes", ["component"]
        )

        self.system_cpu_usage = Gauge(
            "mcp_system_cpu_percent", "System CPU usage percentage", ["component"]
        )
        ...
        self.compliance_scores = Histogram(
            ...
            buckets=[0, 25, 50, 75, 90, 95, 98, 100],
        )
        ...
        self.cache_hit_ratio = Gauge(
            "mcp_cache_hit_ratio", "Cache hit ratio", ["cache_type"]
        )
    ...
    def record_validation(self, metrics: ValidationMetrics):
        ...
        try:
            ...
            self.validation_duration.labels(
                validation_type="comprehensive", jurisdiction=metrics.jurisdiction
            ...
            if metrics.warnings_count > 0:
                self.warnings_found.labels(
                    category="general", jurisdiction=metrics.jurisdiction
            ...
            if metrics.total_rules > 0:
                ...
                self.compliance_scores.labels(
                    jurisdiction=metrics.jurisdiction, building_type="unknown"
            ...
            logger.debug(
                f"Recorded validation metrics for building {metrics.building_id}"
            )
    ...
    def record_api_request(
        ...
        try:
            ...
            if status_code >= 400:
                ...
                self.errors_total.labels(error_type=error_type, component="api").inc()
    ...
    def record_websocket_connection(self, building_id: str, action: str):
        ...
        try:
            self.websocket_connections.labels(
                building_id=building_id, action=action
            ).inc()
    ...
    def record_websocket_message(self, message_type: str, building_id: str):
        ...
        try:
            self.websocket_messages.labels(
                message_type=message_type, building_id=building_id
            ).inc()
    ...
    def record_jurisdiction_match(self, country: str, state: str, match_level: str):
        ...
        try:
            self.jurisdiction_matches.labels(
                country=country, state=state, match_level=match_level
            ).inc()
    ...
    def record_mcp_file_loaded(self, file_type: str, jurisdiction: str):
        ...
        try:
            self.mcp_files_loaded.labels(
                file_type=file_type, jurisdiction=jurisdiction
            ).inc()
    ...
    def record_login_attempt(self, result: str, user_role: str):
        ...
        try:
            self.login_attempts.labels(result=result, user_role=user_role).inc()
    ...
    def record_validation_error(self, error_type: str, jurisdiction: str):
        ...
        try:
            self.validation_errors.labels(
                error_type=error_type, jurisdiction=jurisdiction
            ).inc()
    ...
    def generate_metrics_response(self) -> Response:
        ...
        try:
            ...
            return FastAPIResponse(
                content=f"Error generating metrics: {str(e)}", status_code=500
            )
