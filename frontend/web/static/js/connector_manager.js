...
// Handles connector-specific operations with FloorID support
...
class ConnectorManager {
    ...
    setupEventListeners() {
        // Floor filter
        const floorFilter = document.getElementById('connector-floor-filter');
        if (floorFilter) {
            floorFilter.addEventListener('change', (e) => {
                this.filters.floor_id = e.target.value || null;
                this.loadConnectors();
            });
        }

        // System filter
        const systemFilter = document.getElementById('connector-system-filter');
        if (systemFilter) {
            systemFilter.addEventListener('change', (e) => {
                this.filters.system = e.target.value || null;
                this.loadConnectors();
            });
        }
        ...
        if (typeFilter) {
            typeFilter.addEventListener('change', (e) => {
                this.filters.connector_type = e.target.value || null;
                this.loadConnectors();
            });
        }

        // Search filter
        const searchFilter = document.getElementById('connector-search-filter');
        if (searchFilter) {
        ...
        const createBtn = document.getElementById('create-connector-btn');
        if (createBtn) {
            createBtn.addEventListener('click', () => this.showCreateConnectorModal());
        }

        // Bulk operations
        const bulkFloorBtn = document.getElementById('bulk-floor-assign-btn');
        if (bulkFloorBtn) {
            bulkFloorBtn.addEventListener('click', () => this.showBulkFloorAssignment());
        }
    }
    ...
    async loadConnectors() {
        try {
            ...
            if (response.ok) {
                ...
                this.updateConnectorStats();
            } else {
                console.error('Failed to load connectors:', response.statusText);
                this.showNotification('Failed to load connectors', 'error');
            }
        } catch (error) {
            console.error('Error loading connectors:', error);
            this.showNotification('Error loading connectors', 'error');
        }
    }
    ...
    renderConnectorList() {
        ...
        let html = '';
        for (const [floorName, connectors] of Object.entries(grouped)) {
            html += `<div class="mb-6">
    ...
    updateConnectorStats() {
        const statsContainer = document.getElementById('connector-stats');
        if (!statsContainer) return;

        const total = this.connectors.length;
        const active = this.connectors.filter(c => c.status === 'active').length;
        const connected = this.connectors.filter(c => c.connection_status === 'connected').length;
        const byFloor = this.groupConnectorsByFloor();
    ...
    groupConnectorsByFloor() {
        return this.connectors.reduce((groups, connector) => {
            const floorName = connector.floor?.name || 'Unknown Floor';
            if (!groups[floorName]) {
                groups[floorName] = [];
            }
            groups[floorName].push(connector);
            return groups;
        }, {});
    }
    ...
    async loadFloorsForModal() {
        try {
            const response = await fetch('/api/buildings/1/floors', {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('arx_jwt') || ''}`,
                    'Content-Type': 'application/json'
                }
            ...
            if (response.ok) {
                ...
                const floorSelect = document.getElementById('new-connector-floor');
                if (floorSelect) {
    ...
    async createConnector(formData) {
        try {
            ...
            if (response.ok) {
                ...
                this.showNotification('Connector created successfully', 'success');
                this.hideCreateConnectorModal();
                this.loadConnectors();
                return connector;
            } else {
                const error = await response.json();
                this.showNotification(`Failed to create connector: ${error.message}`, 'error');
                return null;
            }
        } catch (error) {
            console.error('Error creating connector:', error);
            this.showNotification('Error creating connector', 'error');
            return null;
        }
    }

    async editConnector(connectorId) {
        try {
            ...
            if (response.ok) {
                const connector = await response.json();
                this.showEditConnectorModal(connector);
            } else {
                this.showNotification('Failed to load connector details', 'error');
            }
        } catch (error) {
            console.error('Error loading connector:', error);
            this.showNotification('Error loading connector', 'error');
        }
    }

    showEditConnectorModal(connector) {
        const modal = document.getElementById('edit-connector-modal');
        if (modal) {
            // Populate form fields
            document.getElementById('edit-connector-name').value = connector.name || '';
            document.getElementById('edit-connector-floor').value = connector.floor_id || '';
            document.getElementById('edit-connector-type').value = connector.connector_type || '';
            document.getElementById('edit-connection-type').value = connector.connection_type || '';
            document.getElementById('edit-connection-status').value = connector.connection_status || '';
    ...
    async updateConnector(connectorId, formData) {
        try {
            const response = await fetch(`/api/connectors/${connectorId}`, {
                method: 'PUT',
                headers: {
            ...
            if (response.ok) {
                this.showNotification('Connector updated successfully', 'success');
                this.hideEditConnectorModal();
                this.loadConnectors();
                return true;
            } else {
                const error = await response.json();
                this.showNotification(`Failed to update connector: ${error.message}`, 'error');
                return false;
            }
        } catch (error) {
            console.error('Error updating connector:', error);
            this.showNotification('Error updating connector', 'error');
            return false;
        }
    }
    ...
    async deleteConnector(connectorId) {
        ...
        try {
            ...
            if (response.ok) {
                this.showNotification('Connector deleted successfully', 'success');
                this.loadConnectors();
            } else {
                this.showNotification('Failed to delete connector', 'error');
            }
        } catch (error) {
            console.error('Error deleting connector:', error);
            this.showNotification('Error deleting connector', 'error');
        }
    }

    showBulkFloorAssignment() {
        const modal = document.getElementById('bulk-floor-assignment-modal');
        if (modal) {
            modal.classList.remove('hidden');
            this.loadFloorsForBulkAssignment();
        }
    }
    ...
    async loadFloorsForBulkAssignment() {
        try {
            ...
            if (response.ok) {
                const floors = await response.json();
                const floorSelect = document.getElementById('bulk-assignment-floor');
                if (floorSelect) {
                    floorSelect.innerHTML = '<option value="">-- Select Floor --</option>' +
                        floors.map(floor => `<option value="${floor.id}">${floor.name}</option>`).join('');
                }
            ...
            console.error('Error loading floors for bulk assignment:', error);
        }
    }

    async assignConnectorsToFloor(connectorIds, floorId) {
        try {
            const promises = connectorIds.map(connectorId =>
                fetch(`/api/connectors/${connectorId}`, {
                    ...
                    body: JSON.stringify({ floor_id: parseInt(floorId) })
                })
            );
            ...
            if (successful > 0) {
                this.showNotification(`Successfully assigned ${successful} connectors to floor`, 'success');
                if (failed > 0) {
                    this.showNotification(`${failed} connectors failed to update`, 'warning');
                }
                this.loadConnectors();
            } else {
                this.showNotification('Failed to assign connectors to floor', 'error');
            }
        } catch (error) {
            console.error('Error in bulk floor assignment:', error);
            this.showNotification('Error assigning connectors to floor', 'error');
        }
    }
