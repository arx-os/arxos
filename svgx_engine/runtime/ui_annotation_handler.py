...
class AnnotationHandler:
    ...
    def _handle_create(self, event: Event, canvas_id: str) -> Dict[str, Any]:
        ...
        # Record in history
        self._record_annotation_action(
            canvas_id,
            "create",
            {
    ...
    def _handle_delete(self, event: Event, canvas_id: str) -> Dict[str, Any]:
        ...
        self._record_annotation_action(
            canvas_id,
            "delete",
            {"annotation_id": annotation_id, "deleted_annotation": deleted_annotation},
        )
    ...
    def _handle_show(self, event: Event, canvas_id: str) -> Dict[str, Any]:
        ...
        old_visibility = self.visibility_states[canvas_id].get(
            annotation_id, AnnotationVisibility.HIDDEN
        )
        self.visibility_states[canvas_id][annotation_id] = AnnotationVisibility.VISIBLE
    ...
    def _handle_hide(self, event: Event, canvas_id: str) -> Dict[str, Any]:
        ...
        old_visibility = self.visibility_states[canvas_id].get(
            annotation_id, AnnotationVisibility.VISIBLE
        )
    ...
    def _handle_toggle(self, event: Event, canvas_id: str) -> Dict[str, Any]:
        ...
        current_visibility = self.visibility_states[canvas_id].get(
            annotation_id, AnnotationVisibility.VISIBLE
        )
        new_visibility = (
            AnnotationVisibility.HIDDEN
            if current_visibility == AnnotationVisibility.VISIBLE
            else AnnotationVisibility.VISIBLE
        )
    ...
    def _record_annotation_action(
        self, canvas_id: str, action: str, data: Dict[str, Any]
    ):
        """Record annotation action in history."""
        self.annotation_history[canvas_id].append(
            {"action": action, "timestamp": datetime.utcnow(), "data": data}
        )
    ...
    def get_annotation(
        self, canvas_id: str, annotation_id: str
    ) -> Optional[Annotation]:
        """Get annotation by ID."""
        return self.annotations.get(canvas_id, {}).get(annotation_id)

    def get_annotations(
        self, canvas_id: str, visible_only: bool = False
    ) -> List[Annotation]:
        ...
        if visible_only:
            annotations = [
                ...
                if self.visibility_states.get(canvas_id, {}).get(
                    ann.id, AnnotationVisibility.VISIBLE
                )
    ...
    def get_annotations_by_target(
        self, canvas_id: str, target_id: str
    ) -> List[Annotation]:
        """Get annotations for a specific target object."""
        return [
            ann
            for ann in self.annotations.get(canvas_id, {}).values()
...
def _register_annotation_handler():
    def handler(event: Event):
        if (
            ...
            and event.data.get("event_subtype") == "annotation"
        ):
