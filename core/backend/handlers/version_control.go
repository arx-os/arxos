...
func GetVersionHistory(w http.ResponseWriter, r *http.Request) {
	...
	if len(versions) > 0 {
		...
		// User info is available in userMap for reference
		// Note: DrawingVersion model doesn't have a User field
	}
...
func CreateVersion(w http.ResponseWriter, r *http.Request) {
	...
	// Check if user has access to this floor (simplified - in real app, check permissions)
	// Note: Floor model doesn't have Building field, so we'll skip this check for now
	// In a real implementation, you would check user permissions properly
