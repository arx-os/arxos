...
class FluidDynamicsService:
    ...
    def _laminar_flow_analysis(
        self, request: FluidAnalysisRequest
    ) -> FluidAnalysisResult:
    ...
    def _turbulent_flow_analysis(
        ...
        k_field, epsilon_field = self._calculate_turbulent_quantities(
            request, velocity_field
        )

        # Calculate temperature field
        temperature_field = self._calculate_temperature_field_turbulent(
            request, velocity_field, k_field
        )
    ...
    def _transitional_flow_analysis(
        self, request: FluidAnalysisRequest
    ) -> FluidAnalysisResult:
    ...
    def _solve_navier_stokes_laminar(
        self, request: FluidAnalysisRequest
        ...
        for bc in request.boundary_conditions:
            if bc.type == BoundaryConditionType.INLET:
                ...
                for node_idx in self._find_nodes_in_boundary(bc.location, request.mesh):
                    velocity_field[node_idx] = np.array(
                        bc.value.get("velocity", [1.0, 0.0, 0.0])
                    )
        ...
        for iteration in range(100):
            # Pressure correction
            pressure_correction = self._pressure_correction(
                velocity_field, pressure_field, request
            )
            pressure_field += pressure_correction

            # Velocity correction
            velocity_correction = self._velocity_correction(
                velocity_field, pressure_field, request
            )
    ...
    def _solve_rans_equations(
        ...
        for bc in request.boundary_conditions:
            if bc.type == BoundaryConditionType.INLET:
                for node_idx in self._find_nodes_in_boundary(bc.location, request.mesh):
                    ...
                    pressure_field[node_idx] = bc.value.get("pressure", 101325.0)
                    k_field[node_idx] = (
                        bc.value.get("turbulent_intensity", 0.05) ** 2 * 1.5
                    )
        ...
        for iteration in range(200):
            ...
            velocity_correction = self._momentum_correction_turbulent(
                velocity_field, pressure_field, k_field, epsilon_field, request
            )
            velocity_field += velocity_correction

            # Solve pressure equation
            pressure_correction = self._pressure_correction_turbulent(
                velocity_field, pressure_field, request
            )
            pressure_field += pressure_correction

            # Solve k-epsilon equations
            k_correction, epsilon_correction = self._turbulent_correction(
                velocity_field, k_field, epsilon_field, request
            )
            ...
            # Check convergence
            if (
                np.max(np.abs(velocity_correction)) < 1e-5
                and np.max(np.abs(pressure_correction)) < 1e-5
            ):
                break
    ...
    def _calculate_temperature_field(
        self, request: FluidAnalysisRequest, velocity_field: np.ndarray
    ) -> List[float]:
        ...
        for iteration in range(50):
            temperature_correction = self._temperature_correction(
                temperature_field, velocity_field, request
            )
    ...
    def _calculate_temperature_field_turbulent(
        self,
        request: FluidAnalysisRequest,
    ...
    def _calculate_turbulent_quantities(
        self, request: FluidAnalysisRequest, velocity_field: np.ndarray
    ...
    def _calculate_streamlines(
        self, velocity_field: List[Tuple[float, float, float]], mesh: List[MeshElement]
    ...
    def _pressure_correction_turbulent(
        self,
        velocity_field: np.ndarray,
        pressure_field: np.ndarray,
        request: FluidAnalysisRequest,
    ...
    def _momentum_correction_turbulent(
        self,
        velocity_field: np.ndarray,
        pressure_field: np.ndarray,
        k_field: np.ndarray,
    ...
    def _calculate_reynolds_number(
        self, request: FluidAnalysisRequest, result: FluidAnalysisResult
    ) -> float:
        ...
        reynolds = (
            request.fluid_properties.density
            * avg_velocity
