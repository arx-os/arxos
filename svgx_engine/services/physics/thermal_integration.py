...
class ThermalIntegrationService:
    ...
    def _convert_basic_result(self, result) -> Dict[str, Any]:
        ...
        return {
            "analysis_mode": "basic",
            "temperature_field": result.temperature_field if hasattr(result, 'temperature_field') else [],
    ...
    def get_material_properties(self, material_name: str, temperature: float = 293.15) -> Dict[str, float]:
        ...
        if material_name in self.advanced_service.materials:
            ...
            return {
                "thermal_conductivity": material.get_property("thermal_conductivity", temperature),
    ...
    def generate_report(self, results: Dict[str, Any], 
        ...
        report = {
            "analysis_summary": {
                ...
                "convergence": results.get("convergence_info", {}).get("converged", False),
            ...
            "thermal_results": {
                ...
                "heat_flux_summary": {
                    "total_heat_flux": sum(np.linalg.norm(flux) for flux in results.get("final_heat_flux", [])),
                    "max_heat_flux": max(np.linalg.norm(flux) for flux in results.get("final_heat_flux", []))
                }
            },
            "material_analysis": {
                "phases_present": list(set(phase.value for phase in results.get("material_phases", {}).values())),
                "phase_change_locations": self._identify_phase_change_locations(results)
            },
            "performance_metrics": {
        ...
        if material_phases:
            ...
            for time_step, phases in material_phases.items():
                for element_id, phase in phases.items():
                    if phase != MaterialPhase.SOLID:  # Non-solid phases indicate phase change
