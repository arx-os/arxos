...
class SchemaExporter:
    ...
    def export_schema(self) -> bool:
        ...
        try:
            ...
            logger.error("schema_export_failed", error=str(e))
            return False
        finally:
            self.disconnect()
    ...
    def _export_tables(self) -> List[TableInfo]:
        ...
        with self.connection.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            for row in cursor.fetchall():
                ...
                logger.debug("exporting_table", table_name=table_name)
        ...
        with self.connection.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            for row in cursor.fetchall():
                ...
                column_info = ColumnInfo(
                    ...
                    foreign_key_table=(
                        fk_result["referenced_table_name"] if fk_result else None
                    ),
                    foreign_key_column=(
                        fk_result["referenced_column_name"] if fk_result else None
                    ),
                )
    ...
    def _export_migration_history(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("no_migration_history_found", error=str(e))
    ...
    def generate_documentation(
        ...
        logger.info(
            "generating_schema_documentation",
    ...
    def _generate_json_documentation(self, output_file: Optional[str] = None) -> str:
        ...
        if output_file:
            with open(output_file, "w") as f:
                f.write(content)
            logger.info("json_documentation_saved", file=output_file)
...
"""
        ...
        if output_file:
            with open(output_file, "w") as f:
                f.write(content)
            logger.info("markdown_documentation_saved", file=output_file)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Export database schema documentation")
    parser.add_argument(
        "--database-url",
