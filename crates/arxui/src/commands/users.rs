...
use crate::tui::handle_user_browser;
use arx::identity::{
    get_key_fingerprint_for_email, is_gpg_available, PendingUserRegistry, User, UserRegistry,
};
    ...
    if is_gpg_available() {
        match get_key_fingerprint_for_email(&email) {
            ...
            Err(_) => {
                ...
                println!(
                    "💡 No GPG key found for {}. User can configure GPG later.",
                    email
                );
            }
        }
    ...
    for user in users {
        ...
        if user.verified {
            ...
            if let Some(ref verified_at) = user.verified_at {
                println!("   Verified at: {}", format_timestamp(verified_at));
            }
        }
        ...
        if let Some(ref last_active) = user.last_active {
            println!("   Last active: {}", format_timestamp(last_active));
        }
        println!();
    }
...
fn handle_users_show(email: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Added: {}", format_timestamp(&user.added_at));
    if let Some(ref last_active) = user.last_active {
        println!("Last active: {}", format_timestamp(last_active));
    }
...
fn handle_users_approve(email: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _pending_request = pending_registry
    ...
    println!(
        "✅ User approved and added to registry: {} <{}>",
        verified_user.name, verified_user.email
    );
