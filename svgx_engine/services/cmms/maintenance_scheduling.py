...
class MaintenanceSchedule(BaseModel):
    ...
    trigger_value: Union[int, float, str] = Field(
        ..., description="Trigger value (hours, days, etc.)"
    )
    estimated_duration: int = Field(..., description="Estimated duration in minutes")
...
class MaintenanceCalendar(BaseModel):
    ...
    working_days: Set[int] = Field(
        default={0, 1, 2, 3, 4}, description="Working days (0=Monday)"
    )
...
class MaintenanceSchedulingService:
    ...
    async def start_maintenance_task(
        self, task_id: UUID, performer: str
    ) -> Optional[MaintenanceTask]:
    ...
    async def get_maintenance_calendar(
        self, calendar_id: UUID, start_date: datetime, end_date: datetime
    ) -> List[MaintenanceTask]:
    ...
    async def _get_assets_for_maintenance(
        ...
        try:
            ...
            if task.assigned_to:
                await self.notification_system.send_email_notification(
                    to_email=task.assigned_to, subject=subject, message=message
                )
    ...
    async def get_maintenance_statistics(
        ...
        history = await self.get_maintenance_history(
            start_date=start_date, end_date=end_date
        )

        total_tasks = len(tasks)
        completed_tasks = len(
            [t for t in tasks if t.status == MaintenanceStatus.COMPLETED]
        )
        overdue_tasks = len([t for t in tasks if t.status == MaintenanceStatus.OVERDUE])
        in_progress_tasks = len(
            [t for t in tasks if t.status == MaintenanceStatus.IN_PROGRESS]
        )
        ...
        return {
            ...
            "average_duration_per_task": (
                (total_duration / len(history)) if history else 0
            ),
        }
