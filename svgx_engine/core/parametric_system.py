    ...
    def validate(self) -> bool:
        ...
        try:
            ...
            if self.parameter_type == ParameterType.LENGTH:
                ...
                return (
                    isinstance(self.value, (int, float, Decimal))
                    and 0 <= self.value <= 360
                )
...
class ParametricGeometry:
    ...
    def _generate_line(self) -> Dict[str, Any]:
        ...
        start_x = self.parameters.get(
            "start_x", Parameter("", "start_x", ParameterType.REAL, 0)
        )
        start_y = self.parameters.get(
            "start_y", Parameter("", "start_y", ParameterType.REAL, 0)
        )
        length = self.parameters.get(
            "length", Parameter("", "length", ParameterType.LENGTH, 10)
        )
        angle = self.parameters.get(
            "angle", Parameter("", "angle", ParameterType.ANGLE, 0)
        )

        end_x = start_x.get_value() + length.get_value() * math.cos(
            math.radians(angle.get_value())
        )
        end_y = start_y.get_value() + length.get_value() * math.sin(
            math.radians(angle.get_value())
        )
    ...
    def _generate_circle(self) -> Dict[str, Any]:
        ...
        center_x = self.parameters.get(
            "center_x", Parameter("", "center_x", ParameterType.REAL, 0)
        )
        center_y = self.parameters.get(
            "center_y", Parameter("", "center_y", ParameterType.REAL, 0)
        )
        radius = self.parameters.get(
            "radius", Parameter("", "radius", ParameterType.RADIUS, 5)
        )
    ...
    def _generate_rectangle(self) -> Dict[str, Any]:
        ...
        width = self.parameters.get(
            "width", Parameter("", "width", ParameterType.LENGTH, 10)
        )
        height = self.parameters.get(
            "height", Parameter("", "height", ParameterType.LENGTH, 10)
        )
        angle = self.parameters.get(
            "angle", Parameter("", "angle", ParameterType.ANGLE, 0)
        )
        ...
        for dx, dy in [(-1, -1), (1, -1), (1, 1), (-1, 1)]:
            x = (
                center_x.get_value()
                + dx * half_width * math.cos(rad_angle)
                - dy * half_height * math.sin(rad_angle)
            )
            y = (
                center_y.get_value()
                + dx * half_width * math.sin(rad_angle)
                + dy * half_height * math.cos(rad_angle)
            )
    ...
    def _generate_polygon(self) -> Dict[str, Any]:
        ...
        sides = self.parameters.get(
            "sides", Parameter("", "sides", ParameterType.INTEGER, 6)
        )
        start_angle = self.parameters.get(
            "start_angle", Parameter("", "start_angle", ParameterType.ANGLE, 0)
        )
...
class ParametricModelingSystem:
    ...
    def add_expression(
        self, expression: str, target_parameter: str, source_parameters: List[str]
    ) -> ParameterExpression:
    ...
    def create_parametric_geometry(
        ...
        geometry_type: str,
        parameters: Dict[str, Parameter],
        expressions: List[ParameterExpression] = None,
    ) -> ParametricGeometry:
    ...
    def create_parametric_assembly(
        self,
        name: str,
        components: List[ParametricGeometry],
        relationships: List[ParameterExpression] = None,
    ) -> ParametricAssembly:
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "parameter_types": list(
                set(p.parameter_type.value for p in self.parameters.values())
            ),
