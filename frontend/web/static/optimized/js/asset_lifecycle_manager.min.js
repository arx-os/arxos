class AssetLifecycleManager{constructor(){this.baseUrl='/v1/asset-lifecycle';this.eventListeners={'lifecycle_event_logged':[],'replacement_schedule_created':[],'analytics_updated':[],'compliance_report_updated':[]};}
...
renderComplianceTable(assets,container){if(!container)return;container.innerHTML='';const table=document.createElement('table');table.className='compliance-report-table';table.innerHTML=`
            ...
            <tbody>
                ${assets.map(a => `<tr${a.overdue?' class="overdue"':''}><td>${a.asset_id}</td><td>${a.asset_name}</td><td>${a.asset_type}</td><td>${a.status}</td><td>${a.condition}</td><td>${a.next_maintenance??''}</td><td>${a.overdue?'⚠️':''}</td></tr>`).join('')}
            </tbody>
        `;container.appendChild(table);}}
window.assetLifecycleManager=new AssetLifecycleManager();if(typeof module!=='undefined'&&module.exports)module.exports=AssetLifecycleManager;
