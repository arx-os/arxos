...
class EmailIntegrationService:
        ...
        try:
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to get email notifications: {response.status} - {error_text}")
                    raise aiohttp.ClientError(f"Failed to get email notifications: {response.status}")
        ...
        try:
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to get email notification: {response.status} - {error_text}")
                    raise aiohttp.ClientError(f"Failed to get email notification: {response.status}")
        ...
        try:
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to get delivery history: {response.status} - {error_text}")
                    raise aiohttp.ClientError(f"Failed to get delivery history: {response.status}")
        ...
        try:
            async with self.session.post(
                ...
                if response.status == 201:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to create email template: {response.status} - {error_text}")
                    raise aiohttp.ClientError(f"Failed to create email template: {response.status}")

        except aiohttp.ClientError as e:
            logger.error(f"Failed to create email template: {e}")
            raise

    async def get_email_templates(self, active_only: bool = True) -> List[Dict[str, Any]]:
        """
        Get email templates.
        ...
        try:
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to get email templates: {response.status} - {error_text}")
                    raise aiohttp.ClientError(f"Failed to get email templates: {response.status}")

        except aiohttp.ClientError as e:
            logger.error(f"Failed to get email templates: {e}")
            raise

    async def update_email_template(self, template_id: int, template: EmailTemplate) -> Dict[str, Any]:
        """
        ...
        try:
            async with self.session.put(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to update email template: {response.status} - {error_text}")
                    raise aiohttp.ClientError(f"Failed to update email template: {response.status}")
        ...
        try:
            async with self.session.delete(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to delete email template: {response.status} - {error_text}")
                    raise aiohttp.ClientError(f"Failed to delete email template: {response.status}")
    ...
    async def get_email_statistics(
        ...
        try:
            async with self.session.get(
                f"{self.base_url}/api/notifications/email/statistics",
                params=params
            ) as response:
                if response.status == 200:
                    return await response.json()
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to get email statistics: {response.status} - {error_text}")
                    raise aiohttp.ClientError(f"Failed to get email statistics: {response.status}")
        ...
        try:
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to test email config: {response.status} - {error_text}")
                    raise aiohttp.ClientError(f"Failed to test email config: {response.status}")
