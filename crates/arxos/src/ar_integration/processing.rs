...
use super::pending::{
    DetectedEquipmentInfo, DetectionMethod, PendingEquipment, PendingEquipmentManager,
};
...
pub struct ARScanData {
    ...
    /// Optional floor level provided by the scan source
    pub floor_level: Option<i32>,
    /// Optional room name provided by the scan source
...
pub fn process_ar_scan_to_pending(
    ...
    let storage_file = PathBuf::from(format!("{}_pending.json", building_name));
    ...
    let floor_level = scan_data.floor_level.unwrap_or(0);
    let room_name = scan_data.room_name.as_deref();

    for detected_eq in &scan_data.detected_equipment {
        // Validate confidence threshold
        if detected_eq.confidence < confidence_threshold {
            warn!(
                "Filtered equipment '{}' due to low confidence: {:.2} < {:.2}",
        ...
        if let Some(pending_id) = manager.add_pending_equipment(
            ...
            info!(
                "Created pending equipment: {} (confidence: {:.2})",
                detected_info.name, detected_info.confidence
            );
        }
    }

    manager.save_to_storage_path(&storage_file)?;

    info!(
        "Created {} pending equipment items from AR scan",
        pending_ids.len()
    );
    Ok(pending_ids)
}
...
pub fn validate_ar_scan_data(scan_data: &ARScanData) -> Result<(), Vec<String>> {
    ...
    for (i, equipment) in scan_data.detected_equipment.iter().enumerate() {
        ...
        if equipment.name.is_empty() {
            errors.push(format!("Equipment at index {} has empty name", i));
        }
    }
...
pub fn pending_equipment_to_json(
...
pub fn pending_equipment_from_json(
    json: &str,
) -> Result<PendingEquipment, Box<dyn std::error::Error>> {
...
pub fn process_ar_scan_and_save_pending(
    ...
    // Process to pending
    let pending_ids =
        process_ar_scan_to_pending(scan_data, building_name, confidence_threshold, None)?;
