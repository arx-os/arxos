...
def test_privacy_controls_service():
    ...
    try:
        ...
        test_cases = [
            (
                "svgx_content",
                "<svgx:behavior>test</svgx:behavior>",
            ...
            (
                "svgx_behavior_profiles",
                {"behavior_script": "console.log('test')"},
                DataClassification.CLASSIFIED,
            ),
        ]
        ...
        print(
            f"✓ Privacy controls applied: {secured_data['privacy_metadata']['classification']}"
        )
...
def test_audit_trail_service():
    ...
    try:
        ...
        filtered_logs = audit_service.get_svgx_audit_logs(
            {"event_type": "svgx_access", "user_id": "test_user"}
        )
        print(f"✓ Filtered audit logs: {len(filtered_logs)} events")

        # Test compliance report generation
        date_range = (datetime.now() - timedelta(days=1), datetime.now())
        report = audit_service.generate_svgx_compliance_report(
            "svgx_data_access", date_range
        )
...
def test_rbac_service():
    ...
    try:
        ...
        for action, resource, expected in permissions:
            ...
            status = "✓" if result == expected else "✗"
            print(
                f"{status} Permission {action} on {resource}: {result} (expected: {expected})"
            )
...
def test_advanced_security_service():
    ...
    try:
        ...
        print(
            f"✓ Secure data access: {secured_data['privacy_metadata']['classification']}"
        )
...
def test_error_handling():
    ...
    try:
        ...
        try:
            raise SecurityError(
                "Test security error", "TEST_ERROR", {"details": "test"}
            )
        ...
        try:
            raise ValidationError(
                "Test validation error", "VALIDATION_ERROR", {"field": "test"}
            )
...
def test_integration():
    ...
    try:
        ...
        role_id = rbac_service.create_svgx_role(
            "integration_role", ["svgx:read", "svgx:write"], ""
        )
        ...
        print(
            f"  - Data secured with classification: {secured_data['privacy_metadata']['classification']}"
        )
