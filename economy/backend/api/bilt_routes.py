...
try:
    from economy.backend.services.bilt_token.minting_engine_mock import (
        MockBiltMintingEngine as BiltMintingEngine,
    )
    from economy.backend.services.bilt_token.dividend_calculator_mock import (
        MockBiltDividendCalculator as BiltDividendCalculator,
    )
except ImportError:
...
class ContributionRequest(BaseModel):
    ...
    verifier_wallet: Optional[str] = Field(
        None, description="Wallet address of secondary verifier"
    )
...
class DividendDistributionRequest(BaseModel):
    amount: Optional[float] = Field(
        None, description="Amount to distribute (if None, distributes entire pool)"
    )
...
async def distribute_dividends(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail="Failed to distribute dividends"
            )
...
@router.get(
    "/dividends/claimable/{wallet_address}", response_model=DividendClaimResponse
)
async def get_claimable_dividends(
    ...
    """
    try:
        claimable = await dividend_calculator.calculate_claimable_dividends(
            wallet_address
        )
...
async def get_dividend_yield(
    ...
    try:
        ...
        return DividendYieldResponse(
            ...
            average_dividend_per_token=float(
                yield_data.get("average_dividend_per_token", 0)
            ),
