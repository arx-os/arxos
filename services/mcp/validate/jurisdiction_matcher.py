...
class JurisdictionMatcher:
    ...
    def extract_building_location(self, building_model: Dict[str, Any]) -> Optional[BuildingLocation]:
        ...
        location_data = (
            ...
            metadata.get("site") or
            building_model.get("location")
        )
    ...
    def match_jurisdictions(self, building_location: BuildingLocation) -> List[JurisdictionMatch]:
        ...
        for code_id in base_codes:
            if code_id in self.available_mcp_files:
                ...
                matches.append(JurisdictionMatch(
                    mcp_file=mcp_file,
                    match_level="country",
                    confidence=1.0,
        ...
        if building_location.country == "US" and building_location.state:
            state_codes = country_mappings.get("states", {}).get(building_location.state, [])
    ...
    def get_applicable_codes(self, building_model: Dict[str, Any]) -> List[str]:
        ...
        if not location:
            logger.warning("No location data found in building model, using fallback codes")
    ...
    def validate_jurisdiction_match(self, building_model: Dict[str, Any], mcp_file_id: str) -> bool:
    ...
    def get_jurisdiction_info(self, building_model: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "jurisdiction_level": max([match.match_level for match in matches]) if matches else "unknown",
            "matches": [
                {
