...
class TestComprehensiveSystem:
    ...
    def test_collaborative_editing_locks(self, engine):
        ...
        # Test lock conflict
        conflict_result = engine.lock_object(
            canvas_id, object_id, "session-2", "user-2"
        )
    ...
    def test_rest_api_endpoints(self, base_event):
        ...
        resp = client.get(
            "/runtime/lock-status/?canvas_id=test-canvas&object_id=test-object"
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_system_health_metrics(self):
        ...
        metrics_collector.record_system_health(
            component="test_component", status="healthy", details={"test": "data"}
        )
