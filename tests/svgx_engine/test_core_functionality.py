...
class TestWASMIntegration:
    ...
    def test_precision_calculations(self, wasm_integration):
        ...
        # Test rounding
        data = {"operation": "round", "value": 123.456789, "precision": 0.01}
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:arx="http://arxos.io/svgx">
  <rect x="0" y="0" width="100" height="100"/>
        ...
        assert (
            duration < 32.0
        ), f"Parse response time {duration:.2f}ms exceeds 32ms target"
    ...
    def test_interactive_response_time_target(self, client):
        ...
        assert (
            duration < 16.0
        ), f"Interactive response time {duration:.2f}ms exceeds 16ms target"
  ...
  <arx:object id="obj01" type="mechanical.component">
    <arx:physics>
      ...
      <forces>
        ...
        assert (
            duration < 100.0
        ), f"Physics simulation time {duration:.2f}ms exceeds 100ms target"
