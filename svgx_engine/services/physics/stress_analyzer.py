...
class StressAnalyzer:
    ...
    def calculate_principal_stresses(
        ...
        logger.info(
            f"Principal stresses: σ1={principal_stresses[0]:.2f}, "
    ...
    def calculate_strain_energy_density(
        self, stress_tensor: StressTensor, strain_tensor: StrainTensor
    ) -> float:
        """
    ...
    def check_failure_criterion(
        self, stress_tensor: StressTensor, material: str, criterion: FailureCriterion
    ) -> Tuple[bool, float]:
        """
        ...
        if criterion == FailureCriterion.VON_MISES:
            ...
            shear_strength = cohesion + normal_stress * math.tan(
    ...
    def calculate_fatigue_stress_range(
        self, stress_history: List[StressTensor]
    ) -> float:
        """
        ...
        von_mises_stresses = [
            self.calculate_von_mises_stress(stress) for stress in stress_history
        ]
    ...
    def calculate_fatigue_life(
        self, stress_range: float, material: str, stress_ratio: float = 0.0
    ) -> float:
        """
    ...
    def calculate_equivalent_stress(
        self, stress_tensor: StressTensor, method: str = "von_mises"
    ) -> float:
        """
    ...
    def calculate_strain_from_stress(
        self, stress_tensor: StressTensor, elastic_modulus: float, poisson_ratio: float
    ) -> StrainTensor:
        """
        ...
        # Hooke's law for 3D
        epsilon_xx = (
            stress_tensor.sigma_xx
            - nu * (stress_tensor.sigma_yy + stress_tensor.sigma_zz)
        ) / E
        epsilon_yy = (
            stress_tensor.sigma_yy
            - nu * (stress_tensor.sigma_xx + stress_tensor.sigma_zz)
        ) / E
        epsilon_zz = (
            stress_tensor.sigma_zz
            - nu * (stress_tensor.sigma_xx + stress_tensor.sigma_yy)
        ) / E
