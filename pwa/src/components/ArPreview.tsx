...
type XRNavigator = Navigator & { xr?: any };
...
export default function ArPreview() {
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      session.addEventListener("end", () => setSessionActive(false), { once: true });
      // For the prototype we immediately end the session—real overlays will stream point clouds.
      await session.end();
    } catch (err) {
      ...
      <header className="mb-3 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Detect immersive AR support and initiate a browser-native session when available.
          </p>
        </div>
        <span
