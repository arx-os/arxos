...
class PrecisionManager:
    ...
    def create_precise_value(self, value: Union[float, str, Decimal], unit: str = "mm") -> Decimal:
    ...
    def _parse_value_with_units(self, value_str: str) -> Decimal:
        ...
        unit_patterns = {
            r'(\d+\.?\d*)\s*μm': lambda m: Decimal(m.group(1)) / self.config.mm_to_micron,
            r'(\d+\.?\d*)\s*micron': lambda m: Decimal(m.group(1)) / self.config.mm_to_micron,
            r'(\d+\.?\d*)\s*mm': lambda m: Decimal(m.group(1)),
            r'(\d+\.?\d*)\s*in': lambda m: Decimal(m.group(1)) * self.config.inch_to_mm,
            r'(\d+\.?\d*)\s*inch': lambda m: Decimal(m.group(1)) * self.config.inch_to_mm,
            r'(\d+\.?\d*)\s*nm': lambda m: Decimal(m.group(1)) / (self.config.mm_to_micron * self.config.micron_to_nm),
        }
    ...
    def format_for_display(self, value: Decimal, mode: Optional[PrecisionDisplayMode] = None) -> str:
    ...
    def validate_precision(self, value: Decimal, required_precision: PrecisionLevel) -> bool:
