...
use crate::ui::layouts::{dashboard_layout, card_grid, split_horizontal};
                ...
                if let Ok(commits) = manager.list_commits(5) {
                    for commit in commits {
                        ...
                        activities.push(ActivityItem {
                            timestamp,
                            message: commit.message.lines().next().unwrap_or("No message").to_string(),
...
fn render_summary_cards<'a>(stats: &DashboardStats, area: Rect, theme: &'a Theme) -> Vec<Paragraph<'a>> {
            ...
            Paragraph::new(vec![
                Line::from(vec![
                    Span::styled(
                        ...
                        Style::default().fg(critical_color).add_modifier(Modifier::BOLD),
                    ),
                ]),
                Line::from(vec![
...
fn render_status_breakdown<'a>(stats: &DashboardStats, _area: Rect, theme: &'a Theme) -> Paragraph<'a> {
    let lines = vec![
        ...
        Line::from(vec![
            Span::styled(StatusColor::Healthy.icon(), Style::default().fg(StatusColor::Healthy.color())),
            Span::styled(" Healthy: ", Style::default().fg(theme.text)),
            Span::styled(stats.healthy_count.to_string(), Style::default().fg(StatusColor::Healthy.color())),
        ]),
        Line::from(vec![
            Span::styled(StatusColor::Warning.icon(), Style::default().fg(StatusColor::Warning.color())),
            Span::styled(" Warning: ", Style::default().fg(theme.text)),
            Span::styled(stats.warning_count.to_string(), Style::default().fg(StatusColor::Warning.color())),
        ]),
        Line::from(vec![
            Span::styled(StatusColor::Critical.icon(), Style::default().fg(StatusColor::Critical.color())),
            Span::styled(" Critical: ", Style::default().fg(theme.text)),
            Span::styled(stats.critical_count.to_string(), Style::default().fg(StatusColor::Critical.color())),
        ]),
        Line::from(vec![
            Span::styled(StatusColor::Unknown.icon(), Style::default().fg(StatusColor::Unknown.color())),
            Span::styled(" Unknown: ", Style::default().fg(theme.text)),
            Span::styled(stats.unknown_count.to_string(), Style::default().fg(StatusColor::Unknown.color())),
        ]),
    ];
    
    Paragraph::new(lines)
        .block(Block::default().borders(Borders::ALL).title("Status Breakdown"))
...
fn render_activity_list<'a>(activities: &[ActivityItem], _area: Rect, theme: &'a Theme) -> Paragraph<'a> {
    ...
    let content = if lines.is_empty() {
        vec![Line::from(vec![
            Span::styled("No recent activity", Style::default().fg(theme.muted)),
        ])]
    } else {
        lines
    };
...
pub fn handle_status_dashboard(building_name: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
