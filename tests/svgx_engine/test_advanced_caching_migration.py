...
def test_svgx_cache_key_generator():
    ...
    try:
        ...
        symbol_key = SVGXCacheKeyGenerator.generate_symbol_key(
            "test_symbol", "test_namespace"
        )
        print(f"✓ Symbol key: {symbol_key}")

        # Test behavior key generation
        behavior_key = SVGXCacheKeyGenerator.generate_behavior_key(
            "test_behavior", "test_namespace"
        )
        print(f"✓ Behavior key: {behavior_key}")

        # Test compilation key generation
        svgx_content = "<svgx:behavior>test</svgx:behavior>"
        compilation_key = SVGXCacheKeyGenerator.generate_compilation_key(
            svgx_content, "svg"
        )
...
def test_memory_cache():
    ...
    try:
        ...
        success = cache.set(
            ...
            svgx_namespace="default",
            svgx_user_id="user123",
        )
...
def test_advanced_caching_system():
    ...
    try:
        ...
        try:
            ...
            print(
                f"✓ Overall metrics: {overall_metrics['overall_hit_rate']:.2f} hit rate"
            )

            svgx_metrics = cache_system.get_svgx_metrics()
            print(
                f"✓ SVGX metrics: {len(svgx_metrics['access_patterns'])} access patterns"
            )

            size_info = cache_system.get_cache_size_info()
            print(
                f"✓ Cache size info: {size_info['memory_cache']['used_mb']:.2f} MB used"
            )
...
def test_cache_integration():
    ...
    try:
        ...
        try:
            ...
            # Set in memory cache
            cache_system.memory_cache.set(
                key,
                test_data,
                3600,
                True,
                SVGXCacheType.SVGX_CONTENT,
                "test",
                "user123",
            )

            # Set in disk cache
            cache_system.disk_cache.set(
                key,
                test_data,
                86400,
                True,
            ...
            cache_system.database_cache.set(
                key,
                test_data,
                604800,
                True,
...
def test_error_handling():
    ...
    try:
        ...
        try:
            raise CacheError(
                "Test cache error", "TEST_CACHE_ERROR", {"details": "test"}
            )
