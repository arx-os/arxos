// Comparison View Component for Floor Version Control
class ComparisonView {
                ...
                <div class="p-4 border-b border-gray-200">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            ${this.options.showExportButton ? `
                                <button id="export-comparison" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md text-sm">
    ...
    initializeEventListeners() {
        ...
        document.getElementById('deselect-all-changes').addEventListener('click', () => {
            this.deselectAllChanges();
        });
    }

    // Load comparison
    async loadComparison(fromVersionId, toVersionId) {
        if (!this.options.floorId) {
            this.showError('No floor selected');
            return;
        }
    ...
    renderComparison() {
        ...
        this.initializeChangeSelection();
    }
    ...
    // Render timeline change details
    renderTimelineChangeDetails(change) {
        switch (change.type) {
            case 'added':
                return `<div class="text-green-700 text-sm">${change.new_value}</div>`;
            case 'removed':
                return `<div class="text-red-700 text-sm">${change.old_value}</div>`;
            case 'modified':
                return `
                    <div class="space-y-1">
                        <div class="text-red-700 text-sm">- ${change.old_value}</div>
                ...
                `;
            case 'unchanged':
                return `<div class="text-gray-600 text-sm">${change.value}</div>`;
            default:
                return '';
        }
    }
    ...
    updateChangeCounts() {
        ...
        document.getElementById('change-count').textContent = `${changes.length} changes`;
        document.getElementById('comparison-status').textContent = 'Comparison loaded';
    }

    // Show change summary
    showChangeSummary() {
        document.getElementById('change-summary').style.display = 'block';
    }

    // Initialize change selection
    initializeChangeSelection() {
        const checkboxes = document.querySelectorAll('.change-checkbox');
        checkboxes.forEach(checkbox => {
            checkbox.addEventListener('change', (e) => {
                const index = parseInt(e.target.dataset.changeIndex);
                if (e.target.checked) {
                    this.selectedChanges.push(index);
                } else {
                    this.selectedChanges = this.selectedChanges.filter(i => i !== index);
                }
            });
        });
    }
    ...
    selectAllChanges() {
        ...
        checkboxes.forEach(checkbox => {
            ...
            const index = parseInt(checkbox.dataset.changeIndex);
            if (!this.selectedChanges.includes(index)) {
                this.selectedChanges.push(index);
            }
        });
    }
    ...
    async exportComparison() {
        if (!this.comparison) {
            this.showError('No comparison to export');
            return;
        }

        try {
            const response = await fetch(`/api/floors/${this.options.floorId}/versions/export-comparison`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    ...
                    selected_changes: this.selectedChanges,
                    format: 'pdf'
                })
            });
            ...
            this.showError('Error exporting comparison: ' + error.message);
        }
    }
    ...
    close() {
        ...
        document.getElementById('change-count').textContent = '0 changes';
