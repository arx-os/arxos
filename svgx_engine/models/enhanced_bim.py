...
class EnhancedBIMElement:
    ...
    def get_property_value(self, property_set_id: str, property_id: str) -> Optional[Any]:
...
class EnhancedBIMModel:
    ...
    def get_elements_by_type(self, element_type: BIMElementType) -> List[EnhancedBIMElement]:
        """Get all elements of a specific type."""
        return [elem for elem in self.elements.values() if elem.element_type == element_type]
    
    def get_elements_by_system(self, system_type: BIMSystemType) -> List[EnhancedBIMElement]:
        """Get all elements of a specific system type."""
        return [elem for elem in self.elements.values() if elem.system_type == system_type]
        ...
        for element_id, element in self.elements.items():
            for relationship in element.relationships:
                if relationship.source_element_id not in self.elements:
                    issues.append(f"Relationship {relationship.id} references non-existent source element {relationship.source_element_id}")
                if relationship.target_element_id not in self.elements:
                    issues.append(f"Relationship {relationship.id} references non-existent target element {relationship.target_element_id}")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "systems": {k: {ek: ev.to_dict() for ek, ev in v.items()} for k, v in self.systems.items()},
            "metadata": self.metadata,
            "created_at": self.created_at.isoformat(),
            "updated_at": self.updated_at.isoformat()
        }
...
class BIMTransformer:
    ...
    def _extract_properties(svgx_element: Dict[str, Any]) -> Dict[str, BIMPropertySet]:
        ...
        if 'width' in svgx_element:
            physical_props.add_property(BIMProperty(
                id="width",
                name="Width",
        ...
        if 'capacity' in svgx_element:
            performance_props.add_property(BIMProperty(
                id="capacity",
                name="Capacity",
...
class BIMAnalyzer:
    ...
    def analyze_spatial_hierarchy(bim_model: EnhancedBIMModel) -> Dict[str, Any]:
        ...
        for element in bim_model.elements.values():
            if element.element_type in [BIMElementType.BUILDING, BIMElementType.FLOOR, 
                                      BIMElementType.ROOM, BIMElementType.ZONE, BIMElementType.SPACE]:
                spatial_elements[element.element_type.value] = spatial_elements.get(element.element_type.value, [])
    ...
    def analyze_relationships(bim_model: EnhancedBIMModel) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_relationships_per_element": total_relationships / len(bim_model.elements) if bim_model.elements else 0
        }
