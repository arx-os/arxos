...
class MonitoringDashboard:
    ...
    def get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "retention": {"metrics_days": 90, "reports_days": 30, "logs_days": 7},
    ...
    def check_alert(
        ...
        severity = (
            "critical"
            if metric_name in ["security_issues", "build_success_rate"]
            else "warning"
        )
    ...
    def send_alert_notifications(
        self, service: str, language: str, metric_name: str, message: str, severity: str
    ):
        """Send alert notifications"""
    ...
    def send_email_alert(
        ...
        try:
            ...
            msg["Subject"] = (
                f"[{severity.upper()}] Arxos SDK Alert: {service} ({language})"
            )
    ...
    def get_dashboard_data(self, days: int = 7) -> Dict[str, Any]:
        ...
        build_success_rate = (
            (build_result[1] / build_result[0] * 100) if build_result[0] > 0 else 0
        )
        ...
        pub_success_rate = (
            (pub_result[1] / pub_result[0] * 100) if pub_result[0] > 0 else 0
        )
    ...
    def cleanup_old_data(self):
        ...
        cursor.execute(
            "DELETE FROM metrics WHERE timestamp < ?", (metrics_cutoff.isoformat(),)
        )

        # Clean up old builds
        builds_cutoff = datetime.now() - timedelta(days=retention["reports_days"])
        cursor.execute(
            "DELETE FROM builds WHERE timestamp < ?", (builds_cutoff.isoformat(),)
        )

        # Clean up old tests
        cursor.execute(
            "DELETE FROM tests WHERE timestamp < ?", (builds_cutoff.isoformat(),)
        )

        # Clean up old publications
        cursor.execute(
            "DELETE FROM publications WHERE timestamp < ?", (builds_cutoff.isoformat(),)
        )

        # Clean up old alerts
        alerts_cutoff = datetime.now() - timedelta(days=retention["logs_days"])
        cursor.execute(
            "DELETE FROM alerts WHERE timestamp < ?", (alerts_cutoff.isoformat(),)
        )
...
def main():
    ...
    parser.add_argument(
        "--config", default="config/monitoring.yaml", help="Config file path"
    )
    parser.add_argument(
        "--generate-dashboard", action="store_true", help="Generate dashboard HTML"
    )
