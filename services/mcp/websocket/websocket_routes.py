            ...
            while True:
                ...
                try:
                    ...
                    await handle_client_message(
                        websocket, building_id, message, session_id
                    )
                except json.JSONDecodeError:
                    await websocket_manager.broadcast_error(
                        building_id, {"error": "Invalid JSON message"}, session_id
                    )
...
async def get_websocket_status():
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={"error": "Failed to get WebSocket status"}
        )
...
async def handle_client_message(
    websocket: WebSocket, building_id: str, message: dict, session_id: Optional[str]
):
    """
    ...
    if message_type == "ping":
        ...
        await handle_compliance_score_request(
            websocket, building_id, message, session_id
        )
        ...
        await websocket_manager.broadcast_error(
            building_id, {"error": f"Unknown message type: {message_type}"}, session_id
        )
...
async def handle_validation_request(
    ...
    try:
        ...
        # Perform validation
        validation_report = await rule_engine.validate_building_model_async(
            building_model
        )
        ...
        await websocket_manager.broadcast_error(
            building_id, {"error": f"Validation failed: {str(e)}"}, session_id
        )


async def handle_violation_highlight(
    websocket: WebSocket, building_id: str, message: dict, session_id: Optional[str]
):
    ...
    try:
        ...
        logger.info(
            f"Violation highlight broadcast for building {building_id}, violation {violation_id}"
        )

    except Exception as e:
        logger.error(f"Error handling violation highlight: {e}")
        await websocket_manager.broadcast_error(
            building_id, {"error": f"Violation highlight failed: {str(e)}"}, session_id
        )


async def handle_compliance_score_request(
    websocket: WebSocket, building_id: str, message: dict, session_id: Optional[str]
):
    ...
    try:
        ...
        await websocket_manager.broadcast_compliance_score(
            building_id, compliance_score, session_id
        )
