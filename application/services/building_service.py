...
class BuildingApplicationService:
    ...
    def create_building(
        ...
        try:
            ...
            if result.success:
                ...
                if self.metrics:
                    self.metrics.record_timing(
                        "building.create", time.time() - start_time
                    )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to create building", building_name=name, error=str(e)
                )
    ...
    def update_building(
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    "Updating building", building_id=building_id, updated_by=updated_by
                )
            ...
            if result.success:
                ...
                if self.event_store:
                    building_updated_event = BuildingUpdated(
                        ...
                        updated_fields=[
                            field
                            for field in [name, address, description, status]
                ...
                if self.metrics:
                    self.metrics.record_timing(
                        "building.update", time.time() - start_time
                    )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to update building", building_id=building_id, error=str(e)
                )
        ...
        try:
            ...
            if self.cache_service:
                ...
                if cached_building:
                    ...
                    return GetBuildingResponse(success=True, building=cached_building)
            ...
            if result.success:
                ...
                if self.cache_service:
                    self.cache_service.set(
                        f"building:{building_id}", result.building, ttl=300
                    )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to get building", building_id=building_id, error=str(e)
                )
    ...
    def list_buildings(
        ...
        try:
            ...
            if result.success:
                ...
                if self.metrics:
                    self.metrics.record_timing(
                        "building.list", time.time() - start_time
                    )
    ...
    def delete_building(self, building_id: str) -> DeleteBuildingResponse:
        ...
        try:
            # Log the operation
            if self.logger:
                self.logger.info("Deleting building", building_id=building_id)
            ...
            if result.success:
                ...
                if self.event_store:
                    building_deleted_event = BuildingDeleted(
                        building_id=building_id,
                        building_name="",  # Would need to get from building
                        deleted_by="system",
                    )
                ...
                if self.metrics:
                    self.metrics.record_timing(
                        "building.delete", time.time() - start_time
                    )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to delete building", building_id=building_id, error=str(e)
                )
    ...
    def get_building_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for building in buildings:
                ...
                stats["by_type"][building_type] = (
                    stats["by_type"].get(building_type, 0) + 1
                )
            ...
            if self.metrics:
                self.metrics.record_timing(
                    "building.statistics", time.time() - start_time
                )
            ...
            # Log error
            if self.logger:
                self.logger.error("Failed to get building statistics", error=str(e))

            # Record error metrics
            if self.metrics:
                self.metrics.increment_counter("building.statistics.error")

            return {"error": f"Failed to get building statistics: {str(e)}"}
    ...
    def search_buildings(
        ...
        try:
            ...
            for building in all_buildings:
                if (
                    query_lower in building.name.lower()
                    or query_lower in str(building.address).lower()
                    or (
                        building.description
                        and query_lower in building.description.lower()
                    )
                ):
            ...
            if self.logger:
                self.logger.error(
                    "Failed to search buildings", query=query, error=str(e)
                )
            ...
            return ListBuildingsResponse(
                success=False, error_message=f"Failed to search buildings: {str(e)}"
            )
