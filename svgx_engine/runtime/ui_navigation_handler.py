...
class NavigationHandler:
    ...
    def _handle_pan(self, event: Event, canvas_id: str) -> Dict[str, Any]:
        ...
        # Record in history
        self._record_navigation_action(
            canvas_id,
            "pan",
            {
                "dx": dx,
    ...
    def _record_navigation_action(
        ...
        self.navigation_history[canvas_id].append(
            {"action": action, "timestamp": event.timestamp, "data": data}
        )
    ...
    def add_focus_target(
        ...
        self.focus_targets[canvas_id][target_id] = {"x": x, "y": y, "zoom": zoom}
...
def _register_navigation_handler():
    def handler(event: Event):
        if (
            ...
            and event.data.get("event_subtype") == "navigation"
        ):
