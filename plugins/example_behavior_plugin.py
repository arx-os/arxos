...
class ExampleBehaviorPlugin(BehaviorHandlerPlugin):
    ...
    async def handle_behavior(
        self, behavior_data: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        logger.info(
            f"Processing behavior: {behavior_type} for object {object_id} on canvas {canvas_id}"
        )
        ...
        logger.info(
            f"Behavior processed successfully: {behavior_type} -> {result['modifications']}"
        )
