...
class NLPEngine:
    ...
    async def _analyze_text(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            words = text.split()
            if words:
                analysis["average_word_length"] = sum(
    ...
    async def _detect_intent(self, text: str) -> Dict[str, Any]:
        ...
        try:
                ...
                for pattern in patterns:
                    if pattern in text_lower:
                        detected_intents.append(
                            {"intent": intent, "pattern": pattern, "confidence": 0.8}
                        )
                        break
            ...
            return {
                ...
                "confidence": (
                    detected_intents[0]["confidence"] if detected_intents else 0.0
                ),
            }
        ...
        for category, keywords in cad_keywords.items():
            for keyword in keywords:
                if keyword in text_lower:
                    cad_entities.append(
                        {"text": keyword, "category": category, "confidence": 0.9}
                    )
    ...
    async def _analyze_domain_context(
        ...
        try:
            ...
            if any(keyword in text_lower for keyword in cad_keywords):
                ...
                domain_analysis["domain_keywords"].extend(
                    [k for k in cad_keywords if k in text_lower]
                )
            ...
            if any(keyword in text_lower for keyword in bim_keywords):
                ...
                domain_analysis["domain_keywords"].extend(
                    [k for k in bim_keywords if k in text_lower]
                )
