...
class TestPhase3Enhancements:
        ...
        return BuildingModel(
            ...
            objects=[
                ...
                BuildingObject(
                    ...
                    properties={"type": "exit_access", "width": 36.0},
                    location={
                        "x": 300,
                        "y": 100,
                        "z": 0,
                ...
                BuildingObject(
                    ...
                    properties={"type": "exit_discharge", "width": 36.0},
                    location={
                        "x": 318,
                        "y": 0,
                        "z": 0,
    ...
    def _create_test_mcp_files(self) -> list:
        ...
        for mcp_data, filename in [
            (nec_base, "nec_base.json"),
            (ibc_base, "ibc_base.json"),
        ]:
    ...
    def test_spatial_engine_adjacency_detection(self):
        ...
        is_adjacent = self.spatial_engine.check_spatial_relationship(
            obj_a, obj_b, "adjacent", max_distance=5.0
        )
        assert is_adjacent
    ...
    def test_spatial_engine_egress_distance_calculation(self):
        ...
        room = self.building_model.objects[8]  # room_bathroom_1
        exits = [
            obj for obj in self.building_model.objects if obj.object_type == "exit_door"
        ]
    ...
    def test_enhanced_formula_evaluation(self):
        ...
        hvac_unit = next(
            obj for obj in self.building_model.objects if obj.object_type == "hvac_unit"
        )
    ...
    def test_cross_system_validation(self):
        ...
        electrical_load = sum(
            ...
            for obj in self.building_model.objects
        ...
        assert (
            hvac_capacity > electrical_load
        )  # HVAC should be larger than electrical load
    ...
    def test_comprehensive_validation(self):
        ...
        try:
            compliance_report = self.engine.validate_building_model(
                self.building_model, self.mcp_files
            )
    ...
    def test_egress_requirements_validation(self):
        ...
        rooms = [
            obj for obj in self.building_model.objects if obj.object_type == "room"
        ]
        exits = [
            obj for obj in self.building_model.objects if obj.object_type == "exit_door"
        ]

        violations = self.spatial_engine.validate_egress_requirements(
            rooms, exits, max_egress_distance=50.0
        )
