...
class TestLogicEngineMigration:
    ...
    def test_rule_execution(self, logic_engine):
        ...
        conditions = [{"field": "data.value", "operator": ">", "value": 10}]

        actions = [{"type": "set_field", "field": "data.result", "value": "high"}]
    ...
    def test_rule_chain_execution(self, logic_engine):
        ...
        for i in range(2):
            rule_id = logic_engine.create_rule(
                ...
                actions=[
                    {
                        "type": "set_field",
                        "field": f"data.step_{i}",
                        "value": f"completed_{i}",
                    }
                ],
            )
            rule_ids.append(rule_id)
        ...
        assert all(
            execution.status == ExecutionStatus.SUCCESS for execution in executions
        )
    ...
    def test_builtin_functions(self, logic_engine):
        ...
        assert (
            logic_engine.builtin_functions["contains"]("hello world", "world") == True
        )
    ...
    def test_complex_condition_evaluation(self, logic_engine):
        ...
        actions = [{"type": "set_field", "field": "user.can_access", "value": True}]
        ...
        test_data = {"user": {"age": 25, "status": "active"}}
        ...
        test_data = {"user": {"age": 16, "status": "active"}}
    ...
    def test_field_path_evaluation(self, logic_engine):
        ...
        conditions = [
            {"field": "data.user.profile.email", "operator": "contains", "value": "@"}
        ]

        actions = [
            {"type": "set_field", "field": "data.user.profile.verified", "value": True}
        ]
        ...
        test_data = {"data": {"user": {"profile": {"email": "test@example.com"}}}}
    ...
    def test_error_handling(self, logic_engine):
        ...
        rule_id = logic_engine.create_rule(
            ...
            rule_type=RuleType.CONDITIONAL,
            conditions=[
                {"field": "nonexistent.field", "operator": "==", "value": True}
            ],
            actions=[{"type": "set_field", "field": "result", "value": "success"}],
        )
        ...
        )  # Conditions not met, not an error
