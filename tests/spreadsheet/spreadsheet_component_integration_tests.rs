...
use arxos::tui::spreadsheet::{
    editor::{CellEditor, EditState},
    undo_redo::UndoRedoManager,
    validation::validate_cell,
    CellType, CellValue, ColumnDefinition, FilterCondition, Grid, ValidationRule,
};
...
fn create_test_grid() -> Grid {
    ...
    for i in 0..3 {
        if let Some(row) = grid.rows.get_mut(i) {
            ...
            row[2].value = CellValue::Enum(if i % 2 == 0 {
                "HVAC".to_string()
            } else {
...
fn test_undo_redo_with_grid() {
    ...
    // Verify change
    assert_eq!(
        grid.get_cell(0, 1).unwrap().value,
...
fn test_filter_with_navigation() {
    ...
    use arxos::tui::spreadsheet::filter_sort;
