...
class SnapSettings:
    ...
    enabled: bool = True
    snap_types: Set[SnapType] = field(default_factory=lambda: {
        SnapType.GRID, SnapType.ENDPOINT, SnapType.MIDPOINT, SnapType.INTERSECTION
    })
...
class GridSystem:
    ...
    def get_grid_lines(self, bounds: Tuple[PrecisionPoint, PrecisionPoint]) -> List[Tuple[PrecisionPoint, PrecisionPoint]]:
        """
...
class ObjectSnapper:
    ...
    def _get_entity_snap_points(self, entity: Any, cursor_point: PrecisionPoint) -> List[SnapPoint]:
    ...
    def _get_tangent_points(self, entity: Any, cursor_point: PrecisionPoint) -> List[PrecisionPoint]:
        ...
        if hasattr(entity, 'center') and hasattr(entity, 'radius'):
            ...
            if distance_sq > radius * radius:
                ...
                if distance > 0:
                    ...
                    tangent_points.append(PrecisionPoint(
...
class AngleSnapper:
    ...
    def snap_angle(self, angle: decimal.Decimal) -> decimal.Decimal:
        ...
        snapped_degrees = round(angle_degrees / float(self.angle_snap)) * float(self.angle_snap)

        # Convert back to radians
        return decimal.Decimal(str(snapped_degrees)) * decimal.Decimal(str(math.pi)) / 180
    ...
    def get_snap_angles(self, base_angle: decimal.Decimal) -> List[decimal.Decimal]:
        ...
        for i in range(-4, 5):  # 9 angles total
            ...
            angles.append(decimal.Decimal(str(snap_angle)) * decimal.Decimal(str(math.pi)) / 180)
...
class GridSnapSystem:
    ...
    def __init__(self, grid_settings: Optional[GridSettings] = None,
    ...
    def snap_point(self, point: PrecisionPoint,
                   snap_to_grid: bool = True,
                   snap_to_objects: bool = True,
                   snap_to_angles: bool = True) -> PrecisionPoint:
        """
    ...
    def get_snap_feedback(self, cursor_point: PrecisionPoint) -> Dict[str, Any]:
        ...
        if self.object_snapper.settings.enabled:
            ...
            if snap_points:
                ...
                feedback["snap_points"] = [sp.position for sp in snap_points[:3]]  # Top 3
        ...
        if self.grid_system.settings.visible:
            ...
            feedback["grid_lines"] = self.grid_system.get_grid_lines((min_point, max_point))
    ...
    def get_snap_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_snap_type": self.last_snap_point.snap_type.value if self.last_snap_point else None,
            "grid_enabled": self.grid_system.settings.enabled,
            "snap_enabled": self.object_snapper.settings.enabled,
            "active_snap_types": [st.value for st in self.object_snapper.settings.snap_types]
        }
...
def create_grid_snap_system(grid_settings: Optional[GridSettings] = None,
                           snap_settings: Optional[SnapSettings] = None) -> GridSnapSystem:
    """Create a grid and snap system."""
    return GridSnapSystem(grid_settings, snap_settings)


def create_grid_settings(spacing: Union[float, decimal.Decimal] = 10.0,
                        origin_x: Union[float, decimal.Decimal] = 0.0,
                        origin_y: Union[float, decimal.Decimal] = 0.0,
                        enabled: bool = True,
                        visible: bool = True) -> GridSettings:
...
def create_snap_settings(enabled: bool = True,
                        tolerance: Union[float, decimal.Decimal] = 5.0,
                        angle_snap: Union[float, decimal.Decimal] = 15.0,
                        snap_types: Optional[Set[SnapType]] = None) -> SnapSettings:
    """Create snap settings."""
    if snap_types is None:
        snap_types = {SnapType.GRID, SnapType.ENDPOINT, SnapType.MIDPOINT, SnapType.INTERSECTION}
