...
class DeviceApplicationService:
    ...
    def create_device(
        ...
        try:
            ...
            if result.success:
                ...
                if self.metrics:
                    self.metrics.record_timing(
                        "device.create", time.time() - start_time
                    )
            ...
            return CreateDeviceResponse(
                success=False, error_message=f"Failed to create device: {str(e)}"
            )
    ...
    def update_device(
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    "Updating device", device_id=device_id, updated_by=updated_by
                )
            ...
            if result.success:
                ...
                if self.event_store:
                    device_updated_event = DeviceUpdated(
                        ...
                        updated_fields=[
                            field
                            for field in [name, device_type, status]
                ...
                if self.metrics:
                    self.metrics.record_timing(
                        "device.update", time.time() - start_time
                    )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to update device", device_id=device_id, error=str(e)
                )
            ...
            return UpdateDeviceResponse(
                success=False, error_message=f"Failed to update device: {str(e)}"
            )
        ...
        try:
            ...
            if self.cache_service:
                ...
                if cached_device:
                    ...
                    return GetDeviceResponse(success=True, device=cached_device)
            ...
            if result.success:
                ...
                if self.cache_service:
                    self.cache_service.set(
                        f"device:{device_id}", result.device, ttl=300
                    )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to get device", device_id=device_id, error=str(e)
                )
            ...
            return GetDeviceResponse(
                success=False, error_message=f"Failed to get device: {str(e)}"
            )
    ...
    def list_devices(
        ...
        try:
            ...
            result = list_devices_uc.execute(
                room_id, device_type, status, page, page_size
            )
            ...
            return ListDevicesResponse(
                success=False, error_message=f"Failed to list devices: {str(e)}"
            )
    ...
    def delete_device(self, device_id: str) -> DeleteDeviceResponse:
        ...
        try:
            # Log the operation
            if self.logger:
                self.logger.info("Deleting device", device_id=device_id)
            ...
            if result.success:
                ...
                if self.metrics:
                    self.metrics.record_timing(
                        "device.delete", time.time() - start_time
                    )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to delete device", device_id=device_id, error=str(e)
                )
            ...
            return DeleteDeviceResponse(
                success=False, error_message=f"Failed to delete device: {str(e)}"
            )
    ...
    def get_device_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for device in devices:
                ...
                stats["by_manufacturer"][manufacturer] = (
                    stats["by_manufacturer"].get(manufacturer, 0) + 1
                )
            ...
            if self.metrics:
                self.metrics.record_timing(
                    "device.statistics", time.time() - start_time
                )
            ...
            # Log error
            if self.logger:
                self.logger.error("Failed to get device statistics", error=str(e))

            # Record error metrics
            if self.metrics:
                self.metrics.increment_counter("device.statistics.error")

            return {"error": f"Failed to get device statistics: {str(e)}"}
