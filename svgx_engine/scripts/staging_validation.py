...
class StagingValidator:
    ...
    async def performance_test(
        ...
        logger.info(
            f"Performance test completed for {endpoint}: {stats['mean']:.2f}ms avg"
        )
        return stats
    ...
    async def load_test(
        ...
        logger.info(
            f"Starting load test for {endpoint} with {concurrent_users} users..."
        )
        ...
        async def user_workload():
            ...
            while time.time() - start_time < duration_seconds:
                ...
                try:
                    ...
                    user_results.append(
                        {"duration": 0, "status": 0, "success": False, "error": str(e)}
                    )
    ...
    async def validate_performance_targets(self) -> Dict:
        ...
        for endpoint, payload in test_payloads.items():
            performance_stats = await self.performance_test(
                endpoint, payload, iterations=20
            )
        ...
        validation_results = {
            "ui_response_time": {
                ...
                "passed": target_results.get("evaluate", {}).get("mean", 0)
            ...
            "redraw_time": {
                ...
                "passed": target_results.get("parse", {}).get("mean", 0)
            ...
            "physics_simulation": {
                ...
                "passed": target_results.get("simulate", {}).get("mean", 0)
                <= self.performance_targets.physics_simulation,
            },
        }

        logger.info("Performance target validation completed")
        return {"targets": validation_results, "detailed_results": target_results}
    ...
    async def run_comprehensive_validation(self) -> Dict:
        ...
        load_results = await self.load_test(
            "parse", load_test_payload, concurrent_users=20, duration_seconds=30
        )
