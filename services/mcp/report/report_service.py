...
class ReportService:
                ...
                try:
                    with open(file_path, "rb") as file:
                        self.s3_client.upload_fileobj(
                            file, self.s3_bucket, f"reports/{filename}"
                        )

                    cloud_urls["aws_s3"] = (
                        f"https://{self.s3_bucket}.s3.amazonaws.com/reports/{filename}"
                    )
            ...
            if self.azure_client and self.azure_container:
                try:
                    blob_client = self.azure_client.get_blob_client(
                        container=self.azure_container, blob=f"reports/{filename}"
                    )
    ...
    async def generate_and_send_report(
        ...
        try:
            ...
            email_result = await self.send_report_email(
                generation_result["report_path"], recipients
            )
    ...
    async def get_report_history(
        self, building_id: str = None, limit: int = 50
    ) -> List[Dict[str, Any]]:
        """
            ...
            if self.s3_client and self.s3_bucket:
                try:
                    self.s3_client.delete_object(
                        Bucket=self.s3_bucket, Key=f"reports/{filename}"
                    )
