...
def test_access_control_migration():
    ...
    try:
        ...
        audit_logs = ac_service.get_audit_logs(user_id=user_data["user_id"], limit=10)
        ...
        cad_capabilities = ac_service.get_user_svgx_capabilities(
            cad_user_data["user_id"]
        )
        ...
        bim_capabilities = ac_service.get_user_svgx_capabilities(
            bim_user_data["user_id"]
        )
...
def test_clean_code_practices():
    ...
    try:
        ...
        user_data = ac_service.create_user(
            username="test", email="test@example.com", primary_role=UserRole.VIEWER
        )
