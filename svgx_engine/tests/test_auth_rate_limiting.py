...
class TestAuthenticationAndRateLimiting:
    ...
    def test_rate_limiting_basic(self):
        ...
        for i in range(35):  # Exceed the 30 requests per minute limit for POST
            response = self.client.post(
                "/runtime/ui-event/",
                json={"event_type": "test", "canvas_id": "test-canvas"},
            )
    ...
    def test_rate_limiting_by_user(self):
        ...
        for i in range(25):  # Under the limit
            response = self.client.get(
                "/health/", headers={"Authorization": f"Bearer {user1_token}"}
            )
        ...
        for i in range(25):  # Under the limit
            response = self.client.get(
                "/health/", headers={"Authorization": f"Bearer {user2_token}"}
            )
    ...
    def test_invalid_token_format(self):
        ...
        response = self.client.get(
            "/metrics/", headers={"Authorization": "Bearer invalid.token.format"}
        )
    ...
    def test_permission_based_access(self):
        ...
        write_endpoint_data = {"event_type": "test", "canvas_id": "test-canvas"}

        # Admin should have write permission
        admin_write_response = self.client.post(
            "/runtime/ui-event/",
        ...
        viewer_write_response = self.client.post(
            "/runtime/ui-event/",
            json=write_endpoint_data,
    ...
    def test_token_validation_edge_cases(self):
        ...
        response = self.client.get(
            "/metrics/", headers={"Authorization": "Bearer malformed.token.here"}
        )
        assert response.status_code == 401

        # Test with empty token
        response = self.client.get("/metrics/", headers={"Authorization": "Bearer "})
        assert response.status_code == 401

        # Test with wrong token type
        response = self.client.get(
            "/metrics/", headers={"Authorization": "Basic dGVzdDp0ZXN0"}
        )
        assert response.status_code == 401
