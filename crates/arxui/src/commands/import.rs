...
pub fn handle_import(
    ...
    match processor.extract_hierarchy(&ifc_file) {
        Ok((building, floors)) => {
            progress_reporter.finish_success(&format!(
                "Successfully extracted building hierarchy: {}",
                building.name
            ));
            ...
            println!(
                "   Created: {}",
                building.created_at.format("%Y-%m-%d %H:%M:%S")
            );
            ...
            if dry_run {
                println!(
                    "🔍 DRY RUN: Would generate YAML file for building: {}",
                    building.name
                );
                if let Some(ref repo_path) = repo {
                    println!(
                        "🔍 DRY RUN: Would initialize Git repository at: {}",
                        repo_path
                    );
                }
                return Ok(());
        ...
        Err(e) => {
            ...
            warn!(
                "Hierarchy extraction failed, trying fallback parsing: {}",
                e
            );
            ...
            match processor.process_file_with_progress(&ifc_file, progress_context) {
                ...
                Err(fallback_error) => {
                    progress_reporter
                        .finish_error(&format!("Error processing IFC file: {}", fallback_error));
                    return Err(format!(
...
fn initialize_git_repo(
    ...
    let git_config = GitConfig {
        author_name: std::env::var("GIT_AUTHOR_NAME")
            .unwrap_or_else(|_| "ArxOS System".to_string()),
        author_email: std::env::var("GIT_AUTHOR_EMAIL")
            .unwrap_or_else(|_| "system@arxos.dev".to_string()),
    ...
    let content = PathSafety::read_file_safely(std::path::Path::new(yaml_file), &base_dir)
    ...
    let result = git_manager.export_building(
        &building_data,
        Some(&format!("Initial import from IFC: {}", building_name)),
    )?;
