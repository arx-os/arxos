...
class TestGenerationRequest:
    ...
    def test_invalid_output_format(self):
        ...
        with pytest.raises(ValidationError):
            GenerationRequest(category="geometric", output_format="invalid_format")

    def test_invalid_quality_level(self):
        """Test validation of quality level."""
        with pytest.raises(ValidationError):
            GenerationRequest(category="geometric", quality_level="invalid_level")
...
class TestSVGXSymbolGenerator:
    ...
    def test_generate_symbol_template_not_found(self, generator):
        ...
        request = GenerationRequest(
            template_id="nonexistent_template", category="geometric"
        )
    ...
    def test_generate_batch(self, generator):
        ...
        requests = [
            GenerationRequest(
                category="geometric", parameters={"shape_type": "circle", "size": 100}
            ),
            GenerationRequest(
                category="geometric", parameters={"shape_type": "square", "size": 120}
            ),
            GenerationRequest(
                category="geometric", parameters={"shape_type": "triangle", "size": 80}
            ),
        ]
    ...
    def test_validate_parameters_against_constraints(self, generator):
        ...
        with pytest.raises(SVGXValidationError):
            generator._validate_parameters_against_constraints(
                {"size": 150}, constraints
            )
    ...
    def test_batch_generation_workflow(self):
        ...
        requests = [
            ...
            GenerationRequest(
                template_id="technical_symbol",
                category="technical",
                parameters={"symbol_type": "valve"},
            ),
            GenerationRequest(
                template_id="abstract_pattern",
                category="abstract",
                parameters={"pattern_type": "geometric"},
            ),
        ]
