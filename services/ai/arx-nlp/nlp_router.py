...
from .models.nlp_models import NLPResponse, CLICommand
...
class NLPRouter:
    ...
    def parse_natural_language(
        self, text: str, context: Optional[NLPContext] = None
    ) -> NLPResponse:
        """
        ...
        try:
            ...
            slot_result = self.slot_filler.extract_slots(
                text, intent_result.intent_type
            )
            ...
            self.logger.info(
                f"NLP processing completed with confidence: {intent_result.confidence}"
            )
            return response
    ...
    def _create_error_response(self, text: str, error: str) -> NLPResponse:
        ...
        return NLPResponse(
            original_text=text,
            intent=Intent(intent_type=IntentType.QUERY, confidence=0.0, raw_text=text),
            slots=[],
            cli_command=CLICommand(
                command="help", arguments=["--error", error], options={}
            ),
            confidence=0.0,
            context=NLPContext(),
            timestamp=datetime.now(),
    ...
    def batch_process(
        self, texts: List[str], context: Optional[NLPContext] = None
    ) -> List[NLPResponse]:
        """
    ...
    def get_suggestions(
        self, partial_text: str, context: Optional[NLPContext] = None
    ) -> List[str]:
        """
...
if __name__ == "__main__":
    ...
    for text in test_inputs:
        ...
        print(
            f"CLI Command: {result.cli_command.command} {' '.join(result.cli_command.arguments)}"
        )
        print("-" * 50)
