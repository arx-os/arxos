...
class CodeQualityEngine:
    ...
    def analyze_code_quality(self, file_path: str) -> CodeQualityCheck:
        ...
        try:
            ...
            overall_score = (solid_score + clean_code_score + documentation_score + 
                           performance_score + security_score) / 5
            ...
            recommendations = self._generate_recommendations(
                solid_score, clean_code_score, documentation_score,
    ...
    def _analyze_solid_principles(self, tree: ast.AST, content: str) -> float:
        ...
        for cls in classes:
            ...
            if len(methods) > 15:  # Interface too large
                score -= 10
                issues.append(f"Class {cls.name} has too many methods - consider interface segregation")
    ...
    def _generate_recommendations(self, solid_score: float, clean_code_score: float,
                                documentation_score: float, performance_score: float,
    ...
    def generate_quality_report(self, file_paths: List[str]) -> Dict[str, Any]:
        ...
        excellent_count = sum(1 for check in checks if check.level == CodeQualityLevel.EXCELLENT)
        good_count = sum(1 for check in checks if check.level == CodeQualityLevel.GOOD)
        acceptable_count = sum(1 for check in checks if check.level == CodeQualityLevel.ACCEPTABLE)
        needs_improvement_count = sum(1 for check in checks if check.level == CodeQualityLevel.NEEDS_IMPROVEMENT)
        ...
        logger.info(f"✅ Code quality analysis completed. Overall score: {average_score:.1f}")
        return report
