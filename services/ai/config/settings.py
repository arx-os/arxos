...
class AIServiceSettings(BaseSettings):
    ...
    voice_max_duration: int = Field(default=30, description="Maximum voice duration in seconds")
    
    # Geometry Validation Configuration
    geometry_precision: float = Field(default=1e-6, description="Geometry precision tolerance")
    geometry_max_vertices: int = Field(default=10000, description="Maximum vertices per geometry")
...
def get_settings() -> AIServiceSettings:
    ...
    if _settings is None:
        ...
        logger.info("AI Service settings loaded", 
    ...
    for env_var, setting_name in ENV_MAPPINGS.items():
        ...
        if env_value is not None:
            if hasattr(settings, setting_name):
                ...
                logger.info(f"Loaded {setting_name} from environment variable {env_var}")
