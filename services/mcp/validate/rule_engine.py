...
try:
    ...
    spec = importlib.util.spec_from_file_location(
        "mcp_models", 
        Path(__file__).parent.parent / "models" / "mcp_models.py"
    )
    ...
    spatial_spec = importlib.util.spec_from_file_location(
        "spatial_engine",
        Path(__file__).parent / "spatial_engine.py"
    )
...
class ConditionEvaluator:
    ...
    def _evaluate_spatial_condition(self, condition: RuleCondition, 
        ...
        for obj in objects:
            ...
            if condition.property == 'area':
                ...
                if self._evaluate_property_value(area, condition.operator, condition.value):
                    matched_objects.append(obj)
                    
            elif condition.property == 'volume':
                volume = self.spatial_engine.calculate_room_volume(obj)
                if self._evaluate_property_value(volume, condition.operator, condition.value):
                    matched_objects.append(obj)
                ...
                if isinstance(condition.value, dict) and 'target_id' in condition.value:
                    target_obj = self._find_object_by_id(objects, condition.value['target_id'])
                    if target_obj:
                        distance = self.spatial_engine.calculate_distance(obj, target_obj)
                        if self._evaluate_property_value(distance, condition.operator, condition.value.get('max_distance', 0)):
                            matched_objects.append(obj)
                            
            elif condition.property == 'height':
                height = self._get_object_height(obj)
                if self._evaluate_property_value(height, condition.operator, condition.value):
                    matched_objects.append(obj)
                    
            elif condition.property == 'floor_level':
                floor_level = self.spatial_engine.get_floor_level(obj)
                if self._evaluate_property_value(floor_level, condition.operator, condition.value):
                    matched_objects.append(obj)
                ...
                if target_type:
                    target_objects = [o for o in objects if o.object_type == target_type]
                    for target_obj in target_objects:
                        if self.spatial_engine.check_spatial_relationship(obj, target_obj, 'adjacent', max_distance=max_distance):
                            matched_objects.append(obj)
                            break
                ...
                if target_type:
                    target_objects = [o for o in objects if o.object_type == target_type]
                    for target_obj in target_objects:
                        if self.spatial_engine.check_spatial_relationship(obj, target_obj, 'within_distance', max_distance=max_distance):
                            matched_objects.append(obj)
                            break
        
        return matched_objects
    
    def _evaluate_property_value(self, value: Any, operator: str, target_value: Any) -> bool:
    ...
    def _find_object_by_id(self, objects: List[BuildingObject], object_id: str) -> Optional[BuildingObject]:
        """Find object by ID"""
        for obj in objects:
...
class ActionExecutor:
    ...
    def _execute_validation_action(self, action: RuleAction, 
                                 context: RuleExecutionContext) -> List[ValidationViolation]:
    ...
    def _evaluate_formula(self, formula: str, context: RuleExecutionContext) -> float:
        ...
        if not variables and not context.matched_objects:
            variables = self._extract_variables_from_objects(context.building_model.objects)
        ...
        try:
            ...
            logger.error(f"Error evaluating formula '{formula}' with variables {variables}: {e}")
            # Return 0 as fallback instead of raising
            return 0.0
    
    def _extract_variables_from_objects(self, objects: List[BuildingObject]) -> Dict[str, Any]:
...
class MCPRuleEngine:
    ...
    def validate_building_model(self, building_model: BuildingModel, 
                              mcp_files: List[str] = None) -> ComplianceReport:
        """
        ...
        try:
            ...
            if mcp_files is None and self.jurisdiction_matcher:
                ...
                applicable_codes = self.jurisdiction_matcher.get_applicable_codes(building_dict)
