...
fn test_complete_ios_ar_workflow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    unsafe {
        ...
        assert!(
            load_parsed.get("file_path").is_some(),
        ...
        assert!(
            save_parsed
                ...
                .and_then(|s| s.as_bool())
                .unwrap_or(false),
            "AR scan save should succeed. Response: {}",
            save_json_str
        );
        ...
        assert!(
            ...
            "Should have pending IDs. Response: {}",
            save_json_str
        );
        ...
        assert!(
            list_parsed
                ...
                .and_then(|s| s.as_bool())
                .unwrap_or(false),
            "List pending should succeed. Response: {}",
            list_json_str
        );
        ...
        assert!(
            !items.is_empty(),
            "Should have pending items. Response: {}",
            list_json_str
        );
        ...
        let confirm_result =
            ffi::arxos_confirm_pending_equipment(building, pending_id, std::ptr::null(), 0); // Don't commit to Git
        ...
        let equipment = floor
            .equipment
            .iter()
            .find(|e| e.name == "iOS Workflow Equipment");
...
fn test_ios_ar_workflow_with_rejection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    unsafe {
        ...
        let equipment = floor
            .equipment
            .iter()
            .find(|e| e.name == "Reject Test Equipment");
        assert!(
            equipment.is_none(),
            "Rejected equipment should NOT be in building"
        );
