...
class LoadCalculator:
    ...
    def calculate_dead_load(self, volume: float, material: str) -> float:
        ...
        logger.info(
            f"Calculated dead load: {dead_load:.2f} kN for {volume:.2f} m³ of {material}"
        )
        return dead_load
    ...
    def calculate_live_load(self, area: float, occupancy_type: str) -> float:
        ...
        logger.info(
            f"Calculated live load: {live_load:.2f} kN for {area:.2f} m² {occupancy_type}"
        )
        return live_load

    def calculate_wind_load(
        self, area: float, height: float, wind_params: Optional[WindParameters] = None
    ) -> float:
        """
        ...
        design_pressure = (
            ...
            * exposure_factor
            * height_factor
            * wind_params.topographic_factor
            * wind_params.gust_factor
            * wind_params.directionality_factor
            * wind_params.importance_factor
        )

        wind_load = area * design_pressure / 1000.0  # Convert to kN
        logger.info(
            f"Calculated wind load: {wind_load:.2f} kN for {area:.2f} m² at height {height:.1f} m"
        )
        return wind_load

    def calculate_seismic_load(
        self, weight: float, seismic_params: Optional[SeismicParameters] = None
    ) -> float:
        """
        ...
        base_shear = (
            weight
            * seismic_params.spectral_acceleration
            * seismic_params.importance_factor
            / seismic_params.response_modification_factor
        )

        logger.info(
            f"Calculated seismic load: {base_shear:.2f} kN for weight {weight:.2f} kN"
        )
        return base_shear

    def calculate_snow_load(
        self, area: float, roof_slope: float, ground_snow_load: float = 1.44
    ) -> float:
        """
        ...
        logger.info(
            f"Calculated snow load: {snow_load:.2f} kN for {area:.2f} m² roof with {roof_slope:.1f}° slope"
        )
        return snow_load

    def calculate_impact_load(
        self, static_load: float, impact_factor: float = 1.5
    ) -> float:
        """
        ...
        logger.info(
            f"Calculated impact load: {impact_load:.2f} kN with factor {impact_factor}"
        )
        return impact_load

    def calculate_thermal_load(
        self, area: float, temperature_diff: float, thermal_coefficient: float = 12e-6
    ) -> float:
        """
        ...
        logger.info(
            f"Calculated thermal load: {thermal_load:.2f} kN for {temperature_diff:.1f}°C change"
        )
        return thermal_load

    def combine_loads(
        self, loads: Dict[str, float], combination: LoadCombination
    ) -> float:
        """
    ...
    def calculate_dynamic_load(
        self, static_load: float, frequency: float, damping_ratio: float = 0.05
    ) -> float:
        """
        ...
        logger.info(
            f"Calculated dynamic load: {dynamic_load:.2f} kN (amplification: {amplification_factor:.2f})"
        )
        return dynamic_load
    ...
    def add_material_density(
        self, material: str, density: float, unit_weight: float
    ) -> None:
        """Add custom material density."""
        self.material_densities[material] = MaterialDensity(
            material, density, unit_weight
        )
