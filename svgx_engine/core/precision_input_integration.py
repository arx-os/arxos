...
class PrecisionInputGeometryIntegration:
    ...
    def __init__(self, config: Optional[PrecisionConfig] = None):
        ...
        self.geometry_factory = PrecisionGeometryFactory(
            self.config, self.geometry_adapter
        )
        ...
        # Feedback callbacks
        self.on_input_feedback: Optional[Callable[[PrecisionInputFeedback], None]] = (
            None
        )
    ...
    def set_input_feedback_callback(
        self, callback: Callable[[PrecisionInputFeedback], None]
    ):
    ...
    def set_validation_error_callback(
        self, callback: Callable[[str, Dict[str, Any]], None]
    ):
        """Set callback for validation error events"""
        self.on_validation_error = callback

    def start_geometry_creation(
        self, geometry_type: str, input_mode: InputMode = InputMode.FREEHAND
    ) -> str:
        """
        ...
        context = GeometryCreationContext(
            geometry_type=geometry_type, input_mode=input_mode
        )
        ...
        # Provide feedback
        self._provide_creation_feedback(
            session_id,
            "creation_started",
            {"geometry_type": geometry_type, "input_mode": input_mode.value},
        )
    ...
    def handle_mouse_input(
        ...
        if coordinate:
            ...
            validation_result = self._validate_coordinate_for_geometry(
                coordinate, context
            )
            ...
            self._provide_creation_feedback(
                session_id,
                "invalid_coordinate",
                {"coordinates": (x, y, z), "event_type": event_type},
            )
            return None
    ...
    def handle_keyboard_input(
        self, session_id: str, x_str: str, y_str: str, z_str: str = "0.0"
    ) -> Optional[PrecisionCoordinate]:
        """
        ...
        if coordinate:
            ...
            # Handle invalid input
            self._provide_creation_feedback(
                session_id, "invalid_coordinate", {"coordinates": (x_str, y_str, z_str)}
            )
            return None
    ...
    def _validate_coordinate_for_geometry(
        self, coordinate: PrecisionCoordinate, context: GeometryCreationContext
    ) -> Dict[str, Any]:
        """
        Validate coordinate for geometry creation.
        ...
        try:
            ...
            if not coord_validation.is_valid:
                ...
                validation_result["errors"].append(
                    f"Coordinate validation failed: {coord_validation.errors}"
                )
            ...
            if context.geometry_type == "line" and context.get_coordinate_count() == 2:
                ...
                and context.get_coordinate_count() == 2
            ):
                ...
                min_radius = self.config.validation_rules.get(
    ...
    def _complete_geometry_creation(self, session_id: str):
        ...
        try:
            ...
            if geometry:
                ...
                logger.info(
                    f"Completed geometry creation for session {session_id}: {context.geometry_type}"
                )
            else:
            ...
            self._provide_creation_feedback(
                session_id,
                "creation_error",
                {"error": str(e), "geometry_type": context.geometry_type},
            )

    def _create_geometry_from_context(
        self, context: GeometryCreationContext
        ...
        try:
            if context.geometry_type == "point":
                ...
                if context.get_coordinate_count() >= 2:
                    ...
                    return self.geometry_factory.create_rectangle_2d(
                        x, y, width, height
                    )
    ...
    def _provide_creation_feedback(
        self, session_id: str, feedback_type: str, data: Dict[str, Any]
    ):
        """
    ...
    def _handle_error(self, error_message: str):
        ...
        if self.on_validation_error:
            self.on_validation_error(
                "input_integration_error", {"message": error_message}
            )
...
class PrecisionInputGeometryValidator:
    ...
    def validate_geometry_input(
        self, coordinates: List[PrecisionCoordinate], geometry_type: str
    ) -> Dict[str, Any]:
        """
    ...
    def _validate_line_geometry(
        self, coordinates: List[PrecisionCoordinate], validation_result: Dict[str, Any]
    ):
