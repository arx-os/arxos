...
class TestSymbolSchemaValidationService:
    ...
    def service(self, temp_dir):
        ...
        with patch(
            "services.symbol_schema_validation.SymbolSchemaValidationService._init_databases"
        ) as mock_init:
...
<svgx xmlns="http://www.svgx.org/schema/1.0">
        ...
        return json.dumps(
            {
                ...
                "behaviors": {
                    "actions": [
                        {"name": "click", "type": "interaction", "action": "highlight"}
                    ]
                },
                "physics": {"mass": 1.0, "friction": 0.1},
            }
        )
    ...
    def test_validate_symbol_xml(self, service, sample_svgx_xml):
        ...
        result = service.validate_symbol(symbol_id=symbol_id, content=sample_svgx_xml)
    ...
    def test_validate_symbol_json(self, service, sample_svgx_json):
        ...
        result = service.validate_symbol(symbol_id=symbol_id, content=sample_svgx_json)
    ...
    def test_validate_invalid_xml(self, service):
        ...
        result = service.validate_symbol(symbol_id=symbol_id, content=invalid_xml)
    ...
    def test_validate_invalid_json(self, service):
        ...
        result = service.validate_symbol(symbol_id=symbol_id, content=invalid_json)
    ...
    def test_validation_caching(self, service, sample_svgx_xml):
        ...
        # First validation
        result1 = service.validate_symbol(symbol_id=symbol_id, content=sample_svgx_xml)

        # Second validation (should use cache)
        result2 = service.validate_symbol(symbol_id=symbol_id, content=sample_svgx_xml)
    ...
    def test_validation_statistics(self, service, sample_svgx_xml):
        ...
        for i in range(3):
            service.validate_symbol(
                symbol_id=f"test_symbol_{i}", content=sample_svgx_xml
            )
    ...
    def test_clear_cache(self, service, sample_svgx_xml):
        ...
        service.validate_symbol(symbol_id="test_symbol_cache", content=sample_svgx_xml)
...
<svgx xmlns="http://www.svgx.org/schema/1.0">
    ...
    <geometry>
        ...
        result = service.validate_symbol(
            symbol_id="test_svgx_specific", content=valid_svgx
        )
...
<svg xmlns="http://www.w3.org/2000/svg">
    <rect x="0" y="0" width="100" height="50"/>
        ...
        result = service.validate_symbol(
            symbol_id="test_svgx_invalid", content=invalid_svgx
        )
    ...
    def test_custom_validation_rules(self, service):
        ...
        result = service.validate_symbol(
            symbol_id="test_custom_valid", content=valid_content
        )

        # Test with invalid content
        invalid_content = "<svg xmlns='http://www.w3.org/2000/svg'></svg>"
        result = service.validate_symbol(
            symbol_id="test_custom_invalid", content=invalid_content
        )
    ...
    def test_performance_monitoring(self, service, sample_svgx_xml):
        ...
        result = service.validate_symbol(
            symbol_id="test_performance", content=sample_svgx_xml
        )
    ...
    def test_error_handling(self, service):
        ...
        with pytest.raises(ValidationError):
            service.validate_symbol(symbol_id="test_empty", content="")

        # Test with None content
        with pytest.raises(ValidationError):
            service.validate_symbol(symbol_id="test_none", content=None)
    ...
    def test_cleanup(self, service):
        ...
        # Add some test data
        service.validate_symbol(symbol_id="test_cleanup", content="<svgx></svgx>")
    ...
    def test_concurrent_validations(self, service, sample_svgx_xml):
        ...
        def validate_symbol(symbol_id):
            try:
                result = service.validate_symbol(
                    symbol_id=symbol_id, content=sample_svgx_xml
                )
        ...
        for i in range(5):
            thread = threading.Thread(
                target=validate_symbol, args=(f"concurrent_symbol_{i}",)
            )
