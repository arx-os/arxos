...
class ReportGenerator:
    ...
    def generate_compliance_report(self, validation_data: Dict[str, Any], 
        ...
        """
        try:
            logger.info(f"Generating compliance report for building {building_info.get('building_id', 'unknown')}")
    ...
    def _create_executive_summary(self, validation_data: Dict[str, Any], 
                                 building_info: Dict[str, Any]) -> List:
    ...
    def _create_violations_section(self, validation_data: Dict[str, Any]) -> List:
        ...
        # Section title
        title = Paragraph("Detailed Violations and Warnings", self.styles['CustomHeading'])
        elements.append(title)
        ...
        # Critical violations
        if violations:
            elements.append(Paragraph("Critical Violations", self.styles['CustomSubheading']))
    ...
    def _create_compliance_summary(self, validation_data: Dict[str, Any]) -> List:
        ...
        if recommendations:
            elements.append(Paragraph("Recommendations", self.styles['CustomSubheading']))
    ...
    def generate_violation_summary(self, validation_data: Dict[str, Any],
                                 output_path: str) -> str:
        ...
        try:
            ...
            # Summary statistics
            total_violations = validation_data.get('total_violations', 0)
            critical_violations = len([v for v in validation_data.get('violations', []) 
            ...
            summary_data = [
                ...
                ['Compliance Rate:', f"{validation_data.get('compliance_rate', 0):.1f}%"]
            ]
            ...
            violations = validation_data.get('violations', [])
            if violations:
                story.append(Paragraph("Critical Violations:", self.styles['CustomHeading']))
    ...
    def generate_executive_summary(self, validation_data: Dict[str, Any],
                                 building_info: Dict[str, Any],
                                 output_path: str) -> str:
        ...
        try:
            ...
            if total_violations == 0:
                findings.append("✅ Building meets all critical compliance requirements")
            else:
                findings.append(f"❌ {total_violations} violations require immediate attention")
            
            if validation_data.get('total_warnings', 0) > 0:
                findings.append(f"⚠️ {validation_data.get('total_warnings', 0)} warnings for review")
