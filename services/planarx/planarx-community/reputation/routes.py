...
async def record_contribution(
    ...
    try:
        ...
        if not event:
            raise HTTPException(
                status_code=400, detail="Invalid contribution or daily limit exceeded"
            )
...
async def get_user_reputation(
    ...
    recommendations = badge_system.get_badge_recommendations(
        user_id, contribution_stats
    )
...
async def get_badge_details(
    badge_id: str, current_user: User = Depends(get_current_user)
):
...
async def get_badges_by_rarity(
    rarity: str, current_user: User = Depends(get_current_user)
):
...
async def get_badge_progress(
    user_id: str, badge_id: str, current_user: User = Depends(get_current_user)
):
...
async def get_abuse_reports(current_user: User = Depends(get_current_user)):
...
async def flag_user_for_review(
    user_id: str, reason: str, current_user: User = Depends(get_current_user)
):
    ...
    return {"success": True, "message": f"User {user_id} flagged for review"}
...
async def clear_abuse_flag(
    ...
    return {"success": True, "message": f"Abuse flag cleared for user {user_id}"}
...
async def get_grant_visibility_rules(
    grant_type: str, current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        user_reputation = reputation_engine.get_user_reputation(current_user.id)
        user_tier = (
            user_reputation.current_tier.value if user_reputation else "newcomer"
        )
        ...
        return {"success": True, "visibility_rules": visibility_rules}
...
async def get_escrow_rules(
    ...
    try:
        ...
        return {"success": True, "escrow_rules": escrow_rules}
...
async def get_review_requirements(
    ...
    try:
        grant_enum = GrantType(grant_type)
        review_requirements = grant_eligibility_engine.get_review_requirements(
            grant_enum
        )

        return {"success": True, "review_requirements": review_requirements}
...
async def get_eligibility_stats(current_user: User = Depends(get_current_user)):
...
async def get_reputation_stats(current_user: User = Depends(get_current_user)):
