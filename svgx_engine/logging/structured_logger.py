...
class SVGXLogger:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "include_timestamp": os.getenv("SVGX_LOG_INCLUDE_TIMESTAMP", "true").lower()
            == "true",
            "include_level": os.getenv("SVGX_LOG_INCLUDE_LEVEL", "true").lower()
            == "true",
        }
    ...
    def database(self, operation: str, table: str, duration_ms: float, **kwargs):
        ...
        self.logger.info(
            f"Database: {operation} on {table} took {duration_ms:.2f}ms", extra=extra
        )
    ...
    def cache(self, operation: str, key: str, hit: bool, duration_ms: float, **kwargs):
        ...
        self.logger.info(
            f"Cache {cache_type}: {operation} {key} ({duration_ms:.2f}ms)", extra=extra
        )
...
class LoggingContext:
    ...
    def __enter__(self):
        ...
        self.logger.set_context(
            request_id=self.request_id, session_id=self.session_id, user_id=self.user_id
        )
        return self.logger
