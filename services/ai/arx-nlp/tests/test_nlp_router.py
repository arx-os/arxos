...
class TestNLPRouter:
    ...
    def setup_method(self):
        ...
        self.context = NLPContext(
            user_id="test_user", building_id="test_building", floor_id="floor_1"
        )
    ...
    def test_slot_extraction(self):
        ...
        object_slots = [
            slot for slot in result.slots if slot.slot_type == SlotType.OBJECT_TYPE
        ]
        assert len(object_slots) > 0

        # Check that property slot is extracted
        property_slots = [
            slot for slot in result.slots if slot.slot_type == SlotType.PROPERTY
        ]
    ...
    def test_batch_processing(self):
        ...
        texts = ["create a bedroom", "modify the kitchen", "find all doors"]
    ...
    def test_error_handling(self):
        ...
        result = self.router.parse_natural_language(
    ...
    def test_object_mappings(self):
        ...
        kitchen_slot = next(
            (slot for slot in object_slots if "kitchen" in slot.original_value), None
        )
        if kitchen_slot:
    ...
    def test_complex_commands(self):
        ...
        result = self.router.parse_natural_language(
            "create a red bedroom with size 20x15 on floor 2"
        )
...
class TestIntentMapper:
    ...
    def test_validation(self):
        ...
        intent = Intent(
            intent_type=IntentType.CREATE, confidence=0.8, raw_text="create a room"
        )
...
class TestSlotFiller:
    ...
    def test_slot_extraction(self):
        """Test slot extraction"""
        slot_result = self.filler.extract_slots(
            "create a red bedroom", IntentType.CREATE
        )
    ...
    def test_validation(self):
        ...
        slot = Slot(slot_type=SlotType.OBJECT_TYPE, value="room", confidence=0.8)
...
class TestCLITranslator:
    ...
    def test_command_generation(self):
        ...
        slot_result = SlotResult(
            slots=[Slot(slot_type=SlotType.OBJECT_TYPE, value="room", confidence=0.9)],
            confidence=0.9,
        )
    ...
    def test_validation(self):
        """Test command validation"""
        cli_command = CLICommand(command="arx", subcommand="create", arguments=["room"])
