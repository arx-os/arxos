...
use esp_idf_svc::bt::{BleGatts, BleAdvertising};
...
impl BLEBridge {
    fn new() -> Self {
        ...
        let gatts = BleGatts::new().unwrap();
        let advertising = BleAdvertising::new().unwrap();
        
        // Start advertising
        advertising.start().unwrap();
        
        Self { gatts, advertising }
    }
    
    fn bridge_task(&mut self) {
        loop {
            // Bridge BLE ↔ LoRa
            self.handle_ble_to_lora();
            self.handle_lora_to_ble();
            task::delay_ms(10);
        }
    }
}
...
| Local CI/CD | $0 | Self-hosted infrastructure |
