...
class TestCoverageReporting:
                ...
                if report_result.returncode == 0:
                    ...
                    for file_path, file_data in coverage_data.get("files", {}).items():
                        if "arx_backend" in file_path:
                            ...
                            covered_lines += file_data.get("summary", {}).get(
                                "covered_lines", 0
                            )
                    ...
                    if result.returncode == 0:
                        ...
                        if coverage_file.exists():
                            ...
                            if total_statements > 0:
                                ...
                                # Check against threshold
                                threshold = coverage_config["language_thresholds"][
                                    "typescript"
                                ]
                                assert (
                                    coverage_percentage >= threshold
                                ...
                                html_report = (
                                    sdk_path / "coverage" / "lcov-report" / "index.html"
                                )
        ...
        try:
            ...
            if pom_xml.exists():
                ...
                if result.returncode == 0:
                    ...
                    jacoco_report = (
                        sdk_path / "target" / "site" / "jacoco" / "index.html"
                    )
                    if jacoco_report.exists():
                        ...
                        xml_report = (
                            sdk_path / "target" / "site" / "jacoco" / "jacoco.xml"
                        )
    ...
    def test_overall_coverage_summary(self, coverage_config):
        ...
        for language, sdk_path in sdk_languages:
            ...
            if path.exists():
                ...
                coverage_summary[language] = {
                    ...
                    "threshold": coverage_config["language_thresholds"].get(
                        language, 80
                    ),
                }
            else:
        ...
        existing_sdks = [
            lang for lang, data in coverage_summary.items() if data["exists"]
        ]
        total_threshold = sum(
            coverage_summary[lang]["threshold"] for lang in existing_sdks
        )
    ...
    def test_coverage_quality_gates(self, coverage_config):
        ...
        ), f"Too many quality gates failing: {passed_gates}/{total_gates}"
