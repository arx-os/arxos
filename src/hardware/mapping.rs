...
impl MappingManager {
    ...
    pub fn load(&mut self) -> HardwareResult<()> {
        if !self.mapping_file.exists() {
            info!(
                "No mapping file found at {:?}, starting with empty mappings",
                self.mapping_file
            );
            return Ok(());
        }
        ...
        info!(
            "Loading sensor-equipment mappings from {:?}",
            self.mapping_file
        );

        let base_dir = self.mapping_file.parent().unwrap_or_else(|| Path::new("."));

        let content = PathSafety::read_file_safely(&self.mapping_file, base_dir).map_err(|e| {
        ...
        let collection: MappingCollection =
            serde_yaml::from_str(&content).map_err(HardwareError::YamlError)?;
    ...
    pub fn save(&self) -> HardwareResult<()> {
        info!(
            "Saving {} sensor-equipment mappings to {:?}",
            self.mappings.len(),
            self.mapping_file
        );
        ...
        let yaml_content = serde_yaml::to_string(&collection).map_err(HardwareError::YamlError)?;
        ...
        std::fs::write(&self.mapping_file, yaml_content).map_err(|e| {
            HardwareError::IoError(std::io::Error::other(format!(
    ...
    pub fn add_mapping(&mut self, mapping: EquipmentSensorMapping) {
        info!(
            "Adding/updating mapping: sensor {} -> equipment {}",
            mapping.sensor_id, mapping.equipment_id
        );
        self.mappings.insert(mapping.sensor_id.clone(), mapping);
    }
