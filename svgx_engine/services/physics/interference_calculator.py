...
class InterferenceCalculator:
    ...
    def analyze_interference(
        ...
        try:
            ...
            sir = self._calculate_signal_to_interference_ratio(
                request, interference_level
            )

            # Calculate carrier-to-interference ratio
            cir = self._calculate_carrier_to_interference_ratio(
                request, interference_level
            )
    ...
    def _calculate_interference_level(
        ...
        # Add interference from sources
        for source in request.interference_sources:
            source_interference = self._calculate_source_interference(
                request.desired_signal, source
            )
            total_interference += source_interference

        return total_interference

    def _calculate_source_interference(
        self,
        desired_signal: InterferenceSource,
        ...
        distance = self._calculate_distance(
            desired_signal.position, interference_source.position
        )
        ...
        if freq_separation == 0:
            ...
            interference = self._calculate_co_channel_interference(
                desired_signal, interference_source, distance
            )
        elif freq_separation <= desired_signal.bandwidth:
            # Adjacent channel interference
            interference = self._calculate_adjacent_channel_interference(
                desired_signal, interference_source, distance
            )
        else:
            # Out-of-band interference (reduced)
            interference = self._calculate_out_of_band_interference(
                desired_signal, interference_source, distance, freq_separation
            )
    ...
    def _calculate_co_channel_interference(
        self,
        desired_signal: InterferenceSource,
        interference_source: InterferenceSource,
        distance: float,
        ...
        received_interference = (
            interference_power_dbm + interference_source.antenna_gain - path_loss
        )

        return received_interference

    def _calculate_adjacent_channel_interference(
        self,
        desired_signal: InterferenceSource,
        interference_source: InterferenceSource,
        distance: float,
        ...
        co_channel_interference = self._calculate_co_channel_interference(
            desired_signal, interference_source, distance
        )

        # Apply adjacent channel rejection
        channel_spacing = abs(desired_signal.frequency - interference_source.frequency)
        rejection_factor = self._calculate_adjacent_channel_rejection(
            channel_spacing, desired_signal.bandwidth
        )
    ...
    def _calculate_adjacent_channel_rejection(
        self, channel_spacing: float, bandwidth: float
    ) -> float:
    ...
    def _calculate_out_of_band_interference(
        self,
        desired_signal: InterferenceSource,
        interference_source: InterferenceSource,
        distance: float,
        ...
        rejection_factor = self._calculate_out_of_band_rejection(
            freq_separation, desired_signal.bandwidth
        )
    ...
    def _calculate_out_of_band_rejection(
        self, freq_separation: float, bandwidth: float
    ) -> float:
    ...
    def _calculate_signal_to_interference_ratio(
        self, request: InterferenceAnalysisRequest, interference_level: float
    ) -> float:
        """Calculate signal-to-interference ratio."""
    ...
    def _calculate_carrier_to_interference_ratio(
        self, request: InterferenceAnalysisRequest, interference_level: float
    ) -> float:
        """Calculate carrier-to-interference ratio."""
        # Similar to SIR but with different reference levels
        cir = (
            self._calculate_signal_to_interference_ratio(request, interference_level)
            - 3.0
        )

        return cir

    def _determine_interference_type(
        self, request: InterferenceAnalysisRequest
    ) -> InterferenceType:
        ...
        for source in request.interference_sources:
            if (
                abs(source.frequency - request.desired_signal.frequency)
                < 0.1 * request.desired_signal.bandwidth
            ):
        ...
        for source in request.interference_sources:
            if (
                abs(source.frequency - request.desired_signal.frequency)
                <= request.desired_signal.bandwidth
            ):
        ...
        for source in request.interference_sources:
            for harmonic in range(2, 6):  # Check 2nd to 5th harmonics
                ...
                if (
                    abs(harmonic_freq - request.desired_signal.frequency)
                    < 0.1 * request.desired_signal.bandwidth
                ):
        ...
        if len(request.interference_sources) >= 2:
            ...
            if (
                abs(imd_freq - request.desired_signal.frequency)
                < 0.1 * request.desired_signal.bandwidth
            ):
    ...
    def _determine_severity_level(
        self, sir: float, cir: float, interference_level: float
    ...
    def _calculate_interference_spectrum(
        self, request: InterferenceAnalysisRequest
        ...
        spectrum.append(
            (request.desired_signal.frequency, request.environment.noise_floor)
        )

        # Add interference sources
        for source in request.interference_sources:
            # Calculate interference at desired frequency
            interference_level = self._calculate_source_interference(
                request.desired_signal, source
            )
            ...
            for harmonic in range(2, 4):  # 2nd and 3rd harmonics
                harmonic_freq = source.frequency * harmonic
                harmonic_power = 10 * math.log10(source.power * 1000) - 20 * math.log10(
                    harmonic
                )
    ...
    def predict_interference(
        self, request: InterferenceAnalysisRequest, time_horizon: float
    ) -> Dict[str, Any]:
    ...
    def calculate_interference_margin(
        self, request: InterferenceAnalysisRequest
    ) -> float:
