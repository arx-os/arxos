...
use arxos::hardware::{EquipmentStatusUpdater, SensorData, SensorDataValues, SensorMetadata};
...
fn test_yaml_to_ar_export_workflow() {
    ...
    // Export to glTF
    ar_exporter
        .export(ARFormat::GLTF, &gltf_path)
    ...
    let json: serde_json::Value =
        serde_json::from_str(&content).expect("glTF should be valid JSON");
    ...
    assert!(
        node_count > 0,
        "glTF should have at least one node for equipment"
    );
}
...
fn test_yaml_to_ifc_export_workflow() {
    ...
    assert!(
        ...
        "Should contain IFC header"
    );
    assert!(
        content.contains("IFCROOM") || content.contains("IFCSPACE"),
        "Should contain room entities"
    );
    assert!(
        content.contains("IFCAIRTERMINAL") || content.contains("IFCDISTRIBUTIONELEMENT"),
        "Should contain HVAC equipment"
    );
}
...
fn test_sensor_ingestion_to_equipment_update_workflow() {
    ...
    let mut status_updater =
        EquipmentStatusUpdater::new(&building_name).expect("Should create status updater");
        ...
        data: SensorDataValues {
            values: {
                ...
                map.insert(
                    ...
                    serde_yaml::Value::Number(serde_yaml::Number::from(95)),
                );
                map.insert(
                    "unit".to_string(),
    ...
    // Process sensor data
    let update_result = status_updater
    ...
    assert_ne!(
        update_result.old_status, update_result.new_status,
    ...
    assert!(
        ...
        "Should generate alerts for out-of-range temperature"
    );
    assert!(
        update_result
            .alerts
...
fn test_complete_workflow_yaml_ifc_ar_hardware() {
    ...
    fs::write(
        &building_yaml_path,
        serde_yaml::to_string(&workflow_building).unwrap(),
    )
    .unwrap();
    ...
    let critical_sensor_data = SensorData {
        ...
        data: SensorDataValues {
            values: {
                ...
                map.insert(
                    "temperature".to_string(),
                    serde_yaml::Value::Number(serde_yaml::Number::from(110)),
                );
                map
            },
        },
    ...
    let update_result = status_updater
        .process_sensor_data(&critical_sensor_data)
...
fn test_delta_export_with_sensor_updates() {
    ...
    delta_exporter
        .export_delta(Some(&loaded_sync_state), &delta_ifc_path)
