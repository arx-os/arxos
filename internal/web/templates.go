...
func NewTemplates() (*Templates, error) {
	...
	pages := []string{"index", "login", "buildings", "building", "equipment", "settings", "upload", "floor-viewer"}
	...
	// Parse fragment templates (for HTMX partial updates)
	fragments := []string{"building-row", "equipment-card", "notification", "form-errors", "upload-result"}
	for _, fragment := range fragments {
		tmpl, err := template.New(fragment).Funcs(templateFuncs()).ParseFS(templateFS, "templates/fragments/"+fragment+".html")
		if err != nil {
			logger.Warn("Fragment %s not found, skipping", fragment)
			continue
		}
		t.fragments[fragment] = tmpl
	}
