...
func (uc *IssueUseCase) CreateIssue(
	ctx context.Context,
	req versioncontrol.CreateIssueRequest,
	...
	if req.IssueType == versioncontrol.IssueTypeSafety || req.IssueType == versioncontrol.IssueTypeEmergency {
		if priority == versioncontrol.IssuePriorityNormal || priority == versioncontrol.IssuePriorityLow {
			priority = versioncontrol.IssuePriorityUrgent
...
func (uc *IssueUseCase) StartWork(
	...
	issueID types.ID,
	workerID types.ID,
) (*versioncontrol.Branch, *versioncontrol.PullRequest, error) {
	...
	branch, err := uc.branchUC.CreateBranch(ctx, versioncontrol.CreateBranchRequest{
		RepositoryID: issue.RepositoryID,
		Name:         branchName,
		DisplayName:  fmt.Sprintf("Issue #%d: %s", issue.Number, issue.Title),
		Description:  issue.Body,
		BranchType:   versioncontrol.BranchTypeIssue,
		CreatedBy:    &workerID,
	})
	if err != nil {
	...
	// Create PR for issue
	pr, err := uc.prUC.CreatePullRequest(ctx, versioncontrol.CreatePRRequest{
		RepositoryID:   issue.RepositoryID,
		...
		PRType:         versioncontrol.PRTypeIssueFix,
		Priority:       versioncontrol.PRPriority(issue.Priority),
		CreatedBy:      workerID,
	})
	if err != nil {
	...
	issue.PRID = &pr.ID
	issue.Status = versioncontrol.IssueStatusInProgress
...
func (uc *IssueUseCase) ResolveIssue(
	ctx context.Context,
	req versioncontrol.ResolveIssueRequest,
) error {
...
func (uc *IssueUseCase) VerifyIssue(
	ctx context.Context,
	req versioncontrol.VerifyIssueRequest,
) error {
