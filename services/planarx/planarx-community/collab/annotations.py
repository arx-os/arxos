...
class AnnotationManager:
    ...
    def create_thread(
        ...
        self.logger.info(
            f"Created comment thread {thread_id} for object {arx_object_id}"
        )
        return thread
    ...
    def update_comment(self, comment_id: str, user_id: str, new_content: str) -> bool:
    ...
    def resolve_thread(
        self, thread_id: str, resolved_by: str, resolution_note: str = ""
    ) -> bool:
        ...
        for thread in self.threads.values():
            for comment in thread.comments:
                if comment.id == comment_id:
                    ...
                    self.logger.info(
                        f"Added reaction {reaction_type} to comment {comment_id}"
                    )
                    return True
    ...
    def remove_reaction(
        self, comment_id: str, user_id: str, reaction_type: str
    ) -> bool:
        ...
        for thread in self.threads.values():
            for comment in thread.comments:
                if comment.id == comment_id:
                    if (
                        reaction_type in comment.reactions
                        and user_id in comment.reactions[reaction_type]
                    ):
                    ...
                    self.logger.info(
                        f"Removed reaction {reaction_type} from comment {comment_id}"
                    )
                    return True
    ...
    def unsubscribe_from_thread(self, thread_id: str, user_id: str) -> bool:
        ...
        if (
            user_id in self.user_subscriptions
            and thread_id in self.user_subscriptions[user_id]
        ):
    ...
    def get_threads_by_draft(self, draft_id: str) -> List[CommentThread]:
        ...
        return [
            thread for thread in self.threads.values() if thread.draft_id == draft_id
        ]
    ...
    def get_user_threads(self, user_id: str) -> List[CommentThread]:
        ...
        for thread in self.threads.values():
            if (
                user_id in thread.subscribers
                or thread.assigned_to == user_id
                or thread.created_by == user_id
            ):
    ...
    def get_threads_by_status(self, status: CommentStatus) -> List[CommentThread]:
        ...
        return [thread for thread in self.threads.values() if thread.status == status]

    def search_threads(
        self, query: str, draft_id: Optional[str] = None
    ) -> List[CommentThread]:
        ...
        for thread in self.threads.values():
            ...
            if (
                ...
                or query_lower in thread.description.lower()
            ):
