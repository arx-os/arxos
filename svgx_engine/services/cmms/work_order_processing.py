...
class WorkOrderProcessingService:
    ...
    def create_work_order_from_template(self,
                                      template_id: str,
                                      title: str,
                                      asset_id: Optional[str] = None,
                                      location: Optional[str] = None,
        ...
        # Create steps from template import template
    ...
    def add_work_order_part(self,
        ...
        logger.info(f"Added part to work order {work_order.work_order_number}: {part_number}")
        return part_id
    ...
    def update_work_order_status(self, work_order_id: str, status: WorkOrderStatus,
        ...
        logger.info(f"Updated work order {work_order.work_order_number} status: {old_status.value} -> {status.value}")
    ...
    def assign_work_order(self, work_order_id: str, assigned_to: str) -> None:
        ...
        logger.info(f"Assigned work order {work_order.work_order_number} to {assigned_to}")

    def complete_work_order_step(self, step_id: str, completed_by: str,
                               actual_duration: float = 0.0, notes: Optional[str] = None) -> None:
        """
    ...
    def get_work_orders(self, status: Optional[WorkOrderStatus] = None,
                       assigned_to: Optional[str] = None,
                       asset_id: Optional[str] = None) -> List[WorkOrder]:
        """
    ...
    def get_overdue_work_orders(self) -> List[WorkOrder]:
        ...
        for work_order in self.work_orders.values():
            if (work_order.due_date and
                work_order.due_date < now and
                work_order.status not in [WorkOrderStatus.COMPLETED, WorkOrderStatus.CANCELLED, WorkOrderStatus.CLOSED]):
    ...
    def get_templates(self, work_order_type: Optional[WorkOrderType] = None,
                     asset_type: Optional[str] = None) -> List[WorkOrderTemplate]:
        """
    ...
    def _generate_work_order_number(self) -> str:
        ...
        count = len([wo for wo in self.work_orders.values()
                    if wo.created_at.date() == datetime.now().date()]) + 1
    ...
    def _calculate_actual_hours_and_cost(self, work_order_id: str) -> None:
        ...
        # Calculate actual hours from steps import steps
        steps = self.work_order_steps.get(work_order_id, [])
        actual_hours = sum(step.actual_duration for step in steps if step.status == "completed")
        work_order.actual_hours = actual_hours

        # Calculate actual cost from parts import parts
    ...
    def _update_statistics_for_status_change(self, old_status: WorkOrderStatus, new_status: WorkOrderStatus) -> None:
    ...
    def get_work_order_statistics(self) -> Dict[str, Any]:
        ...
        completed_work_orders = [wo for wo in self.work_orders.values()
                               if wo.status == WorkOrderStatus.COMPLETED and wo.completed_date]

        if completed_work_orders:
            total_completion_time = sum(
                (wo.completed_date - wo.created_at).total_seconds() / 3600  # Convert to hours
                for wo in completed_work_orders
            )
            self.statistics["average_completion_time"] = total_completion_time / len(completed_work_orders)
