...
class TestConflictDetector:
    ...
    def test_conflict_types(self):
        ...
        op1 = Operation(
            "op1", OperationType.CREATE, "user1", "session1", time.time(), "element1"
        )
        op2 = Operation(
            "op2",
        ...
        op3 = Operation(
            "op3", OperationType.DELETE, "user1", "session1", time.time(), "element1"
        )
        op4 = Operation(
            "op4",
...
class TestPerformanceTargets:
    ...
    async def test_conflict_detection_time(self):
        ...
        # Create conflicting operations
        op1 = Operation(
            "op1", OperationType.UPDATE, "user1", "session1", time.time(), "element1"
        )
        op2 = Operation(
            "op2",
            OperationType.UPDATE,
...
class TestErrorHandling:
    ...
    async def test_invalid_operation(self):
        ...
        try:
            ...
            operation_data = {"type": "invalid_type", "user_id": "user1"}
...
class TestScalability:
    ...
    def test_operation_batching(self):
        ...
        batch_size = 10
        batches = [
            collaboration.operation_queue[i : i + batch_size]
...
class TestIntegration:
    ...
    async def test_full_collaboration_workflow(self):
        ...
        try:
            ...
            user = collaboration.presence_manager.add_user(
                "user1", "John Doe", "session1"
            )
            assert user.username == "John Doe"
            ...
            if conflicts:
                ...
                success = await resolve_conflict(
                    conflict_id, "last_write_wins", "user1"
                )
                assert success
