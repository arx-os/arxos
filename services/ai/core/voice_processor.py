...
class VoiceProcessor:
    ...
    async def _bytes_to_audio(self, audio_bytes: bytes) -> sr.AudioData:
        ...
        try:
            ...
            audio_data = sr.AudioData(
                y_int16.tobytes(), sample_rate=sr_rate, sample_width=2
            )
    ...
    async def _transcribe_audio(
        ...
        try:
            ...
            text = await asyncio.to_thread(
                self.recognizer.recognize_google, audio, language=lang_code
            )
        ...
        try:
            if task == "transcribe":
                return await self.process_audio(
                    parameters.get("audio_data", ""), parameters.get("language", "en")
                )
            elif task == "analyze":
                audio_bytes = base64.b64decode(parameters.get("audio_data", ""))
                return {"analysis": await self._analyze_audio(audio_bytes)}
    ...
    async def _detect_language(self, audio_data: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"detected_language": "unknown", "text": "", "error": str(e)}
