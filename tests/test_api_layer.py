...
class TestAPILayer:
    ...
    def auth_headers(self, mock_user):
        ...
        return {"X-API-Key": mock_user["api_key"], "Content-Type": "application/json"}
    ...
    def test_project_endpoints_with_auth(self, client, auth_headers):
        ...
        response = client.get(
            f"/api/v1/projects/{project_id}/statistics", headers=auth_headers
        )
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert "success" in data
    ...
    def test_building_endpoints_with_auth(self, client, auth_headers):
        ...
        response = client.post(
            "/api/v1/buildings/", json=building_data, headers=auth_headers
        )
        assert response.status_code == status.HTTP_201_CREATED
        data = response.json()
        assert "success" in data
        ...
        response = client.get(
            f"/api/v1/buildings/{building_id}/rooms", headers=auth_headers
        )
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert "success" in data
        ...
        response = client.get(
            f"/api/v1/buildings/{building_id}/statistics", headers=auth_headers
        )
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert "success" in data
    ...
    def test_pagination_and_filtering(self, client, auth_headers):
        ...
        response = client.get(
            "/api/v1/devices/?page=1&page_size=5", headers=auth_headers
        )
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        ...
        # Test filtering
        response = client.get(
            "/api/v1/devices/?device_type=sensor", headers=auth_headers
        )
        assert response.status_code == status.HTTP_200_OK

        # Test building list with filtering
        response = client.get(
            "/api/v1/buildings/?building_type=commercial", headers=auth_headers
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_error_handling(self, client, auth_headers):
        ...
        response = client.post(
            "/api/v1/devices/", json=invalid_device_data, headers=auth_headers
        )
    ...
    def test_request_logging(self, client, caplog):
        ...
        with caplog.at_level("INFO"):
            ...
            log_records = [
                ...
                if "request" in record.message.lower()
            ]
...
class TestAPIDependencies:
    ...
    def test_user_model(self):
        ...
        user = User(
            user_id="test-123", api_key="test-key", permissions=["read", "write"]
        )
    ...
    def test_permission_requirements(self, client):
        ...
        response = client.post(
            "/api/v1/devices/", json=device_data, headers={"X-API-Key": "test-api-key"}
        )
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_response_formatting_functions(self):
        ...
        success_response = format_success_response(
            data={"test": "data"}, message="Success message"
        )
        ...
        error_response = format_error_response(
            error_code="TEST_ERROR", message="Error message", details={"detail": "test"}
        )
