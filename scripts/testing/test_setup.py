...
def test_service_health(service_name, config):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_status(f"{service_name} health check failed (status: {response.status_code})", "ERROR")
            return False
...
def test_user_registration():
    ...
    try:
        ...
        if response.status_code in [200, 201, 409]:  # 409 means user already exists
            ...
            print_status(f"User registration failed (status: {response.status_code})", "ERROR")
            return False
    except requests.exceptions.RequestException as e:
  ...
  <circle cx="200" cy="175" r="20" fill="red"/>
    ...
    try:
        ...
        if upload_response.status_code in [200, 201]:
            ...
            print_status(f"SVG upload failed (status: {upload_response.status_code})", "WARNING")
            return False
    except requests.exceptions.RequestException as e:
  ...
  <element id="test-rect" type="rectangle" x="100" y="100" width="200" height="150">
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_status(f"SVGX Engine parse failed (status: {response.status_code})", "WARNING")
            return False
    except requests.exceptions.RequestException as e:
...
def main():
    ...
    if passed_tests >= total_tests * 0.8:  # 80% success rate
        ...
        print_status("Some components are not working correctly. Please check the setup.")
        return False
