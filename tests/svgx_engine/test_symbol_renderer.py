...
class TestSymbolRendererService:
    ...
    def service(self, temp_dir):
        ...
        with patch(
            "services.symbol_renderer.SVGXSymbolRendererService._init_databases"
        ) as mock_init:
...
<svgx xmlns="http://www.svgx.org/schema/1.0">
    ...
    def test_render_symbol_svg(self, service, sample_svgx_content):
        ...
        result = service.render_symbol(
            symbol_id=symbol_id, content=sample_svgx_content, options=options
        )
    ...
    def test_render_animation(self, service, sample_svgx_content):
        ...
        frames = service.render_animation(
            symbol_id=symbol_id, content=sample_svgx_content, duration=duration, fps=fps
        )
    ...
    def test_render_interactive(self, service, sample_svgx_content):
        ...
        assert all(
            isinstance(elem, InteractiveElement) for elem in interactive_elements
        )
    ...
    def test_concurrent_rendering(self, service, sample_svgx_content):
        ...
        for i in range(5):
            thread = threading.Thread(
                target=render_symbol, args=(f"concurrent_symbol_{i}",)
            )
    ...
    def test_cleanup(self, service):
        ...
        # Add some test data
        service.render_symbol(
            symbol_id="test_cleanup", content="<svgx></svgx>", options=RenderOptions()
        )
