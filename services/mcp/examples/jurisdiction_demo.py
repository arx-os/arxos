...
def create_building_with_location(
    building_id: str, building_name: str, location_data: dict
) -> BuildingModel:
    ...
    objects = [
        BuildingObject(
            ...
            properties={"voltage": 120, "amperage": 200, "circuit_count": 20},
        ),
        BuildingObject(
            object_id="outlet_bathroom_1",
            object_type="electrical_outlet",
            properties={"voltage": 120, "amperage": 15, "location": "bathroom"},
        ),
        BuildingObject(
            object_id="outlet_kitchen_1",
            object_type="electrical_outlet",
            properties={"voltage": 120, "amperage": 20, "location": "kitchen"},
        ),
    ]
...
def demonstrate_jurisdiction_matching():
    ...
    test_buildings = [
        ...
        },
        {
            ...
            "location": {"country": "EU", "state": "Germany", "city": "Berlin"},
        },
        {
            "id": "building_ca_int",
            "name": "Canadian Residential Building",
            "location": {"country": "CA", "state": "Ontario", "city": "Toronto"},
        },
        {
            "id": "building_au",
            "name": "Australian Commercial Building",
            "location": {"country": "AU", "state": "New South Wales", "city": "Sydney"},
        },
        {
    ...
    for building_info in test_buildings:
        ...
        building_model = create_building_with_location(
            building_info["id"], building_info["name"], building_info["location"] or {}
        )
        ...
        if "error" in jurisdiction_info:
            ...
            print(
                f"   🎯 Jurisdiction Level: {jurisdiction_info['jurisdiction_level']}"
            )
            print(
                f"   📋 Applicable Codes: {len(jurisdiction_info['applicable_codes'])}"
            )
            ...
            if jurisdiction_info.get("matches"):
                ...
                for match in jurisdiction_info["matches"]:
                    print(
                        f"      • {match['name']} ({match['match_level']}) - {match['reasoning']}"
                    )
