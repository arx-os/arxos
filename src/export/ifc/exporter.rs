...
use super::{delta::calculate_delta, sync_state::IFCSyncState};
...
impl IFCExporter {
    ...
    pub fn export(&self, output: &Path) -> ArxResult<()> {
        ...
        info!(
            "Converted {} spatial entities for IFC export",
        ...
        let output_str = output.to_str().ok_or_else(|| {
            crate::error::ArxError::io_error("Invalid output path encoding")
    ...
    pub fn export_delta(&self, sync_state: Option<&IFCSyncState>, output: &Path) -> ArxResult<()> {
        ...
        info!(
            "Delta: {} new, {} updated, {} deleted equipment; {} new, {} updated, {} deleted rooms",
            ...
            for wing in &floor.wings {
                for room in &wing.rooms {
                    ...
                    if delta.new_rooms.iter().any(|r| {
                        r.properties.get("universal_path").unwrap_or(&r.id) == &room_path
                            || r.id == room.id
        ...
        info!(
            "Converted {} delta spatial entities for IFC export",
            spatial_entities.len()
        );
...
mod tests {
    ...
    fn create_test_building_data() -> BuildingData {
        BuildingData {
            ...
            floors: vec![{
                use crate::core::{
                    BoundingBox, Dimensions, Equipment, EquipmentStatus, EquipmentType, Floor,
