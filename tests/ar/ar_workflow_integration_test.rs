...
fn test_ar_workflow_complete() {
    ...
    let ar_scan = ARScanData {
        detected_equipment: vec![
            ...
            DetectedEquipmentData {
                ...
                position: Point3D { x: 12.0, y: 22.0, z: 3.0 },
                confidence: 0.88,
                detection_method: Some("ARKit".to_string()),
            },
        ],
    ...
    let storage_file = temp_dir.path().join("pending-equipment-test_building.json");
    ...
    let detected_info = DetectedEquipmentInfo {
        ...
        bounding_box: BoundingBox3D {
            min: Point3D { x: 9.5, y: 19.5, z: 2.5 },
            max: Point3D { x: 10.5, y: 20.5, z: 3.5 },
        },
        confidence: 0.95,
    ...
    manager2.load_from_storage(&storage_file)
    ...
    let equipment_id = manager2.confirm_pending(&pending_id, &mut building_data_mut)
    ...
    let floor = building_data_mut.floors.iter().find(|f| f.level == 3).unwrap();
    let equipment = floor.equipment.iter().find(|e| e.name == "VAV-301").unwrap();
    ...
    assert!(confirmed_pending.is_empty(), "Confirmed items should not appear in pending list");
}
...
fn test_ar_workflow_rejection() {
    ...
    let pending_list = manager.list_pending();
    assert!(pending_list.is_empty(), "Rejected item should not appear in pending list");
}
