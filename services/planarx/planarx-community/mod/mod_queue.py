...
class ModerationQueue:
    ...
    def _sort_tasks(self, tasks: List[ModerationTask], sort_by: QueueSortOption) -> List[ModerationTask]:
        ...
        if sort_by == QueueSortOption.PRIORITY_TIME:
            ...
            return sorted(tasks, key=lambda t: (priority_order.get(t.priority, 4), t.created_at))
    ...
    def _flag_to_task(self, flag) -> ModerationTask:
        ...
        overdue_hours = max(0, (time_since_creation.total_seconds() / 3600) - response_target)
    ...
    def get_queue_statistics(self) -> Dict:
        ...
        avg_response_time = sum(f.response_time_hours for f in resolved_flags) / len(resolved_flags) if resolved_flags else 0
    ...
    def get_moderator_workload(self, moderator_id: str) -> ModeratorWorkload:
        ...
        pending_review = len([f for f in assigned_flags 
        ...
        # Get last activity
        last_activity = max((f.updated_at for f in assigned_flags), default=datetime.utcnow())
    ...
    def auto_assign_flags(self, available_moderators: List[str]):
        ...
        for flag in unassigned_flags:
            ...
            least_loaded_moderator = min(available_moderators, key=lambda m: moderator_loads.get(m, 0))
            ...
            # Update load count
            moderator_loads[least_loaded_moderator] = moderator_loads.get(least_loaded_moderator, 0) + 1
            
            self.logger.info(f"Auto-assigned flag {flag.id} to moderator {least_loaded_moderator}")
    ...
    def _check_overdue_alerts(self):
        ...
        overdue_flags = [f for f in flagging_system.flags.values() 
                        if self._is_flag_overdue(f) and f.status.value in ["pending", "under_review"]]
    ...
    def _check_critical_alerts(self):
        ...
        critical_flags = [f for f in flagging_system.flags.values() 
                         if f.priority.value == "critical" and f.status.value in ["pending", "under_review"]]
    ...
    def get_active_alerts(self, alert_type: str = None) -> List[QueueAlert]:
        ...
        alerts = [alert for alert in self.queue_alerts.values() if not alert.acknowledged]
    ...
    def get_queue_dashboard_data(self) -> Dict:
        ...
        # Get recent flags
        recent_flags = sorted(flagging_system.flags.values(), 
                            key=lambda f: f.created_at, reverse=True)[:10]
        
        # Get overdue flags
        overdue_flags = [f for f in flagging_system.flags.values() if self._is_flag_overdue(f)]
        ...
        return {
            ...
            "unassigned_flags_count": len([f for f in flagging_system.flags.values() 
