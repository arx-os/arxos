...
class PrecisionValidator:
    ...
    def __init__(
        self,
        precision_math: Optional[PrecisionMath] = None,
    ...
    def validate_coordinate(
        self, coordinate: PrecisionCoordinate, precision: Optional[Decimal] = None
        ...
        for rule_name, rule in self.validation_rules.items():
            if rule.validation_type == ValidationType.COORDINATE:
                ...
                if (
                    not result.is_valid
                    and rule.validation_level == ValidationLevel.CRITICAL
                ):
                    self.logger.error(f"Critical validation failed: {result.message}")

        return results

    def validate_geometric_operation(
        self,
        operation_name: str,
        operation_result: Union[float, Decimal],
        expected_precision: Optional[Decimal] = None,
    ) -> List[ValidationResult]:
        """
        ...
        for rule_name, rule in self.validation_rules.items():
            if rule.validation_type == ValidationType.GEOMETRIC:
                result = rule.execute(
                    operation_name, operation_result, expected_precision
                )
                results.append(result)
                self.validation_history.append(result)
                ...
                if (
                    ...
                    self.logger.error(
                        f"Critical geometric validation failed: {result.message}"
                    )
        ...
        for rule_name, rule in self.validation_rules.items():
            if rule.validation_type == ValidationType.CONSTRAINT:
                ...
                if (
                    ...
                    self.logger.error(
                        f"Critical constraint validation failed: {result.message}"
                    )
    ...
    def validate_batch(
        ...
        for data in validation_data:
            ...
            if validation_type == "coordinate":
                ...
                results = self.validate_geometric_operation(
                    operation_name, operation_result, expected_precision
                )
                ...
                precision = data.get("precision")
                results = self.validate_constraint(
                    constraint_name, constraint_data, precision
                )
            else:
    ...
    def _validate_coordinate_range(
        ...
        return (
            abs(coordinate.x) <= max_coordinate
            and abs(coordinate.y) <= max_coordinate
            and abs(coordinate.z) <= max_coordinate
        )
    ...
    def _validate_coordinate_precision(
        ...
        return (
            self.precision_math.validate_precision(coordinate.x, precision)
            and self.precision_math.validate_precision(coordinate.y, precision)
            and self.precision_math.validate_precision(coordinate.z, precision)
        )

    def _validate_coordinate_nan(
        self, coordinate: PrecisionCoordinate, precision: Optional[Decimal] = None
    ) -> bool:
        ...
        return (
            not math.isnan(coordinate.x)
            and not math.isnan(coordinate.y)
            and not math.isnan(coordinate.z)
            and not math.isinf(coordinate.x)
            and not math.isinf(coordinate.y)
            and not math.isinf(coordinate.z)
        )

    # Geometric validation rule implementations
    def _validate_distance_precision(
        self,
        operation_name: str,
        operation_result: Union[float, Decimal],
        expected_precision: Optional[Decimal] = None,
        ...
        return self.precision_math.validate_precision(
            operation_result, expected_precision
        )

    def _validate_angle_precision(
        self,
        operation_name: str,
        operation_result: Union[float, Decimal],
        expected_precision: Optional[Decimal] = None,
    ...
    def _validate_constraint_precision(
        self,
        constraint_name: str,
        constraint_data: Dict[str, Any],
        precision: Optional[Decimal] = None,
    ...
    def _validate_calculation_performance(
        self,
        operation_name: str,
        operation_result: Union[float, Decimal],
        expected_precision: Optional[Decimal] = None,
    ...
    def get_validation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_level": by_level,
            "recent_validations": [
                result.to_dict() for result in self.validation_history[-10:]
            ],
        }
    ...
    def get_critical_failures(self) -> List[ValidationResult]:
        ...
        return [
            ...
            and result.validation_level == ValidationLevel.CRITICAL
        ]
...
class PrecisionTestingFramework:
    ...
    def run_geometric_tests(self) -> List[Dict[str, Any]]:
        ...
        results = self.validator.validate_geometric_operation(
            "angle_calculation", angle_result
        )
        test_results.append(
            {
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        passed_tests = sum(
            sum(1 for test in suite if test["passed"]) for suite in test_suites.values()
        )
        ...
        self.logger.info(
            f"Precision validation tests completed: {passed_tests}/{total_tests} passed"
        )
        return test_summary
