...
class RoomApplicationService:
    ...
    def update_room(
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    "Updating room", room_id=room_id, updated_by=updated_by
                )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to update room", room_id=room_id, error=str(e)
                )
        ...
        try:
            ...
            if self.cache_service:
                ...
                if cached_room:
                    ...
                    return GetRoomResponse(success=True, room=cached_room)
            ...
            # Log error
            if self.logger:
                self.logger.error("Failed to get room", room_id=room_id, error=str(e))
    ...
    def delete_room(self, room_id: str) -> DeleteRoomResponse:
        ...
        try:
            # Log the operation
            if self.logger:
                self.logger.info("Deleting room", room_id=room_id)
            ...
            if result.success:
                ...
                if self.event_store:
                    room_deleted_event = RoomDeleted(
                        room_id=room_id,
                        room_number="",  # Would need to get from room
                        deleted_by="system",
                    )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to delete room", room_id=room_id, error=str(e)
                )
    ...
    def get_room_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log error
            if self.logger:
                self.logger.error("Failed to get room statistics", error=str(e))

            # Record error metrics
            if self.metrics:
                self.metrics.increment_counter("room.statistics.error")

            return {"error": f"Failed to get room statistics: {str(e)}"}
