...
class BadgeSystem:
    ...
    def __init__(self):
        ...
        self.badge_progress: Dict[str, Dict[str, float]] = (
            {}
    ...
    def _meets_complex_criteria(self, criteria: Dict, user_stats: Dict) -> bool:
        ...
        if "quality_score" in criteria and "total_contributions" in criteria:
            ...
            return (
                ...
                and total_contributions >= criteria["total_contributions"]
            )

        return True

    def award_badge(
        self, user_id: str, badge_id: str, metadata: Dict = None
    ) -> Optional[UserBadge]:
    ...
    def get_user_badge_summary(self, user_id: str) -> Dict:
        ...
        return {
            ...
            "recent_badges": [
                ...
                for badge in sorted(
                    user_badges, key=lambda b: b.earned_at, reverse=True
                )[:5]
            ],
            "all_badges": [
                {
    ...
    def check_consecutive_days(
        self, user_id: str, contribution_dates: List[datetime]
    ) -> int:
    ...
    def calculate_quality_score(self, user_id: str, contributions: List) -> float:
        ...
        positive_feedback = sum(
            1 for c in contributions if getattr(c, "positive_feedback", False)
        )
        helpful_comments = sum(1 for c in contributions if getattr(c, "helpful", False))
        resolved_threads = sum(
            1 for c in contributions if getattr(c, "resolved", False)
        )
                ...
                recommendations.append(
                    {
                        ...
                        "progress": progress,
                        "remaining_effort": self._calculate_remaining_effort(
                            badge.criteria, user_stats
                        ),
                    }
                )
