...
// TestConverterRegistry tests the converter registry functionality
func TestConverterRegistry(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.filename, func(t *testing.T) {
			conv, err := registry.GetConverter(tt.filename)

			if tt.shouldFind {
				if err != nil {
					t.Errorf("Expected to find converter for %s, got error: %v", tt.filename, err)
					return
				}
				if conv.GetFormat() != tt.expectedType {
					t.Errorf("Expected format %s, got %s", tt.expectedType, conv.GetFormat())
				}
			} else {
				if err == nil {
					t.Errorf("Expected error for unsupported file %s, but got converter", tt.filename)
				}
			}
		})
	}
