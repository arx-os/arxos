...
class DataPartitioningManager {
    ...
    async loadFloorPartitions(buildingId, floorId, loadStrategy = 'lazy') {
        try {
            ...
            if (result.success) {
                ...
                floorPartitions.partitions.forEach(partition => {
                    if (partition.is_loaded) {
                        this.loadedPartitions.add(partition.partition_id);
                    }
                });
        ...
        try {
            ...
            result.batch_results.forEach(batchResult => {
                if (batchResult.success) {
                    // Update partitions for this floor
    ...
    startPerformanceMonitoring() {
        ...
        setInterval(async () => {
            try {
                ...
                await this.getCompressionStats();
    ...
    getPerformanceChartData() {
        return this.performanceHistory.map(entry => ({
            ...
            total_partitions: entry.stats.total_partitions || 0,
            loaded_partitions: entry.stats.partitions?.loaded || 0,
            cache_hit_rate: entry.stats.cache_stats?.hit_rate || 0,
    ...
    showErrorNotification(message, operation) {
        ...
        notification.className = 'partitioning-error-notification';
...
// Global instance
window.dataPartitioningManager = new DataPartitioningManager();
