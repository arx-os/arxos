...
### 1. Direct SSH to Mesh Nodes

Data consumers connect directly to mesh nodes via SSH for queries and data export:

```bash
# Insurance company analyst connects to local mesh gateway
ssh insurance-user@cbre-downtown-la-node
...
# Download via SCP (still local/mesh connection)
scp insurance-user@cbre-downtown-la-node:/tmp/*.csv ./data/
```
...
ssh data-consumer@arxos-data-gateway-nyc
...
```bash
ssh risk-analyst@local-arxos-gateway
...
```bash
ssh market-analyst@arxos-gateway-miami
...
```bash
ssh city-inspector@municipal-arxos-node
