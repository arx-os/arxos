...
class DatabaseConfig:
    ...
    url: str = field(
        default_factory=lambda: os.getenv(
            "SVGX_DATABASE_URL", "sqlite:///svgx_engine.db"
        )
    )
    pool_size: int = field(
        default_factory=lambda: int(os.getenv("SVGX_DB_POOL_SIZE", "10"))
    )
    max_overflow: int = field(
        default_factory=lambda: int(os.getenv("SVGX_DB_MAX_OVERFLOW", "20"))
    )
    pool_timeout: int = field(
        default_factory=lambda: int(os.getenv("SVGX_DB_POOL_TIMEOUT", "30"))
    )
    pool_recycle: int = field(
        default_factory=lambda: int(os.getenv("SVGX_DB_POOL_RECYCLE", "3600"))
    )
    echo: bool = field(
        default_factory=lambda: os.getenv("SVGX_DB_ECHO", "false").lower() == "true"
    )
...
class RedisConfig:
    ...
    password: Optional[str] = field(
        default_factory=lambda: os.getenv("SVGX_REDIS_PASSWORD")
    )
    max_connections: int = field(
        default_factory=lambda: int(os.getenv("SVGX_REDIS_MAX_CONNECTIONS", "10"))
    )
    socket_timeout: int = field(
        default_factory=lambda: int(os.getenv("SVGX_REDIS_SOCKET_TIMEOUT", "5"))
    )
    socket_connect_timeout: int = field(
        default_factory=lambda: int(os.getenv("SVGX_REDIS_SOCKET_CONNECT_TIMEOUT", "5"))
    )
    retry_on_timeout: bool = field(
        default_factory=lambda: os.getenv("SVGX_REDIS_RETRY_ON_TIMEOUT", "true").lower()
        == "true"
    )
    health_check_interval: int = field(
        default_factory=lambda: int(
            os.getenv("SVGX_REDIS_HEALTH_CHECK_INTERVAL", "300")
        )
    )
    default_ttl: int = field(
        default_factory=lambda: int(os.getenv("SVGX_REDIS_DEFAULT_TTL", "3600"))
    )
    enable_fallback: bool = field(
        default_factory=lambda: os.getenv("SVGX_REDIS_ENABLE_FALLBACK", "true").lower()
        == "true"
    )
...
class LoggingConfig:
    ...
    max_bytes: int = field(
        default_factory=lambda: int(os.getenv("SVGX_LOG_MAX_BYTES", "10485760"))
    )
    backup_count: int = field(
        default_factory=lambda: int(os.getenv("SVGX_LOG_BACKUP_COUNT", "5"))
    )
    console_output: bool = field(
        default_factory=lambda: os.getenv("SVGX_LOG_CONSOLE", "true").lower() == "true"
    )
    include_timestamp: bool = field(
        default_factory=lambda: os.getenv("SVGX_LOG_INCLUDE_TIMESTAMP", "true").lower()
        == "true"
    )
    include_level: bool = field(
        default_factory=lambda: os.getenv("SVGX_LOG_INCLUDE_LEVEL", "true").lower()
        == "true"
    )
...
class SecurityConfig:
    ...
    secret_key: str = field(
        default_factory=lambda: os.getenv(
            "SVGX_SECRET_KEY", "your-secret-key-change-in-production"
        )
    )
    algorithm: str = field(default_factory=lambda: os.getenv("SVGX_ALGORITHM", "HS256"))
    access_token_expire_minutes: int = field(
        default_factory=lambda: int(os.getenv("SVGX_ACCESS_TOKEN_EXPIRE_MINUTES", "30"))
    )
    refresh_token_expire_days: int = field(
        default_factory=lambda: int(os.getenv("SVGX_REFRESH_TOKEN_EXPIRE_DAYS", "7"))
    )
    bcrypt_rounds: int = field(
        default_factory=lambda: int(os.getenv("SVGX_BCRYPT_ROUNDS", "12"))
    )
    cors_origins: List[str] = field(
        default_factory=lambda: os.getenv("SVGX_CORS_ORIGINS", "*").split(",")
    )
    rate_limit_requests: int = field(
        default_factory=lambda: int(os.getenv("SVGX_RATE_LIMIT_REQUESTS", "100"))
    )
    rate_limit_window: int = field(
        default_factory=lambda: int(os.getenv("SVGX_RATE_LIMIT_WINDOW", "3600"))
    )
...
class PerformanceConfig:
    ...
    max_workers: int = field(
        default_factory=lambda: int(os.getenv("SVGX_MAX_WORKERS", "4"))
    )
    worker_timeout: int = field(
        default_factory=lambda: int(os.getenv("SVGX_WORKER_TIMEOUT", "30"))
    )
    cache_ttl: int = field(
        default_factory=lambda: int(os.getenv("SVGX_CACHE_TTL", "3600"))
    )
    max_cache_size: int = field(
        default_factory=lambda: int(os.getenv("SVGX_MAX_CACHE_SIZE", "1000"))
    )
    enable_compression: bool = field(
        default_factory=lambda: os.getenv("SVGX_ENABLE_COMPRESSION", "true").lower()
        == "true"
    )
    enable_gzip: bool = field(
        default_factory=lambda: os.getenv("SVGX_ENABLE_GZIP", "true").lower() == "true"
    )
...
class APIConfig:
    ...
    title: str = field(
        default_factory=lambda: os.getenv("SVGX_API_TITLE", "SVGX Engine API")
    )
    description: str = field(
        default_factory=lambda: os.getenv(
            "SVGX_API_DESCRIPTION",
            "Programmable spatial markup format and simulation engine",
        )
    )
    version: str = field(default_factory=lambda: os.getenv("SVGX_API_VERSION", "1.0.0"))
    docs_url: str = field(
        default_factory=lambda: os.getenv("SVGX_API_DOCS_URL", "/docs")
    )
    redoc_url: str = field(
        default_factory=lambda: os.getenv("SVGX_API_REDOC_URL", "/redoc")
    )
    debug: bool = field(
        default_factory=lambda: os.getenv("SVGX_API_DEBUG", "false").lower() == "true"
    )
...
class ServerConfig:
    ...
    reload: bool = field(
        default_factory=lambda: os.getenv("SVGX_RELOAD", "false").lower() == "true"
    )
    ...
    access_log: bool = field(
        default_factory=lambda: os.getenv("SVGX_ACCESS_LOG", "true").lower() == "true"
    )
...
class MonitoringConfig:
    ...
    enable_metrics: bool = field(
        default_factory=lambda: os.getenv("SVGX_ENABLE_METRICS", "true").lower()
        == "true"
    )
    metrics_port: int = field(
        default_factory=lambda: int(os.getenv("SVGX_METRICS_PORT", "9090"))
    )
    enable_health_checks: bool = field(
        default_factory=lambda: os.getenv("SVGX_ENABLE_HEALTH_CHECKS", "true").lower()
        == "true"
    )
    health_check_interval: int = field(
        default_factory=lambda: int(os.getenv("SVGX_HEALTH_CHECK_INTERVAL", "30"))
    )
    enable_tracing: bool = field(
        default_factory=lambda: os.getenv("SVGX_ENABLE_TRACING", "false").lower()
        == "true"
    )
    tracing_endpoint: Optional[str] = field(
        default_factory=lambda: os.getenv("SVGX_TRACING_ENDPOINT")
    )
...
class SVGXConfig:
    ...
    environment: str = field(
        default_factory=lambda: os.getenv("SVGX_ENVIRONMENT", "development")
    )
