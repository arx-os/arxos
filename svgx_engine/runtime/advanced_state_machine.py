...
class StateMachine:
    ...
    state_history: List[Tuple[str, str, datetime]] = field(
...
class AdvancedStateMachine:
    ...
    def _initialize_default_state_machines(self):
        ...
        equipment_transitions = [
            ...
            StateTransition(
                id="fault_to_maintenance", from_state="fault", to_state="maintenance"
            ),
            StateTransition(
                id="maintenance_to_off", from_state="maintenance", to_state="off"
            ),
        ]
        ...
        process_transitions = [
            StateTransition(
                id="stopped_to_starting", from_state="stopped", to_state="starting"
            ),
            StateTransition(
                id="starting_to_running", from_state="starting", to_state="running"
            ),
            StateTransition(
                id="running_to_paused", from_state="running", to_state="paused"
            ),
            StateTransition(
                id="paused_to_running", from_state="paused", to_state="running"
            ),
            StateTransition(
                id="running_to_stopping", from_state="running", to_state="stopping"
            ),
            StateTransition(
                id="stopping_to_stopped", from_state="stopping", to_state="stopped"
            ),
        ...
        system_transitions = [
            StateTransition(
                id="normal_to_warning", from_state="normal", to_state="warning"
            ),
            StateTransition(
                id="warning_to_normal", from_state="warning", to_state="normal"
            ),
            StateTransition(
                id="warning_to_critical", from_state="warning", to_state="critical"
            ),
            StateTransition(
                id="critical_to_warning", from_state="critical", to_state="warning"
            ),
            StateTransition(
                id="critical_to_emergency", from_state="critical", to_state="emergency"
            ),
            StateTransition(
                id="emergency_to_shutdown", from_state="emergency", to_state="shutdown"
            ),
        ...
        safety_transitions = [
            StateTransition(
                id="safe_to_warning", from_state="safe", to_state="warning"
            ),
            StateTransition(
                id="warning_to_safe", from_state="warning", to_state="safe"
            ),
            StateTransition(
                id="warning_to_danger", from_state="warning", to_state="danger"
            ),
            StateTransition(
                id="danger_to_warning", from_state="danger", to_state="warning"
            ),
            StateTransition(
                id="danger_to_shutdown", from_state="danger", to_state="shutdown"
            ),
            StateTransition(
                id="any_to_emergency", from_state="*", to_state="emergency"
            ),
            StateTransition(
                id="emergency_to_shutdown", from_state="emergency", to_state="shutdown"
            ),
    ...
    def create_state_machine(
        ...
        try:
            ...
            logger.info(
                f"Created state machine '{name}' with {len(states)} states and {len(transitions)} transitions"
            )
            return True
    ...
    async def transition_state(
        ...
        try:
            ...
            transition = self._find_valid_transition(
                state_machine, current_state, target_state, context
            )
            if not transition:
                logger.warning(
            ...
            if success:
                ...
                state_machine.state_history.append(
                    (current_state, target_state, datetime.utcnow())
                )
                ...
                logger.info(
                    f"Transitioned {machine_id} from {current_state} to {target_state}"
                )
    ...
    def _find_valid_transition(
        ...
        for transition in state_machine.transitions.values():
            ...
            if (
                transition.from_state == from_state or transition.from_state == "*"
    ...
    def get_available_transitions(self, machine_id: str) -> List[StateTransition]:
        ...
        for transition in state_machine.transitions.values():
            if transition.enabled and (
                transition.from_state == current_state or transition.from_state == "*"
            ):
    ...
    def validate_state_machine(self, machine_id: str) -> Dict[str, Any]:
        ...
        if (
            ...
            and state_machine.current_state not in state_machine.states
        ):
            issues.append(
                f"Current state '{state_machine.current_state}' not found in states"
            )
        ...
        for transition_id, transition in state_machine.transitions.items():
            if (
                ...
                and transition.from_state not in state_machine.states
            ):
                issues.append(
                    f"Transition {transition_id}: from_state '{transition.from_state}' not found"
                )

            if transition.to_state not in state_machine.states:
                issues.append(
                    f"Transition {transition_id}: to_state '{transition.to_state}' not found"
                )
