...
class FloorApplicationService:
    ...
    def create_floor(
        ...
        try:
            ...
            return CreateFloorResponse(
                success=False, error_message=f"Failed to create floor: {str(e)}"
            )
    ...
    def update_floor(
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    "Updating floor", floor_id=floor_id, updated_by=updated_by
                )
            ...
            if result.success:
                ...
                if self.event_store:
                    floor_updated_event = FloorUpdated(
                        ...
                        updated_fields=[
                            field
                            for field in [floor_number, status]
            ...
            if self.logger:
                self.logger.error(
                    "Failed to update floor", floor_id=floor_id, error=str(e)
                )
            ...
            return UpdateFloorResponse(
                success=False, error_message=f"Failed to update floor: {str(e)}"
            )
        ...
        try:
            ...
            if self.cache_service:
                ...
                if cached_floor:
                    ...
                    return GetFloorResponse(success=True, floor=cached_floor)
            ...
            if self.logger:
                self.logger.error(
                    "Failed to get floor", floor_id=floor_id, error=str(e)
                )
            ...
            return GetFloorResponse(
                success=False, error_message=f"Failed to get floor: {str(e)}"
            )
    ...
    def list_floors(
        ...
        try:
            ...
            return ListFloorsResponse(
                success=False, error_message=f"Failed to list floors: {str(e)}"
            )
    ...
    def delete_floor(self, floor_id: str) -> DeleteFloorResponse:
        ...
        try:
            # Log the operation
            if self.logger:
                self.logger.info("Deleting floor", floor_id=floor_id)
            ...
            if result.success:
                ...
                if self.event_store:
                    floor_deleted_event = FloorDeleted(
                        floor_id=floor_id,
                        floor_number=0,  # Would need to get from floor
                        deleted_by="system",
                    )
            ...
            if self.logger:
                self.logger.error(
                    "Failed to delete floor", floor_id=floor_id, error=str(e)
                )
            ...
            return DeleteFloorResponse(
                success=False, error_message=f"Failed to delete floor: {str(e)}"
            )
    ...
    def get_floor_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for floor in floors:
                ...
                stats["by_building"][building_id] = (
                    stats["by_building"].get(building_id, 0) + 1
                )
            ...
            # Log error
            if self.logger:
                self.logger.error("Failed to get floor statistics", error=str(e))

            # Record error metrics
            if self.metrics:
                self.metrics.increment_counter("floor.statistics.error")

            return {"error": f"Failed to get floor statistics: {str(e)}"}
