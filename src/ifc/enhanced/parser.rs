...
impl EnhancedIFCParser {
    ...
    pub fn parse_with_recovery(&mut self, file_path: &str) -> ArxResult<ParseResult> {
        ...
        info!(
            "Starting enhanced IFC parsing with error recovery: {}",
            file_path
        );
        ...
        for (line_num, line) in lines.iter().enumerate() {
            if self.collected_errors.len() >= self.error_threshold {
                warn!(
                    "Error threshold reached ({}), stopping parsing",
                    ...
                    if self.is_spatial_entity(&entity.entity_type) {
                        match self.extract_spatial_data_safe(&entity) {
                            ...
                            Err(e) => {
                                ...
                                warn!(
                                    "Failed to extract spatial data for entity {}: {}",
                                    entity.id, e
                                );
        ...
        info!(
            "Parsing completed: {} entities, {} errors, {:.1}% success rate",
    ...
    pub fn parse_with_progress_and_recovery(
        ...
        info!(
            "Parsing with progress completed: {} entities, {} errors, {:.1}% success rate",
            result.parse_stats.successful_entities,
            result.errors.len(),
            result.parse_stats.success_rate() * 100.0
        );
    ...
    fn validate_file_size(&self, file_path: &str) -> ArxResult<()> {
        ...
        let metadata = metadata(file_path).map_err(|e| {
            ArxError::io_error(format!("Cannot read file metadata: {}", e))
    ...
    fn read_file_safely(&self, file_path: &str) -> ArxResult<String> {
        ...
        {
            warn!(
                "File does not have .ifc extension: {}",
                validated_path.display()
            );
        }

        // Validate file size before reading
        self.validate_file_size(validated_path.to_str().ok_or_else(|| {
            ArxError::io_error("Invalid file path encoding").with_file_path(file_path)
        })?)?;
        ...
        match PathSafety::read_file_safely(&validated_path, &current_dir) {
            ...
            Err(e) => Err(
                ArxError::io_error(format!("Failed to read IFC file: {}", e))
                    .with_file_path(file_path)
                    .with_suggestions(vec![
                        "Check file permissions".to_string(),
