...
class RealtimeEditor:
        ...
        try:
            async for message in websocket:
                ...
                if message_type == "join_session":
                    ...
                    if session_id and user_id:
                        await self.update_cursor(session_id, user_id, data["cursor"])
    ...
    async def leave_session(self, session_id: str, user_id: str):
        ...
        # Notify other users
        await self.broadcast_to_session(
            session_id, {"type": "user_left", "user_id": user_id}
        )
    ...
    async def handle_edit_operation(
        self, session_id: str, user_id: str, operation_data: Dict
    ):
        ...
        await self.broadcast_to_session(
            session_id,
            {"type": "edit_operation", "operation": asdict(edit_op)},
            exclude_user=user_id,
        )
    ...
    async def update_cursor(self, session_id: str, user_id: str, cursor_data: Dict):
        ...
        await self.broadcast_to_session(
            session_id,
            {"type": "cursor_update", "user_id": user_id, "cursor": cursor_data},
            exclude_user=user_id,
        )
    ...
    async def update_typing_status(
        ...
        await self.broadcast_to_session(
            session_id,
            {"type": "typing_indicator", "user_id": user_id, "is_typing": is_typing},
            exclude_user=user_id,
        )
