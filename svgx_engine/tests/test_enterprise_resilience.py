...
class TestEnterpriseResilienceService:
    ...
    async def test_add_circuit_breaker(self, resilience_service):
        ...
        assert isinstance(
            resilience_service.circuit_breakers["test_service"], CircuitBreaker
        )
    ...
    async def test_add_retry_handler(self, resilience_service):
        ...
        assert isinstance(
            resilience_service.retry_handlers["test_operation"], RetryHandler
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_with_resilience_with_circuit_breaker(
        self, resilience_service
    ):
        """Test execution with circuit breaker protection"""
        config = CircuitBreakerConfig(
            name="test_service", failure_threshold=5, timeout=1.0, enabled=False
        )
        resilience_service.add_circuit_breaker("test_service", config)
...
class TestIntegration:
    ...
    async def test_full_resilience_workflow(self):
        ...
        # Add circuit breaker
        circuit_config = CircuitBreakerConfig(
            name="database", failure_threshold=3, timeout=1.0, enabled=False
        )
        ...
        health_config = HealthCheckConfig(
            endpoint="http://localhost:5432/health", timeout=1.0
        )
