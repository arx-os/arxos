...
class CreateDrawingRequest(BaseModel):
    ...
    precision_level: Optional[PrecisionLevel] = Field(
        PrecisionLevel.SUB_MILLIMETER, description="Precision level"
    )
...
async def create_drawing(request: CreateDrawingRequest):
    ...
    try:
        drawing_id = cad_system.create_new_drawing(
            request.name, request.precision_level
        )
...
async def add_precision_point(request: AddPointRequest):
    ...
    try:
        point = cad_system.add_precision_point(request.x, request.y, request.z)

        if not point:
            raise HTTPException(status_code=500, detail="Failed to add precision point")

        return AddPointResponse(
            point=point.to_dict(), message="Precision point added successfully"
        )
...
async def add_constraint(request: AddConstraintRequest):
    ...
    try:
        success = cad_system.add_constraint(
            request.constraint_type, request.entities, request.parameters
        )

        if not success:
            raise HTTPException(status_code=500, detail="Failed to add constraint")

        return AddConstraintResponse(message="Constraint added successfully")
...
async def add_dimension(request: AddDimensionRequest):
    ...
    try:
        ...
        start_point = PrecisionPoint(
            ...
            (
                Decimal(str(request.start_point["z"]))
                if "z" in request.start_point
                else None
            ),
        )
        ...
        success = cad_system.add_dimension(
            request.dimension_type, start_point, end_point, request.style_name
        )

        if not success:
            raise HTTPException(status_code=500, detail="Failed to add dimension")

        return AddDimensionResponse(message="Dimension added successfully")
...
async def add_parameter(request: AddParameterRequest):
    ...
    try:
        ...
        return AddParameterResponse(
            parameter=parameter_info, message="Parameter added successfully"
        )
...
async def create_assembly(request: CreateAssemblyRequest):
    ...
    try:
        ...
        return CreateAssemblyResponse(
            assembly=assembly_info, message="Assembly created successfully"
        )
...
async def add_component_to_assembly(request: AddComponentRequest):
    ...
    try:
        success = cad_system.add_component_to_assembly(
            request.assembly_id, request.component
        )

        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to add component to assembly"
            )

        return AddComponentResponse(message="Component added to assembly successfully")
...
async def generate_views(request: GenerateViewsRequest):
    ...
    try:
        ...
        return GenerateViewsResponse(
            views=views, message="Views generated successfully"
        )
...
async def get_drawing_info(drawing_id: str):
    ...
    try:
        ...
        return DrawingInfoResponse(
            info=info, message="Drawing info retrieved successfully"
        )
...
async def export_drawing(request: ExportDrawingRequest):
    ...
    try:
        ...
        return ExportDrawingResponse(
            export_data=export_data, message="Drawing exported successfully"
        )
...
async def get_cad_system_info():
    ...
    try:
        ...
        return CADSystemInfoResponse(
            system_info=system_info, message="CAD system info retrieved successfully"
        )
...
async def get_drawing_history():
    ...
    try:
        ...
        return {"message": "Drawing history retrieved successfully", "history": history}
