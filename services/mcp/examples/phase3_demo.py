    ...
    return BuildingModel(
        ...
        objects=[
            ...
            BuildingObject(
                ...
                object_type="duct",
                properties={"type": "supply", "airflow": 1200.0, "location": "attic"},
                location={
                    "x": 550,
                    "y": 180,
                    "z": 10,
            ...
            BuildingObject(
                ...
                properties={"type": "sink", "flow_rate": 2.5, "location": "bathroom"},
                location={
                    "x": 110,
                    "y": 210,
                    "z": 0,
...
def create_phase3_mcp_files():
    ...
    for mcp_data, filename in [
        (nec_2023, "nec_2023_demo.json"),
        (ibc_2024, "ibc_2024_demo.json"),
    ]:
...
def demonstrate_spatial_engine():
    ...
    panel = next(
        obj for obj in building_model.objects if obj.object_type == "electrical_panel"
    )
    outlet = next(
        obj for obj in building_model.objects if obj.object_type == "electrical_outlet"
    )
    ...
    for i, room_a in enumerate(rooms):
        for j, room_b in enumerate(rooms[i + 1 :], i + 1):
            is_adjacent = spatial_engine.check_spatial_relationship(
                room_a, room_b, "adjacent", max_distance=5.0
            )
            print(
                f"🔗 {room_a.object_id} adjacent to {room_b.object_id}: {is_adjacent}"
            )
...
def demonstrate_enhanced_formula_evaluation():
    ...
    hvac_unit = next(
        obj for obj in building_model.objects if obj.object_type == "hvac_unit"
    )
    ...
    for calc_name, calc_data in result.calculations.items():
        print(
            f"   📈 {calc_name}: {calc_data.get('result', 'N/A')} {calc_data.get('unit', '')}"
        )
...
def demonstrate_cross_system_validation():
    ...
    # Get plumbing flow
    plumbing_flow = sum(
        obj.properties.get("flow_rate", 0)
        for obj in building_model.objects
...
def demonstrate_comprehensive_validation():
    ...
    try:
        ...
        for report in compliance_report.validation_reports:
            print(f"\n📄 {report.mcp_name}:")
            print(
                f"   Rules: {report.total_rules} total, {report.passed_rules} passed, {report.failed_rules} failed"
            )
            print(
                f"   Violations: {report.total_violations}, Warnings: {report.total_warnings}"
            )

            for result in report.results:
                if result.violations:
                    print(
                        f"   ❌ {result.rule_name}: {len(result.violations)} violations"
                    )
                else:
