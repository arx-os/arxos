...
from domain.value_objects import BuildingId, Address, BuildingStatus, Coordinates, Dimensions
from domain.exceptions import (
    InvalidBuildingError, BuildingNotFoundError, DuplicateBuildingError,
...
class CreateBuildingUseCase:
    ...
    def execute(self, request: CreateBuildingRequest) -> CreateBuildingResponse:
        ...
        try:
            ...
            return CreateBuildingResponse(
                success=False,
                error_message=f"Building already exists: {str(e)}"
            )
        except InvalidBuildingError as e:
            return CreateBuildingResponse(
                success=False,
                error_message=f"Invalid building data: {str(e)}"
            )
...
Example:
    ...
    def execute(self, request: UpdateBuildingRequest) -> UpdateBuildingResponse:
        ...
        try:
            ...
            return UpdateBuildingResponse(
                success=False,
                error_message=f"Building not found: {str(e)}"
            )
        ...
        try:
            ...
            if status:
                try:
                    ...
                    buildings = self.unit_of_work.buildings.get_by_status(building_status)
                except ValueError:
