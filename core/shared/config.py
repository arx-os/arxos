    ...
    def from_env(cls) -> "SharedConfig":
        ...
        return cls(
            ...
            enable_structured_logging=os.getenv(
                "ARXOS_ENABLE_STRUCTURED_LOGGING", "true"
            ).lower()
            == "true",
            ...
            enable_object_validation=os.getenv(
                "ARXOS_ENABLE_OBJECT_VALIDATION", "true"
            ).lower()
            == "true",
            max_request_size=int(
                os.getenv("ARXOS_MAX_REQUEST_SIZE", str(10 * 1024 * 1024))
            ),
            enable_request_logging=os.getenv(
                "ARXOS_ENABLE_REQUEST_LOGGING", "true"
            ).lower()
            == "true",
            enable_rate_limiting=os.getenv("ARXOS_ENABLE_RATE_LIMITING", "true").lower()
            == "true",
            rate_limit_window=int(os.getenv("ARXOS_RATE_LIMIT_WINDOW", "60")),
            rate_limit_max_requests=int(
                os.getenv("ARXOS_RATE_LIMIT_MAX_REQUESTS", "100")
            ),
            enable_detailed_errors=os.getenv(
                "ARXOS_ENABLE_DETAILED_ERRORS", "false"
            ).lower()
            == "true",
