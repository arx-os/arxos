...
def test_cad_primitives():
    ...
    print(
        f"✓ Line created: {line.start_x}, {line.start_y} to {line.end_x}, {line.end_y}"
    )

    # Test Circle
    circle = Circle(
        center_x=Decimal("50.0"), center_y=Decimal("60.0"), radius=Decimal("25.5")
    )
    print(
        f"✓ Circle created: center ({circle.center_x}, {circle.center_y}), radius {circle.radius}"
    )
    ...
    print(
        f"✓ Arc created: center ({arc.center_x}, {arc.center_y}), radius {arc.radius}"
    )
    ...
    print(
        f"✓ Polyline created: {len(polyline.points)} points, closed={polyline.closed}"
    )
...
def test_constraints():
    ...
    parallel = ParallelConstraint(
        target_ids=["line1", "line2"], parameters={"tolerance": 0.001}
    )
...
def test_infrastructure_integration():
    ...
    circle = Circle(
        center_x=Decimal("50.0"), center_y=Decimal("50.0"), radius=Decimal("25.0")
    )
    ...
    print(
        f"✓ CAD element created: {element.id}, {len(element.cad_primitives)} primitives"
    )
