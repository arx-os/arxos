    ...
    try:
        ...
        if args.service and args.language:
            ...
            if args.language not in service_config.languages:
                logger.error(f"Language '{args.language}' not supported for service '{args.service}'")
                sys.exit(1)
            ...
            for language in generator.config.languages:
                if language in service_config.languages:
                    try:
                        ...
                        logger.error(f"❌ Failed to generate {language} SDK for {args.service}: {e}")
                else:
                    logger.info(f"⏭️ Skipping {language} for {args.service} (not supported)")
            ...
            for service_name, service_config in generator.services.items():
                if args.language in service_config.languages:
                    try:
                        generator.generate_sdk(service_name, args.language, service_config)
                        logger.info(f"✅ Generated {args.language} SDK for {service_name}")
                    except Exception as e:
                        logger.error(f"❌ Failed to generate {args.language} SDK for {service_name}: {e}")
                else:
                    logger.info(f"⏭️ Skipping {service_name} for {args.language} (not supported)")
