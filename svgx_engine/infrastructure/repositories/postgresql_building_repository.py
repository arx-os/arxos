...
class PostgreSQLBuildingRepository(BuildingRepository):
    ...
    def _initialize_pool(self):
        ...
        try:
            self._pool = SimpleConnectionPool(1, self.pool_size, self.connection_string)
            logger.info(
                f"PostgreSQL connection pool initialized with size {self.pool_size}"
            )
    ...
    def _row_to_building(self, row: Dict[str, Any]) -> Building:
        ...
        try:
            ...
            dimensions = Dimensions(
                ...
                height=(
                    float(row["dimensions_height"])
                    if row["dimensions_height"]
                    else None
                ),
            )
