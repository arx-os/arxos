...
class MockBiltDividendCalculator:
    ...
    async def distribute_dividends(self, amount: Optional[Decimal] = None) -> bool:
        ...
        try:
            ...
            dividend_per_token = amount / Decimal(self.mock_contract_state['total_supply'])
    ...
    async def calculate_claimable_dividends(self, wallet_address: str) -> DividendClaim:
        ...
        try:
            ...
            claimable_amount = Decimal(str(balance)) * Decimal('0.01')  # 1% of balance as mock dividend
            
            # Get claim data
            claim_data = self.mock_contract_state['claim_data'].get(wallet_address, {
                'last_claim_index': 0,
                'total_claimed': 0.0
            })
    ...
    async def claim_dividends(self, wallet_address: str) -> bool:
        ...
        try:
            ...
            self.mock_contract_state['claim_data'][wallet_address]['total_claimed'] += float(claimable.claimable_amount)
            
            logger.info(f"Mock dividends claimed: {claimable.claimable_amount} for {wallet_address}")
            return True
    ...
    async def get_dividend_pool_stats(self) -> Dict[str, Any]:
        ...
        try:
            return {
                'current_pool': float(self.mock_contract_state['current_dividend_pool']),
                'total_revenue': float(self.mock_contract_state['total_revenue']),
                'total_distributed': float(self.mock_contract_state['total_dividends_distributed']),
                'total_supply': self.mock_contract_state['total_supply'],
                'database_total_revenue': float(self.mock_contract_state['total_revenue']),
                'database_total_distributions': len(self.mock_contract_state['dividend_history'])
            }
