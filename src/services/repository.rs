...
pub trait Repository: Send + Sync {
    /// Load building data
    fn load(&self, building_name: &str) -> Result<BuildingData, Box<dyn std::error::Error + Send + Sync>>;
    
    /// Save building data
    fn save(&self, building_name: &str, data: &BuildingData) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
    
    /// Save and commit building data (with Git integration)
    fn save_and_commit(&self, building_name: &str, data: &BuildingData, message: Option<&str>) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
...
impl Repository for InMemoryRepository {
    ...
    fn save_and_commit(&self, building_name: &str, data: &BuildingData, _message: Option<&str>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
