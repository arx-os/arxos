...
class TestDeploymentScript:
    ...
    def test_script_validation(self, mock_run):
        ...
        result = subprocess.run(
            ["bash", str(script_path), "--validate"], capture_output=True, text=True
        )
    ...
    def test_color_determination(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.return_value.stdout = (
                '{"spec":{"template":{"metadata":{"labels":{"color":"blue"}}}}}'
            )
            ...
            result = subprocess.run(
                [
                    ...
                    "arxos-test",
                    "-o",
                    "jsonpath='{.spec.template.metadata.labels.color}'",
                ],
                capture_output=True,
                text=True,
            )
        ...
        with patch("subprocess.run") as mock_run:
            ...
            result = subprocess.run(
                [
                    ...
                    "patch",
                    "service",
                    "arxos-backend-service",
                    "-n",
                    "arxos-test",
                    "-p",
                    f'{{"spec":{{"selector":{{"color":"{test_color}"}}}}}}',
                ],
                capture_output=True,
                text=True,
            )
...
class TestHealthChecks:
    ...
    def test_custom_health_checks(self):
        ...
        custom_checks = ["scripts/check_db.sh", "scripts/check_external.sh"]
...
class TestMonitoringAndAlerting:
    ...
    def test_alert_thresholds(self):
        ...
        thresholds = {"cpu": 80, "memory": 85, "response_time": 2000, "error_rate": 5}

        # Mock metrics
        metrics = {"cpu": 75, "memory": 70, "response_time": 1500, "error_rate": 2}
...
class TestSecurityTests:
    ...
    def test_secrets_validation(self):
        ...
        required_secrets = ["database_password", "api_keys", "ssl_certificates"]
