...
class BiltDividendCalculator:
    ...
    def __init__(
        ...
        self.bilt_contract = self.web3.eth.contract(
            address=bilt_contract_address, abi=bilt_contract_abi
        )
        self.revenue_router = self.web3.eth.contract(
            address=revenue_router_address, abi=revenue_router_abi
        )
    ...
    async def distribute_dividends(self, amount: Optional[Decimal] = None) -> bool:
        ...
        try:
            if amount is None:
                ...
                amount_wei = int(amount * 10**18)
                transaction = self.revenue_router.functions.distributeDividends(
                    amount_wei
                )
            ...
            if tx_receipt.status == 1:
                ...
                distribution_data = {
                    ...
                    "total_amount": (
                        float(amount)
                        if amount
                        else await self._get_dividend_pool_amount()
                    ),
        ...
        try:
            ...
            if tx_receipt.status == 1:
                ...
                await self.dividend_repo.update_claim_data(
                    wallet_address, claimable.claimable_amount, tx_hash.hex()
                )

                logger.info(
                    f"Dividends claimed successfully: {claimable.claimable_amount} for {wallet_address}"
                )
                return True
            else:
    ...
    async def get_revenue_by_source(self, source: str) -> Decimal:
        ...
        try:
            ...
            revenue_wei = self.revenue_router.functions.getRevenueBySource(
                source
            ).call()
