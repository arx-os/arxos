...
pub fn handle_sync(
    ...
    let ifc_path = if let Some(path) = ifc_file {
        ...
        let yaml_file = yaml_files.first().ok_or("No YAML files found to sync")?;
        let ifc_name = yaml_file.replace(".yaml", ".ifc").replace(".yml", ".ifc");
...
fn handle_watch_mode(
    ...
    let yaml_files = loading::find_yaml_files()?;
    let yaml_file = yaml_files.first().ok_or("No YAML files found to watch")?;
