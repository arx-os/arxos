...
class PartitioningBenchmarker:
    ...
    def __init__(self, database_url: str):
        ...
        logger.info(
            "partitioning_benchmarker_initialized",
    ...
    def run_benchmarks(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "partitioning_benchmarks_completed",
            ...
            logger.error("partitioning_benchmarks_failed", error=str(e))
            return False
        finally:
            self.disconnect()
    ...
    def _benchmark_query(
        self, query_info: Dict[str, Any], query_type: str
        ...
        try:
            ...
            logger.debug(
                "benchmarking_query", query_name=query_name, table_name=table_name
            )
            ...
            partitioned_query = self._adapt_query_for_partitioning(
                query_sql, table_name
            )
    ...
    def _execute_query_with_metrics(
        self, query_sql: str, table_name: str, phase: str
    ...
    def _extract_plan_metrics(self, execution_plan: Optional[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "Plan" in execution_plan:
                ...
                cache_hit_ratio = (
                    buffer_hits / total_blocks if total_blocks > 0 else 0.0
                )
            ...
            logger.warning("failed_to_extract_plan_metrics", error=str(e))
            return {}
    ...
    def _generate_benchmark_summary(
        self, db_info: Dict[str, Any], benchmark_results: List[BenchmarkResult]
    ) -> BenchmarkSummary:
    ...
    def _generate_recommendations(
        ...
        if avg_improvement > 50:
            recommendations.append(
                "Excellent performance improvement - partitioning is highly effective"
            )
        elif avg_improvement > 20:
            recommendations.append(
                "Good performance improvement - partitioning provides benefits"
            )
        elif avg_improvement > 0:
            recommendations.append(
                "Modest performance improvement - consider query optimization"
            )
        else:
            recommendations.append(
                "No performance improvement - review partitioning strategy"
            )
        ...
        for query_type, improvements in query_type_results.items():
            avg_improvement = sum(improvements) / len(improvements)
            recommendations.append(
                f"{query_type}: {avg_improvement:.1f}% average improvement"
            )
        ...
        recommendations.append(
            f"Best performing query: {best_query.benchmark_name} ({best_query.improvement_percentage:.1f}% improvement)"
        )
        recommendations.append(
            f"Worst performing query: {worst_query.benchmark_name} ({worst_query.improvement_percentage:.1f}% improvement)"
        )
    ...
    def _generate_json_report(self, output_file: Optional[str] = None) -> str:
        ...
        report = {
            ...
            "benchmark_results": [
                asdict(result) for result in self.benchmark_results.benchmark_results
            ],
        }
...
"""
        ...
        for result in self.benchmark_results.benchmark_results:
            ...
            content += (
                f"- **Cache Hit Ratio:** {result.before_result.cache_hit_ratio:.2f}\n\n"
            )

            content += "#### After Partitioning\n"
            content += (
                f"- **Execution Time:** {result.after_result.execution_time_ms:.2f}ms\n"
            )
            content += f"- **Row Count:** {result.after_result.row_count}\n"
            content += f"- **Plan Type:** {result.after_result.plan_type}\n"
            content += (
                f"- **Cache Hit Ratio:** {result.after_result.cache_hit_ratio:.2f}\n\n"
            )

            if result.after_result.partition_used:
                content += (
                    f"- **Partition Used:** {result.after_result.partition_used}\n\n"
                )
...
def main():
    ...
    if success:
        ...
        if benchmarker.benchmark_results:
            ...
            print(
                f"Average improvement: {benchmarker.benchmark_results.average_improvement:.1f}%"
            )
            print(
                f"Best improvement: {benchmarker.benchmark_results.best_improvement:.1f}%"
            )
            print(
                f"Worst improvement: {benchmarker.benchmark_results.worst_improvement:.1f}%"
            )
            print("\nRecommendations:")
