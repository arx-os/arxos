...
class UserAgreementManager:
    ...
    def accept_agreement(
        ...
        self.logger.info(
            f"User {user_id} accepted {agreement_type.value} version {agreement_version}"
        )
        return agreement
    ...
    def revoke_agreement(
        self, user_id: str, agreement_type: AgreementType, reason: str = None
        ...
        for agreement in self.user_agreements[user_id]:
            if (
                ...
                and agreement.status == AgreementStatus.ACCEPTED
            ):
    ...
    def check_agreement_status(
        self, user_id: str, agreement_type: AgreementType
    ) -> Optional[UserAgreement]:
            ...
            if agreement.agreement_type == agreement_type:
                if agreement.status == AgreementStatus.ACCEPTED:
                    if (
                        ...
                        or agreement.accepted_at > current_agreement.accepted_at
                    ):
                        current_agreement = agreement

        return current_agreement

    def is_agreement_valid(self, user_id: str, agreement_type: AgreementType) -> bool:
    ...
    def _update_compliance_status(self, user_id: str):
        ...
        for agreement_type in self.required_agreements:
            if not self.is_agreement_valid(user_id, agreement_type):
                ...
                if agreement_type == AgreementType.COMMUNITY_GUIDELINES:
                    restrictions.extend(
                        ["cannot_submit_drafts", "cannot_comment", "cannot_collaborate"]
                    )
                elif agreement_type == AgreementType.PRIVACY_POLICY:
                    restrictions.extend(
                        ["cannot_access_personal_data", "limited_profile_visibility"]
                    )
                elif agreement_type == AgreementType.TERMS_OF_SERVICE:
                    restrictions.extend(
                        ["cannot_fund_projects", "cannot_receive_funding"]
                    )
        ...
        for agreement_type in self.required_agreements:
            ...
            if (
                agreement
                and agreement.expires_at
                and agreement.expires_at < datetime.utcnow()
            ):
        ...
        compliance_score = (
            valid_agreements / total_required if total_required > 0 else 0.0
        )
    ...
    def update_agreement_version(self, agreement_type: AgreementType, new_version: str):
        ...
        self.logger.info(
            f"Updated {agreement_type.value} version from {old_version} to {new_version}"
        )
    ...
    def get_expiring_agreements(self, user_id: str, days_ahead: int = 30) -> List[Dict]:
        ...
        for agreement_type in self.required_agreements:
            ...
            if (
                agreement
                and agreement.expires_at
                and agreement.expires_at <= cutoff_date
            ):
    ...
    def get_compliance_stats(self) -> Dict:
        ...
        return {
            ...
            "compliance_rate": (
                (compliant_users / total_users * 100) if total_users > 0 else 0
            ),
