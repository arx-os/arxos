...
class BuildingModel(BaseModel):
    ...
    # Status
    status = Column(
        Enum(BuildingStatus), nullable=False, default=BuildingStatus.PLANNED, index=True
    )
    ...
    # Relationships
    floors = relationship(
        "FloorModel", back_populates="building", cascade="all, delete-orphan"
    )
    projects = relationship(
        "ProjectModel", back_populates="building", cascade="all, delete-orphan"
    )

    def __repr__(self) -> str:
        """String representation of the building model."""
        return (
            f"<BuildingModel(id={self.id}, name='{self.name}', status={self.status})>"
        )
    ...
    def full_address(self) -> str:
        ...
        address_parts.extend(
            [self.address_city, self.address_state, self.address_postal_code]
        )
    ...
    def update_status(
        self, new_status: BuildingStatus, updated_by: Optional[str] = None
    ) -> None:
