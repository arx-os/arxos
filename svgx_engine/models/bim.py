...
class HVACZone(BIMElementBase):
    ...
    def __post_init__(self):
        ...
        if self.temperature_setpoint is not None:
            if not (-50 <= self.temperature_setpoint <= 100):
                raise ValueError(
                    "Temperature setpoint must be between -50 and 100 degrees Celsius"
                )
    ...
    def is_comfort_zone(self) -> bool:
        ...
        return (
            self.temperature_setpoint is not None and self.humidity_setpoint is not None
        )
...
class Device(BIMElementBase):
    ...
    def __post_init__(self):
        ...
        if self.efficiency_rating is not None and not (
            0 <= self.efficiency_rating <= 100
        ):
...
class BIMModel:
    ...
    def get_elements_by_type(self, element_type: str) -> List[BIMElementBase]:
        ...
        return [
            element
            for element in self.elements
            if element.__class__.__name__ == element_type
        ]
