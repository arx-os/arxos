...
class ReportGenerationDemo:
    ...
    def demo_test_data(self):
        ...
        try:
            ...
            logger.info(
                f"   - Categories: {len(self.test_data['compliance_by_category'])}"
            )
            logger.info(
                f"   - Recommendations: {len(self.test_data['recommendations'])}"
            )
            ...
            logger.info(
                f"   - Construction Year: {self.building_info['construction_year']}"
            )
    ...
    def _generate_demo_summary(self, results):
        ...
        for demo_name, result in results.items():
            ...
            logger.info(
                f"{status_icon} {demo_name}: {'PASSED' if result else 'FAILED'}"
            )

        logger.info("\n🎯 Key Features Demonstrated:")
        logger.info("✅ Professional PDF report generation")
        logger.info(
            "✅ Multiple report templates (comprehensive, violation, executive)"
        )
