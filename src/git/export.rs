...
use super::{GitError, GitOperationResult, CommitMetadata, GitConfig};
...
pub fn create_file_structure(
    ...
    // Main building file
    let building_yaml = serializer.to_yaml(building_data)
        .map_err(|e| GitError::Generic(e.to_string()))?;
...
fn create_index_file(
    ...
    index_map.insert(Value::String("floors".to_string()), Value::Sequence(floors_list));
