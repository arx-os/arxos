...
fn test_full_ifc_export_workflow() {
    ...
    assert!(
        content.contains("IFCAIRTERMINAL"),
        "Should contain HVAC equipment"
    );
    assert!(
        content.contains("IFCLIGHTFIXTURE"),
        "Should contain electrical equipment"
    );
}
...
fn test_delta_export_workflow() {
    ...
    assert_eq!(
        ...
        1,
        "Should detect one new equipment"
    );

    // Export delta
    exporter2
        .export_delta(Some(&sync_state), &ifc_path)
    ...
    let content = fs::read_to_string(&ifc_path).unwrap();
    assert!(
        content.contains("IFCFLOWTERMINAL") || content.contains("Pump Unit 1"),
...
fn test_sync_state_persistence() {
    ...
    .into_iter()
    .collect();
    let rooms_paths: std::collections::HashSet<String> = ["building/floor-1/room-101".to_string()]
        .into_iter()
        .collect();
    ...
    assert!(loaded_state
        .exported_equipment_paths
        .contains("building/floor-1/equipment-2"));
    assert!(loaded_state
        .exported_rooms_paths
        .contains("building/floor-1/room-101"));
}
...
fn test_delta_calculation_integration() {
    ...
    assert_eq!(
        delta.new_equipment.len(),
        1,
        "Should have one new equipment"
    );
    assert_eq!(
        delta.deleted_equipment.len(),
        1,
        "Should have one deleted equipment"
    );
    assert!(delta
        .deleted_equipment
        .contains(&"building/floor-1/room-101/equipment-electrical-1".to_string()));
}
...
fn test_multiple_floors_export() {
    ...
    exporter
        .export(&ifc_path)
        .expect("Multi-floor export should succeed");
...
fn test_universal_path_preservation() {
    ...
    assert!(
        content.contains("building/floor-1/room-101/equipment-hvac-1")
