/**
 * Advanced Version Control Manager
 * Handles branching, merging, conflict resolution, annotations, comments, and visualization
 */

class VersionControlManager {
    constructor() {
        this.baseUrl = '/v1/version-control';
        this.eventListeners = {
            'version_created': [],
            'branch_created': [],
    ...
    // --- Version Management ---
    async createVersion(floorData, floorId, buildingId, branchName, commitMessage, author, options = {}) {
        ...
        this.emit('version_created', result);
        return result;
    }
    ...
    async getVersionData(versionId) {
        const res = await fetch(`${this.baseUrl}/version/${versionId}`);
        return await res.json();
    }

    // --- Branch Management ---
    async createBranch(branchName, floorId, buildingId, baseVersionId, createdBy, description = '') {
        ...
        const result = await res.json();
        this.emit('branch_created', result);
        return result;
    }

    async getBranches(buildingId, floorId) {
        const res = await fetch(`${this.baseUrl}/branches/${buildingId}/${floorId}`);
        return await res.json();
    }

    // --- Merge Management ---
    async createMergeRequest(sourceBranch, targetBranch, floorId, buildingId, createdBy, description = '') {
        const res = await fetch(`${this.baseUrl}/merge-request`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        const result = await res.json();
        this.emit('merge_request_created', result);
        return result;
    }

    async resolveConflict(mergeId, conflictId, resolution, resolvedBy) {
        const res = await fetch(`${this.baseUrl}/merge/${mergeId}/resolve-conflict`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
    ...
    async executeMerge(mergeId, executedBy) {
        const res = await fetch(`${this.baseUrl}/merge/${mergeId}/execute`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        const result = await res.json();
        this.emit('merge_executed', result);
        return result;
    }

    // --- Annotations ---
    async addAnnotation(versionId, floorId, buildingId, title, content, author, options = {}) {
        const res = await fetch(`${this.baseUrl}/annotation`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                ...
                annotation_type: options.annotationType || 'note'
            })
        });
        const result = await res.json();
        this.emit('annotation_added', result);
        return result;
    }

    async getAnnotations(versionId) {
        const res = await fetch(`${this.baseUrl}/annotations/${versionId}`);
        return await res.json();
    }

    async searchAnnotations(buildingId, floorId, query) {
        const res = await fetch(`${this.baseUrl}/annotations/search/${buildingId}/${floorId}?query=${encodeURIComponent(query)}`);
        return await res.json();
    }

    // --- Comments ---
    async addComment(parentId, parentType, content, author) {
        const res = await fetch(`${this.baseUrl}/comment`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                parent_id: parentId,
    ...
    async getComments(parentId, parentType) {
        const res = await fetch(`${this.baseUrl}/comments/${parentId}?parent_type=${parentType}`);
        return await res.json();
    }

    // --- Branch Visualization ---
    async getBranchGraph(buildingId, floorId) {
        const res = await fetch(`${this.baseUrl}/branch-graph/${buildingId}/${floorId}`);
        const result = await res.json();
        this.emit('branch_graph_updated', result);
        return result;
    }

    // --- Export/Import ---
    async exportVersionData(buildingId, floorId, options = {}) {
        const url = `${this.baseUrl}/export/${buildingId}/${floorId}?include_annotations=${options.includeAnnotations !== false}&include_comments=${options.includeComments !== false}`;
        const res = await fetch(url);
        return await res.json();
    }
    ...
    renderBranchGraph(graphData, container) {
        ...
        graph.versions.forEach(version => {
            if (!branchVersions[version.branch]) {
                branchVersions[version.branch] = [];
            }
        ...
        Object.keys(branchVersions).forEach(branchName => {
            ...
            branchLane.className = 'branch-lane';
            branchLane.innerHTML = `
                <div class="branch-header">${branchName}</div>
                <div class="version-timeline">
                    ${branchVersions[branchName].map(version => `
                        <div class="version-node" data-version-id="${version.version_id}">
    ...
    showConflictResolutionDialog(conflicts, onResolve) {
        ...
        dialog.querySelector('.btn-resolve').addEventListener('click', () => {
            ...
            dialog.querySelectorAll('.resolution-select').forEach(select => {
                if (select.value) {
                    resolutions[select.dataset.conflictId] = select.value;
                }
            });
    ...
    downloadExport(data, filename) {
        this.utils.downloadJSON(data, filename);
    }

    formatVersionNumber(versionNumber) {
        return versionNumber.replace(/_/g, ' ').replace(/-/g, ' ');
    }
    ...
    getVersionTypeColor(versionType) {
        ...
        return colors[versionType] || '#6c757d';
    }
}
