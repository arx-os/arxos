...
async def get_current_user(
    ...
    logger.info(
        f"User authenticated: {user_id}", user_id=user_id, permissions=permissions
    )
...
async def validate_pagination_params(
    page: int = 1, page_size: int = 10, max_page_size: int = 100
) -> Dict[str, int]:
...
async def validate_sorting_params(
    ...
    return {"sort_by": sort_by, "sort_order": sort_order or "asc"}


async def validate_filter_params(filters: Dict[str, Any] = None) -> Dict[str, Any]:
...
async def set_cache(cache_key: str = Depends(get_cache_key), ttl: int = 300) -> None:
