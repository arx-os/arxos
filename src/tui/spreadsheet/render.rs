...
use ratatui::{
    ...
    text::{Line, Span},
    widgets::{Block, Borders, Cell as TableCell, Paragraph, Row, Table, Wrap},
    Frame,
};

/// Aggregated parameters required to render the spreadsheet view.
pub struct SpreadsheetRenderContext<'a> {
    pub area: Rect,
    pub grid: &'a Grid,
    pub theme: &'a Theme,
    pub workflow_status: &'a WorkflowStatus,
    pub editor: Option<&'a CellEditor>,
    pub save_state: Option<&'a SaveState>,
    pub search_state: Option<&'a SearchState>,
    pub ar_scan_count: Option<usize>,
}

impl<'a> SpreadsheetRenderContext<'a> {
    pub fn new(
        area: Rect,
        grid: &'a Grid,
        theme: &'a Theme,
...
pub fn render_spreadsheet(
    ...
    let context = SpreadsheetRenderContext::new(area, grid, theme, workflow_status);
    render_spreadsheet_with_editor_save_search(frame, context);
}
...
pub fn render_spreadsheet_with_editor(
    ...
    let mut context = SpreadsheetRenderContext::new(area, grid, theme, workflow_status);
...
pub fn render_spreadsheet_with_editor_save_search(
    frame: &mut Frame,
    mut context: SpreadsheetRenderContext<'_>,
) {
    context.ar_scan_count = None;
    render_spreadsheet_with_editor_save_search_ar(frame, context);
}
...
pub fn render_spreadsheet_with_editor_save_search_ar(
    frame: &mut Frame,
    context: SpreadsheetRenderContext<'_>,
) {
    ...
    // Render header
    render_header(frame, layout[0], context.grid, context.theme);
    ...
    if let Some(ref address_path) = context.grid.address_modal {
        render_address_modal(frame, context.area, address_path, context.theme);
    }

    // Render status bar
    render_status_bar(frame, layout[2], &context);
}

/// Render header
fn render_header(frame: &mut Frame, area: Rect, _grid: &Grid, theme: &Theme) {
    let header_text = "Spreadsheet: Equipment List".to_string();
...
fn render_grid_body(
    ...
    let header_cells: Vec<TableCell> = grid
        .columns
        .iter()
        .enumerate()
    ...
    let visible_column_indices: Vec<usize> = grid
        .columns
        .iter()
        .enumerate()
        .filter_map(|(col_idx, _col)| {
    ...
    let column_widths = if visible_column_indices.len() == grid.columns.len() {
        ...
        let owned_columns: Vec<super::types::ColumnDefinition> =
            visible_columns.iter().map(|c| (*c).clone()).collect();
...
fn calculate_column_widths(
    columns: &[super::types::ColumnDefinition],
    available_width: u16,
) -> Vec<Constraint> {
    if columns.is_empty() {
        return vec![];
...
/// Render status bar
fn render_status_bar(frame: &mut Frame, area: Rect, context: &SpreadsheetRenderContext<'_>) {
    let layout = Layout::default()
        .direction(Direction::Vertical)
    ...
    let save_state = context.save_state;
    let search_state = context.search_state;
    let ar_scan_count = context.ar_scan_count;
    ...
    let search_status = if let Some(search) = search_state {
        if search.is_active {
            ...
            format!(
                "{}{}",
                match_info,
                if search.case_sensitive { " | Case" } else { "" }
            )
        } else {
            String::new()
        }
    ...
    if !search_status.is_empty() {
        status_line.push_str(&search_status);
    }

    if let Some(count) = ar_scan_count {
        status_line.push_str(&format!(" | Pending AR scans: {}", count));
    }

    let paragraph = Paragraph::new(status_line).style(Style::default().fg(theme.text));
    frame.render_widget(paragraph, layout[0]);

    // Status line 2: Workflow info and theme colors
    let describe = |active: bool| if active { "Active" } else { "Idle" };
    let workflow_text = format!(
        "Workflows: watch {} | sync {} | sensors {}",
        describe(context.workflow_status.watch_mode_active),
        describe(context.workflow_status.sync_active),
        describe(context.workflow_status.sensors_active)
    );

    let color_info = format!(
        "Primary: {:?} | Accent: {:?}",
        context.theme.primary, context.theme.accent
    );

    let second_line = Paragraph::new(format!("{} | {}", workflow_text, color_info))
        .style(Style::default().fg(Color::Gray));
    frame.render_widget(second_line, layout[1]);
}
