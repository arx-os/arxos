...
use crate::ar_integration::pending::{PendingEquipment, PendingEquipmentManager};
...
impl PendingManagerState {
    fn new(building_name: String) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let storage_file =
            std::path::PathBuf::from(format!("pending-equipment-{}.json", building_name));
        ...
        let pending_items: Vec<PendingEquipment> =
            pending_refs.iter().map(|p| (*p).clone()).collect();
...
/// Render header
fn render_header<'a>(state: &'a PendingManagerState, theme: &'a Theme) -> Paragraph<'a> {
    let title = format!(
    ...
    let items: Vec<ListItem> = state
        ...
        .map(|(idx, item)| {
            ...
            let line_parts = vec![
                ...
                Span::styled(") ", Style::default().fg(theme.muted)),
                Span::styled(confidence_bar, Style::default().fg(confidence_color)),
            ];
...
fn render_preview<'a>(
    ...
    let mut lines = vec![
        ...
        Line::from(Span::raw("")),
        Line::from(vec![Span::styled(
            format!("Items to confirm: {}", items_to_confirm.len()),
            Style::default().fg(theme.text),
        )]),
        Line::from(Span::raw("")),
    ];
    ...
    if items_to_confirm.len() > 10 {
        lines.push(Line::from(vec![Span::styled(
            format!("  ... and {} more", items_to_confirm.len() - 10),
            Style::default().fg(theme.muted),
        )]));
    }
...
/// Render footer
fn render_footer<'a>(theme: &'a Theme, selected_count: usize) -> Paragraph<'a> {
