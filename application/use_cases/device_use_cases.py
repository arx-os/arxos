        ...
        try:
            ...
            if not request.name or len(request.name.strip()) == 0:
                return CreateDeviceResponse(
                    success=False, error_message="Device name is required"
                )

            if not request.device_type or len(request.device_type.strip()) == 0:
                return CreateDeviceResponse(
                    success=False, error_message="Device type is required"
                )
            ...
            return CreateDeviceResponse(
                success=False, error_message=f"Device already exists: {str(e)}"
            )
        except InvalidDeviceError as e:
            return CreateDeviceResponse(
                success=False, error_message=f"Invalid device data: {str(e)}"
            )
        ...
        try:
            ...
            if not request.device_id or len(request.device_id.strip()) == 0:
                return UpdateDeviceResponse(
                    success=False, error_message="Device ID is required"
                )
            ...
            if not device:
                return UpdateDeviceResponse(
                    success=False, error_message="Device not found"
                )
            ...
            if request.device_type is not None:
                if len(request.device_type.strip()) == 0:
                    return UpdateDeviceResponse(
                        success=False, error_message="Device type cannot be empty"
                    )
...
class GetDeviceUseCase:
    ...
    def execute(self, device_id: str) -> GetDeviceResponse:
        ...
        try:
            ...
            return GetDeviceResponse(success=True, device=device_dict)
            ...
            if room_id:
                ...
                try:
                    ...
                    return ListDevicesResponse(
                        success=False, error_message=f"Invalid device status: {status}"
                    )
            else:
