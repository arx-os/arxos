...
pub fn get_key_fingerprint_for_email(email: &str) -> GpgResult<String> {
    ...
    if !output.status.success() {
        return Err(GpgError::KeyNotFound(format!(
            "No GPG key found for email: {}",
            email
        )));
    }
...
pub fn get_key_id_for_email(email: &str) -> GpgResult<String> {
    ...
    Err(GpgError::KeyNotFound(format!(
        "Could not extract key ID for email: {}",
        email
    )))
}
...
mod tests {
    ...
    fn test_sanitize_email_for_filename() {
        assert_eq!(
            sanitize_email_for_filename("test@example.com"),
            "test_at_example_com"
        );
        assert_eq!(
            sanitize_email_for_filename("user+tag@domain.co.uk"),
            "user_plus_tag_at_domain_co_uk"
        );
        assert_eq!(
            sanitize_email_for_filename("user.name@example.com"),
    ...
    fn test_git_signing_config_variations() {
        ...
        for value in ["true", "True", "TRUE", "1", "yes", "Yes", "YES"] {
            ...
            assert!(
                is_git_signing_enabled(repo_path),
                "Should recognize '{}' as true",
                value
            );
        }
