...
class SVGXLinter:
    ...
    def _validate_arx_object(self, obj: ET.Element, source_name: str):
        ...
        if not obj.get("id"):
            self.add_error(
                f"arx:object missing required 'id' attribute in {source_name}"
            )

        if not obj.get("type"):
            self.add_error(
                f"arx:object missing required 'type' attribute in {source_name}"
            )
        ...
        if obj_type and "." not in obj_type:
            self.add_warning(
                f"arx:object type '{obj_type}' should use dot notation (e.g., 'electrical.light_fixture') in {source_name}"
            )
    ...
    def _validate_arx_behavior(self, behavior: ET.Element, source_name: str):
        ...
        if variables is not None:
            for var in variables.findall("variable"):
                if not var.get("name"):
                    self.add_error(
                        f"Variable missing 'name' attribute in {source_name}"
                    )
        ...
        if calculations is not None:
            for calc in calculations.findall("calculation"):
                if not calc.get("name"):
                    self.add_error(
                        f"Calculation missing 'name' attribute in {source_name}"
                    )
                if not calc.get("formula"):
                    self.add_error(
                        f"Calculation missing 'formula' attribute in {source_name}"
                    )
        ...
        try:
            ...
            for elem in root.findall(".//*[@arx:precision]"):
                ...
                if precision and not self._is_valid_precision(precision):
                    self.add_error(
                        f"Invalid arx:precision value '{precision}' in {source_name}"
                    )
    ...
    def _check_common_issues(self, content: str, source_name: str):
        ...
        if (
            "arx:precision" in content
            and "mm" not in content
            and "cm" not in content
            and "m" not in content
        ):
            self.add_warning(
                f"Consider adding units to arx:precision attributes in {source_name}"
            )

        # Check for missing system attributes
        if "arx:object" in content and "system=" not in content:
            self.add_warning(
                f"Consider adding 'system' attributes to arx:object elements in {source_name}"
            )

        # Check for missing behavior
        if "arx:object" in content and "arx:behavior" not in content:
            self.add_info(
                f"Consider adding behavior profiles to objects in {source_name}"
            )
