...
type ContributorUseCase struct {
	...
	auditRepo       domain.AccessAuditRepository
...
func (uc *ContributorUseCase) AddContributor(
	ctx context.Context,
	req domain.AddContributorRequest,
	...
	contributor := &domain.Contributor{
		...
		UserID:       req.UserID,
		Role:         req.Role,
		Status:       domain.ContributorStatusActive,
	...
	// Log audit
	_ = uc.auditRepo.Log(&domain.AccessAuditLog{
		ID:            types.NewID(),
		RepositoryID:  &req.RepositoryID,
		Action:        domain.AccessAuditContributorAdded,
...
func (uc *ContributorUseCase) RemoveContributor(
	...
	_ = uc.auditRepo.Log(&domain.AccessAuditLog{
		...
		Action:        domain.AccessAuditContributorRemoved,
...
func (uc *ContributorUseCase) UpdateContributor(
	ctx context.Context,
	req domain.UpdateContributorRequest,
	actorID types.ID,
) error {
	...
	if req.Role != nil && oldRole != *req.Role {
		_ = uc.auditRepo.Log(&domain.AccessAuditLog{
			ID:            types.NewID(),
			RepositoryID:  &contributor.RepositoryID,
			Action:        domain.AccessAuditRoleChanged,
			SubjectUserID: &contributor.UserID,
...
func (uc *ContributorUseCase) CreateTeam(
	...
	team := &domain.Team{
		...
		DefaultRole:     domain.ContributorRoleContributor,
		CreatedAt:       now,
...
func (uc *ContributorUseCase) AddTeamMember(
	ctx context.Context,
	req domain.AddTeamMemberRequest,
) error {
...
func (uc *ContributorUseCase) CheckPermission(
	...
	if contributor.Status != domain.ContributorStatusActive {
		return false, nil
	}
...
func (uc *ContributorUseCase) setRolePermissions(contributor *domain.Contributor) {
	...
	case domain.ContributorRoleOwner:
	...
	case domain.ContributorRoleAdmin:
	...
	case domain.ContributorRoleMaintainer:
	...
	case domain.ContributorRoleContributor:
	...
	case domain.ContributorRoleReporter:
		// Reporter can only create issues
		contributor.CanRead = true
		contributor.CanManageIssues = true

	case domain.ContributorRoleReader:
