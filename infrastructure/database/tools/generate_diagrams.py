...
class DiagramGenerator:
    ...
    def __init__(
        self, database_url: str, output_dir: str = "arx-docs/database/diagrams"
    ):
        """
    ...
    def _generate_er_diagram(self) -> None:
        ...
        for table_name, table_info in self.tables.items():
            ...
            table_cell.set(
                "geometry",
                f"x={x},y={y},width=200,height={50 + len(table_info.columns) * 20}",
            )
        ...
        for relationship in self.relationships:
            if (
                relationship.from_table in table_positions
                and relationship.to_table in table_positions
            ):
                # Create relationship arrow
                rel_cell = ET.SubElement(root_element, "mxCell")
                rel_cell.set(
                    "id", f"rel_{relationship.from_table}_{relationship.to_table}"
                )
                rel_cell.set("edge", "1")
                ...
                if relationship.cascade_delete:
                    ...
                    rel_cell.set(
                        "style",
                        "edgeStyle=orthogonalEdgeStyle;rounded=0;orthogonalLoop=1;jettySize=auto;html=1;strokeDasharray=5 5;",
                    )

                # Add label
                label_cell = ET.SubElement(root_element, "mxCell")
                label_cell.set(
                    "id", f"label_{relationship.from_table}_{relationship.to_table}"
                )
                label_cell.set(
                    "value", f"{relationship.from_column} → {relationship.to_column}"
                )
                label_cell.set(
                    "style",
                ...
                label_cell.set(
                    "parent", f"rel_{relationship.from_table}_{relationship.to_table}"
                )
        ...
        logger.info("er_diagram_generated", file=str(drawio_file))
        self.metrics["diagrams_generated"] += 1

    def _calculate_table_position(
    ...
    def _generate_data_flow_diagram(self) -> None:
        ...
        for service in services:
            ...
            service_cell.set(
                "geometry", f"x={service['x']},y={service['y']},width=120,height=60"
            )
        ...
        for flow in flows:
            ...
            flow_cell.set(
                "id", f"flow_{flow[0].replace(' ', '_')}_{flow[1].replace(' ', '_')}"
            )
            ...
            label_cell = ET.SubElement(root_element, "mxCell")
            label_cell.set(
                "id", f"label_{flow[0].replace(' ', '_')}_{flow[1].replace(' ', '_')}"
            )
            ...
            label_cell.set(
                "parent",
                f"flow_{flow[0].replace(' ', '_')}_{flow[1].replace(' ', '_')}",
            )
        ...
        logger.info("data_flow_diagram_generated", file=str(drawio_file))
        self.metrics["diagrams_generated"] += 1
    ...
    def _generate_relationship_diagram(self) -> None:
        ...
        logger.info("relationship_diagram_generated", file=str(drawio_file))
        self.metrics["diagrams_generated"] += 1
...
"""
        ...
        logger.info("legend_generated", file=str(legend_file))
