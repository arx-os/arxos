...
class DeviceRegistry:
    ...
    def unregister_device(self, device_id: str) -> bool:
        ...
        try:
            with self.lock:
                ...
                # Remove from database import database
                with sqlite3.connect(self.db_path) as conn:
                    conn.execute("DELETE FROM devices WHERE id = ?", (device_id,)
                    conn.execute("DELETE FROM device_telemetry WHERE device_id = ?", (device_id,)
                    conn.execute("DELETE FROM device_events WHERE device_id = ?", (device_id,)
                    conn.commit()

                # Remove from memory import memory
    ...
    def get_devices(self,
                   ...
                   location: Optional[Dict[str, str]] = None) -> List[Device]:
        ...
        if location:
            devices = [d for d in devices if self._location_matches(d.location, location)]

        return devices

    def _location_matches(self, device_location: DeviceLocation, filter_location: Dict[str, str]) -> bool:
    ...
    def discover_devices(self, protocol: CommunicationProtocol,
                        address_range: Optional[str] = None) -> List[Device]:
        ...
        try:
            ...
            logger.info(f"Discovered {len(discovered_devices)} devices using {protocol}")
    ...
    def _get_driver_for_protocol(self, protocol: CommunicationProtocol) -> Optional[Any]:
        """Get driver for communication protocol."""
        driver_name = f"{protocol.value}_driver"
        return self.drivers.get(driver_name)

    def _create_device_from_discovery(self, device_info: Dict[str, Any],
                                    protocol: CommunicationProtocol) -> Device:
    ...
    def _notify_telemetry_handlers(self, device_id: str, capability: str,
                                  value: Any, timestamp: datetime) -> None:
    ...
    async def _collect_device_telemetry(self, device: Device):
        ...
        try:
            ...
            for capability_name, value in telemetry_data.items():
                ...
                # Notify handlers
                self._notify_telemetry_handlers(device.id, capability_name, value, timestamp)
    ...
    def _store_telemetry(self, device_id: str, capability_name: str,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?)
                """, (device_id, capability_name, json.dumps(value), timestamp.isoformat()))
                conn.commit()
        except Exception as e:
            logger.error(f"Failed to store telemetry: {e}")

    def get_telemetry(self, device_id: str, capability_name: Optional[str] = None,
