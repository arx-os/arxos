...
pub fn handle_command_palette(
    ...
    loop {
        ...
        if let Some(Event::Key(key_event)) = terminal.poll_event(Duration::from_millis(100))? {
            // Handle help
            if crate::tui::handle_help_event(Event::Key(key_event), palette.help_system_mut()) {
                continue;
            }
...
#[cfg(test)]
mod tests {
    use crate::tui::command_palette::palette::CommandPalette;
    ...
    fn test_command_palette_creation_in_handler() {
        ...
        assert!(
            !palette.commands().is_empty(),
            "Palette should have commands"
        );
    }
