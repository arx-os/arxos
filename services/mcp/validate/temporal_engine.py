...
from models.mcp_models import (
    ...
    RuleCondition, RuleAction, ValidationViolation, RuleCategory
)
...
class TemporalRuleEngine:
    ...
    def evaluate_temporal_condition(self, condition: TemporalCondition, 
    ...
    def _evaluate_before(self, condition: TemporalCondition, current_time: datetime) -> bool:
    ...
    def _evaluate_after(self, condition: TemporalCondition, current_time: datetime) -> bool:
    ...
    def _evaluate_during(self, condition: TemporalCondition, current_time: datetime) -> bool:
    ...
    def _evaluate_between(self, condition: TemporalCondition, current_time: datetime) -> bool:
        """Evaluate 'between' condition (same as during)"""
        return self._evaluate_during(condition, current_time)
    
    def _evaluate_on(self, condition: TemporalCondition, current_time: datetime) -> bool:
        ...
        return (current_time.date() == target_time.date() and
                current_time.hour == target_time.hour and
                current_time.minute == target_time.minute)
    
    def _evaluate_every(self, condition: TemporalCondition, current_time: datetime) -> bool:
    ...
    def _evaluate_business_hours(self, condition: TemporalCondition, current_time: datetime) -> bool:
        ...
        start_time = datetime.strptime(condition.business_hours_start, condition.time_format).time()
        end_time = datetime.strptime(condition.business_hours_end, condition.time_format).time()
        
        return start_time <= current_time.time() <= end_time
    
    def _evaluate_off_hours(self, condition: TemporalCondition, current_time: datetime) -> bool:
        """Evaluate off-hours condition"""
        return not self._evaluate_business_hours(condition, current_time)
    
    def _parse_time(self, time_value: Union[str, datetime], condition: TemporalCondition) -> datetime:
                ...
                if len(time_value) == 10:  # Date only
                    ...
                    try:
                        ...
                        if 'T' in time_value or '+' in time_value:
                            ...
                            parsed = datetime.strptime(time_value, condition.datetime_format)
                            return parsed.replace(tzinfo=pytz.timezone(condition.timezone))
                    except ValueError:
                        continue
            except ValueError:
    ...
    def create_temporal_rule(self, rule_id: str, name: str, description: str,
                           temporal_condition: TemporalCondition,
                           base_conditions: List[RuleCondition],
                           actions: List[RuleAction]) -> MCPRule:
        """
    ...
    def validate_building_model_temporal(self, building_model: BuildingModel,
                                       mcp_files: List[str],
        ...
        for mcp_file in loaded_mcp_files:
            for rule in mcp_file.rules:
                ...
                if temporal_condition:
                    ...
                    temporal_satisfied = self.evaluate_temporal_condition(temporal_condition, current_time)
        ...
        return {
            ...
            'temporal_rules_executed': len([r for r in temporal_results if r['temporal_satisfied']])
        }
    
    def _extract_temporal_condition(self, rule: MCPRule) -> Optional[TemporalCondition]:
        """Extract temporal condition from rule"""
        for condition in rule.conditions:
            if condition.type == 'temporal' or condition.property == 'temporal_condition':
    ...
    def get_temporal_schedule(self, mcp_files: List[str],
                             start_date: datetime,
                             end_date: datetime,
        ...
        while current <= end_date:
            ...
            results = self.validate_building_model_temporal(
                BuildingModel(building_id="schedule_test", building_name="Schedule Test", objects=[]),
                mcp_files,
                current
            )
            
            # Add to schedule if any temporal rules would execute
            temporal_rules = [r for r in results['temporal_results'] if r['temporal_satisfied']]
            if temporal_rules:
                schedule.append({
