...
class CMMSDataSynchronizationService:
    ...
    def add_connection(self, connection: CMMSConnection) -> None:
        ...
        logger.info(
            f"Added CMMS connection: {connection.name} ({connection.system_type})"
        )
    ...
    def add_field_mapping(self, mapping: FieldMapping) -> None:
        ...
        logger.info(
            f"Added field mapping: {mapping.cmms_field} -> {mapping.arxos_field}"
        )
            ...
            for work_order_data in work_orders_data:
                ...
                try:
                    ...
                    transformed_data = self._transform_work_order_data(
                        work_order_data, self.field_mappings.get(connection_id, [])
                    )

                    # Create or update work order
                    work_order = self._create_work_order_from_data(
                        transformed_data, connection_id
                    )
    ...
    async def sync_maintenance_schedules(self, connection_id: str) -> SyncResult:
        ...
        try:
            ...
            schedules_data = await self._fetch_maintenance_schedules_from_cmms(
                connection
            )
            ...
            for schedule_data in schedules_data:
                ...
                try:
                    ...
                    transformed_data = self._transform_schedule_data(
                        schedule_data, self.field_mappings.get(connection_id, [])
                    )

                    # Create or update maintenance schedule
                    schedule = self._create_maintenance_schedule_from_data(
                        transformed_data, connection_id
                    )
            ...
            for asset_data in assets_data:
                ...
                try:
                    ...
                    transformed_data = self._transform_asset_data(
                        asset_data, self.field_mappings.get(connection_id, [])
                    )

                    # Create or update asset
                    asset = self._create_asset_from_data(
                        transformed_data, connection_id
                    )
    ...
    async def _fetch_work_orders_from_cmms(
        self, connection: CMMSConnection
    ) -> List[Dict[str, Any]]:
        """Fetch work orders from CMMS API."""
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(endpoint, headers=headers) as response:
                if response.status == 200:
                    ...
                    return (
                        data.get("work_orders", []) if isinstance(data, dict) else data
                    )
                else:
                    raise Exception(f"CMMS API returned status {response.status}")

    async def _fetch_maintenance_schedules_from_cmms(
        self, connection: CMMSConnection
    ) -> List[Dict[str, Any]]:
        """Fetch maintenance schedules from CMMS API."""
        async with aiohttp.ClientSession() as session:
    ...
    async def _fetch_assets_from_cmms(
        self, connection: CMMSConnection
    ) -> List[Dict[str, Any]]:
        """Fetch assets from CMMS API."""
        async with aiohttp.ClientSession() as session:
    ...
    def _transform_work_order_data(
        self, data: Dict[str, Any], mappings: List[FieldMapping]
    ) -> Dict[str, Any]:
    ...
    def _create_work_order_from_data(
        ...
        return WorkOrder(
            ...
            maintenance_type=MaintenanceType(
                data.get("maintenance_type", "corrective")
            ),
    ...
    def _create_maintenance_schedule_from_data(
        self, data: Dict[str, Any], connection_id: str
    ) -> MaintenanceSchedule:
        ...
        return MaintenanceSchedule(
            ...
            next_due_date=self._parse_datetime(data.get("next_due_date"))
            or datetime.now(),
            last_completed=self._parse_datetime(data.get("last_completed")),
        )

    def _create_asset_from_data(
        self, data: Dict[str, Any], connection_id: str
    ) -> Asset:
    ...
    def _update_sync_statistics(self, result: SyncResult) -> None:
        ...
        if result.completed_at:
            ...
            total_syncs = (
                self.statistics["successful_syncs"] + self.statistics["failed_syncs"]
            )
            if total_syncs > 0:
                self.statistics["average_sync_time"] = (
                    self.statistics["average_sync_time"] * (total_syncs - 1) + sync_time
                ) / total_syncs
    ...
    def get_work_orders(
        ...
        if connection_id:
            work_orders = [
                wo for wo in work_orders if wo.connection_id == connection_id
            ]
    ...
    def get_maintenance_schedules(
        self, connection_id: Optional[str] = None, asset_id: Optional[str] = None
    ...
    def get_assets(
        self, connection_id: Optional[str] = None, status: Optional[AssetStatus] = None
    ) -> List[Asset]:
