...
Example:
    ...
    async def record_performance_data(
        ...
        if maintenance_hours:
            asset.total_operating_hours = (asset.total_operating_hours or 0) + maintenance_hours
    ...
    async def get_asset_performance_history(
        ...
        performance_data = [p for p in self.performance_data.values() if p.asset_id == asset_id]
        
        if start_date:
            performance_data = [p for p in performance_data if p.timestamp >= start_date]
        if end_date:
    ...
    async def _check_performance_alerts(self, asset_id: str, performance: AssetPerformance):
        ...
        try:
            ...
            if asset.responsible_person:
                await self.notification_system.send_email_notification(
                    to_email=asset.responsible_person,
                    subject=subject,
                    message=message
                )
    ...
    async def acknowledge_alert(self, alert_id: UUID, acknowledged_by: str) -> Optional[AssetAlert]:
    ...
    async def get_asset_statistics(
        ...
        operational_assets = len([a for a in assets if a.status == AssetStatus.OPERATIONAL])
        maintenance_assets = len([a for a in assets if a.status == AssetStatus.MAINTENANCE])
        ...
        if performance_data:
            avg_uptime = sum(p.uptime_percentage for p in performance_data) / len(performance_data)
            avg_efficiency = sum(p.efficiency_rating for p in performance_data) / len(performance_data)
        ...
        return {
            ...
            "operational_rate": (operational_assets / total_assets * 100) if total_assets > 0 else 0,
    ...
    async def update_maintenance_history(
        ...
        if duration_hours:
            asset.total_operating_hours = (asset.total_operating_hours or 0) + duration_hours
