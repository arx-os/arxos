...
class SVGXRateLimiter:
    ...
    def check_rate_limit(self, user_id: str, endpoint: str, 
                        request_id: Optional[str] = None) -> RateLimitResult:
        """
        ...
        try:
            ...
            logger.error("Rate limit check failed", error=str(e), user_id=user_id, endpoint=endpoint)
    ...
    def _check_user_limit(self, user_id: str, limit_config: Dict[str, Any]) -> RateLimitResult:
    ...
    def _check_endpoint_limit(self, endpoint: str, limit_config: Dict[str, Any]) -> RateLimitResult:
    ...
    def _check_token_bucket_limit(self, limiter_key: str, limit_config: Dict[str, Any]) -> RateLimitResult:
    ...
    def _check_sliding_window_limit(self, limiter_key: str, limit_config: Dict[str, Any]) -> RateLimitResult:
    ...
    def _check_fixed_window_limit(self, limiter_key: str, limit_config: Dict[str, Any]) -> RateLimitResult:
    ...
    def _log_rate_limit_check(self, user_id: str, endpoint: str, result: RateLimitResult, 
    ...
    def _log_rate_limit_exceeded(self, user_id: str, endpoint: str, limit_type: str, 
                                request_id: Optional[str] = None):
    ...
    def clear_rate_limits(self, user_id: Optional[str] = None, endpoint: Optional[str] = None):
        """Clear rate limits for specific user or endpoint."""
        if user_id:
            # Clear user-specific limiters
            keys_to_remove = [key for key in self.limiters.keys() if key.startswith(f"user:{user_id}")]
        ...
        if endpoint:
            ...
            keys_to_remove = [key for key in self.limiters.keys() if key.startswith(f"endpoint:{endpoint}")]
            for key in keys_to_remove:
                del self.limiters[key]
...
def check_rate_limit(user_id: str, endpoint: str, request_id: Optional[str] = None) -> RateLimitResult:
