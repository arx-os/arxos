        ...
        for placement in scenario.equipment_items.iter() {
            if placement.constraint_validation.is_valid {
                commentary.push(ExpertCommentary {
                    ...
                    content: format!("This {} equipment was placed successfully with all constraints satisfied. Notice the positioning relative to structural support and clearance requirements.", 
                        format!("{:?}", placement.equipment.equipment_type)),
                    category: CommentaryCategory::BestPractice,
                    timestamp: None,
                });
            } else {
