...
class DocumentationGenerator:
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    "--title",
                    f"{spec_file.stem.replace('_', ' ').title()} API Documentation",
                ],
                check=True,
                capture_output=True,
            )
    ...
    def generate_markdown_api_docs(self, spec_file: Path, output_path: Path):
        ...
        try:
            ...
            logger.error(
                f"❌ Failed to generate Markdown docs for {spec_file.name}: {e}"
            )
                    ...
                    if method.upper() in ["GET", "POST", "PUT", "DELETE", "PATCH"]:
                        ...
                        if "parameters" in details:
                            ...
                            for param in details["parameters"]:
                                content.append(
                                    f"- `{param['name']}` ({param.get('in', 'unknown')}) - {param.get('description', 'No description')}"
                                )
                            content.append("")
    ...
    def generate_sdk_documentation(self):
        ...
        for language_dir in (self.sdk_path / "generated").iterdir():
            if language_dir.is_dir():
                ...
                self.generate_language_sdk_docs(
                    language, language_dir, language_docs_path
                )
    ...
    def generate_language_sdk_docs(self, language: str, sdk_dir: Path, docs_path: Path):
        ...
        for service_dir in sdk_dir.iterdir():
            if service_dir.is_dir():
                ...
                service_readme = self.generate_service_docs(
                    language, service_name, service_dir
                )
    ...
    def generate_sdk_readme(self, language: str, sdk_dir: Path) -> str:
        ...
        content.append("")
        content.append(
            f"This directory contains the {language.title()} SDKs for all Arxos services."
        )
        content.append("")
    ...
    def generate_service_docs(
        self, language: str, service_name: str, service_dir: Path
    ) -> str:
        ...
        content.append("")
        content.append(
            f"This SDK provides type-safe access to the {service_title} API."
        )
        content.append("")
        ...
        if language == "typescript":
            content.append(
                f"import {{ {service_title.replace(' ', '')}Client }} from '@arxos/{service_name}';"
            )
            content.append("")
            content.append(
                f"const client = new {service_title.replace(' ', '')}Client('http://localhost:8080');"
            )
            content.append("const health = await client.health.getHealth();")
        elif language == "python":
            content.append(
                f"from arxos_{service_name.replace('-', '_')} import {service_title.replace(' ', '')}Client"
            )
            content.append("")
            content.append(
                f"client = {service_title.replace(' ', '')}Client('http://localhost:8080')"
            )
            content.append("health = client.health.get_health()")
        elif language == "go":
            ...
            content.append("")
            content.append(
                f"client := arxos.New{service_title.replace(' ', '')}Client(\"http://localhost:8080\")"
            )
            content.append("health, err := client.Health.GetHealth()")
        content.append("```")
        content.append("")
    ...
    def generate_examples(self):
        ...
        for language_dir in (self.sdk_path / "generated").iterdir():
            if language_dir.is_dir():
                ...
                self.generate_language_examples(
                    language, language_dir, language_examples_path
                )

    def generate_language_examples(
        self, language: str, sdk_dir: Path, examples_path: Path
    ):
    ...
    def generate_error_handling_example(self, language: str) -> str:
        ...
        if language == "typescript":
            content.append(
                "import { ArxBackendClient, ArxosError, AuthenticationError } from '@arxos/arx-backend';"
            )
            content.append("")
            ...
            content.append(
                "from arxos_arx_backend import ArxBackendClient, ArxosError, AuthenticationError"
            )
            content.append("")
    ...
    def generate_getting_started_tutorial(self) -> str:
        ...
        content.append("")
        content.append(
            "This tutorial will guide you through setting up and using Arxos SDKs."
        )
        content.append("")
        ...
        content.append(
            "- Learn about [Error Handling](advanced_usage.md#error-handling)"
        )
        content.append("")
    ...
    def generate_advanced_usage_tutorial(self) -> str:
        ...
        content.append("try {")
        content.append(
            "    const result = await client.projects.createProject(projectData);"
        )
        content.append("} catch (error) {")
        content.append("    if (error instanceof AuthenticationError) {")
        ...
        content.append("")
        content.append(
            "The SDK handles rate limiting automatically, but you can configure it:"
        )
        content.append("")
        content.append("```typescript")
        content.append("const client = new ArxBackendClient('http://localhost:8080', {")
    ...
    def generate_main_index(self) -> str:
        ...
        content.append("")
        content.append(
            "Welcome to the Arxos SDK documentation. This documentation covers all SDKs generated for the Arxos platform."
        )
        content.append("")
        ...
        content.append("")
        content.append(
            "- **[API Documentation](./api/)** - OpenAPI specifications and API references"
        )
        content.append(
            "- **[SDK Documentation](./sdk/)** - Language-specific SDK documentation"
        )
        content.append(
        ...
        content.append("")
        content.append(
            "- **TypeScript/JavaScript** - Full type safety and modern ES6+ features"
        )
        ...
        content.append(
            "- **Java** - Bean validation, Optional types, and Maven packaging"
        )
        ...
        content.append(
            "- **CMMS Service API** - Maintenance management (30+ endpoints)"
        )
        content.append(
            "- **Database Infrastructure API** - Schema management (15+ endpoints)"
        )
        content.append("")
        ...
        content.append("")
        content.append(
            "1. Choose your preferred language from the [SDK Documentation](./sdk/)"
        )
    ...
    def generate_api_index(self) -> str:
        ...
        content.append("")
        content.append(
            "This section contains the API documentation for all Arxos services."
        )
        content.append("")
        ...
        content.append("")
        content.append(
            "- **[Arx Backend API](./arx-backend/)** - Core backend services"
        )

        content.append("- **[CMMS Service API](./arx-cmms/)** - Maintenance management")
        content.append(
            "- **[Database Infrastructure API](./arx-database/)** - Schema management"
        )
        content.append("")
    ...
    def generate_sdk_index(self) -> str:
        ...
        content.append("")
        content.append(
            "- **[TypeScript/JavaScript](./typescript/)** - Full type safety and modern features"
        )
...
def main():
    ...
    parser.add_argument("--sdk-path", help="Path to SDK directory", default="sdk")
