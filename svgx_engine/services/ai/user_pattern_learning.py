...
class UserRecommendation(BaseModel):
    ...
    source_patterns: List[UUID] = Field(
        default_factory=list, description="Source pattern IDs"
    )
    is_dismissed: bool = Field(
        default=False, description="Whether recommendation was dismissed"
    )
    is_applied: bool = Field(
        default=False, description="Whether recommendation was applied"
    )
    created_at: datetime = Field(default_factory=datetime.utcnow)
    expires_at: Optional[datetime] = Field(
        None, description="Recommendation expiration"
    )
...
class UserPatternLearningService:
    ...
    async def get_user_preferences(
        ...
        if preference_type:
            preferences = [
                p for p in preferences if p.preference_type == preference_type
            ]
    ...
    async def get_user_recommendations(
        ...
        recommendations = [
            r for r in self.recommendations.values() if r.user_id == user_id
        ]
        ...
        if active_only:
            recommendations = [
                r for r in recommendations if not r.is_dismissed and not r.is_applied
            ]
            ...
            recommendations = [
                r
                for r in recommendations
                if not r.expires_at or r.expires_at > current_time
            ]

        return sorted(
            recommendations, key=lambda x: (x.priority, x.confidence), reverse=True
        )
    ...
    async def _analyze_patterns(self, user_id: str):
        ...
        recent_actions = await self.get_user_actions(
            user_id=user_id, start_date=datetime.utcnow() - timedelta(days=30)
        )
    ...
    async def _analyze_preference_patterns(
        ...
        for action in actions:
            if action.resource_id:
                resource_counts[action.resource_id] = (
                    resource_counts.get(action.resource_id, 0) + 1
                )

            context_preferences[action.context] = (
                context_preferences.get(action.context, 0) + 1
            )
    ...
    async def _find_existing_pattern(
        ...
        for pattern in self.patterns.values():
            if (
                ...
                and pattern.context == context
                and pattern.pattern_data.get("action_type") == pattern_key
            ):
                return pattern
        return None
    ...
    async def _generate_recommendations(self, user_id: str):
        ...
        for pattern in patterns:
            if pattern.pattern_type == PatternType.FREQUENCY:
                if (
                    pattern.pattern_data.get("action_type")
                    == UserActionType.CREATE.value
                ):
    ...
    async def _update_user_analytics(self, user_id: str):
        ...
        most_used_context = (
            max(context_counts, key=context_counts.get) if context_counts else None
        )
        ...
        collaboration_actions = sum(
            1
            for action in actions
            if action.action_type
            in [UserActionType.SHARE, UserActionType.COMMENT, UserActionType.LIKE]
        )
        collaboration_score = (
            collaboration_actions / total_actions if total_actions > 0 else 0.0
        )
            ...
            "summary": {
                ...
                "active_recommendations": len(
                    [
                        r
                        for r in recommendations
                        if not r.is_dismissed and not r.is_applied
                    ]
                ),
                "efficiency_score": analytics.efficiency_score if analytics else 0.0,
                "collaboration_score": (
                    analytics.collaboration_score if analytics else 0.0
                ),
            },
        }
