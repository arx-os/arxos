...
class CADSystem:
    ...
    def create_point(
        self,
        x: Union[float, Decimal],
        y: Union[float, Decimal],
        z: Optional[Union[float, Decimal]] = None,
    ) -> PrecisionPoint:
    ...
    def create_vector(
        ...
        dz: Optional[Union[float, Decimal]] = None,
    ) -> PrecisionVector:
        """Create a precision vector."""
        return self.precision_system.create_vector(dx, dy, dz)

    def snap_point(
        self, target_point: PrecisionPoint, use_grid: bool = True, use_snap: bool = True
    ...
    def add_constraint(
        ...
        if constraint_type_enum == ConstraintType.DISTANCE:
            ...
            return self.constraint_system.create_distance_constraint(
                entities[0], entities[1], distance
            )

        elif constraint_type_enum == ConstraintType.ANGLE:
            angle = parameters.get("angle", 0.0) if parameters else 0.0
            return self.constraint_system.create_angle_constraint(
                entities[0], entities[1], angle
            )

        elif constraint_type_enum == ConstraintType.PARALLEL:
            return self.constraint_system.create_parallel_constraint(
                entities[0], entities[1]
            )

        elif constraint_type_enum == ConstraintType.PERPENDICULAR:
            return self.constraint_system.create_perpendicular_constraint(
                entities[0], entities[1]
            )
            ...
            return self.constraint_system.create_coincident_constraint(
                entities[0], entities[1]
            )
    ...
    def create_linear_dimension(
        ...
        offset: Union[float, Decimal] = 10.0,
    ) -> str:
        """Create a linear dimension."""
        return self.dimensioning_system.create_linear_dimension(
            start_point, end_point, style_name, offset
        )

    def create_radial_dimension(
        self,
        center_point: PrecisionPoint,
        radius_point: PrecisionPoint,
        style_name: str = "standard",
    ) -> str:
        """Create a radial dimension."""
        return self.dimensioning_system.create_radial_dimension(
            center_point, radius_point, style_name
        )

    def create_angular_dimension(
        self,
        center_point: PrecisionPoint,
        start_vector: PrecisionVector,
        end_vector: PrecisionVector,
        style_name: str = "standard",
    ) -> str:
        """Create an angular dimension."""
        return self.dimensioning_system.create_angular_dimension(
            center_point, start_vector, end_vector, style_name
        )
