...
class PrecisionSettings:
    ...
    def _validate_precision_settings(self):
        ...
        if self.millimeter_precision <= self.micron_precision:
            raise ValueError(
                "Millimeter precision must be greater than micron precision"
            )
        if self.micron_precision <= self.nanometer_precision:
            raise ValueError(
                "Micron precision must be greater than nanometer precision"
            )

    def _configure_rounding(self):
        """Configure rounding mode."""
        rounding_map = {"HALF_UP": ROUND_HALF_UP, "DOWN": ROUND_DOWN, "UP": ROUND_UP}
...
class PrecisionMath:
    ...
    def round_to_precision(
        self, value: Union[float, Decimal], precision: Optional[Decimal] = None
    ) -> Decimal:
        """
    ...
    def power(
        self,
        base: Union[float, Decimal],
        exponent: Union[float, Decimal],
        precision: Optional[Decimal] = None,
    ) -> Decimal:
        """
    ...
    def distance_2d(
        self,
        x1: Union[float, Decimal],
        y1: Union[float, Decimal],
        x2: Union[float, Decimal],
        y2: Union[float, Decimal],
        precision: Optional[Decimal] = None,
    ) -> Decimal:
        """
    ...
    def distance_3d(
        ...
        x1: Union[float, Decimal],
        y1: Union[float, Decimal],
        z1: Union[float, Decimal],
        x2: Union[float, Decimal],
        y2: Union[float, Decimal],
        z2: Union[float, Decimal],
        precision: Optional[Decimal] = None,
    ) -> Decimal:
        """
        ...
        sum_squared = self.add(
            dx_squared, self.add(dy_squared, dz_squared, precision), precision
        )
        return self.sqrt(sum_squared, precision)
    ...
    def min(
        self, values: List[Union[float, Decimal]], precision: Optional[Decimal] = None
    ) -> Decimal:
        """
    ...
    def validate_geometric_calculation(
        self,
        operation: str,
        result: Decimal,
        ...
        if not is_valid and self.settings.strict_mode:
            error_msg = (
                f"Geometric calculation '{operation}' failed precision validation"
            )
            if self.settings.log_precision_errors:
...
class PrecisionValidator:
    ...
    def validate_distance_precision(
        self, distance: Union[float, Decimal], precision: Optional[Decimal] = None
    ) -> bool:
        """
    ...
    def validate_angle_precision(
        self, angle: Union[float, Decimal], precision: Optional[Decimal] = None
    ) -> bool:
        """
