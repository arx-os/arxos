    ...
    fn previous_component(&mut self) {
        if !self.components.is_empty() {
            self.selected_component =
                (self.selected_component + self.components.len() - 1) % self.components.len();
        }
    }
...
fn check_git_component() -> ComponentHealth {
    ...
    match crate::git::BuildingGitManager::new(
        ".",
        "test",
        crate::git::GitConfig {
...
fn check_yaml_component() -> ComponentHealth {
    ...
    let serializer = BuildingYamlSerializer::new();
    match serializer.to_yaml(&test_data) {
        Ok(yaml_str) => match serde_yaml::from_str::<BuildingData>(&yaml_str) {
            Ok(_) => {
...
/// Render header
fn render_header<'a>(state: &'a HealthDashboardState, theme: &'a Theme) -> Paragraph<'a> {
...
fn render_footer<'a>(theme: &'a Theme) -> Paragraph<'a> {
    let help_text =
        "←/→: Navigate | r: Refresh | d: Toggle Details | a: Toggle Auto-refresh | q: Quit";
