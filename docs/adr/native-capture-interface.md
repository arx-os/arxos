...
| **Related** | [`adr-capture-model.md`](capture-model.md) · agent `lidar.import` · `import_lidar_path` |
...
1. Size/point refuse per `docs/reference/resource-limits.md`.  
...
**Implemented on agent:** optional `provenance` object on `lidar.import` is stamped onto building metadata and rooms (`capture_client`, …). Pipeline still uses `capture_source=lidar_file`. See [native-file-handoff.md](../reference/native-file-handoff.md).
...
3. ~~Repo split~~ — companion in **`ios`** (Decision 12); contract in [agent-client-interface.md](../reference/agent-client-interface.md).  
4. **Next:** Develop RoomPlan/ARKit + polish in **arx-os/ios**; lab loop [ios-lab-loop.md](../reference/ios-lab-loop.md).
