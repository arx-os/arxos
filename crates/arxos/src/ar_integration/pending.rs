...
impl PendingEquipmentManager {
    ...
    fn find_or_create_room(
        ...
        if let Some((wing_idx, room_idx)) = floor.wings.iter().enumerate().find_map(|(wi, wing)| {
            wing.rooms
                .iter()
        ...
        let wing_index =
            if let Some(idx) = floor.wings.iter().position(|w| w.name == default_wing_name) {
                idx
        ...
        if let (Some(room_name), Some((wing_idx, room_idx))) = (room_name_opt, room_location) {
            if let Some(wing) = floor.wings.get_mut(wing_idx) {
                ...
                warn!(
                    ...
                    wing_idx, equipment_id, room_name
                );
            }
        }
