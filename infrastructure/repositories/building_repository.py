...
class SQLAlchemyBuildingRepository(
    BaseRepository[Building, BuildingModel], BuildingRepository
):
    ...
    def get_all(self) -> List[Building]:
        ...
        try:
            models = (
                self.session.query(BuildingModel)
                .filter(BuildingModel.deleted_at.is_(None))
                .order_by(BuildingModel.name)
                .all()
            )
    ...
    def find_by_name(self, name: str) -> Optional[Building]:
        ...
        try:
            model = (
                self.session.query(BuildingModel)
                .filter(
                    and_(BuildingModel.name == name, BuildingModel.deleted_at.is_(None))
                )
                .first()
            )
    ...
    def get_building_statistics(self) -> dict:
        ...
        try:
            ...
            return {"total_buildings": total_buildings, "status_counts": status_counts}
        except Exception as e:
    ...
    def _model_to_entity(self, model: BuildingModel) -> Building:
        ...
        if model.width is not None and model.length is not None:
            dimensions = Dimensions(
                width=model.width, length=model.length, height=model.height
            )
