...
fn handle_ar_export_command(
    ...
    println!(
        "📤 Exporting building '{}' to AR format: {}",
        building, format
    );
...
fn handle_pending_command(command: PendingCommands) -> Result<(), Box<dyn std::error::Error>> {
    match command {
        ...
        PendingCommands::Reject { pending_id } => handle_pending_reject_command(&pending_id),
        PendingCommands::BatchConfirm {
            pending_ids,
            building,
            commit,
...
pub fn handle_ar_integrate_command(
    ...
    println!(
        "📱 Integrating AR scan data for room: {} on floor: {}",
        room, floor
    );
    ...
    let ar_scan_data =
        crate::ar_integration::convert_mobile_ar_data(scan_data_bytes, room.clone(), floor)?;
    ...
    println!(
        "  📦 Equipment added: {}",
        integration_result.equipment_added
    );
    println!(
        "  🔄 Equipment updated: {}",
        integration_result.equipment_updated
    );
    ...
    if commit {
        let commit_message = message
            .unwrap_or_else(|| format!("Integrate AR scan for room {} on floor {}", room, floor));
...
fn handle_pending_list_command(
    building: &str,
    floor: Option<i32>,
    verbose: bool,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, item) in pending_items.iter().enumerate() {
        ...
        if verbose {
            ...
            println!(
                "      Detected: {}",
                item.detected_at.format("%Y-%m-%d %H:%M:%S")
            );
        } else {
            println!(
                "      Floor: {} | Confidence: {:.2}",
                item.floor_level, item.confidence
            );
        }
    }
...
fn handle_pending_confirm_command(
    pending_id: &str,
    building: &str,
    commit: bool,
) -> Result<(), Box<dyn std::error::Error>> {
    info!(
        "Confirming pending equipment: {} for building: {}",
        pending_id, building
    );
...
fn handle_pending_batch_confirm_command(
    pending_ids: Vec<String>,
    building: &str,
    commit: bool,
) -> Result<(), Box<dyn std::error::Error>> {
    info!(
        "Batch confirming {} pending equipment items",
        pending_ids.len()
    );
    println!(
        "✅ Batch confirming {} pending equipment items",
    ...
    if commit {
        let commit_message = format!(
            "Batch confirm {} pending equipment items",
            equipment_ids.len()
        );
        persistence.save_and_commit(&building_data, Some(&commit_message))?;
        info!("Changes committed to Git");
