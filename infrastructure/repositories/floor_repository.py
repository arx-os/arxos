...
class SQLAlchemyFloorRepository(BaseRepository[Floor, FloorModel], FloorRepository):
    ...
    def get_by_id(self, floor_id: FloorId) -> Optional[Floor]:
        ...
        try:
            model = (
                self.session.query(FloorModel)
                .filter(
                    and_(
                        FloorModel.id == floor_id.value, FloorModel.deleted_at.is_(None)
                    )
                )
                .first()
            )
    ...
    def get_by_building_and_number(
        self, building_id: BuildingId, floor_number: int
    ) -> Optional[Floor]:
    ...
    def get_by_status(self, status: FloorStatus) -> List[Floor]:
        ...
        try:
            models = (
                self.session.query(FloorModel)
                .filter(
                    and_(FloorModel.status == status, FloorModel.deleted_at.is_(None))
                )
                .order_by(FloorModel.floor_number)
                .all()
            )
