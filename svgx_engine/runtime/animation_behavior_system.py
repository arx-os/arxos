...
class AnimationBehaviorSystem:
    ...
    async def _update_animation(self, animation: Animation, current_time: float):
        ...
        try:
            ...
            iteration_progress = (
                animation_time % iteration_duration
            ) / iteration_duration
            ...
            easing_func = self.easing_functions.get(
                animation.config.easing, self.easing_functions[EasingFunction.LINEAR]
            )
            ...
            await self._trigger_animation_event(
                animation, "progress", {"progress": eased_progress}
            )
    ...
    def _interpolate_keyframes(self, animation: Animation, progress: float) -> Any:
        ...
        easing_func = self.easing_functions.get(
            prev_keyframe.easing, self.easing_functions[EasingFunction.LINEAR]
        )
        eased_factor = easing_func(factor)

        # Interpolate values
        return self._interpolate_values(
            prev_keyframe.value, next_keyframe.value, eased_factor
        )

    def _interpolate_values(
        self, start_value: Any, end_value: Any, factor: float
    ) -> Any:
    ...
    async def _trigger_animation_event(
        self,
        animation: Animation,
        event_type: str,
        ...
        try:
            with self._lock:
                ...
                if animation.target_element in self.animations_by_element:
                    self.animations_by_element[animation.target_element].discard(
                        animation_id
                    )

                if animation.animation_type in self.animations_by_type:
                    self.animations_by_type[animation.animation_type].discard(
                        animation_id
                    )
        ...
        try:
            with self._lock:
                if animation_id:
                    ...
                    playing_animations = len(
                        self.animations_by_status.get(AnimationStatus.PLAYING, set())
                    )
                    completed_animations = len(
                        self.animations_by_status.get(AnimationStatus.COMPLETED, set())
                    )
