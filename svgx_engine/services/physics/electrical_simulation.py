...
class ElectricalSimulationService:
    ...
    def analyze_electrical_circuit(
        ...
        try:
            ...
            result.max_voltage = (
                max(result.voltage_field) if result.voltage_field else 0.0
            )
            result.max_current = (
                max(np.linalg.norm(current) for current in result.current_field)
    ...
    def _solve_electrical_circuit(
        ...
        for bc in request.boundary_conditions:
            if bc.type == "voltage":
                ...
                for node_idx in self._find_nodes_in_boundary(bc.location, request.mesh):
                    current_field[node_idx] = np.array(
                        bc.value.get("current", [0.0, 0.0, 0.0])
                    )

        # Solve circuit equations (simplified)
        for iteration in range(100):
            voltage_correction = self._voltage_correction(
                voltage_field, current_field, request
            )
            voltage_field += voltage_correction

            current_correction = self._current_correction(
                voltage_field, current_field, request
            )
            current_field += current_correction

            if (
                np.max(np.abs(voltage_correction)) < 1e-6
                and np.max(np.abs(current_correction)) < 1e-6
            ):
                break

        return voltage_field.tolist(), current_field.tolist()

    def _solve_transient_circuit(
        self, request: ElectricalAnalysisRequest
    ) -> Tuple[List[float], List[Tuple[float, float, float]]]:
        ...
        for step in range(time_steps):
            ...
            voltage_correction = self._transient_voltage_correction(
                voltage_field, current_field, request, step * dt
            )
            voltage_field += voltage_correction * dt

            current_correction = self._transient_current_correction(
                voltage_field, current_field, request, step * dt
            )
    ...
    def _solve_frequency_domain_circuit(
        self, request: ElectricalAnalysisRequest
    ) -> Tuple[List[float], List[Tuple[float, float, float]]]:
        ...
        if request.frequency_range:
            ...
            for freq in frequencies:
                voltage_correction = self._frequency_voltage_correction(
                    voltage_field, current_field, request, freq
                )
                voltage_field += voltage_correction

                current_correction = self._frequency_current_correction(
                    voltage_field, current_field, request, freq
                )
                current_field += current_correction
    ...
    def _solve_fault_circuit(
        ...
        for iteration in range(50):
            voltage_correction = self._fault_voltage_correction(
                voltage_field, current_field, request
            )
            voltage_field += voltage_correction

            current_correction = self._fault_current_correction(
                voltage_field, current_field, request
            )
            current_field += current_correction

            if (
                np.max(np.abs(voltage_correction)) < 1e-5
                and np.max(np.abs(current_correction)) < 1e-5
            ):
                break

        return voltage_field.tolist(), current_field.tolist()

    def _calculate_power_field(
        self,
        voltage_field: List[float],
        current_field: List[Tuple[float, float, float]],
    ) -> List[float]:
    ...
    def _calculate_electric_field(
        self, voltage_field: List[float], mesh: List[Dict[str, Any]]
    ...
    def _transient_voltage_correction(
        self,
        voltage_field: np.ndarray,
        current_field: np.ndarray,
        request: ElectricalAnalysisRequest,
    ...
    def _transient_current_correction(
        self,
        voltage_field: np.ndarray,
        current_field: np.ndarray,
        request: ElectricalAnalysisRequest,
    ...
    def _frequency_voltage_correction(
        self,
        voltage_field: np.ndarray,
        current_field: np.ndarray,
        request: ElectricalAnalysisRequest,
    ...
    def _frequency_current_correction(
        self,
        voltage_field: np.ndarray,
        current_field: np.ndarray,
        request: ElectricalAnalysisRequest,
    ...
    def _find_component_nodes(
        self, component: ElectricalComponent, mesh: List[Dict[str, Any]]
    ) -> List[int]:
    ...
    def _validate_request(self, request: ElectricalAnalysisRequest) -> None:
        ...
        if request.circuit_type not in [
            CircuitType.DC,
            CircuitType.AC,
