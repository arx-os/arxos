...
class BaseRepository(ABC, Generic[T, M]):
    ...
    def save(self, entity: T) -> T:
        ...
        try:
            ...
            raise RepositoryError(
                f"Failed to save {self.entity_class.__name__}: {str(e)}"
            )
    ...
    def get_by_id(self, entity_id) -> Optional[T]:
        ...
        try:
            ...
            raise RepositoryError(
                f"Failed to retrieve {self.entity_class.__name__}: {str(e)}"
            )
    ...
    def find_all(
        ...
        try:
            ...
            raise RepositoryError(
                f"Failed to retrieve {self.entity_class.__name__}s: {str(e)}"
            )
    ...
    def count(self) -> int:
        ...
        try:
            ...
            raise RepositoryError(
                f"Failed to count {self.entity_class.__name__}s: {str(e)}"
            )
    ...
    def hard_delete(self, entity_id) -> bool:
        ...
        try:
            ...
            self.session.rollback()
            raise RepositoryError(
                f"Failed to hard delete {self.entity_class.__name__}: {str(e)}"
            )
    ...
    def exists(self, entity_id) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error checking existence of {self.entity_class.__name__}: {e}"
            )
            raise RepositoryError(
                f"Failed to check existence of {self.entity_class.__name__}: {str(e)}"
            )
