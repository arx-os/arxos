        ...
        match self.command {
            ...
            Commands::Render { building, interactive, .. } => {
                if interactive {
                    ...
                    {
                        crate::render_3d::start_interactive_renderer(&building)?;
                    }
                    #[cfg(not(feature = "render3d"))]
                    {
                        println!("❌ Interactive 3D rendering requires --features render3d");
                        return Err(arx::error::ArxError::ConfigError("Render3D feature not enabled".to_string()));
                    }
                } else {
