...
impl CounterStorage {
    ...
    pub fn load() -> Result<Self> {
        ...
        let content = fs::read_to_string(&counters_path)
            .with_context(|| format!("Failed to read counters file: {}", counters_path.display()))?;
        
        let storage: CounterStorage = toml::from_str(&content)
            .with_context(|| format!("Failed to parse counters file: {}", counters_path.display()))?;
    ...
    pub fn save(&self) -> Result<()> {
        ...
        if let Some(parent) = counters_path.parent() {
            fs::create_dir_all(parent)
                .with_context(|| format!("Failed to create .arxos directory: {}", parent.display()))?;
        }

        let content = toml::to_string_pretty(self)
            .context("Failed to serialize counters to TOML")?;
        
        fs::write(&counters_path, content)
            .with_context(|| format!("Failed to write counters file: {}", counters_path.display()))?;
