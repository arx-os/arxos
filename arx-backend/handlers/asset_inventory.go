...
func CreateBuildingAsset(w http.ResponseWriter, r *http.Request) {
	...
	if cacheService != nil {
		...
		cacheService.InvalidatePattern(fmt.Sprintf("assets:list:building:%s*", asset.BuildingID))
		// Invalidate asset relationships cache
		cacheService.InvalidatePattern(fmt.Sprintf("assets:relationships:building:%s*", asset.BuildingID))
	}
...
func GetBuildingAssets(w http.ResponseWriter, r *http.Request) {
	...
	cacheKey := fmt.Sprintf("assets:list:building:%s", buildingID)
	if system != "" {
		cacheKey += fmt.Sprintf(":system:%s", system)
	}
	if assetType != "" {
		cacheKey += fmt.Sprintf(":type:%s", assetType)
	}
	if status != "" {
	...
	if cacheService != nil {
		cacheService.Set(cacheKey, assets, 5*time.Minute)
...
func GetBuildingAsset(w http.ResponseWriter, r *http.Request) {
	...
	if cacheService != nil {
		cacheKey := fmt.Sprintf("asset:details:%s", assetID)
		cacheService.Set(cacheKey, asset, 10*time.Minute)
		w.Header().Set("X-Cache", "MISS")
	}
...
func UpdateBuildingAsset(w http.ResponseWriter, r *http.Request) {
	...
	if cacheService != nil {
		...
		// Invalidate asset list cache for this building
		cacheService.InvalidatePattern(fmt.Sprintf("assets:list:building:%s*", currentAsset.BuildingID))
		// Invalidate asset relationships cache
		cacheService.InvalidatePattern(fmt.Sprintf("assets:relationships:building:%s*", currentAsset.BuildingID))
	}
