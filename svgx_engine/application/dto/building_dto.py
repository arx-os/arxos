...
class DimensionsDTO(BaseModel):
    ...
    volume: Optional[float] = Field(
        None, description="Calculated volume in cubic meters"
    )
...
class UpdateBuildingRequestDTO(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=255, description="Building name"
    )
...
class BuildingSearchRequestDTO(BaseModel):
    ...
    coordinates: Optional[CoordinatesDTO] = Field(
        None, description="Search by coordinates"
    )
...
def status_to_dto(status: Status) -> StatusDTO:
    ...
    return StatusDTO(value=status.value, description=status.description)
...
def create_request_dto_to_internal(
    ...
    return CreateBuildingRequest(
        ...
        cost=Money(amount=dto.cost.amount, currency=dto.cost.currency),
