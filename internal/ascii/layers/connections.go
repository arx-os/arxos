...
func (c *ConnectionLayer) Render(viewport abim.Viewport) [][]rune {
...
func (c *ConnectionLayer) renderConnection(buffer [][]rune, conn ConnectionPath, vp abim.Viewport) {
...
func (c *ConnectionLayer) renderPathSegment(buffer [][]rune, from, to models.Point, char rune, vp abim.Viewport) {
...
func (c *ConnectionLayer) renderJunction(buffer [][]rune, point models.Point, connType connections.ConnectionType, vp abim.Viewport) {
	// Convert to viewport coordinates
...
func (c *ConnectionLayer) renderConnectionLabel(buffer [][]rune, conn ConnectionPath, vp abim.Viewport) {
...
func (c *ConnectionLayer) SetDirty(regions []abim.Region) {
	c.dirty = regions
}
...
func (c *ConnectionLayer) SetShowLabels(show bool) {
	c.showLabels = show
	c.dirty = []abim.Region{{0, 0, 1000, 1000}} // Force redraw
}
	...
	for i := range c.connections {
		if c.connections[i].ID == connectionID {
			...
			for _, point := range c.connections[i].Points {
				x := int(point.X)
				y := int(point.Y)
				c.dirty = append(c.dirty, abim.Region{x-1, y-1, 3, 3})
			}
			break
		}
	}
...
func (c *ConnectionLayer) GetConnectionsAt(x, y int, vp abim.Viewport, tolerance float64) []ConnectionPath {
