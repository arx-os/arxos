...
class EventDrivenBehaviorEngine:
            ...
            for handler in handlers:
                ...
                try:
                    ...
                    handler_result = await asyncio.wait_for(
                        self._execute_handler(handler, event), timeout=handler.timeout
                    )
                    results.append(handler_result)

                except asyncio.TimeoutError:
                    logger.error(
                        f"Handler {handler.id} timed out after {handler.timeout}s"
                    )
    ...
    async def _execute_handler(
        ...
        try:
            ...
            return {"handler_id": handler.id, "success": True, "result": result}

        except Exception as e:
            logger.error(f"Handler {handler.id} execution failed: {e}")
            return {"handler_id": handler.id, "success": False, "error": str(e)}
    ...
    def _update_event_stats(self, event_result: EventResult):
        ...
        with self.processing_lock:
            ...
            self.event_stats["avg_processing_time"] = (
                current_avg * (total_events - 1) + event_result.processing_time
            ) / total_events
