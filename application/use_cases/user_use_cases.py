        ...
        try:
            ...
            if not request.username or len(request.username.strip()) == 0:
                return CreateUserResponse(
                    success=False, error_message="Username is required"
                )

            if not request.email or len(request.email.strip()) == 0:
                return CreateUserResponse(
                    success=False, error_message="Email is required"
                )
            ...
            if request.phone_number:
                try:
                    ...
                    return CreateUserResponse(
                        success=False, error_message="Invalid phone number format"
                    )
            ...
            return CreateUserResponse(
                success=False, error_message=f"User already exists: {str(e)}"
            )
        except InvalidUserError as e:
            return CreateUserResponse(
                success=False, error_message=f"Invalid user data: {str(e)}"
            )
        ...
        try:
            ...
            if not request.user_id or len(request.user_id.strip()) == 0:
                return UpdateUserResponse(
                    success=False, error_message="User ID is required"
                )
            ...
            if not user:
                return UpdateUserResponse(success=False, error_message="User not found")

            # Update user fields
            if request.username is not None:
                if len(request.username.strip()) == 0:
                    return UpdateUserResponse(
                        success=False, error_message="Username cannot be empty"
                    )
...
class GetUserUseCase:
    ...
    def execute(self, user_id: str) -> GetUserResponse:
        ...
        try:
            ...
            if not user:
                return GetUserResponse(success=False, error_message="User not found")
            ...
            return GetUserResponse(success=True, user=user_dict)
            ...
            if role:
                ...
                try:
                    ...
                    return ListUsersResponse(
                        success=False, error_message=f"Invalid user role: {role}"
                    )
            elif active_only:
...
class DeleteUserUseCase:
    ...
    def execute(self, user_id: str) -> DeleteUserResponse:
        ...
        try:
            ...
            if not user:
                return DeleteUserResponse(success=False, error_message="User not found")
