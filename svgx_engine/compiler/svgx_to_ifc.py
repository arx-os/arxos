...
Example:
    ...
    def _generate_ifc_content(self, elements: List) -> str:
        ...
        for element in elements:
            if element.arx_object:
                ifc_lines.extend(self._generate_object_entities(element, entity_id, entity_id - 1))
    ...
    def _generate_object_entities(self, element, start_id: int, container_id: int) -> List[str]:
        ...
        if 'room' in obj_type:
            ...
            entities.extend(self._generate_electrical_entity(current_id, container_id, obj))
            current_id += 1
        elif 'mechanical' in obj_type:
            entities.extend(self._generate_mechanical_entity(current_id, container_id, obj))
            current_id += 1
        else:
            # Generic object
            entities.extend(self._generate_generic_entity(current_id, container_id, obj))
            current_id += 1
        
        return entities
    
    def _generate_room_entity(self, entity_id: int, container_id: int, obj) -> List[str]:
        """Generate IFC room entity."""
        return [
            f"#{entity_id}=IFCSPACE('{obj.object_id}',$,{obj.object_type},$,$,#{container_id},$,$,.ELEMENT.,$,$,$);"
        ]
    
    def _generate_wall_entity(self, entity_id: int, container_id: int, obj) -> List[str]:
        """Generate IFC wall entity."""
        return [
            f"#{entity_id}=IFCWALL('{obj.object_id}',$,{obj.object_type},$,$,#{container_id},$,$,.ELEMENT.,$);"
        ]
    
    def _generate_electrical_entity(self, entity_id: int, container_id: int, obj) -> List[str]:
        """Generate IFC electrical entity."""
        return [
            f"#{entity_id}=IFCDISTRIBUTIONELEMENT('{obj.object_id}',$,{obj.object_type},$,$,#{container_id},$,$,.ELEMENT.,$);"
        ]
    
    def _generate_mechanical_entity(self, entity_id: int, container_id: int, obj) -> List[str]:
        """Generate IFC mechanical entity."""
        return [
            f"#{entity_id}=IFCDISTRIBUTIONELEMENT('{obj.object_id}',$,{obj.object_type},$,$,#{container_id},$,$,.ELEMENT.,$);"
        ]
    
    def _generate_generic_entity(self, entity_id: int, container_id: int, obj) -> List[str]:
