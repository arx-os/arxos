    ...
    match type_lower.as_str() {
        ...
        _ => {
            ...
            if type_lower.contains("light")
                || type_lower.contains("lamp")
                || type_lower.contains("fixture")
            {
                "IFCLIGHTFIXTURE".to_string()
            } else if type_lower.contains("air")
                || type_lower.contains("vent")
                || type_lower.contains("hvac")
            {
...
mod tests {
    ...
    const VALID_IFC_ID_RANGE: Range<u32> = 10000..910000;
    ...
    fn test_universal_path_to_ifc_entity_id_deterministic() {
        ...
        assert!(
            VALID_IFC_ID_RANGE.contains(&id1),
    ...
    fn test_universal_path_to_ifc_entity_id_range() {
        ...
        assert!(VALID_IFC_ID_RANGE.contains(&id1));
        assert!(VALID_IFC_ID_RANGE.contains(&id2));
    }
    ...
    fn test_map_equipment_type_string_to_ifc() {
        ...
        assert_eq!(map_equipment_type_string_to_ifc("hvac"), "IFCAIRTERMINAL");
        assert_eq!(
            map_equipment_type_string_to_ifc("Electrical"),
            "IFCLIGHTFIXTURE"
        );
        assert_eq!(
            map_equipment_type_string_to_ifc("plumbing"),
            "IFCFLOWTERMINAL"
        );
        assert_eq!(
            map_equipment_type_string_to_ifc("network"),
            "IFCCABLECARRIERSEGMENT"
        );
        assert_eq!(
            map_equipment_type_string_to_ifc("furniture"),
            "IFCFURNISHINGELEMENT"
        );

        // Pattern matching
        assert_eq!(
            map_equipment_type_string_to_ifc("light fixture"),
            "IFCLIGHTFIXTURE"
        );
        assert_eq!(
            map_equipment_type_string_to_ifc("air terminal"),
            "IFCAIRTERMINAL"
        );
        ...
        // Default case
        assert_eq!(
            map_equipment_type_string_to_ifc("unknown"),
    ...
    fn test_map_equipment_type_edge_cases() {
        // Empty string
        assert_eq!(
            map_equipment_type_string_to_ifc(""),
            "IFCDISTRIBUTIONELEMENT"
        );
        ...
        assert_eq!(
            map_equipment_type_string_to_ifc("LED Light"),
            "IFCLIGHTFIXTURE"
        );
        assert_eq!(
            map_equipment_type_string_to_ifc("HVAC Unit"),
            "IFCAIRTERMINAL"
        );
        ...
        assert_eq!(
            map_equipment_type_string_to_ifc("Control Valve"),
            "IFCVALVE"
        );
        assert_eq!(
            map_equipment_type_string_to_ifc("Electrical Panel"),
    ...
    fn test_universal_path_to_ifc_entity_id_distinct_paths() {
        ...
        assert!(VALID_IFC_ID_RANGE.contains(&id1));
        assert!(VALID_IFC_ID_RANGE.contains(&id2));
        assert!(VALID_IFC_ID_RANGE.contains(&id3));
    ...
    fn test_universal_path_to_ifc_entity_id_empty_string() {
        ...
        assert!(VALID_IFC_ID_RANGE.contains(&id));
    }
