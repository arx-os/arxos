...
class PrecisionUndoRedo:
    ...
    def create_state(
        ...
        try:
            ...
            if "coordinates" in data or "precision_coordinates" in data:
                ...
                state.precision_violations = validation_result.get(
    ...
    def _validate_state_coordinates(self, state: PrecisionState) -> Dict[str, Any]:
        ...
        validation_result = {"is_valid": True, "errors": [], "precision_violations": []}
            ...
            if "precision_coordinates" in state.data:
                for i, coord_data in enumerate(state.data["precision_coordinates"]):
                    if isinstance(coord_data, dict):
                        coord = PrecisionCoordinate(
                            coord_data["x"], coord_data["y"], coord_data.get("z", 0.0)
                        )
                    else:
                        coord = coord_data  # Assume it's already a PrecisionCoordinate

                    coord_validation = self.coordinate_validator.validate_coordinate(
                        coord
                    )
                    if not coord_validation.is_valid:
                        validation_result["is_valid"] = False
                        validation_result["errors"].append(
                            f"Coordinate {i}: {coord_validation.errors}"
                        )

                    # Check precision violations
                    precision_violations = self._check_precision_violations(coord)
                    if precision_violations:
                        validation_result["precision_violations"].extend(
                            precision_violations
                        )
            ...
            # Check for legacy coordinates
                ...
                for i, coord in enumerate(state.data["coordinates"]):
                    if isinstance(coord, list) and len(coord) >= 2:
                        ...
                        coord_validation = (
                            self.coordinate_validator.validate_coordinate(
                                precision_coord
                            )
                        )
                        if not coord_validation.is_valid:
                        ...
                        # Check precision violations
                        precision_violations = self._check_precision_violations(
                            precision_coord
                        )
                        if precision_violations:
                            validation_result["precision_violations"].extend(
    ...
    def _check_precision_violations(self, coord: PrecisionCoordinate) -> List[str]:
        ...
        if abs(coord.x - round(coord.x / precision_value) * precision_value) > 1e-10:
            violations.append(
                f"X-coordinate {coord.x} not at precision level {precision_value}"
            )
        if abs(coord.y - round(coord.y / precision_value) * precision_value) > 1e-10:
            violations.append(
                f"Y-coordinate {coord.y} not at precision level {precision_value}"
            )
        if abs(coord.z - round(coord.z / precision_value) * precision_value) > 1e-10:
            violations.append(
                f"Z-coordinate {coord.z} not at precision level {precision_value}"
            )
    ...
    def push_operation(
        ...
        try:
            # Generate entry ID
            entry_id = (
                f"{operation_type.value}_{self.operation_count}_{int(time.time())}"
            )
            ...
            entry = UndoRedoEntry(
                ...
                validation_result=self._validate_entry(
                    entry_id, before_state, after_state
                ),
            )
    ...
    def _validate_entry(
        self,
        entry_id: str,
        before_state: Optional[PrecisionState],
        after_state: Optional[PrecisionState],
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if before_state:
                ...
                if not before_validation["is_valid"]:
                    validation_result["errors"].append(
                        f"Before state validation failed: {before_validation['errors']}"
                    )
            ...
            if after_state:
                ...
                if not after_validation["is_valid"]:
                    validation_result["errors"].append(
                        f"After state validation failed: {after_validation['errors']}"
                    )

            # Check precision preservation
            if before_state and after_state:
                precision_preserved = self._check_precision_preservation(
                    before_state, after_state
                )
                validation_result["precision_preserved"] = precision_preserved
                if not precision_preserved:
                    validation_result["errors"].append(
                        "Precision not preserved between states"
                    )

            # Overall validation
            validation_result["is_valid"] = (
                validation_result["before_state_valid"]
                and validation_result["after_state_valid"]
                and validation_result["precision_preserved"]
            )
    ...
    def _check_precision_preservation(
        self, before_state: PrecisionState, after_state: PrecisionState
    ) -> bool:
        ...
        try:
            ...
            if (
                "precision_coordinates" in before_state.data
                and "precision_coordinates" in after_state.data
            ):
                ...
                for i, (before_coord, after_coord) in enumerate(
                    zip(before_coords, after_coords)
                ):
                    if isinstance(before_coord, dict):
                        before_precision = PrecisionCoordinate(
                            before_coord["x"],
                            before_coord["y"],
                            before_coord.get("z", 0.0),
                        )
                    else:
                        before_precision = before_coord

                    if isinstance(after_coord, dict):
                        after_precision = PrecisionCoordinate(
                            after_coord["x"],
                            after_coord["y"],
                            after_coord.get("z", 0.0),
                        )
                    else:
                    ...
                    precision_value = self.config.get_precision_value()
                    if (
                        abs(before_precision.x - after_precision.x) > precision_value
                        or abs(before_precision.y - after_precision.y) > precision_value
                        or abs(before_precision.z - after_precision.z) > precision_value
                    ):
                        return False
    ...
    def _validate_entry_for_undo(self, entry: UndoRedoEntry) -> bool:
        ...
        try:
            ...
            if not validation_result["is_valid"]:
                logger.error(
                    f"Before state validation failed for entry {entry.entry_id}"
                )
                return False
    ...
    def _validate_entry_for_redo(self, entry: UndoRedoEntry) -> bool:
        ...
        try:
            ...
            if not validation_result["is_valid"]:
                logger.error(
                    f"After state validation failed for entry {entry.entry_id}"
                )
                return False
    ...
    def _perform_undo(self, entry: UndoRedoEntry):
        ...
        try:
            if entry.before_state:
                ...
                self.current_state[entry.before_state.object_id] = (
                    entry.before_state.data
                )
    ...
    def _restore_state(self, state: PrecisionState):
        ...
        try:
            ...
            if (
                ...
                and self.config.should_fail_on_violation()
            ):
                raise ValueError(
                    f"Cannot restore invalid state: {validation_result['errors']}"
                )
    ...
    def _deserialize_entry(self, entry_data: Dict[str, Any]) -> UndoRedoEntry:
        ...
        return UndoRedoEntry(
            ...
            before_state=(
                PrecisionState(**entry_data["before_state"])
                if entry_data["before_state"]
                else None
            ),
            after_state=(
                PrecisionState(**entry_data["after_state"])
                if entry_data["after_state"]
                else None
            ),
