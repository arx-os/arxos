...
class LinearDimension(Dimension):
    ...
    def format_text(self) -> str:
        ...
        if self.tolerance_plus is not None or self.tolerance_minus is not None:
            ...
            if self.tolerance_plus is not None:
                tolerance_text += (
                    f"+{float(self.tolerance_plus):.{self.style.precision}f}"
                )
            if self.tolerance_minus is not None:
                tolerance_text += (
                    f"-{float(self.tolerance_minus):.{self.style.precision}f}"
                )
    ...
    def get_leader_points(self) -> List[PrecisionPoint]:
        ...
        leader_start = PrecisionPoint(
            mid_x + perp_x * offset_distance, mid_y + perp_y * offset_distance
        )
...
class RadialDimension(Dimension):
    ...
    def get_leader_points(self) -> List[PrecisionPoint]:
        ...
        leader_end = PrecisionPoint(
            center.x
            + unit_x * (length + self.style.extension_offset + self.style.text_offset),
            center.y
            + unit_y * (length + self.style.extension_offset + self.style.text_offset),
        )
...
class AngularDimension(Dimension):
    ...
    def get_leader_points(self) -> List[PrecisionPoint]:
        ...
        radius2 = (
            center.distance_to(self.text_position) if self.text_position else radius1
        )
        ...
        return [arc_center, PrecisionPoint(arc_center.x + arc_radius, arc_center.y)]
...
class DimensionManager:
    ...
    def _create_dimension_from_data(
        self, dim_data: Dict[str, Any]
    ) -> Optional[Dimension]:
        ...
        try:
            ...
            if dim_type in [
                ...
                return self.create_linear_dimension(
                    dim_data["start_point"], dim_data["end_point"], dim_type, style_name
                )
